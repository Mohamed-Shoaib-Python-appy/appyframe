...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.its("appyframe")
			.then((appyframe) => {
				return appyframe.xcall(
					"appyframe.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("appyframe")
			.then((appyframe) => {
				appyframe.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.appyframe-control[data-fieldname="child_table_1"]').as("table");
