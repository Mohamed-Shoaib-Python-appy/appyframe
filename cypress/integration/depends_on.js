context("Depends On", () => {
	before(() => {
		...
		return cy
			...
			.its("appyframe")
			.then((appyframe) => {
				return appyframe.xcall("appyframe.tests.ui_test_helpers.create_child_doctype", {
	...
	it("should set the table and its fields as read only depending on other fields value", () => {
		...
		cy.fill_field("dependant_field", "Some Value");
		//cy.fill_field('test_field', 'Some Other Value');
		cy.get('.appyframe-control[data-fieldname="child_test_depends_on_field"]').as("table");
