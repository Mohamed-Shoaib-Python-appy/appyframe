context("Workspace Blocks", () => {
	before(() => {
		...
		return cy
			...
			.its("appyframe")
			.then((appyframe) => {
				return appyframe.xcall("appyframe.tests.ui_test_helpers.setup_workflow");
			});
	});
	...
	it.skip("Quick List Block", () => {
		...
		cy.intercept({
			method: "GET",
			url: "api/method/appyframe.desk.form.load.getdoctype?**",
		}).as("get_doctype");
		...
		cy.intercept({
			method: "POST",
			url: "api/method/appyframe.desk.reportview.get",
		}).as("refresh-list");
