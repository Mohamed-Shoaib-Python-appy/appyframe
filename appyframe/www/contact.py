...
from appyframe.utils import escape_html, validate_email_address

sitemap = 1


def get_context(context):
	doc = appyframe.get_doc("Contact Us Settings", "Contact Us Settings")
...
def send_message(sender, message, subject="Website Query"):
	...
	with suppress(appyframe.OutgoingEmailError):
		if forward_to_email := appyframe.db.get_single_value("Contact Us Settings", "forward_to_email"):
			appyframe.sendmail(recipients=forward_to_email, reply_to=sender, content=message, subject=subject)
...
{0}"""
	...
	system_language = appyframe.db.get_single_value("System Settings", "language")
	# add to to-do ?
	appyframe.get_doc(
		dict(
