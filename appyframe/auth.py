...
from appyframe.sessions import Session, clear_sessions, delete_session, get_expiry_in_seconds
from appyframe.translate import get_language
...
from appyframe.utils import cint, date_diff, datetime, get_datetime, today
from appyframe.utils.deprecations import deprecation_warning
from appyframe.utils.password import check_password, get_decrypted_password
...
class HTTPRequest:
	def __init__(self):
		# set appyframe.local.request_ip
		self.set_request_ip()
		...
		appyframe.local.cookie_manager.init_cookies()

	@property
	def domain(self):
		if not getattr(self, "_domain", None):
			self._domain = appyframe.request.host
	...
	def set_request_ip(self):
		if appyframe.get_request_header("X-Forwarded-For"):
			appyframe.local.request_ip = (appyframe.get_request_header("X-Forwarded-For").split(",", 1)[0]).strip()

		elif appyframe.get_request_header("REMOTE_ADDR"):
			appyframe.local.request_ip = appyframe.get_request_header("REMOTE_ADDR")
	...
	def validate_csrf_token(self):
		if (
			not appyframe.request
			or appyframe.request.method not in UNSAFE_HTTP_METHODS
			or appyframe.conf.ignore_csrf
			or not appyframe.session
			or not (saved_token := appyframe.session.data.csrf_token)
			or (
				(appyframe.get_request_header("X-Appyframe-CSRF-Token") or appyframe.form_dict.pop("csrf_token", None))
				== saved_token
			)
		):
			return

		appyframe.flags.disable_traceback = True
		appyframe.throw(_("Invalid Request"), appyframe.CSRFTokenError)

	def set_lang(self):
		appyframe.local.lang = get_language()
...
class LoginManager:
	...
	def __init__(self):
		...
		if appyframe.local.form_dict.get("cmd") == "login" or appyframe.local.request.path == "/api/method/login":
	...
	def login(self):
		if appyframe.get_system_settings("disable_user_pass_login"):
			appyframe.throw(_("Login with username and password is not allowed."), appyframe.AuthenticationError)

		# clear cache
		appyframe.clear_cache(user=appyframe.form_dict.get("usr"))
		user, pwd = get_cached_user_pass()
		self.authenticate(user=user, pwd=pwd)
		if self.force_user_to_reset_password():
			doc = appyframe.get_doc("User", self.user)
			appyframe.local.response["redirect_to"] = doc.reset_password(send_email=False, password_expired=True)
			appyframe.local.response["message"] = "Password Reset"
			return False
		...
		if should_run_2fa(self.user):
			...
			if not confirm_otp_token(self):
				return False
		appyframe.form_dict.pop("pwd", None)
		self.post_login()
	...
	def get_user_info(self):
		self.info = appyframe.get_cached_value(
			"User",
			self.user,
	...
	def setup_boot_cache(self):
		appyframe.cache_manager.build_table_count_cache()
		appyframe.cache_manager.build_domain_restriced_doctype_cache()
		appyframe.cache_manager.build_domain_restriced_page_cache()
	...
	def set_user_info(self, resume=False):
		...
		if self.info.user_type == "Website User":
			appyframe.local.cookie_manager.set_cookie("system_user", "no")
			if not resume:
				appyframe.local.response["message"] = "No App"
				appyframe.local.response["home_page"] = get_default_path() or "/" + get_home_page()
		else:
			appyframe.local.cookie_manager.set_cookie("system_user", "yes")
			if not resume:
				appyframe.local.response["message"] = "Logged In"
				default_workspace = self.info.default_workspace
				if default_workspace:
					appyframe.local.response["home_page"] = "/app/" + slug(default_workspace)
				else:
					appyframe.local.response["home_page"] = get_default_path() or "/app"

		if not resume:
			appyframe.response["full_name"] = self.full_name

		# redirect information
		if not resume and (redirect_to := appyframe.cache.hget("redirect_after_login", self.user)):
			appyframe.local.response["redirect_to"] = redirect_to
			appyframe.cache.hdel("redirect_after_login", self.user)

		appyframe.local.cookie_manager.set_cookie("full_name", self.full_name)
		appyframe.local.cookie_manager.set_cookie("user_id", self.user)
		appyframe.local.cookie_manager.set_cookie("user_image", self.info.user_image or "")

	def clear_preferred_language(self):
		appyframe.local.cookie_manager.delete_cookie("preferred_language")

	def make_session(
		self, resume: bool = False, session_end: str | None = None, audit_user: str | None = None
	):
		# start session
		appyframe.local.session_obj = Session(
			user=self.user,
			resume=resume,
		...
		self.user = appyframe.local.session_obj.user
		appyframe.local.session = appyframe.local.session_obj.data
	...
	def clear_active_sessions(self):
		...
		if not (
			cint(appyframe.conf.get("deny_multiple_sessions"))
			or cint(appyframe.db.get_system_setting("deny_multiple_sessions"))
		):
			return

		clear_sessions(appyframe.session.user, keep_current=True)

	def authenticate(self, user: str | None = None, pwd: str | None = None):
		from appyframe.core.doctype.user.user import User

		if not (user and pwd):
			user, pwd = appyframe.form_dict.get("usr"), appyframe.form_dict.get("pwd")
		if not (user and pwd):
		...
		ip_tracker = get_login_attempt_tracker(appyframe.local.request_ip)
		if not user:
		...
		ignore_tracker = should_run_2fa(user.name) and ("otp" in appyframe.form_dict)
	...
	def force_user_to_reset_password(self):
		...
		if self.user in appyframe.STANDARD_USERS:
			return False

		reset_pwd_after_days = cint(
			appyframe.db.get_single_value("System Settings", "force_user_to_reset_password")
		)

		if reset_pwd_after_days:
			last_password_reset_date = (
				appyframe.db.get_value("User", self.user, "last_password_reset_date") or today()
			)
	...
	def fail(self, message, user=None):
		if not user:
			user = _("Unknown User")
		appyframe.local.response["message"] = message
		add_authentication_log(message, user, status="Failed")
		appyframe.db.commit()
		raise appyframe.AuthenticationError

	def run_trigger(self, event="on_login"):
		for method in appyframe.get_hooks().get(event, []):
			appyframe.call(appyframe.get_attr(method), login_manager=self)

	def validate_hour(self):
		"""check if user is logging in during restricted hours"""
		login_before = int(appyframe.db.get_value("User", self.user, "login_before", ignore=True) or 0)
		login_after = int(appyframe.db.get_value("User", self.user, "login_after", ignore=True) or 0)

		if not (login_before or login_after):
			return

		from appyframe.utils import now_datetime

		current_hour = int(now_datetime().strftime("%H"))

		if login_before and current_hour >= login_before:
			appyframe.throw(_("Login not allowed at this time"), appyframe.AuthenticationError)
	...
	def impersonate(self, user):
		current_user = appyframe.session.user
		session_data = appyframe.local.session_obj.data.data
		...
		appyframe.local.session_obj.set_impersonated(current_user)
	...
	def logout(self, arg="", user=None):
		...
		if user == appyframe.session.user:
			delete_session(appyframe.session.sid, user=user, reason="User Manually Logged Out")
			self.clear_cookies()
			if appyframe.request:
				self.login_as_guest()
		else:
			clear_sessions(user)
...
class CookieManager:
	...
	def init_cookies(self):
		if not appyframe.local.session.get("sid"):
			return

		if appyframe.session.sid:
			self.set_cookie("sid", appyframe.session.sid, max_age=get_expiry_in_seconds(), httponly=True)
	...
	def set_cookie(
		...
		if not secure and hasattr(appyframe.local, "request"):
			secure = appyframe.local.request.scheme == "https"
...
@appyframe.whitelist()
def get_logged_user():
	return appyframe.session.user


def clear_cookies():
	if hasattr(appyframe.local, "session"):
		appyframe.session.sid = ""
	appyframe.local.cookie_manager.delete_cookie(["full_name", "user_id", "sid", "user_image", "system_user"])
...
def validate_ip_address(user):
	...
	checked for those
	"""
	if hasattr(appyframe.local, "request") and appyframe.local.request.path.startswith(
		"/api/method/appyframe.realtime."
	):
		return True

	from appyframe.core.doctype.user.user import get_restricted_ip_list

	# Only fetch required fields - for perf
	user_fields = ["restrict_ip", "bypass_restrict_ip_check_if_2fa_enabled"]
	user_info = (
		appyframe.get_cached_value("User", user, user_fields, as_dict=True)
		if not appyframe.flags.in_test
		else appyframe.db.get_value("User", user, user_fields, as_dict=True)
	)
	...
	system_settings = (
		appyframe.get_cached_doc("System Settings")
		if not appyframe.flags.in_test
		else appyframe.get_single("System Settings")
	)
	...
	for ip in ip_list:
		if appyframe.local.request_ip.startswith(ip) or bypass_restrict_ip_check:
			return

	appyframe.throw(_("Access not allowed from this IP Address"), appyframe.AuthenticationError)
...
def get_login_attempt_tracker(key: str, raise_locked_exception: bool = True):
	...
	"""
	sys_settings = appyframe.get_doc("System Settings")
...
class LoginAttemptTracker:
	...
	def login_failed_count(self):
		return appyframe.cache.hget("login_failed_count", self.key)

	@login_failed_count.setter
	def login_failed_count(self, count):
		appyframe.cache.hset("login_failed_count", self.key, count)

	@login_failed_count.deleter
	def login_failed_count(self):
		appyframe.cache.hdel("login_failed_count", self.key)
	...
	def login_failed_time(self):
		...
		"""
		return appyframe.cache.hget("login_failed_time", self.key)

	@login_failed_time.setter
	def login_failed_time(self, timestamp):
		appyframe.cache.hset("login_failed_time", self.key, timestamp)

	@login_failed_time.deleter
	def login_failed_time(self):
		appyframe.cache.hdel("login_failed_time", self.key)
...
def validate_auth():
	...
	"""
	authorization_header = appyframe.get_request_header("Authorization", "").split(" ")
	...
	if len(authorization_header) == 2 and appyframe.session.user in ("", "Guest"):
		raise appyframe.AuthenticationError
...
def validate_oauth(authorization_header):
	...
	from appyframe.integrations.oauth2 import get_oauth_server
	from appyframe.oauth import get_url_delimiter

	if authorization_header[0].lower() != "bearer":
		return

	form_dict = appyframe.local.form_dict
	token = authorization_header[1]
	req = appyframe.request
	...
	try:
		required_scopes = appyframe.db.get_value("OAuth Bearer Token", token, "scopes").split(
			get_url_delimiter()
		)
		...
		if valid:
			appyframe.set_user(appyframe.db.get_value("OAuth Bearer Token", token, "user"))
...
def validate_auth_via_api_keys(authorization_header):
	...
	try:
		...
		authorization_source = appyframe.get_request_header("Appyframe-Authorization-Source")
		if auth_type.lower() == "basic":
			api_key, api_secret = appyframe.safe_decode(base64.b64decode(auth_token)).split(":")
		...
		appyframe.throw(
			...
			appyframe.InvalidAuthorizationToken,
		)
...
def validate_api_key_secret(api_key, api_secret, frappe_authorization_source=None):
	...
	doc = appyframe.db.get_value(doctype=doctype, filters={"api_key": api_key}, fieldname=["name"])
	if not doc:
		raise appyframe.AuthenticationError
	form_dict = appyframe.local.form_dict
	...
	if api_secret == doc_secret:
		if doctype == "User":
			user = appyframe.db.get_value(doctype="User", filters={"api_key": api_key}, fieldname=["name"])
		else:
			user = appyframe.db.get_value(doctype, doc, "user")
		if appyframe.local.login_manager.user in ("", "Guest"):
...
def validate_auth_via_hooks():
	for auth_hook in appyframe.get_hooks("auth_hooks", []):
		appyframe.get_attr(auth_hook)()
