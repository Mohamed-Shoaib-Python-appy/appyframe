...
import appyframe
from appyframe.defaults import _clear_cache
from appyframe.utils import cint, is_git_url
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		...
		appyframe.connect()
		enable_scheduler = cint(appyframe.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		appyframe.db.close()
...
def _new_site(
	...
	"""Install a new Appyframe site"""

	from appyframe.utils import scheduler
	...
	if not db_name:
		...
		db_name = (
			"_"
			+ hashlib.sha1(
				os.path.realpath(appyframe.get_site_path()).encode(), usedforsecurity=False
			).hexdigest()[:16]
		)
	...
	with filelock("bench_new_site", timeout=1):
		...
		apps_to_install = ["appyframe"] + (appyframe.conf.get("install_apps") or []) + (list(install_apps) or [])
	...
	scheduler_status = "disabled" if appyframe.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from appyframe.database import bootstrap_database, setup_database
	...
	appyframe.flags.in_install_db = True

	appyframe.flags.root_login = root_login
	appyframe.flags.root_password = root_password
	...
	appyframe.conf.admin_password = appyframe.conf.admin_password or admin_password

	remove_missing_apps()

	appyframe.db.create_auth_table()
	appyframe.db.create_global_search_table()
	appyframe.db.create_user_settings_table()

	appyframe.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/appyframe/bench/blob/develop/bench/utils/__init__.py#L390
	...
	from appyframe.exceptions import InvalidRemoteException

	for org in ["appyframe", "erpnext"]:
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/appyframe/bench/blob/develop/bench/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/appyframe/bench/blob/develop/bench/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from appyframe.model.sync import sync_for
	from appyframe.modules.utils import sync_customizations
	from appyframe.utils.fixtures import sync_fixtures

	appyframe.flags.in_install = name
	appyframe.flags.ignore_in_install = False

	appyframe.clear_cache()
	app_hooks = appyframe.get_hooks(app_name=name)
	installed_apps = appyframe.get_installed_apps()
	...
	if name not in appyframe.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = appyframe.get_attr(before_install)()
		if out is False:
			return

	for fn in appyframe.get_hooks("before_app_install"):
		appyframe.get_attr(fn)(name)
	...
	appyframe.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		appyframe.get_attr(after_install)()

	for fn in appyframe.get_hooks("after_app_install"):
		appyframe.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		appyframe.get_attr(after_sync)()  #

	appyframe.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		appyframe.db.set_global("installed_apps", json.dumps(installed_apps))
		appyframe.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = appyframe.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in appyframe.get_installed_apps():
	...
	for app in appyframe.get_installed_apps():
		if app != app_name:
			hooks = appyframe.get_hooks(app_name=app)
	...
	if not (dry_run or no_backup):
		from appyframe.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	appyframe.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		appyframe.get_attr(before_uninstall)()

	for fn in appyframe.get_hooks("before_app_uninstall"):
		appyframe.get_attr(fn)(app_name)

	modules = appyframe.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		appyframe.get_attr(after_uninstall)()

	for fn in appyframe.get_hooks("after_app_uninstall"):
		appyframe.get_attr(fn)(app_name)

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	appyframe.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in appyframe.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					appyframe.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			appyframe.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in appyframe.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				appyframe.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = appyframe.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if appyframe.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			appyframe.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			appyframe.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from appyframe.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		appyframe.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	appyframe.db.commit()


def init_singles():
	singles = appyframe.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if appyframe.db.get_singles_dict(single):
			continue

		try:
			doc = appyframe.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, appyframe.DoesNotExistError):
...
def make_conf(
	...
	sites_path = appyframe.local.sites_path
	appyframe.destroy()
	appyframe.init(site, sites_path=sites_path)
...
def make_site_config(
	...
	appyframe.create_folder(os.path.join(appyframe.local.site_path))
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(appyframe.local, "conf"):
		appyframe.local.conf[key] = value


def get_site_config_path():
	return os.path.join(appyframe.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = appyframe.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = appyframe.get_module_list(app)
	for module in modules:
		d = appyframe.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	installed_apps = json.loads(appyframe.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_files(site_name, file_path):
	...
	from appyframe.utils import get_bench_relative_path

	file_path = get_bench_relative_path(file_path)

	# Need to do appyframe.init to maintain the site locals
	appyframe.init(site=site_name)
	abs_site_path = os.path.abspath(appyframe.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	current_version = Version(appyframe.__version__)
	...
	if verbose and is_downgrade:
		print(f"Your site is currently on Appyframe {current_version} and your backup is {backup_version}.")

	return is_downgrade


def get_old_backup_version(sql_file_path: str) -> Version | None:
	"""Return the appyframe version used to create the specified database dump.

	This methods supports older versions of Appyframe wich used a different format.
	"""
	header = get_db_dump_header(sql_file_path).split("\n")
	if match := re.search(r"Appyframe (\d+\.\d+\.\d+)", header[0]):
...
def get_backup_version(sql_file_path: str) -> Version | None:
	...
	header = get_db_dump_header(sql_file_path).split("\n")
	metadata = ""
	if "begin appyframe metadata" in header[0]:
		for line in header[1:]:
			if "end appyframe metadata" in line:
				break
			metadata += line.replace("--", "").strip() + "\n"
		parser = configparser.ConfigParser()
		parser.read_string(metadata)
		return Version(parser["appyframe"]["version"])
...
def partial_restore(sql_file_path, verbose=False):
	if appyframe.conf.db_type == "mariadb":
		from appyframe.database.mariadb.setup_db import import_db_from_sql
	elif appyframe.conf.db_type == "postgres":
		import warnings

		from appyframe.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		...
		try:
			appyframe.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise appyframe.InvalidDatabaseFile
