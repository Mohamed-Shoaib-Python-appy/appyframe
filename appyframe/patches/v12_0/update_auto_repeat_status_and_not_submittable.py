...
def execute():
	...
	appyframe.reload_doc("automation", "doctype", "Auto Repeat")
	appyframe.db.sql("update `tabDocPerm` set submit=0, cancel=0, amend=0 where parent='Auto Repeat'")
	appyframe.db.sql("update `tabAuto Repeat` set docstatus=0 where docstatus=1 or docstatus=2")

	for entry in appyframe.get_all("Auto Repeat"):
		doc = appyframe.get_doc("Auto Repeat", entry.name)

		# create custom field for allow auto repeat
		fields = appyframe.get_meta(doc.reference_doctype).fields
