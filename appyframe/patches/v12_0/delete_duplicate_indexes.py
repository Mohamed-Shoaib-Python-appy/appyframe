...
def execute():
	...
	all_tables = appyframe.db.get_tables()
	final_deletion_map = appyframe._dict()

	for table in all_tables:
		indexes_to_keep_map = appyframe._dict()
		indexes_to_delete = []
		index_info = appyframe.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				appyframe.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
			except Exception as e:
				appyframe.log_error("Failed to drop index")
