...
def execute():
	...
	for name, content in appyframe.get_all(
		...
		appyframe.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in appyframe.get_all(
		...
		if content and content.startswith("Removed "):
			appyframe.db.set_value("Comment", name, "content", content[8:], update_modified=False)
