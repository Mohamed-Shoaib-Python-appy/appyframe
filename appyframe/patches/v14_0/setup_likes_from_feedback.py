...
def execute():
	...
	if appyframe.db.count("Feedback") > 20000:
		appyframe.db.auto_commit_on_many_writes = True

	for feedback in appyframe.get_all("Feedback", fields=["*"]):
		if feedback.like:
	...
	# clean up
	appyframe.db.delete("Feedback")
	appyframe.db.commit()

	appyframe.delete_doc("DocType", "Feedback")
