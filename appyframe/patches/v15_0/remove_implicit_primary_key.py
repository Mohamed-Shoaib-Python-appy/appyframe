import appyframe
from appyframe.model.naming import is_autoincremented
...
def execute():
	...
	for doctype in possible_log_types:
		if (
			appyframe.db.exists("DocType", doctype)
			and _is_implicit_int_pk(doctype)
			and not is_autoincremented(doctype)
		):
			appyframe.db.change_column_type(
				doctype,
				"name",
				type=f"varchar({appyframe.db.VARCHAR_LEN})",
				nullable=True,
			)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if appyframe.db.db_type == "mariadb":
		...
		values = (appyframe.db.cur_db_name,)

	col_type = appyframe.db.sql(query, values)
