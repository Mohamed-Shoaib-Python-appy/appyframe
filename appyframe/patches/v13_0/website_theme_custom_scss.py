...
def execute():
	...
	appyframe.reload_doc("website", "doctype", "color")
	appyframe.reload_doc("website", "doctype", "website_theme", force=True)

	for theme in appyframe.get_all("Website Theme"):
...
def setup_color_record(doc):
	...
	for color_field in color_fields:
		...
		if not color_code or appyframe.db.exists("Color", color_code):
			continue
