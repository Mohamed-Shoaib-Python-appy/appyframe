...
def execute():
	...
	appyframe.reload_doc("integrations", "doctype", "social_login_key", force=True)

	if not appyframe.db.exists("DocType", "Social Login Keys"):
		return

	social_login_keys = appyframe.get_doc("Social Login Keys", "Social Login Keys")
	if social_login_keys.get("facebook_client_id") or social_login_keys.get("facebook_client_secret"):
		facebook_login_key = appyframe.new_doc("Social Login Key")
	...
	if social_login_keys.get("frappe_server_url"):
		frappe_login_key = appyframe.new_doc("Social Login Key")
		frappe_login_key.get_social_login_provider("Appyframe", initialize=True)
		frappe_login_key.social_login_provider = "Appyframe"
	...
	if social_login_keys.get("github_client_id") or social_login_keys.get("github_client_secret"):
		github_login_key = appyframe.new_doc("Social Login Key")
	...
	if social_login_keys.get("google_client_id") or social_login_keys.get("google_client_secret"):
		google_login_key = appyframe.new_doc("Social Login Key")
	...
	appyframe.delete_doc("DocType", "Social Login Keys")


def run_patch():
	appyframe.reload_doc("core", "doctype", "user", force=True)
	appyframe.reload_doc("core", "doctype", "user_social_login", force=True)

	users = appyframe.get_all("User", fields=["*"], filters={"name": ("not in", ["Administrator", "Guest"])})

	for user in users:
		idx = 0
		if user.frappe_userid:
			insert_user_social_login(user.name, user.modified_by, "appyframe", idx, userid=user.frappe_userid)
			idx += 1
...
def insert_user_social_login(user, modified_by, provider, idx, userid=None, username=None):
	...
	creation_time = appyframe.utils.get_datetime_str(appyframe.utils.get_datetime())
	values = [
		appyframe.generate_hash(length=10),
		creation_time,
		creation_time,
		user,
		modified_by,
	...
	query = """INSERT INTO `tabUser Social Login` (`{source_cols}`)
		VALUES ({values})
	""".format(source_cols="`, `".join(source_cols), values=", ".join([appyframe.db.escape(d) for d in values]))
