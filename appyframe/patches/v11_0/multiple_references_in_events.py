...
def execute():
	appyframe.reload_doctype("Event")
	# Rename "Cancel" to "Cancelled"
	appyframe.db.sql("""UPDATE tabEvent set event_type='Cancelled' where event_type='Cancel'""")
	...
	for event in events:
		if event.ref_type and event.ref_name:
			try:
				e = appyframe.get_doc("Event", event.name)
				e.append(
					"event_participants",
				...
				appyframe.log_error(appyframe.get_traceback())
