...
function toggle_edit_letterhead() {
	...
	if (!control.value) {
		control.value = appyframe.ui.form.make_control({
			parent: editor.value,
			df: {
...
// mounted
onMounted(() => {
	if (!letterhead.value && appyframe.boot.sysdefaults.letter_head) {
		set_letterhead(appyframe.boot.sysdefaults.letter_head);
	}
