...
export const useStore = defineStore("workflow-builder-store", () => {
	...
	async function fetch() {
		await appyframe.model.clear_doc("Workflow", workflow_name.value);
		await appyframe.model.with_doc("Workflow", workflow_name.value);

		workflow_doc.value = appyframe.get_doc("Workflow", workflow_name.value);
		await appyframe.model.with_doctype(workflow_doc.value.document_type);

		if (!workflowfields.value.length) {
			await appyframe.model.with_doctype("Workflow");
			workflowfields.value = appyframe.get_meta("Workflow").fields;
		}

		if (!statefields.value.length) {
			await appyframe.model.with_doctype("Workflow Document State");
			statefields.value = appyframe.get_meta("Workflow Document State").fields;
		}

		if (!transitionfields.value.length) {
			await appyframe.model.with_doctype("Workflow Transition");
			transitionfields.value = appyframe.get_meta("Workflow Transition").fields;
		}

		if (!workflow_doc_fields.value.length) {
			let doc_type = workflow_doc.value.document_type;
			await appyframe.model.with_doctype(doc_type);
			workflow_doc_fields.value = appyframe.meta
				.get_docfields(doc_type, null, {
					fieldtype: ["not in", appyframe.model.no_value_type],
				})
				.sort((a, b) => {
					if (a.label && b.label) {
	...
	async function save_changes() {
		appyframe.dom.freeze(__("Saving..."));
		...
		try {
			...
			await appyframe.call("appyframe.client.save", { doc });
			appyframe.toast(__("Workflow updated successfully"));
			fetch();
		} catch (e) {
			console.error(e);
		} finally {
	...
	function setup_breadcrumbs() {
		...
		`;
		appyframe.breadcrumbs.clear();
		appyframe.breadcrumbs.$breadcrumbs.append(breadcrumbs);
	}
