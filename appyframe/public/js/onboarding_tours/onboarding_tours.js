appyframe.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new appyframe.Driver({
			className: "appyframe-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				appyframe.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!appyframe.boot.user.onboarding_status[this.tour.name] &&
								(appyframe.boot.user.onboarding_status[this.tour.name] = {});
							appyframe.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								appyframe.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							appyframe.call({
								method: "appyframe.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(appyframe.boot.user.onboarding_status),
				...
				if ($input) appyframe.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await appyframe.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: appyframe.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
appyframe.ui.init_onboarding_tour = () => {
	...
	typeof appyframe.boot.onboarding_tours == "undefined" && appyframe.boot.onboarding_tours == [];
	typeof appyframe.boot.user.onboarding_status == "undefined" &&
		appyframe.boot.user.onboarding_status == {};
	let route = appyframe.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		appyframe.boot.onboarding_tours &&
			appyframe.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (appyframe.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (appyframe.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(appyframe.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == appyframe.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == appyframe.ui.next_form_tour);
	if (current_tour) {
		...
		appyframe.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (appyframe.ui.currentTourInstance?.driver) {
		appyframe.ui.currentTourInstance.driver_steps = [];
		appyframe.ui.currentTourInstance.driver.reset(true);
		appyframe.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (appyframe.ui.currentTourInstance = new appyframe.ui.OnboardingTour());
	...
	const wait_for_data = setInterval(() => {
		...
		if (document.body.getAttribute("data-ajax-state") === "complete") {
			appyframe.utils.sleep(500).then(() => {
				tour.init({
					tour_name,
					start_step,
				});
