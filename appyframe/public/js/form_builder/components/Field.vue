...
function remove_field() {
	if (store.is_customize_form && props.field.df.is_custom_field == 0) {
		appyframe.msgprint(__("Cannot delete standard field. You can hide it if you want"));
...
function duplicate_field() {
	...
	duplicate_field.df.owner = appyframe.session.user;
...
function make_dialog(frm) {
	frm.dialog = new appyframe.ui.Dialog({
	...
	if (frm.doctype === "Customize Form") {
		...
		appyframe.db.exists("Property Setter", property_setter_id).then((exits) => {
			if (exits) {
				frm.dialog.set_secondary_action_label(__("Reset To Default"));
				frm.dialog.set_secondary_action(() => {
					appyframe.call({
						method: "appyframe.custom.doctype.customize_form.customize_form.get_link_filters_from_doc_without_customisations",
						args: {
...
function edit_filters() {
	...
	appyframe.model.with_doctype(field_doctype, () => {
		frm.dialog.show();
		add_existing_filter(frm, props.field.df);
	});
}
			...
			<template #label>
				<div class="field-label">
					...
					<div
						...
						v-html="appyframe.utils.icon('help', 'sm')"
					></div>
				</div>
			</template>
			<template #actions>
				<div class="field-actions" :hidden="store.read_only">
					<button
						...
						@click="edit_filters"
					>
						<div v-html="appyframe.utils.icon('filter', 'sm')"></div>
					</button>
					<AddFieldButton ref="add_field_ref" :column="column" :field="field">
						<div v-html="appyframe.utils.icon('add', 'sm')" />
					</AddFieldButton>
					<button
					...
					>
						<div v-html="appyframe.utils.icon('move', 'sm')"></div>
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="appyframe.utils.icon('duplicate', 'sm')"></div>
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="appyframe.utils.icon('remove', 'sm')"></div>
					</button>
				</div>
			</template>
		</component>
