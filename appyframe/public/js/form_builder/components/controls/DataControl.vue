...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = appyframe.boot.time_zone
		? appyframe.boot.time_zone.user
		: appyframe.sys_defaults.time_zone;
...
<template>
	<div class="control appyframe-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="appyframe.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="appyframe.utils.icon('down', 'sm')"
		/>
	</div>
</template>
