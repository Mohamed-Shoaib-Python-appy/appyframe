	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="appyframe.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && appyframe.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: appyframe.utils.icon("folder-open", "md"),
		closed: appyframe.utils.icon("folder-normal", "md"),
		leaf: appyframe.utils.icon("primitive-dot", "xs"),
		search: appyframe.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return appyframe.utils.get_form_link("File", filename);
};
