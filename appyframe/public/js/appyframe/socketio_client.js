...
appyframe.provide("appyframe.realtime");
...
class RealTimeClient {
	...
	init(port = 9000, lazy_connect = false) {
		if (appyframe.boot.disable_async) {
			return;
		}
	...
	get_host(port = 9000) {
		...
		if (window.dev_server) {
			...
			port = appyframe.boot.socketio_port || port.toString() || "9000";
			if (parts.length > 2) {
				host = parts[0] + ":" + parts[1];
			}
			host = host + ":" + port;
		}
		return host + `/${appyframe.boot.sitename}`;
	}
	...
	doc_subscribe(doctype, docname) {
		if (appyframe.flags.doc_subscribe) {
		...
		appyframe.flags.doc_subscribe = true;

		// throttle to 1 per sec
		setTimeout(function () {
			appyframe.flags.doc_subscribe = false;
		}, 1000);
	...
	process_response(data, method) {
		...
		// "callback" is std appyframe term
		if (method === "success") {
...
appyframe.realtime = new RealTimeClient();

// backward compatbility
appyframe.socketio = appyframe.realtime;
