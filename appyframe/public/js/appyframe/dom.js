...
appyframe.provide("appyframe.dom");
...
appyframe.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + appyframe.dom.id_count;
		appyframe.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + appyframe.dom.id_count;
		$ele.attr("id", id);
		appyframe.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = appyframe.dom.by_id(parent);
	...
	freeze: function (msg, css_class) {
		...
		appyframe.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!appyframe.dom.freeze_count) return; // anything open?
		appyframe.dom.freeze_count--;
		if (!appyframe.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				appyframe.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
appyframe.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
appyframe.run_serially = function (tasks) {
...
appyframe.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
appyframe.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

appyframe.scrub = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

appyframe.unscrub = function (txt) {
	return appyframe.model.unscrub(txt);
};

appyframe.get_data_pill = (
	label,
	target_id = null,
	remove_action = null,
	image = null,
	...
	let color = "",
		style = "";
	if (colored) {
		color = appyframe.get_palette(label);
	}
...
appyframe.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${appyframe.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${appyframe.utils.icon("close-alt", "sm", "close-alt")}
						</button>
					</div>
				</div>
...
appyframe.is_online = function () {
	if (appyframe.boot.developer_mode == 1) {
...
appyframe.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + appyframe.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = appyframe.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = appyframe.assets.bundled_asset("desk.bundle.css");
