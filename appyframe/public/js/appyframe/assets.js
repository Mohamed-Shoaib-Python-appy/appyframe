...
// depends on appyframe.versions to manage versioning

appyframe.require = function (items, callback) {
	if (typeof items === "string") {
		items = [items];
	}
	items = items.map((item) => appyframe.assets.bundled_asset(item));

	return new Promise((resolve) => {
		appyframe.assets.execute(items, () => {
			resolve();
			callback && callback();
		});
	});
...
class AssetManager {
	constructor() {
		...
		this._handlers = {
			js: function (txt) {
				appyframe.dom.eval(txt);
			},
			css: function (txt) {
				appyframe.dom.set_style(txt);
			},
		};
	}
	check() {
	...
	init_local_storage() {
		...
		if (appyframe.boot) localStorage.metadata_version = appyframe.boot.metadata_version;
	}
	...
	execute(items, callback) {
		...
		const version_string =
			appyframe.boot.developer_mode || window.dev_server ? Date.now() : window._version_number;
...
appyframe.assets = new AssetManager();
