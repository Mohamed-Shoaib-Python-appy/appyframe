...
appyframe.provide("appyframe.perm");
...
$.extend(appyframe.perm, {
	...
	has_perm: (doctype, permlevel = 0, ptype = "read", doc) => {
		const perms = appyframe.perm.get_perm(doctype, doc);
	...
	get_perm: (doctype, doc) => {
		...
		if (doc && !doc.__islocal) {
			return appyframe.perm._get_perm(doctype, doc);
		}

		return (appyframe.perm.doctype_perm[doctype] ??= appyframe.perm._get_perm(doctype));
	},

	_get_perm: (doctype, doc) => {
		let perm = [{ read: 0, permlevel: 0 }];

		let meta = appyframe.get_doc("DocType", doctype);
		const user = appyframe.session.user;

		if (user === "Administrator" || appyframe.user_roles.includes("Administrator")) {
			perm[0].read = 1;
		}

		if (!meta) return perm;

		perm = appyframe.perm.get_role_permissions(meta);
		const base_perm = perm[0];

		if (doc) {
			// apply user permissions via docinfo (which is processed server-side)
			let docinfo = appyframe.model.get_docinfo(doctype, doc.name);
			...
			// if owner
			if (doc.owner !== user) {
				for (const right of appyframe.perm.rights) {
			...
			if (docinfo && docinfo.shared) {
				for (const s of docinfo.shared) {
					...
					if (s.read) {
						...
						base_perm.email =
							appyframe.boot.user.can_email.indexOf(doctype) !== -1 ? 1 : 0;
						base_perm.print =
							appyframe.boot.user.can_print.indexOf(doctype) !== -1 ? 1 : 0;
					}
				}
			}
		}

		if (!base_perm.read && appyframe.model.can_read(doctype)) {
			// read via sharing
			base_perm.read = 1;
		}
	...
	get_role_permissions: (meta) => {
		...
		*/
		...
		(meta.permissions || []).forEach((p) => {
			...
			if (appyframe.user_roles.includes(p.role)) {
				appyframe.perm.rights.forEach((right) => {
					if (!p[right]) return;
	...
	get_match_rules: (doctype, ptype) => {
		...
		let perm = appyframe.perm.get_perm(doctype);

		let user_permissions = appyframe.defaults.get_user_permissions();

		if (user_permissions && !$.isEmptyObject(user_permissions)) {
			let rules = {};
			let fields_to_check = appyframe.meta.get_fields_to_check_permissions(doctype);
			$.each(fields_to_check, (i, df) => {
				const user_permissions_for_doctype = user_permissions[df.options] || [];
				const allowed_records = appyframe.perm.get_allowed_docs_for_doctype(
		...
		if (base_perm.read && !base_perm.rights_without_if_owner.has("read")) {
			match_rules.push({ Owner: appyframe.session.user });
		}
		return match_rules;
	},
	...
	get_field_display_status: (df, doc, perm, explain) => {
		...
		if (!perm && doc) {
			perm = appyframe.perm.get_perm(doc.doctype, doc);
		}
	...
	is_visible: (df, doc, perm) => {
		if (typeof df === "string") {
			// df is fieldname
			df = appyframe.meta.get_docfield(doc.doctype, df, doc.parent || doc.name);
		}

		let status = appyframe.perm.get_field_display_status(df, doc, perm);
	...
	get_allowed_docs_for_doctype: (user_permissions, doctype) => {
		...
		return appyframe.perm.filter_allowed_docs_for_doctype(user_permissions, doctype, false);
	},
