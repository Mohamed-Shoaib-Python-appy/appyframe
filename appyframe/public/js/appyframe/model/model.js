...
$.extend(appyframe.model, {
	...
	init: function () {
		...
		appyframe.realtime.on("doc_update", function (data) {
			var doc = locals[data.doctype] && locals[data.doctype][data.name];

			if (doc) {
				// current document is dirty, show message if its not me
				if (
					appyframe.get_route()[0] === "Form" &&
					cur_frm.doc.doctype === doc.doctype &&
					cur_frm.doc.name === doc.name
				) {
					if (data.modified !== cur_frm.doc.modified && !appyframe.ui.form.is_saving) {
						if (!cur_frm.is_dirty()) {
					...
					if (!doc.__unsaved) {
						...
						appyframe.model.remove_from_locals(doc.doctype, doc.name);
					} else {
	...
	is_value_type: function (fieldtype) {
		...
		return appyframe.model.no_value_type.indexOf(fieldtype) === -1;
	},

	is_non_std_field: function (fieldname) {
		return ![...appyframe.model.std_fields_list, ...appyframe.model.child_table_field_list].includes(
			fieldname
		);
	},
	...
	get_std_field: function (fieldname, ignore = false) {
		var docfield = $.map(
			[].concat(appyframe.model.std_fields).concat(appyframe.model.std_fields_table),
			function (d) {
		...
		if (!docfield.length) {
			...
			if (ignore) {
				...
				appyframe.msgprint(__("Unknown Column: {0}", [fieldname]));
			}
		}
		return docfield[0];
	},
	...
	set_in_localstorage: function (doctype, docs) {
		try {
			...
			appyframe.model.clear_local_storage();
			localStorage["_doctype:" + doctype] = JSON.stringify(docs);
		}
	},
	...
	with_doctype: function (doctype, callback, async) {
		if (locals.DocType[doctype]) {
			...
			let cached_docs = appyframe.model.get_from_localstorage(doctype);
			...
			return appyframe.call({
				method: "appyframe.desk.form.load.getdoctype",
				type: "GET",
				args: {
					doctype: doctype,
					with_parent: 1,
				...
				callback: function (r) {
					if (r.exc) {
						appyframe.msgprint(__("Unable to load: {0}", [__(doctype)]));
						throw "No doctype";
					}
					if (r.message == "use_cache") {
						appyframe.model.sync(cached_doc);
					} else {
						appyframe.model.set_in_localstorage(doctype, r.docs);
					}
					appyframe.model.init_doctype(doctype);

					if (r.user_settings) {
						// remember filters and other settings from last view
						appyframe.model.user_settings[doctype] = JSON.parse(r.user_settings);
						appyframe.model.user_settings[doctype].updated_on = moment().toString();
					}
					callback && callback(r);
				},
			});
	...
	init_doctype: function (doctype) {
		...
		if (meta.__templates) {
			$.extend(appyframe.templates, meta.__templates);
		}
	},
	...
	with_doc: function (doctype, name, callback) {
		return new Promise((resolve) => {
			...
			if (
				...
				resolve(appyframe.get_doc(doctype, name));
			} else {
				return appyframe.call({
					method: "appyframe.desk.form.load.getdoc",
					type: "GET",
					args: {
						doctype: doctype,
						name: name,
	...
	get_docinfo: function (doctype, name) {
		return (appyframe.model.docinfo[doctype] && appyframe.model.docinfo[doctype][name]) || null;
	},

	set_docinfo: function (doctype, name, key, value) {
		if (appyframe.model.docinfo[doctype] && appyframe.model.docinfo[doctype][name]) {
			appyframe.model.docinfo[doctype][name][key] = value;
		}
	},

	get_shared: function (doctype, name) {
		return appyframe.model.get_docinfo(doctype, name).shared;
	},

	get_server_module_name: function (doctype) {
		var dt = appyframe.model.scrub(doctype);
		var module = appyframe.model.scrub(locals.DocType[doctype].module);
		var app = appyframe.boot.module_app[module];
	...
	can_create: function (doctype) {
		return appyframe.boot.user.can_create.indexOf(doctype) !== -1;
	},

	can_select: function (doctype) {
		if (appyframe.boot.user) {
			return appyframe.boot.user.can_select.indexOf(doctype) !== -1;
		}
	},

	can_read: function (doctype) {
		if (appyframe.boot.user) {
			return appyframe.boot.user.can_read.indexOf(doctype) !== -1;
		}
	},

	can_write: function (doctype) {
		return appyframe.boot.user.can_write.indexOf(doctype) !== -1;
	},

	can_get_report: function (doctype) {
		return appyframe.boot.user.can_get_report.indexOf(doctype) !== -1;
	},

	can_delete: function (doctype) {
		if (!doctype) return false;
		return appyframe.boot.user.can_delete.indexOf(doctype) !== -1;
	},

	can_submit: function (doctype) {
		if (!doctype) return false;
		return appyframe.boot.user.can_submit.indexOf(doctype) !== -1;
	},

	can_cancel: function (doctype) {
		if (!doctype) return false;
		return appyframe.boot.user.can_cancel.indexOf(doctype) !== -1;
	},

	has_workflow: function (doctype) {
		return appyframe.get_list("Workflow", { document_type: doctype, is_active: 1 }).length;
	},
	...
	is_single: function (doctype) {
		if (!doctype) return false;
		return appyframe.boot.single_types.indexOf(doctype) != -1;
	},
	...
	can_import: function (doctype, frm, meta = null) {
		...
		if (appyframe.user_roles.includes("System Manager")) return true;

		if (frm) return frm.perm[0].import === 1;
		return appyframe.boot.user.can_import.indexOf(doctype) !== -1;
	},
	...
	can_export: function (doctype, frm) {
		...
		return appyframe.boot.user.can_export.indexOf(doctype) !== -1;
	},

	can_print: function (doctype, frm) {
		if (frm) return frm.perm[0].print === 1;
		return appyframe.boot.user.can_print.indexOf(doctype) !== -1;
	},

	can_email: function (doctype, frm) {
		if (frm) return frm.perm[0].email === 1;
		return appyframe.boot.user.can_email.indexOf(doctype) !== -1;
	},

	can_share: function (doctype, frm) {
		let disable_sharing = cint(appyframe.sys_defaults.disable_document_sharing);

		if (disable_sharing && appyframe.session.user !== "Administrator") {
			return false;
		}
		...
		return appyframe.boot.user.can_share.indexOf(doctype) !== -1;
	},

	has_value: function (dt, dn, fn) {
		// return true if property has value
		var val = locals[dt] && locals[dt][dn] && locals[dt][dn][fn];
		var df = appyframe.meta.get_docfield(dt, fn, dn);
	...
	get_list: function (doctype, filters) {
		...
		return appyframe.utils.filter_dict(docsdict, filters);
	},
	...
	get_value: function (doctype, filters, fieldname, callback) {
		if (callback) {
			...
			if (
				...
				var l = appyframe.get_list(doctype, filters);
	...
	set_value: function (
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => appyframe.model.trigger(key, value, doc, skip_dirty_trigger));
			} else {
	...
	on: function (doctype, fieldname, fn) {
		...
		*/
		/* example: appyframe.model.on("Customer", "age", function(fieldname, value, doc) {
		  if(doc.age < 16) {
		   	appyframe.msgprint("Warning, Customer must atleast be 16 years old.");
		    raise "CustomerAgeError";
		  }
		}) */
		appyframe.provide("appyframe.model.events." + doctype);
		if (!appyframe.model.events[doctype][fieldname]) {
			appyframe.model.events[doctype][fieldname] = [];
		}
		appyframe.model.events[doctype][fieldname].push(fn);
	},
		...
		function enqueue_events(events) {
			...
			for (const fn of events) {
				...
				tasks.push(() => {
					...
					// or use the default promise appyframe.after_ajax
					if (return_value && return_value.then) {
						return return_value;
					} else {
						return appyframe.after_server_call();
					}
				});
			}
		}

		if (appyframe.model.events[doc.doctype]) {
			enqueue_events(appyframe.model.events[doc.doctype][fieldname]);
			enqueue_events(appyframe.model.events[doc.doctype]["*"]);
		}
	...
	get_doc: function (doctype, name) {
		...
		if ($.isPlainObject(name)) {
			var doc = appyframe.get_list(doctype, name);
	...
	get_children: function (doctype, parent, parentfield, filters) {
		...
		if ($.isPlainObject(doctype)) {
			...
			doc = appyframe.get_doc(doctype, parent);
		}

		var children = doc[parentfield] || [];
		if (filters) {
			return appyframe.utils.filter_dict(children, filters);
		} else {
			return children;
		}
	},
	...
	remove_from_locals: function (doctype, name) {
		...
		if (appyframe.views.formview[doctype]) {
			delete appyframe.views.formview[doctype].frm.opendocs[name];
		}
	},
	...
	get_no_copy_list: function (doctype) {
		...
		var docfields = appyframe.get_doc("DocType", doctype).fields || [];
	...
	delete_doc: function (doctype, docname, callback) {
		let title = docname;
		const title_field = appyframe.get_meta(doctype).title_field;
		if (appyframe.get_meta(doctype).autoname == "hash" && title_field) {
			const value = appyframe.model.get_value(doctype, docname, title_field);
			if (value) {
				title = `${value} (${docname})`;
			}
		}
		appyframe.confirm(__("Permanently delete {0}?", [title.bold()]), function () {
			return appyframe.call({
				method: "appyframe.client.delete",
				args: {
					doctype: doctype,
					name: docname,
				},
				...
				callback: function (r, rt) {
					if (!r.exc) {
						appyframe.utils.play_sound("delete");
						appyframe.model.clear_doc(doctype, docname);
	...
	round_floats_in: function (doc, fieldnames) {
		...
		if (!fieldnames) {
			fieldnames = appyframe.meta.get_fieldnames(doc.doctype, doc.parent, {
	...
	validate_missing: function (doc, fieldname) {
		if (!doc[fieldname]) {
			appyframe.throw(
				__("Please specify") +
					": " +
					__(appyframe.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name))
			);
		}
	},
	...
	is_numeric_field: function (fieldtype) {
		...
		return appyframe.model.numeric_fieldtypes.includes(fieldtype);
	},
	...
	set_default_views_for_doctype(doctype, frm) {
		appyframe.model.with_doctype(doctype, () => {
			...
			if (meta.is_calendar_and_gantt && appyframe.views.calendar[doctype]) {
			...
			if (doctype === "Communication" && appyframe.boot.email_accounts.length) {
...
// legacy
appyframe.get_doc = appyframe.model.get_doc;
appyframe.get_children = appyframe.model.get_children;
appyframe.get_list = appyframe.model.get_list;
