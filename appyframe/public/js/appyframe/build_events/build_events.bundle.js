...
appyframe.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						appyframe.assets._executed = appyframe.assets._executed.filter(
		...
		// update assets json
		appyframe.call("appyframe.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				appyframe.boot.assets_json = r.message;

				if (appyframe.hot_update) {
					appyframe.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
