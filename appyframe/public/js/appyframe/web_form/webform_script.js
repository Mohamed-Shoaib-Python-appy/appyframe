...
appyframe.ready(function () {
	let web_form_doc = appyframe.web_form_doc;
	let reference_doc = appyframe.reference_doc;
	...
	function show_login_prompt() {
		if (appyframe.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new appyframe.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(appyframe.form.formatters.FileSize($(this).text()));
		});
	}
