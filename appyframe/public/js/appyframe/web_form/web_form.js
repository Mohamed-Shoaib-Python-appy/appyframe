...
appyframe.provide("appyframe.web_form");
import EventEmitterMixin from "../../appyframe/event_emitter";

export default class WebForm extends appyframe.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		appyframe.web_form = this;
		appyframe.web_form.events = {};
		Object.assign(appyframe.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		appyframe.init_client_script && appyframe.init_client_script();
		this.setup_listeners();
		appyframe.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = appyframe.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${appyframe.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".appyframe-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		appyframe.form_dirty = false;

		appyframe.call({
			type: "POST",
			method: "appyframe.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					appyframe.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						appyframe.call({
							type: "POST",
							method: "appyframe.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			appyframe.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
