 ...
 *
 * @returns {appyframe.ui.Page}
 */

/**
 * @typedef {Object} appyframe.ui.Page
 */

appyframe.ui.make_app_page = function (opts) {
	opts.parent.page = new appyframe.ui.Page(opts);
	return opts.parent.page;
};

appyframe.ui.pages = {};

appyframe.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		this.make();
		appyframe.ui.pages[appyframe.get_route_str()] = this;
	}
	...
	load_lib(callback) {
		appyframe.require(this.required_libs, callback);
	}

	add_main_section() {
		$(appyframe.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_page() {
		...
		this.card_layout && this.main.addClass("appyframe-card");
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (appyframe.utils.is_xs() || appyframe.utils.is_sm()) {
	...
	add_action_icon(icon, click, css_class = "", tooltip_label) {
		...
		if (!tooltip_label) {
			...
			tooltip_label = appyframe.unscrub(icon);
		}
	...
	get_icon_label(icon, label) {
		...
		return `${icon ? appyframe.utils.icon(icon_name, size) : ""} <span class="hidden-xs"> ${__(
			label
		)} </span>`;
	}
	...
	set_action(btn, opts) {
		...
		appyframe.ui.keys.get_shortcut_group(this).add(btn, text_span.length ? text_span : btn);
	}
	...
	add_dropdown_item({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${appyframe.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			appyframe.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
			...
			$group = $(
				`<div class="inner-group-button" data-label="${encodeURIComponent(label)}">
					<button type="button" class="btn btn-default ellipsis" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
						${label}
						${appyframe.utils.icon("select", "xs")}
					</button>
	...
	set_title(title, icon = null, strip = true, tab_title = "", tooltip_label = "") {
		...
		this.title = title;
		appyframe.utils.set_title(tab_title || title);
		if (icon) {
			title = `${appyframe.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || "btn-default"} ${opts.btn_size || "btn-sm"} ellipsis">
				${opts.icon ? appyframe.utils.icon(opts.icon) : ""}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		appyframe.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = appyframe.ui.form.make_control({
			df: df,
