...
appyframe.provide("appyframe.ui.keys.handlers");

appyframe.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = appyframe.ui.keys.get_key(e);
		if (appyframe.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = appyframe.ui.keys.handlers[key].length; i < l; i++) {
				var handler = appyframe.ui.keys.handlers[key][i];
...
appyframe.ui.keys.standard_shortcuts = standard_shortcuts;
appyframe.ui.keys.add_shortcut = ({
	...
	appyframe.ui.keys.off(shortcut, page);
	// attach new handler
	appyframe.ui.keys.on(shortcut, handler);
...
appyframe.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (appyframe.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(appyframe.utils.to_title_case)
					.join("+");
				if (appyframe.utils.is_mac()) {
	...
	let dialog = new appyframe.ui.Dialog({
		...
		on_hide() {
			appyframe.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	appyframe.ui.keys.is_dialog_shown = true;
};

appyframe.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = appyframe.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
appyframe.ui.keys.on = function (key, handler) {
	if (!appyframe.ui.keys.handlers[key]) {
		appyframe.ui.keys.handlers[key] = [];
	}
	appyframe.ui.keys.handlers[key].push(handler);
};

appyframe.ui.keys.off = function (key, page) {
	let handlers = appyframe.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	appyframe.ui.keys.handlers[key] = handlers.filter((h) => {
...
appyframe.ui.keys.add_shortcut({
	shortcut: "ctrl+s",
	action: function (e) {
		appyframe.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
appyframe.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

appyframe.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

appyframe.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

appyframe.ui.keys.on("ctrl+down", function (e) {
	var grid_row = appyframe.ui.form.get_open_grid_form();
	grid_row &&
...
appyframe.ui.keys.on("ctrl+up", function (e) {
	var grid_row = appyframe.ui.form.get_open_grid_form();
	grid_row &&
		grid_row.toggle_view(false, function () {
			grid_row.open_prev();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	appyframe.ui.keys.key_map[65 + i] = letter;
});

// keyCode map
appyframe.ui.keyCode = {
	ESCAPE: 27,
	LEFT: 37,
	RIGHT: 39,
	UP: 38,
