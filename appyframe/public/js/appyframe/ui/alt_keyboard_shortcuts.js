appyframe.provide("appyframe.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
appyframe.ui.keys.shortcut_groups = shortcut_groups;

appyframe.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new appyframe.ui.keys.AltShortcutGroup());
	}
...
appyframe.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (appyframe.ui.keys.key_map[e.which] || "").toLowerCase();
...
appyframe.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = appyframe.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
