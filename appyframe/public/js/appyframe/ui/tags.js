...
appyframe.ui.Tags = class {
	...
	bind() {
		...
		const select_tag = function () {
			const tagValue = appyframe.utils.xss_sanitise(me.$input.val());
			me.addTag(tagValue);
			me.$input.val("");
		};
	...
	removeTag(label) {
		label = appyframe.utils.xss_sanitise(label);
	...
	get_tag(label) {
		let colored = true;
		let $tag = appyframe.get_data_pill(
			label,
			label,
			(target, pill_wrapper) => {
