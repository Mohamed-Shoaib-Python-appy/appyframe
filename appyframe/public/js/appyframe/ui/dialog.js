...
appyframe.ui.open_dialogs = [];

appyframe.ui.Dialog = class Dialog extends appyframe.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		this.$wrapper = appyframe.get_modal("", "");
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				appyframe.ui.form.get_open_grid_form?.()?.hide_form();

				if (appyframe.ui.open_dialogs[appyframe.ui.open_dialogs.length - 1] === me) {
					appyframe.ui.open_dialogs.pop();
					if (appyframe.ui.open_dialogs.length) {
						window.cur_dialog =
							appyframe.ui.open_dialogs[appyframe.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				appyframe.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("appyframe.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(appyframe.utils.icon(icon));
