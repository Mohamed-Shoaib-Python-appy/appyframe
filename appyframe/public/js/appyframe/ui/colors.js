...
appyframe.ui.color = {
	get: function (color_name, shade) {
		...
		return appyframe.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(appyframe.ui.color_map);
		if (color_names.includes(color_name)) {
			return appyframe.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(appyframe.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(appyframe.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in appyframe.ui.color_map) {
			const colors = appyframe.ui.color_map[key];
...
appyframe.ui.color_map = appyframe.ui.color.get_color_map();
