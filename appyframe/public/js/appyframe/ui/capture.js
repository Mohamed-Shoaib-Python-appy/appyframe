// appyframe.ui.Capture
// Author - Achilles Rasquinha <achilles@appyframe.io>
 ...
 *
 * @example
 * appyframe._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
appyframe._.get_data_uri = (element) => {
...
/**
 * @description Appyframe's Capture object.
 *
 * @example
 * const capture = appyframe.ui.Capture()
 * capture.show()
 *
 ...
 */
appyframe.ui.Capture = class {
	constructor(options = {}) {
		this.options = appyframe.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...appyframe.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(appyframe.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (appyframe.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(appyframe.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					appyframe.show_alert(appyframe.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = appyframe._.get_data_uri(me.video);
...
appyframe.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
appyframe.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
appyframe.ui.Capture.TEMPLATE = `
<div class="appyframe-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${appyframe.ui.Capture.ERR_MESSAGE}</video>
	</div>
