...
appyframe.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = appyframe.ui.keys.get_key(e);
			let increment_by;
	...
	toggle_theme(theme) {
		...
		appyframe.show_alert(__("Theme Changed"), 3);

		appyframe.xcall("appyframe.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
appyframe.ui.add_system_theme_switch_listener = () => {
	appyframe.ui.dark_theme_media_query.addEventListener("change", () => {
		appyframe.ui.set_theme();
	});
};

appyframe.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

appyframe.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = appyframe.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
