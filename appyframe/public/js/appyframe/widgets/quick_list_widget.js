...
export default class QuickListWidget extends Widget {
	...
	set_actions() {
		...
		if (appyframe.model.can_create(this.document_type)) {
	...
	setup_add_new_button() {
		...
		this.add_new_button.on("click", () => {
			appyframe.set_route(
				appyframe.utils.generate_route({
					type: "doctype",
	...
	setup_refresh_list_button() {
		this.refresh_list = $(
			`<div class="refresh-list btn btn-xs pull-right" title="${__("Refresh List")}">
				${appyframe.utils.icon("es-line-reload", "sm")}
			</div>`
		);
	...
	setup_filter(doctype) {
		...
		this.filters = appyframe.utils.process_filter_expression(this.quick_list_filter);
	...
	setup_quick_list_item(doc) {
		const indicator = appyframe.get_indicator(doc, this.document_type);
			...
			<div class="quick-list-item">
				<div class="ellipsis left">
					...
					<div class="timestamp text-muted">
						${appyframe.datetime.prettyDate(doc.modified)}
					</div>
				</div>
			</div>
		`);
		...
		let icon_to_append = `<div class="right-arrow">${appyframe.utils.icon("right", "xs")}</div>`;
		if (appyframe.utils.is_rtl(appyframe.boot.lang)) {
			icon_to_append = `<div class="left-arrow">${appyframe.utils.icon("left", "xs")}</div>`;
		}
		...
		$quick_list_item.click((e) => {
			...
			appyframe.set_route(`${appyframe.utils.get_form_link(this.document_type, doc.name)}`);
		});
	...
	set_body() {
		...
		appyframe.model.with_doctype(this.document_type, () => {
			let fields = ["name"];

			// get name of title field
			if (!this.title_field_name) {
				let meta = appyframe.get_meta(this.document_type);
			...
			this.has_status_field = appyframe.meta.has_field(this.document_type, "status");
			...
			let workflow_fieldname = appyframe.workflow.get_state_fieldname(this.document_type);
			workflow_fieldname && fields.push(workflow_fieldname);
			fields.push("modified");

			let add_fields = appyframe.listview_settings?.[this.document_type]?.add_fields;
			if (Array.isArray(add_fields)) {
				fields.push(...add_fields);
				fields = [...new Set(fields)];
			}

			let quick_list_filter = appyframe.utils.process_filter_expression(this.quick_list_filter);

			let args = {
				method: "appyframe.desk.reportview.get",
				args: {
	...
	set_footer() {
		...
		let filters = appyframe.utils.get_filter_from_json(this.quick_list_filter);
		let route = appyframe.utils.generate_route({ type: "doctype", name: this.document_type });
		this.see_all_button = $(`
