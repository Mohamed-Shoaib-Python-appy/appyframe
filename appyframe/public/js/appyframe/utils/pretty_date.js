function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = appyframe.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff =
		(new Date(appyframe.datetime.now_datetime().replace(/-/g, "/")).getTime() - date.getTime()) /
		1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = appyframe.datetime.str_to_user ? appyframe.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="appyframe-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
appyframe.datetime.comment_when = comment_when;
appyframe.datetime.prettyDate = prettyDate;

appyframe.datetime.refresh_when = function () {
	if (jQuery) {
		$(".appyframe-timestamp").each(function () {
