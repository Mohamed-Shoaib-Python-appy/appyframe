...
appyframe.provide("appyframe.tools");

appyframe.tools.downloadify = function (data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		appyframe.msgprint(
			__("Export not allowed. You need {0} role to export.", [appyframe.utils.comma_or(roles)])
		);
		return;
	}

	var filename = title + ".csv";
	var csv_data = appyframe.tools.to_csv(data);
...
appyframe.markdown = function (txt) {
	if (!appyframe.md2html) {
		appyframe.md2html = new showdown.Converter();
	}
	...
	return appyframe.md2html.makeHtml(txt);
};

appyframe.tools.to_csv = function (data) {
	var res = [];
