...
appyframe.avatar = function (
	...
	if (user) {
		// desk
		user_info = appyframe.user_info(user);
	} else {
		// website
		let full_name = title || appyframe.get_cookie("full_name");
		user_info = {
			image: image_url === null ? appyframe.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: appyframe.get_abbr(full_name),
			color: appyframe.get_palette(full_name),
		};
	}
...
appyframe.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let abbr = appyframe.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = appyframe.get_palette(title);
...
appyframe.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			appyframe.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
	if (extra_users.length === 1) {
		html += appyframe.avatar(
			extra_users[0],
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => appyframe.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${appyframe.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
appyframe.ui.scroll = function (element, animate, additional_offset) {
...
appyframe.get_palette = function (txt) {
	if (!txt) return appyframe.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return appyframe.palette[idx % 8];
};

appyframe.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
appyframe.gravatars = {};
appyframe.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!appyframe.gravatars[email_id]) {
		// TODO: check if gravatar exists
		appyframe.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return appyframe.gravatars[email_id];
};
...
appyframe.get_cookie = function getCookie(name) {
	return appyframe.get_cookies()[name];
};

appyframe.get_cookies = function getCookies() {
...
appyframe.is_mobile = function () {
	return $(document).width() < 768;
};

appyframe.is_large_screen = function () {
	return $(document).height() > 1180;
};

appyframe.utils.xss_sanitise = function (string, options) {
...
appyframe.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = appyframe.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(appyframe.utils.xss_sanitise(url, { strategies: ["js"] }));
};

appyframe.utils.strip_url = (url) => {
...
appyframe.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
		...
		},
		{
			...
			label: __("Start Date"),
			reqd: 1,
			default: appyframe.datetime.nowdate(),
		},
		{
			fieldname: "end_date",
			fieldtype: "Date",
	...
	appyframe.prompt(
		...
		function (values) {
			appyframe.call({
				method: "appyframe.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
appyframe.utils.get_page_view_count = function (route) {
	return appyframe.call("appyframe.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};
