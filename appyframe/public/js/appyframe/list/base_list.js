...
appyframe.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || appyframe.router.doctype_layout || __(this.doctype);
		this.meta = appyframe.get_meta(this.doctype);
		this.settings = appyframe.listview_settings[this.doctype] || {};
		this.user_settings = appyframe.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = appyframe.is_large_screen() ? 100 : 20;
		this.data = [];
		this.method = "appyframe.desk.reportview.get";

		this.can_create = appyframe.model.can_create(this.doctype);
	...
	get_list_view_settings() {
		return appyframe
			.call("appyframe.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(appyframe.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			appyframe.model.std_fields_list.includes(fieldname) ||
			appyframe.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = appyframe.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!appyframe.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return appyframe.model.with_doctype(this.doctype);
	}
	...
	setup_page() {
		...
		!this.hide_card_layout && this.page.main.addClass("appyframe-card");
	...
	setup_view_menu() {
		if (appyframe.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			const icon_map = {
				Image: "image-view",
				List: "list",
				Report: "small-file",
			...
			this.views_list = new appyframe.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		appyframe.breadcrumbs.add(this.meta.module, this.doctype);
	}
	...
	setup_side_bar() {
		...
		this.list_sidebar = new appyframe.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$frappe_list = $('<div class="appyframe-list">').appendTo(this.page.main);
	}
	...
	get_fields() {
		...
		return this.fields.map((f) => appyframe.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return appyframe.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	refresh() {
		...
		return appyframe.call(args).then((r) => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		...
		Object.assign(appyframe.boot.user_info, data.user_info);
		delete data.user_info;

		data = !Array.isArray(data) ? appyframe.utils.dict(data.keys, data.values) : data;
...
class FilterArea {
	constructor(list_view) {
		...
		this.debounced_refresh_list_view = appyframe.utils.debounce(
	...
	clear(refresh = true) {
		...
		return appyframe.run_serially(promises).then(() => {
			this.trigger_refresh = true;
			...
			doctype_fields
				.filter(
					(df) =>
						...
						(df.in_standard_filter && appyframe.model.is_value_type(df.fieldtype))
				)
				.map((df) => {
					...
					if (
						df.fieldtype == "Link" &&
						df.options &&
						appyframe.boot.treeviews.includes(df.options)
					) {
						condition = "descendants of (inclusive)";
					}
	...
	make_filter_list() {
			...
			<div class="btn-group">
				<button class="btn btn-default btn-sm filter-button">
					<span class="filter-icon">
						${appyframe.utils.icon("es-line-filter")}
					</span>
				...
				<button class="btn btn-default btn-sm filter-x-button" title="${__("Clear all filters")}">
					<span class="filter-icon">
						${appyframe.utils.icon("es-small-close")}
					</span>
				</button>
			</div>
		...
		this.filter_list = new appyframe.ui.FilterGroup({
...
appyframe.views.is_valid = (view_mode) => appyframe.views.view_modes.includes(view_mode);
