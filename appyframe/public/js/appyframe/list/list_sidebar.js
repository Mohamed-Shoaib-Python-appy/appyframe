...
appyframe.views.ListSidebar = class ListSidebar {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		var sidebar_content = appyframe.render_template("list_sidebar", { doctype: this.doctype });
	...
	setup_views() {
		...
		if (this.doctype === "Communication" && appyframe.boot.email_accounts.length) {
			this.sidebar.find('.list-link[data-view="Inbox"]').removeClass("hide");
			show_list_link = true;
		}

		if (appyframe.treeview_settings[this.doctype] || appyframe.get_meta(this.doctype).is_tree) {
			this.sidebar.find(".tree-link").removeClass("hide");
		}

		this.current_view = "List";
		var route = appyframe.get_route();
		if (route.length > 2 && appyframe.views.view_modes.includes(route[2])) {
	...
	setup_kanban_boards() {
		...
		appyframe.views.KanbanView.setup_dropdown_in_sidebar(this.doctype, $dropdown);
	}

	setup_keyboard_shortcuts() {
		this.sidebar.find(".list-link > a, .list-link > .btn-group > a").each((i, el) => {
			appyframe.ui.keys.get_shortcut_group(this.page).add($(el));
		});
	}

	setup_list_group_by() {
		this.list_group_by = new appyframe.views.ListGroupBy({
			doctype: this.doctype,
			sidebar: this,
			list_view: this.list_view,
			page: this.page,
	...
	get_stats() {
		...
		appyframe.call({
			method: "appyframe.desk.reportview.get_sidebar_stats",
			type: "GET",
			args: {
				stats: me.stats,
				doctype: me.doctype,
			...
			callback: function (r) {
				...
				appyframe.utils.setup_search(stats_dropdown, ".stat-link", ".stat-label");
			},
		});
	}
	...
	render_stat(stats) {
		...
		let tag_list = $(appyframe.render_template("list_sidebar_stat", args)).on(
			"click",
			".stat-link",
			(e) => {
	...
	add_insights_banner() {
		...
		const link = "https://appyframe.io/s/insights";
		const cta = "Appyframe Insights";
	...
	add_crm_banner() {
		...
		const message = "";
		const link =
			"https://appyframe.io/crm?utm_source=crm-sidebar&utm_medium=sidebar&utm_campaign=appyframe-ad";
		const cta = __("Switch to Appyframe CRM for smarter sales");
		this.add_banner(message, link, cta);
	}
	...
	add_helpdesk_banner() {
		...
		const message = "";
		const link =
			"https://appyframe.io/helpdesk?utm_source=support-sidebar&utm_medium=sidebar&utm_campaign=appyframe-ad";
		const cta = __("Upgrade your support experience with Appyframe Helpdesk");
		this.add_banner(message, link, cta);
	}
};
