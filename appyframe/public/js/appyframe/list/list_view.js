...
appyframe.views.ListView = class ListView extends appyframe.views.BaseList {
	static load_last_view() {
		const route = appyframe.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = appyframe.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			appyframe.set_route(
				"list",
				appyframe.router.doctype_layout || doctype,
				appyframe.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		const meta = appyframe.get_meta(this.doctype);
		this.is_large_table = meta?.is_large_table;

		this.debounced_refresh = appyframe.utils.debounce(
	...
	has_permissions() {
		return appyframe.perm.has_perm(this.doctype, 0, "read");
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			appyframe.set_route("");
			appyframe.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(appyframe.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = appyframe.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${appyframe.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		appyframe.msgprint(
			appyframe.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? appyframe.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						appyframe.boot.link_title_doctypes.includes(df.options)
					) {
						appyframe.model.with_doctype(df.options, () => {
							const meta = appyframe.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = appyframe.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = appyframe.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				appyframe.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !appyframe.boot.read_only) {
			const doctype_name = __(appyframe.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (allowed_filter_types.includes(f[2]) && appyframe.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		appyframe.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = appyframe.meta.get_docfield.bind(null, this.doctype);
		...
		if (appyframe.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (appyframe.has_indicator(this.doctype) && df.fieldname === "status") {
						return false;
					}
	...
	get_no_result_message() {
		...
		let empty_state_image =
			...
			"/assets/appyframe/images/ui-states/list-empty-state.svg";
	...
	before_refresh() {
		if (appyframe.route_options && this.filter_area) {
	...
	before_render() {
		...
		appyframe.model.user_settings.save(this.doctype, "last_view", this.view_name);
		...
		const $columns = this.columns
			.map((col) => {
				let classes = [
					...
					appyframe.model.is_numeric_field(col.df) ? "text-right" : "",
				].join(" ");
		...
		const right_html = `
			...
			<span class="level-item list-liked-by-me hidden-xs">
				<span title="${__("Liked by me")}">
					${appyframe.utils.icon("es-solid-heart", "sm", "like-icon")}
				</span>
			</span>
		`;
	...
	get_column_html(col, doc) {
		...
		let translated_doctypes = appyframe.boot?.translated_doctypes || [];
		...
		const format = () => {
			if (df.fieldtype === "Percent") {
				...
				return appyframe.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? appyframe.utils.escape_html(value_display)
						: value_display;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${appyframe.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${appyframe.utils.guess_colour(
					_value
				)} ellipsis"
			...
			} else if (appyframe.model.html_fieldtypes.includes(df.fieldtype)) {
				html = `<span class="ellipsis">
					${_value}
				</span>`;
			} else {
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${appyframe.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${appyframe.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		const css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			appyframe.model.is_numeric_field(df) ? "text-right" : "",
		].join(" ");
	...
	get_tags_html(user_tags, limit, colored = false) {
		let get_tag_html = (tag) => {
			...
			if (tag) {
				if (colored) {
					color = appyframe.get_palette(tag);
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments d-flex align-items-center">
					${appyframe.avatar_group(assigned_users, 3, { filterable: true })[0].outerHTML}
				</div>`;
		}
		...
		if (this.list_view_settings && !this.list_view_settings.disable_comment_count) {
			comment_count = `<span class="comment-count d-flex align-items-center">
				${appyframe.utils.icon("es-line-chat-alt")}
	...
	get_form_link(doc) {
		...
		return `/app/${encodeURIComponent(
			appyframe.router.slug(appyframe.router.doctype_layout || this.doctype)
		)}/${encodeURIComponent(cstr(doc.name))}`;
	}

	get_seen_class(doc) {
		const seen_by = doc._seen ? JSON.parse(doc._seen) : [];
		return seen_by.includes(appyframe.session.user) ? "" : "bold";
	}

	get_like_html(doc) {
		const liked_by = doc._liked_by ? JSON.parse(doc._liked_by) : [];
		const is_liked = liked_by.includes(appyframe.session.user);
		const title = liked_by.map((u) => appyframe.user_info(u).fullname).join(", ");
	...
	get_subject_text(doc, title) {
		...
		if (appyframe.model.html_fieldtypes.includes(subject_field.fieldtype)) {
			// NOTE: this is very slow, so only do it for HTML fields
			return appyframe.utils.html2text(value);
		} else {
			return value;
		}
	}
	...
	get_indicator_html(doc, show_workflow_state) {
		const indicator = appyframe.get_indicator(doc, this.doctype, show_workflow_state);
	...
	get_indicator_dot(doc) {
		const indicator = appyframe.get_indicator(doc, this.doctype);
		if (!indicator) return "";
	...
	get_image_url(doc) {
		...
		if (window.cordova && !appyframe.utils.is_url(url)) {
			url = appyframe.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = appyframe.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = appyframe.session.user;
				}
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if (link) {
				appyframe.set_route(link.pathname);
				return false;
			}
		});
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", (e) => {
			...
			appyframe.ui.toggle_like($this, doctype, name);
	...
	setup_realtime_updates() {
		...
		appyframe.realtime.doctype_subscribe(this.doctype);
		appyframe.realtime.off("list_update");
		appyframe.realtime.on("list_update", (data) => {
	...
	disable_realtime_updates() {
		appyframe.realtime.doctype_unsubscribe(this.doctype);
	...
	process_document_refreshes() {
		...
		appyframe.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = appyframe.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return appyframe.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (appyframe.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (appyframe.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					appyframe.set_route("list", "data-import", {
		...
		if (
			appyframe.model.can_create("Custom Field") &&
			appyframe.model.can_create("Property Setter")
		) {
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						appyframe.set_route("form", "doctype", doctype);
					} else if (!this.meta.custom) {
						appyframe.set_route("form", "customize-form", {
							doc_type: doctype,
						});
					}
				},
		...
		if (appyframe.user.has_role("System Manager") && appyframe.boot.developer_mode === 1) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => appyframe.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		appyframe.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: appyframe.get_meta(this.doctype),
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (appyframe.model.has_workflow(this.doctype)) {
			const actions = appyframe.workflow.get_all_transition_actions(this.doctype);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						...
						appyframe
							.xcall("appyframe.model.workflow.bulk_workflow_approval", {
	...
	toggle_workflow_actions() {
		if (!appyframe.model.has_workflow(this.doctype)) return;
		...
		appyframe
			.xcall("appyframe.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				appyframe.model.is_value_type(field_doc) &&
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					appyframe.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk printing
		if (appyframe.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			appyframe.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!appyframe.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (appyframe.model.can_cancel(doctype) && !appyframe.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (appyframe.model.can_delete(doctype) && !appyframe.model.has_workflow(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		if (!params.toString() && appyframe.route_options) {
			params = new Map(Object.entries(appyframe.route_options));
		}
		...
		params.forEach((value, field) => {
			...
			if (!doctype) {
				doctype = appyframe.meta.get_doctype_for_field(this.doctype, field);
			}
...
appyframe.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return appyframe.views.list_view[route];
};
...
class ElementFactory {
	...
	create_like_element(doctype) {
		...
		like.innerHTML = appyframe.utils.icon("es-solid-heart", "sm", "like-icon");
