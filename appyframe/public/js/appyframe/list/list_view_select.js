...
appyframe.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = appyframe.utils.to_title_case(route[2] || "");
		if (route.length > 2 && appyframe.views.view_modes.includes(view_name)) {
	...
	set_route(view, calendar_name) {
		...
		if (search_params) {
			appyframe.route_options = Object.fromEntries(search_params);
		}
	...
	setup_views() {
		const views = {
			...
			Report: {
				...
				current_view_handler: () => {
					...
					if (appyframe.get_route().length > 3) {
						default_action = {
			...
			Calendar: {
				condition: appyframe.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && appyframe.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(appyframe.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => appyframe.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					appyframe.treeview_settings[this.doctype] ||
					appyframe.get_meta(this.doctype).is_tree,
		...
		appyframe.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => appyframe.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return appyframe.utils.to_title_case(appyframe.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(appyframe.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			appyframe.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						appyframe.set_route("list", doctype, "kanban", board.name);
					} else {
						appyframe.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			appyframe.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			appyframe.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					appyframe.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return appyframe.db
			...
			.then((result) => {
				...
				if (appyframe.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = appyframe.boot.email_accounts;
	...
	slug() {
		return appyframe.router.slug(appyframe.router.doctype_layout || this.doctype);
	}
};
