appyframe.ui.form.FormViewers = class FormViewers {
	...
	refresh() {
		...
		let avatar_group = appyframe.avatar_group(this.active_users, 5, {
			align: "left",
			overlap: true,
		});
	...
	setup_events() {
		let me = this;
		appyframe.realtime.off("doc_viewers");
		appyframe.realtime.on("doc_viewers", function (data) {
			me.update_users(data);
		});
	}

	async update_users({ doctype, docname, users = [] }) {
		users = users.filter((u) => u != appyframe.session.user);
	...
	async fetch_user_info(users) {
		...
		for (let user of users) {
			if (!appyframe.boot.user_info[user]) unknown_users.push(user);
		}
		if (!unknown_users.length) return;

		const data = await appyframe.xcall("appyframe.desk.form.load.get_user_info_for_viewers", {
			users: unknown_users,
		});
		Object.assign(appyframe.boot.user_info, data);
	}
};
