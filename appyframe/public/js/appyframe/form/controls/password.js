appyframe.ui.form.ControlPassword = class ControlPassword extends appyframe.ui.form.ControlData {
	...
	make_input() {
		...
		this.toggle_password = $(`
			<div class="toggle-password hidden">
				${appyframe.utils.icon("unhide", "sm")}
			</div>
		`).insertAfter(this.$input);
		...
		this.toggle_password.on("click", () => {
			if (this.$input.attr("type") === "password") {
				...
				this.toggle_password.html(appyframe.utils.icon("hide", "sm"));
			} else {
				this.$input.attr("type", "password");
				this.toggle_password.html(appyframe.utils.icon("unhide", "sm"));
			}
		});
	...
	get_password_strength(value) {
		...
		appyframe.call({
			type: "POST",
			method: "appyframe.core.doctype.user.user.test_password_strength",
			args: {
				new_password: value || "",
			},
			callback: function (r) {
