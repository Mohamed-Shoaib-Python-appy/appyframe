appyframe.ui.form.ControlDate = class ControlDate extends appyframe.ui.form.ControlData {
	...
	set_formatted_input(value) {
		...
		if (should_refresh) {
			this.datepicker.selectDate(appyframe.datetime.str_to_obj(value));
		}
	}
	set_date_options() {
		// webformTODO:
		...
		let lang = "en";
		appyframe.boot.user && (lang = appyframe.boot.user.language);
		...
		this.date_format = appyframe.defaultDateFormat;
		this.datepicker_options = {
			...
			firstDay: appyframe.datetime.get_first_day_of_the_week_index(),
			onSelect: () => {
	...
	get_now_date() {
		return appyframe.datetime
			.convert_to_system_tz(appyframe.datetime.now_date(true), false)
			.toDate();
	}
	set_t_for_today() {
		var me = this;
		this.$input.on("keydown", function (e) {
			if (e.which === 84) {
				...
				if (me.df.fieldtype == "Date") {
					me.set_value(appyframe.datetime.nowdate());
				}
				if (me.df.fieldtype == "Datetime") {
					me.set_value(appyframe.datetime.now_datetime());
				}
				if (me.df.fieldtype == "Time") {
					me.set_value(appyframe.datetime.now_time());
				}
				return false;
			}
		});
	...
	parse(value) {
		if (value) {
			...
			return appyframe.datetime.user_to_str(value, false, true);
		}
	}
	format_for_input(value) {
		if (value) {
			return appyframe.datetime.str_to_user(value, false, true);
		}
		return "";
	}
	validate(value) {
		if (value && !appyframe.datetime.validate(value)) {
			let sysdefaults = appyframe.sys_defaults;
			let date_format =
				sysdefaults && sysdefaults.date_format ? sysdefaults.date_format : "yyyy-mm-dd";
			appyframe.msgprint(__("Date {0} must be in format: {1}", [value, date_format]));
			return "";
		}
		return value;
	}
