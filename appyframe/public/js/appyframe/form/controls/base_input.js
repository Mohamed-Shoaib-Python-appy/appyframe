appyframe.ui.form.ControlInput = class ControlInput extends appyframe.ui.form.Control {
	...
	make_wrapper() {
		if (this.only_input) {
			this.$wrapper = $('<div class="form-group appyframe-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $(
				`<div class="appyframe-control">
	...
	refresh_input() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if (me.frm) {
				me.value = appyframe.model.get_value(me.doctype, me.docname, me.df.fieldname);
			} else if (me.doc) {
				...
				if (is_fetch_from_read_only) {
					$(me.disp_area).attr(
						...
						__(
							...
							me.df.fetch_from.split(".").map((value) => __(appyframe.unscrub(value)))
						)
					);
				}
			}
	...
	set_disp_area(value) {
		...
		if (this.df.fieldtype === "Data") {
			value = appyframe.utils.escape_html(value);
		}
		let doc = this.doc || (this.frm && this.frm.doc);
		let display_value = appyframe.format(value, this.df, { no_icon: true, inline: true }, doc);
