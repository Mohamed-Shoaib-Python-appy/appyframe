appyframe.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	appyframe.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${appyframe.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
