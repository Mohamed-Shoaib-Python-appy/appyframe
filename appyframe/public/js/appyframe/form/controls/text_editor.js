...
appyframe.ui.form.ControlTextEditor = class ControlTextEditor extends appyframe.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			appyframe.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = appyframe.ui.keys && appyframe.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: appyframe.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "appyframe.desk.search.get_names_for_mentions";
				let values = await appyframe.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? appyframe.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return appyframe.dom.remove_script_and_style(value);
	}
