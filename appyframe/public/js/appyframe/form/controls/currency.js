appyframe.ui.form.ControlCurrency = class ControlCurrency extends appyframe.ui.form.ControlFloat {
	...
	get_precision() {
		...
		if (typeof this.df.precision != "number" && !this.df.precision) {
			if (appyframe.boot.sysdefaults.currency_precision) {
				this.df.precision = appyframe.boot.sysdefaults.currency_precision;
			} else {
