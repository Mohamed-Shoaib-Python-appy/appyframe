appyframe.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='appyframe-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = appyframe.perm.get_field_display_status(
			this.df,
			appyframe.model.get_doc(this.doctype, this.docname),
		...
		let value = appyframe.model.get_value(this.doctype, this.docname, this.df.fieldname);
	...
	show_translatable_button(value) {
		...
		if (
			...
			!appyframe.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!appyframe.views || !appyframe.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new appyframe.views.TranslationManager({
						df: this.df,
