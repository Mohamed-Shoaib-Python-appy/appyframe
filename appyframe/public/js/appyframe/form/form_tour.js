appyframe.ui.form.FormTour = class FormTour {
	constructor({ frm }) {
		this.frm = frm;
		this.driver_steps = [];
	}
	...
	init_driver() {
		...
		appyframe.router.on("change", () => this.driver.reset());
	...
	async init({ tour_name, on_finish }) {
		if (tour_name) {
			...
			const doctype_tour_exists = await appyframe.db.exists("Form Tour", this.frm.doctype);
			if (doctype_tour_exists) {
				this.tour = await appyframe.db.get_doc("Form Tour", this.frm.doctype);
			} else {
				this.tour = { steps: appyframe.tour[this.frm.doctype] };
			}
		}
		...
		this.tour.steps.forEach((step) => {
			const on_next = () => {
				...
				if (field?.tab && !field.tab.is_active()) {
					...
					appyframe.utils.sleep(200).then(() => {
						this.start(step.idx);
						this.driver.overlay.refresh();
					});
				}
	...
	get_step(step_info, on_next, on_prev) {
		...
		let element = `.appyframe-control[data-fieldname='${fieldname}']`;
		...
		if (is_table_field) {
			...
			element = `.grid-row-open .appyframe-control[data-fieldname='${fieldname}']`;
		}
	...
	add_new_row_step(step) {
		const $add_row = `.appyframe-control[data-fieldname='${step.fieldname}'] .grid-add-row`;
		const add_row_step = {
			element: $add_row,
	...
	expand_row_and_proceed(step, start_from) {
		...
		appyframe.utils.sleep(300).then(() => this.driver.start(start_from));
	}
	...
	open_first_row_of(fieldname) {
		...
		$($close_row).one("click", () => {
			...
			appyframe.utils.scroll_to(next_element, true, 150, null, () => {
				this.driver.moveNext();
				appyframe.flags.disable_auto_scroll = false;
			});
			appyframe.flags.disable_auto_scroll = true;
		});
	}
