...
appyframe.quick_edit = function (doctype, name) {
	appyframe.db.get_doc(doctype, name).then((doc) => {
		appyframe.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

appyframe.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (appyframe.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	appyframe.quick_entry = new appyframe.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return appyframe.quick_entry.setup();
};

appyframe.ui.form.QuickEntryForm = class QuickEntryForm {
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = appyframe.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.dialog.onhide = () => (appyframe.quick_entry = null);
		this.dialog.show();
			...
			appyframe.call({
				...
				callback: function (r) {
					if (
						...
						appyframe.model.can_submit(me.doctype) &&
						!appyframe.model.has_workflow(me.doctype)
					) {
						...
						me.dialog.hide();
						// delete the old doc
						appyframe.model.clear_doc(me.dialog.doc.doctype, me.dialog.doc.name);
						me.dialog.doc = r.message;
						if (appyframe._from_link) {
							appyframe.ui.form.update_calling_link(me.dialog.doc);
						} else {
							if (me.after_insert) {
	...
	submit(doc) {
		var me = this;
		appyframe.call({
			method: "appyframe.client.submit",
			args: {
				doc: doc,
			},
			callback: function (r) {
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			appyframe.run_serially([() => appyframe.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			appyframe.route_options = appyframe.route_options || {};
			appyframe.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		this.doc.__run_link_triggers = false;
		appyframe.set_route("Form", this.doctype, this.doc.name);
	}
