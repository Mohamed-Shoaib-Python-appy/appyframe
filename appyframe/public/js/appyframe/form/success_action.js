...
appyframe.provide("appyframe.success_action");

appyframe.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = appyframe.boot.success_action.find(
	...
	show_alert() {
		appyframe.db.get_list(this.form.doctype, { limit: 2 }).then((result) => {
	...
	get default_actions() {
		return {
			new: {
				label: __("New"),
				action: (frm) => appyframe.new_doc(frm.doctype),
			},
			print: {
				label: __("Print"),
			...
			list: {
				label: __("View All"),
				action: (frm) => {
					appyframe.set_route("List", frm.doctype);
				},
			},
		};
	}
