...
appyframe.provide("appyframe.ui.form.handlers");
...
appyframe.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!appyframe.ui.form.handlers[doctype]) {
		appyframe.ui.form.handlers[doctype] = {};
	}
	if (!appyframe.ui.form.handlers[doctype][fieldname]) {
		appyframe.ui.form.handlers[doctype][fieldname] = [];
	}
	return appyframe.ui.form.handlers[doctype][fieldname];
};

appyframe.ui.form.on = appyframe.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = appyframe.ui.form.get_event_handler_list(doctype, fieldname);
...
appyframe.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = appyframe.ui.form.get_event_handler_list(doctype, fieldname);
...
appyframe.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

appyframe.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = appyframe.get_doc(doctype, name);
	...
	get_handlers(event_name, doctype) {
		...
		if (appyframe.ui.form.handlers[doctype] && appyframe.ui.form.handlers[doctype][event_name]) {
			$.each(appyframe.ui.form.handlers[doctype][event_name], function (i, fn) {
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (appyframe.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					appyframe.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && appyframe.dom.set_style(doctype.__css);

		this.trigger("setup");
	}
	log_error(caller, e) {
		appyframe.show_alert({ message: __("Error in Client Script."), indicator: "error" });
