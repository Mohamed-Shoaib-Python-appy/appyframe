...
appyframe.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make() {
		var sidebar_content = appyframe.render_template("form_sidebar", {
			doctype: this.frm.doctype,
			frm: this.frm,
			can_write: appyframe.model.can_write(this.frm.doctype, this.frm.docname),
		});
		...
		appyframe.ui.form.setup_user_image_event(this.frm);
	...
	bind_events() {
		...
		this.comments.on("click", function () {
			appyframe.utils.scroll_to(me.frm.footer.wrapper.find(".comment-box"), true);
		});

		this.like_icon.on("click", function () {
			appyframe.ui.toggle_like(me.like_wrapper, me.frm.doctype, me.frm.doc.name, function () {
				me.refresh_like();
			});
		});
	}
	...
	setup_keyboard_shortcuts() {
		...
		appyframe.ui.keys.get_shortcut_group(this.page).add(assignment_link);
	}
	...
	refresh() {
		if (this.frm.doc.__islocal) {
			...
			if (this.frm.doc.route && cint(appyframe.boot.website_tracking_enabled)) {
				...
				appyframe.utils.get_page_view_count(route).then((res) => {
					this.sidebar
						.find(".pageview-count")
			...
			appyframe.ui.form.set_user_image(this.frm);
		}
	}
			...
			appyframe.call({
				...
				callback: function (res) {
					...
					me.sidebar.find(".auto-repeat-status").on("click", function () {
						appyframe.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				},
			});
		}
	...
	make_tags() {
		...
		this.frm.tags = new appyframe.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new appyframe.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new appyframe.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new appyframe.ui.form.Share({
			frm: this.frm,
	...
	make_like() {
		...
		appyframe.ui.setup_like_popover(this.sidebar.find(".form-stats-likes"), ".like-icon");
	}
	...
	make_follow() {
		...
		this.follow_button.on("click", () => {
			...
			appyframe
				.call("appyframe.desk.form.document_follow.update_follow", {
					doctype: this.frm.doctype,
					doc_name: this.frm.doc.name,
					following: !is_followed,
				})
				.then(() => {
					appyframe.model.set_docinfo(
						this.frm.doctype,
						this.frm.doc.name,
	...
	refresh_like() {
		...
		const liked = appyframe.ui.is_liked(this.frm.doc);
		this.like_wrapper
	...
	make_review() {
		...
		if (appyframe.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new appyframe.ui.form.Review({
	...
	reload_docinfo(callback) {
		appyframe.call({
			method: "appyframe.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,
