...
appyframe.ui.form.Attachments = class Attachments {
	constructor(opts) {
		$.extend(this, opts);
	...
	add_attachment(attachment) {
		...
		let file_label = `
			<a href="${file_url}" target="_blank" title="${appyframe.utils.escape_html(file_name)}"
		...
		if (appyframe.model.can_write(this.frm.doctype, this.frm.name)) {
			remove_action = function (target_id) {
				appyframe.confirm(__("Are you sure you want to delete the attachment?"), function () {
		...
		const icon = `<a href="/app/file/${fileid}">
				${appyframe.utils.icon(attachment.is_private ? "es-line-lock" : "es-line-unlock", "sm ml-0")}
			</a>`;

		$(`<li class="attachment-row">`)
			.append(appyframe.get_data_pill(file_label, fileid, remove_action, icon))
	...
	remove_attachment(fileid, callback) {
		...
		return appyframe.call({
			method: "appyframe.desk.form.utils.remove_attach",
			type: "DELETE",
			args: {
				fid: fileid,
				dt: me.frm.doctype,
			...
			callback: function (r, rt) {
				if (r.exc) {
					if (!r._server_messages) appyframe.msgprint(__("There were errors"));
					return;
				}
