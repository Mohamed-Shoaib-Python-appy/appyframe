appyframe.ui.form.SidebarUsers = class {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	show_in_sidebar(users, type, show_alert) {
		...
		users.current.forEach((username) => {
			if (username === appyframe.session.user) {
				// current user
				return;
			}

			var user_info = appyframe.user_info(username);
			sidebar_users.push({
		...
		if (sidebar_users.length) {
			...
			this.parent.append(
				appyframe.render_template("users_in_sidebar", { users: sidebar_users })
			);
		} else {
	...
	show_alert(users, message) {
		if (users.length) {
			if (users.length === 1) {
				appyframe.show_alert(__("{0} is currently {1}", [users[0], message]));
			} else {
				appyframe.show_alert(
					__("{0} are currently {1}", [appyframe.utils.comma_and(users), message])
				);
			}
		}
	}
