...
appyframe.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = appyframe.boot.points;
	...
	update_points() {
		return appyframe
			.xcall("appyframe.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: appyframe.session.user,
			})
			.then((data) => {
				appyframe.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new appyframe.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return appyframe.msgprint(__("You do not have enough points"));
				}
				appyframe
					.xcall("appyframe.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = appyframe.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${appyframe.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${appyframe.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${appyframe.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${appyframe.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
