...
appyframe.ui.form.AssignTo = class AssignTo {
	constructor(opts) {
		$.extend(this, opts);
	...
	render(assignments) {
		...
		let avatar_group = appyframe.avatar_group(assigned_users, 5, {
			align: "left",
			overlap: true,
		});
		...
		avatar_group.click(() => {
			new appyframe.ui.form.AssignmentDialog({
	...
	add() {
		...
		if (this.frm.is_new()) {
			appyframe.throw(__("Please save the document before assignment"));
			return;
		}

		if (!me.assign_to) {
			me.assign_to = new appyframe.ui.form.AssignToDialog({
	...
	remove(owner) {
		if (this.frm.is_new()) {
			appyframe.throw(__("Please save the document before removing assignment"));
			return;
		}

		return appyframe
			.xcall("appyframe.desk.form.assign_to.remove", {
...
appyframe.ui.form.AssignToDialog = class AssignToDialog {
	constructor(opts) {
		$.extend(this, opts);
	...
	make() {
		...
		me.dialog = new appyframe.ui.Dialog({
	...
	assign_to_me() {
		...
		if (me.dialog.get_value("assign_to_me")) {
			assign_to.push(appyframe.session.user);
		}
		...
		return [
			...
			},
			{
				...
				get_data: function (txt) {
					return appyframe.db.get_link_options("User", txt, {
...
appyframe.ui.form.AssignmentDialog = class {
	constructor(opts) {
		this.frm = opts.frm;
	...
	make() {
		...
		this.assignment_list.removeClass("appyframe-control");
	...
	add_assignment(assignment) {
		return appyframe
			.xcall("appyframe.desk.form.assign_to.add", {
				doctype: this.frm.doctype,
				name: this.frm.docname,
	...
	remove_assignment(assignment) {
		return appyframe.xcall("appyframe.desk.form.assign_to.remove", {
			doctype: this.frm.doctype,
			name: this.frm.docname,
			assign_to: assignment,
		});
	...
	close_assignment(assignment) {
		return appyframe.xcall("appyframe.desk.form.assign_to.close", {
			doctype: this.frm.doctype,
			name: this.frm.docname,
			assign_to: assignment,
		});
	...
	get_assignment_row(assignment) {
		const row = $(`
			<div class="dialog-assignment-row" data-user="${assignment}">
				<div class="assignee">
					${appyframe.avatar(assignment)}
					${appyframe.user.full_name(assignment)}
				</div>
		...
		if (assignment === appyframe.session.user || this.frm.perm[0].write) {
			btn_group.append(`
				<button type="button" class="btn btn-default remove-btn" title="${__("Cancel")}">
				${appyframe.utils.icon("close")}
				</button>
			`);
