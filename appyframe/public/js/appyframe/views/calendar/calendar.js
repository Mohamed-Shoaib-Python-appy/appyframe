...
appyframe.provide("appyframe.views.calendar");
appyframe.provide("appyframe.views.calendars");

appyframe.views.CalendarView = class CalendarView extends appyframe.views.ListView {
	static load_last_view() {
		const route = appyframe.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = appyframe.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = appyframe.views.calendar[this.doctype] || {};
			this.calendar_name = appyframe.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new appyframe.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, appyframe.views.calendar[this.doctype]);
				resolve(options);
			} else {
				appyframe.model.with_doc("Calendar View", calendar_name, () => {
					const doc = appyframe.get_doc("Calendar View", calendar_name);
					if (!doc) {
						...
						appyframe.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
	...
	get required_libs() {
		let assets = [
			"assets/appyframe/js/lib/fullcalendar/fullcalendar.min.css",
			"assets/appyframe/js/lib/fullcalendar/fullcalendar.min.js",
		];
		let user_language = appyframe.boot.lang;
		if (user_language && user_language !== "en") {
			assets.push("assets/appyframe/js/lib/fullcalendar/locale-all.js");
		}
		return assets;
	}
};

appyframe.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(appyframe.boot.calendars, function (i, doctype) {
			if (appyframe.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					appyframe.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = appyframe.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(appyframe.utils.icon("left"));
		this.$wrapper
			.find(".fc-next-button span")
			.attr("class", "")
			.html(appyframe.utils.icon("right"));

		this.$wrapper.find(".fc-today-button").prepend(appyframe.utils.icon("today"));
	...
	get_system_datetime(date) {
		date._offset = moment(date).tz(appyframe.sys_defaults.time_zone)._offset;
		return appyframe.datetime.convert_to_system_tz(moment(date).locale("en"));
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			locale: appyframe.boot.lang,
			header: {
			...
			events: function (start, end, timezone, callback) {
				return appyframe.call({
					method: me.get_events_method || "appyframe.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (event) {
				...
				if (appyframe.model.can_read(doctype)) {
					appyframe.set_route("Form", doctype, event.name);
				}
			},
			...
			select: function (startDate, endDate, jsEvent, view) {
				...
				var event = appyframe.model.get_new_doc(me.doctype);
				...
				appyframe.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = appyframe.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = appyframe.datetime.convert_to_user_tz(d.start);
				d.end = appyframe.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!appyframe.datetime.validate(d.start) && d.end) {
				d.start = appyframe.datetime.add_days(d.end, -1);
			}

			if (d.start && !appyframe.datetime.validate(d.end)) {
				d.end = appyframe.datetime.add_days(d.start, 1);
			}

			me.fix_end_date_for_event_render(d);
			me.prepare_colors(d);

			d.title = appyframe.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = appyframe.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = appyframe.ui.color.get(color_name, "extra-light");
			d.textColor = appyframe.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!appyframe.ui.color.validate_hex(color) || !color) {
				color = appyframe.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = appyframe.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		appyframe.model.remove_from_locals(me.doctype, event.name);
		return appyframe.call({
			method: me.update_event_method || "appyframe.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					appyframe.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {
