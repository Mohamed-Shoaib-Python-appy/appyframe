...
appyframe.provide("appyframe.treeview_settings");
appyframe.provide("appyframe.views.trees");
window.cur_tree = null;

appyframe.views.TreeFactory = class TreeFactory extends appyframe.views.Factory {
	make(route) {
		appyframe.model.with_doctype(route[1], function () {
			var options = {
				doctype: route[1],
				meta: appyframe.get_meta(route[1]),
			};

			if (
				!appyframe.treeview_settings[route[1]] &&
				!appyframe.meta.get_docfield(route[1], "is_group")
			) {
				appyframe.msgprint(__("Tree view is not available for {0}", [route[1]]));
				return false;
			}
			$.extend(options, appyframe.treeview_settings[route[1]] || {});
			appyframe.views.trees[options.doctype] = new appyframe.views.TreeView(options);
		});
	}
	...
	on_show() {
		...
		let treeview = appyframe.views.trees[route[1]];
...
appyframe.views.TreeView = class TreeView {
	constructor(opts) {
		var me = this;
		...
		this.get_tree_nodes = me.opts.get_tree_nodes || "appyframe.desk.treeview.get_children";
	...
	get_permissions() {
		this.can_read = appyframe.model.can_read(this.doctype);
		this.can_create =
			appyframe.boot.user.can_create.indexOf(this.doctype) !== -1 ||
			appyframe.boot.user.in_create.indexOf(this.doctype) !== -1;
		this.can_write = appyframe.model.can_write(this.doctype);
		this.can_delete = appyframe.model.can_delete(this.doctype);
	}
	make_page() {
		var me = this;
		if (!this.opts || !this.opts.do_not_make_page) {
			this.parent = appyframe.container.add_page(this.page_name);
			appyframe.ui.make_app_page({ parent: this.parent, single_column: true });
			this.page = this.parent.page;
			appyframe.container.change_to(this.page_name);
			appyframe.breadcrumbs.add(
			...
			this.page.main.addClass("appyframe-card");
		} else {
			this.page = this.opts.page;
			$(this.page[0]).addClass("appyframe-card");
		}
	...
	make_filters() {
		var me = this;
		appyframe.treeview_settings.filters = [];
		$.each(this.opts.filters || [], function (i, filter) {
			if (appyframe.route_options && appyframe.route_options[filter.fieldname]) {
				filter.default = appyframe.route_options[filter.fieldname];
			}
	...
	make_tree() {
		...
		this.tree = new appyframe.ui.Tree({
			parent: this.body,
			label: use_label,
	...
	rebuild_tree() {
		...
		appyframe.call({
			method: "appyframe.utils.nestedset.rebuild_tree",
			args: {
				doctype: me.doctype,
			},
			callback: function (r) {
	...
	select_node(node) {
		...
		if (this.opts.view_template) {
			...
			$(
				appyframe.render_template(me.opts.view_template, {
					data: node.data,
					doctype: me.doctype,
				})
		...
		var toolbar = [
			{
				...
				click: function (node) {
					appyframe.set_route("Form", me.doctype, node.label);
				},
			},
			{
				label: __("Add Child"),
			...
			},
			{
				...
				condition: function (node) {
					...
					if (me.doctype && appyframe.get_meta(me.doctype)) {
						if (!appyframe.get_meta(me.doctype).allow_rename) allow_rename = false;
					}
					return !node.is_root && me.can_write && allow_rename;
				},
				click: function (node) {
					appyframe.model.rename_doc(me.doctype, node.label, function (new_name) {
			...
			},
			{
				...
				click: function (node) {
					appyframe.model.delete_doc(me.doctype, node.label, function () {
						node.parent.remove();
					});
				},
				btnClass: "hidden-xs",
	...
	new_node() {
		...
		if (!(node && node.expandable)) {
			appyframe.msgprint(__("Select a group node first."));
			return;
		}
		...
		d.set_primary_action(__("Create New"), function () {
			...
			d.hide();
			appyframe.dom.freeze(__("Creating {0}", [me.doctype]));

			$.extend(args, v);
			return appyframe.call({
				method: me.opts.add_tree_node || "appyframe.desk.treeview.add_node",
				args: args,
				callback: function (r) {
					if (!r.exc) {
	...
	print_tree() {
		if (!appyframe.model.can_print(this.doctype)) {
			appyframe.msgprint(__("You are not allowed to print this report"));
			return false;
		}
		var tree = $(".tree:visible").html();
		var me = this;
		appyframe.ui.get_print_settings(false, function (print_settings) {
			var title = __(me.docname || me.doctype);
			appyframe.render_tree({ title: title, tree: tree, print_settings: print_settings });
			appyframe.call({
				method: "appyframe.core.doctype.access_log.access_log.make_access_log",
				args: {
					doctype: me.doctype,
	...
	set_menu_item() {
		...
		this.menu_items = [
			{
				label: __("View List"),
				action: function () {
					appyframe.set_route("List", me.doctype);
				},
			},
			{
				label: __("Print"),
		...
		if (
			...
			appyframe.meta.has_field(me.doctype, "lft") &&
			appyframe.meta.has_field(me.doctype, "rgt")
		) {
