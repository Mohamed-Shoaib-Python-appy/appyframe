...
appyframe.standard_pages["Workspaces"] = function () {
	var wrapper = appyframe.container.add_page("Workspaces");
	...
	appyframe.workspace = new appyframe.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		appyframe.workspace.show();
	});
};

appyframe.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = appyframe.workspace_block.blocks;
	...
	async setup_pages(reload) {
		...
		if (this.all_pages) {
			...
			for (let page of this.all_pages) {
				appyframe.workspaces[appyframe.router.slug(page.name)] = {
					title: page.title,
					public: page.public,
				};
			}
	...
	get_pages() {
		return appyframe.xcall("appyframe.desk.desktop.get_workspace_sidebar_items");
	}
			...
			>
				<div class="desk-sidebar-item standard-sidebar-item ${item.selected ? "selected" : ""}">
					<a
						href="/app/${
							item.public
								? appyframe.router.slug(item.title)
								: "private/" + appyframe.router.slug(item.title)
						}"
					...
					>
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>
							${
								item.public
									? appyframe.utils.icon(item.icon || "folder-normal", "md")
	...
	make_sidebar() {
		...
		this.sidebar.find(".selected").length &&
			!appyframe.dom.is_element_in_viewport(this.sidebar.find(".selected")) &&
	...
	build_sidebar_section(category, root_pages) {
		...
		let $title = $(`<button class="btn-reset standard-sidebar-label">
			<span>${appyframe.utils.icon("es-line-down", "xs")}</span>
	...
	append_item(item, container) {
		let is_current_page =
			appyframe.router.slug(item.title) == appyframe.router.slug(this.get_page_to_show().name) &&
	...
	add_drop_icon(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(`<button class="btn-reset drop-icon hidden">`)
			.html(appyframe.utils.icon(drop_icon, "sm"))
	...
	show() {
		...
		if (!appyframe.router.current_route[0]) {
			appyframe.route_flags.replace_route = true;
			appyframe.set_route(appyframe.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return appyframe
			.call("appyframe.desk.desktop.get_desktop_page", {
				page: page,
			})
			.then((data) => {
				...
				return appyframe.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		if (appyframe.boot.user.default_workspace) {
			default_page = {
				name: appyframe.boot.user.default_workspace.title,
				public: appyframe.boot.user.default_workspace.public,
			};
		} else if (
	...
	async show_page(page) {
		...
		if (this.all_pages.length) {
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await appyframe.after_ajax(() => this.get_data(current_page));
			}
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			this.reload();
			appyframe.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				appyframe.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	add_sidebar_actions(item, sidebar_control, is_new) {
		if (!item.is_editable) {
			...
			appyframe.utils.add_custom_button(
				appyframe.utils.icon("es-line-duplicate", "sm"),
			...
			appyframe.utils.add_custom_button(
				appyframe.utils.icon("es-line-preview", "sm"),
			...
			appyframe.utils.add_custom_button(
				appyframe.utils.icon("es-line-drag", "xs"),
				null,
				"drag-handle",
				__("Drag"),
				null,
	...
	edit_page(item) {
		...
		const d = new appyframe.ui.Dialog({
			...
			primary_action: (values) => {
				...
				appyframe.call({
					method: "appyframe.desk.doctype.workspace.workspace.update_page",
					args: {
						name: old_item.name,
						title: values.title,
						icon: values.icon || "",
					...
					callback: function (res) {
						if (res.message) {
							...
							appyframe.show_alert({ message: message, indicator: "green" });
						}
					},
				});
				...
				if (this.make_page_selected) {
					...
					let route = pre_url + appyframe.router.slug(values.title);
	...
	update_cached_values(old_item, new_item, duplicate, new_page) {
		...
		// update appyframe.workspaces
		if (appyframe.workspaces[appyframe.router.slug(old_item.name)] || new_page) {
			!duplicate && delete appyframe.workspaces[appyframe.router.slug(old_item.name)];
			if (new_item) {
				appyframe.workspaces[appyframe.router.slug(new_item.name)] = { title: new_item.title };
			}
		}
	...
	add_settings_button(item, sidebar_control) {
		this.dropdown_list = [
			{
				...
				icon: appyframe.utils.icon("es-line-edit", "sm"),
				action: () => this.edit_page(item),
			},
			{
				label: __("Duplicate"),
				title: __("Duplicate Workspace"),
				icon: appyframe.utils.icon("es-line-duplicate", "sm"),
				action: () => this.duplicate_page(item),
			},
			{
				label: __("Hide"),
				title: __("Hide Workspace"),
				icon: appyframe.utils.icon("es-line-hide", "sm"),
		...
		if (this.is_item_deletable(item)) {
			this.dropdown_list.push({
				...
				icon: appyframe.utils.icon("delete-active", "sm"),
		...
		let $button = $(`
			<div class="btn btn-xs setting-btn dropdown-btn" title="${__("Setting")}">
				${appyframe.utils.icon("es-line-dot-horizontal", "xs")}
			</div>
	...
	is_item_deletable(item) {
		...
		if (
			!item.public ||
			(item.public && (!item.module || (item.module && appyframe.boot.developer_mode)))
		)
			return true;
		return false;
	}
	...
	delete_page(page) {
		appyframe.confirm(
			...
			() => {
				appyframe.call({
					method: "appyframe.desk.doctype.workspace.workspace.delete_page",
					args: { page: page },
					callback: function (res) {
						if (res.message) {
	...
	duplicate_page(page) {
		...
		const d = new appyframe.ui.Dialog({
			...
			primary_action: (values) => {
				...
				d.hide();
				appyframe.call({
					method: "appyframe.desk.doctype.workspace.workspace.duplicate_page",
					args: {
						page_name: page.name,
						new_page: values,
					},
				...
				new_page.name = values.title + (new_page.public ? "" : "-" + appyframe.session.user);
				...
				new_page.for_user = new_page.public ? "" : appyframe.session.user;
	...
	hide_unhide_workspace(page, event, hide) {
		...
		appyframe.call({
			method: "appyframe.desk.doctype.workspace.workspace." + method,
			args: {
				page_name: page.name,
			},
			callback: (r) => {
	...
	update_sorted_sidebar() {
		if (this.sorted_public_items || this.sorted_private_items) {
			appyframe.call({
				method: "appyframe.desk.doctype.workspace.workspace.sort_pages",
				args: {
				...
				callback: function (res) {
					if (res.message) {
						...
						appyframe.show_alert({ message: __(message), indicator: "green" });
					}
				},
			});
		}
	...
	initialize_new_page() {
		...
		const d = new appyframe.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + appyframe.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					title: values.title,
					public: values.is_public || 0,
					for_user: values.is_public ? "" : appyframe.session.user,
					icon: values.icon,
					indicator_color: values.indicator_color,
				...
				this.editor
					...
					.then(async () => {
						...
						appyframe.call({
							method: "appyframe.desk.doctype.workspace.workspace.new_page",
							args: {
								new_page: new_page,
							},
							callback: function (res) {
						...
						let route = pre_url + appyframe.router.slug(new_page.title);
						appyframe.set_route(route);
	...
	validate_page(new_page, old_page) {
		...
		if (appyframe.router.doctype_route_exist(appyframe.router.slug(new_page.title))) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: appyframe.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				appyframe.call({
					method: "appyframe.desk.doctype.workspace.workspace.save_page",
					args: {
						title: page.title,
	...
	create_page_skeleton() {
		...
		this.body.prepend(appyframe.render_template("workspace_loading_skeleton"));
	...
	create_sidebar_skeleton() {
		...
		$(appyframe.render_template("workspace_sidebar_loading_skeleton")).insertBefore(this.sidebar);
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			appyframe.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			appyframe.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};
