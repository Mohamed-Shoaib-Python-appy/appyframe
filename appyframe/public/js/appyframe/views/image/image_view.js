/**
 * appyframe.views.ImageView
 */
appyframe.provide("appyframe.views");

appyframe.views.ImageView = class ImageView extends appyframe.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = appyframe.utils.escape_html(title);
		...
		const expand_button_html = item._image_url
			? `<div class="zoom-view" data-name="${encoded_name}">
				${appyframe.utils.icon("expand", "xs")}
			</div>`
			: "";
	...
	get_attached_images() {
		return appyframe
			.call({
				method: "appyframe.core.api.file.get_attached_images",
				args: {
	...
	get_header_html() {
		...
		// 				${appyframe.utils.icon('heart', 'sm', 'like-icon')}
		// 			</div>
		// 			<div>${__('Liked')}</div>
		// 		</div>
		// 	</div>
	...
	setup_gallery() {
		var me = this;
		this.gallery = new appyframe.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
...
appyframe.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = appyframe.render_template("photoswipe_dom");
	...
	load_lib() {
		return new Promise((resolve) => {
			var asset_dir = "assets/appyframe/js/lib/photoswipe/";
			appyframe.require(
				[
