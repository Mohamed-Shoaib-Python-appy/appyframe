...
appyframe.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[appyframe.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return appyframe.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[appyframe.breadcrumbs.current_page()];
		...
		if (breadcrumbs.type === "Custom") {
			...
			// form / print
			let view = appyframe.get_route()[0];
	...
	get last_route() {
		return appyframe.route_history.slice(-2)[0];
	},
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (
			...
			(breadcrumbs.module_info.blocked ||
				!appyframe.visible_modules.includes(breadcrumbs.module_info.module))
		) {
			return;
		}

		this.append_breadcrumb_element(
			`/app/${appyframe.router.slug(breadcrumbs.workspace)}`,
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			if (
				breadcrumbs.module &&
				appyframe.boot.module_wise_workspaces[breadcrumbs.module]?.includes(last_workspace)
			) {
		...
		if (breadcrumbs.module) {
			...
			breadcrumbs.module_info = appyframe.get_module(breadcrumbs.module);

			// set workspace
			if (
				breadcrumbs.module_info &&
				appyframe.boot.module_wise_workspaces[breadcrumbs.module]
			) {
				breadcrumbs.workspace = appyframe.boot.module_wise_workspaces[breadcrumbs.module][0];
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = appyframe.get_doc("DocType", doctype);
		if (
			(doctype === "User" && !appyframe.user.has_role("System Manager")) ||
			doctype_meta?.issingle
		) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = appyframe.router.slug(appyframe.router.doctype_layout || doctype);
			if (doctype_meta?.is_tree) {
				let view = appyframe.model.user_settings[doctype].last_view || "Tree";
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		let docname = appyframe.get_route().slice(2).join("/");
		let docname_title;
		...
		let form_route = `/app/${appyframe.router.slug(doctype)}/${encodeURIComponent(docname)}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				appyframe.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	},
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = appyframe.get_route()[1];
		let dashboard_route = `/app/${appyframe.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`).appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!appyframe.visible_modules) {
			appyframe.visible_modules = $.map(appyframe.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete appyframe.breadcrumbs.all[old_route_str];
		this.update();
	},
