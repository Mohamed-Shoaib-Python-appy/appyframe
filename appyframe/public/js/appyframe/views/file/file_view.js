...
appyframe.views.FileView = class FileView extends appyframe.views.ListView {
	static load_last_view() {
		const route = appyframe.get_route();
		if (route.length === 2) {
			const view_user_settings = appyframe.get_user_settings("File", "File");
			appyframe.set_route(
				"List",
				"File",
				view_user_settings.last_folder || appyframe.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			appyframe.views.FileView.grid_view = !appyframe.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (appyframe.views.FileView.grid_view) {
			Object.assign(args, {
		...
		return [
			{
				label: __("Home"),
				action: () => {
					appyframe.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					appyframe.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								appyframe.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							appyframe.call({
								method: "appyframe.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new appyframe.ui.FileUploader({
						...
						on_success: (file) => {
							appyframe.show_alert(__("Unzipping files..."));
							appyframe
								.call("appyframe.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										appyframe.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				appyframe.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => appyframe.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/appyframe.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => appyframe.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (appyframe.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
		...
		d.subject_html = `
			${appyframe.utils.icon(icon_class)}
			<span>${title}</span>
	...
	before_render() {
		super.before_render();
		appyframe.model.user_settings.save("File", "grid_view", appyframe.views.FileView.grid_view);
	...
	get_header_html() {
		...
		let header_selector_html = !appyframe.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !appyframe.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === appyframe.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = appyframe.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? appyframe.form.formatters.FileSize(file.file_size) : "";
		...
		return `
			<div class="list-row-col ellipsis list-subject level">
				...
				<span class="level-item  ellipsis" title="${appyframe.utils.escape_html(file.file_name)}">
					<a class="ellipsis" href="${route_url}" title="${appyframe.utils.escape_html(file.file_name)}">
						${file.subject_html}
					</a>
				</span>
			</div>
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					appyframe.file_manager.paste(folder_name);
					appyframe.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!appyframe.file_manager.can_paste ||
			appyframe.file_manager.old_folder === this.current_folder;
...
appyframe.views.FileView.grid_view = appyframe.get_user_settings("File").grid_view || false;
