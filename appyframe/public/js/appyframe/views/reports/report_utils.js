appyframe.provide("appyframe.report_utils");

appyframe.report_utils = {
	make_chart_options: function (
		...
		let datasets = y_fields.map((y_field) => ({
			name: appyframe.model.unscrub(y_field),
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = appyframe.report_utils.prepare_field_from_column(field);
					return { label: field.label, value: field.fieldname };
				}
			});
		}
	...
	get_report_filters: function (report_name) {
		if (appyframe.query_reports[report_name]) {
			let filters = appyframe.query_reports[report_name].filters;
		...
		return appyframe
			...
			.then((r) => {
				appyframe.dom.eval(r.script || "");
				return appyframe.after_ajax(() => {
					if (
						appyframe.query_reports[report_name] &&
						!appyframe.query_reports[report_name].filters &&
						r.filters
					) {
						return (appyframe.query_reports[report_name].filters = r.filters);
					}
					return (
						appyframe.query_reports[report_name] &&
						appyframe.query_reports[report_name].filters
	...
	get_export_dialog(report_name, extra_fields, callback) {
		...
		function update_csv_preview(dialog) {
			const is_query_report = appyframe.get_route()[0] === "query-report";
			const report = is_query_report ? appyframe.query_report : cur_list;
			...
			dialog.set_value(
				"csv_preview",
				appyframe.report_utils.get_csv_preview(
					PREVIEW_DATA,
	...
	get_csv_preview(data, quoting, delimiter) {
		...
		if (delimiter.length > 1) {
			appyframe.throw(__("Delimiter must be a single character"));
		}

		if (0 > quoting || quoting > 3) {
			appyframe.throw(__("Quoting must be between 0 and 3"));
		}
