/**
 * appyframe.views.InboxView
 */

appyframe.provide("appyframe.views");

appyframe.views.InboxView = class InboxView extends appyframe.views.ListView {
	static load_last_view() {
		const route = appyframe.get_route();
		if (!route[3] && appyframe.boot.email_accounts.length) {
			let email_account;
			if (appyframe.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = appyframe.boot.email_accounts[0].email_account;
			}
			appyframe.set_route("List", "Communication", "Inbox", email_account);
			return true;
		...
		function is_valid(email_account) {
			return appyframe.boot.email_accounts.find((d) => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = appyframe.get_route()[3];
	...
	get_seen_class(doc) {
		return Boolean(doc.seen) || JSON.parse(doc._seen || "[]").includes(appyframe.session.user)
			? ""
			: "bold";
	}
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${appyframe.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", appyframe.boot.all_accounts, true],
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = appyframe.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !appyframe.boot.email_accounts.length) {
		...
		const html = appyframe.model.can_create(args.doctype)
			? `<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !appyframe.boot.email_accounts.length) {
			appyframe.route_options = {
				email_id: appyframe.session.user_email,
			};
			appyframe.new_doc("Email Account");
		} else {
