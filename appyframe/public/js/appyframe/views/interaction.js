...
appyframe.provide("appyframe.interaction_settings");

appyframe.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		appyframe.model.with_doctype("Event", () => {
			let categories = appyframe.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new appyframe.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = appyframe.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = appyframe.session.user;
		}
	...
	add_attachments(doc, attachments) {
		appyframe.call({
			method: "appyframe.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
