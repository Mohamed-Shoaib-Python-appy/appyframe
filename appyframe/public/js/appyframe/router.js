...
appyframe.re_route = { "#login": "" };
appyframe.route_titles = {};
appyframe.route_flags = {};
appyframe.route_history = [];
appyframe.view_factory = {};
appyframe.view_factories = [];
appyframe.route_options = null;
appyframe.open_in_new_tab = false;
appyframe.route_hooks = {};

$(window).on("hashchange", function (e) {
	// v1 style routing, route is in hash
	if (window.location.hash && !appyframe.router.is_app_route(e.currentTarget.pathname)) {
		let sub_path = appyframe.router.get_sub_path(window.location.hash);
		appyframe.router.push_state(sub_path);
		return false;
	}
});
...
appyframe.router = {
	...
	setup() {
		...
		for (let doctype of appyframe.boot.user.can_read) {
			this.routes[this.slug(doctype)] = { doctype: doctype };
		}
		if (appyframe.boot.doctype_layouts) {
			for (let doctype_layout of appyframe.boot.doctype_layouts) {
	...
	async route() {
		...
		if (!appyframe.app) return;

		let sub_path = this.get_sub_path();
		if (appyframe.boot.setup_complete) {
			!appyframe.re_route["setup-wizard"] && (appyframe.re_route["setup-wizard"] = "app");
		} else if (!sub_path.startsWith("setup-wizard")) {
			appyframe.re_route["setup-wizard"] && delete appyframe.re_route["setup-wizard"];
			appyframe.set_route(["setup-wizard"]);
		}
	...
	async convert_to_standard_route(route) {
		...
		if (appyframe.workspaces[route[0]]) {
			// public workspace
			route = ["Workspaces", appyframe.workspaces[route[0]].title];
		} else if (route[0] == "private") {
			// private workspace
			let private_workspace = route[1] && `${route[1]}-${appyframe.user.name.toLowerCase()}`;
			if (!appyframe.workspaces[private_workspace] && localStorage.new_workspace) {
				let new_workspace = JSON.parse(localStorage.new_workspace);
				if (appyframe.router.slug(new_workspace.title) === route[1]) {
					appyframe.workspaces[private_workspace] = new_workspace;
				}
			}
			if (!appyframe.workspaces[private_workspace]) {
				appyframe.msgprint(__("Workspace <b>{0}</b> does not exist", [route[1]]));
				return ["Workspaces"];
			}
			route = ["Workspaces", "private", appyframe.workspaces[private_workspace].title];
	...
	set_doctype_route(route) {
		...
		return appyframe.model.with_doctype(doctype_route.doctype).then(() => {
			// doctype route
			let meta = appyframe.get_meta(doctype_route.doctype);
			...
			} else if (appyframe.model.is_single(doctype_route.doctype)) {
	...
	set_history() {
		appyframe.route_history.push(this.current_route);
		appyframe.ui.hide_open_dialog();
	},
	...
	render_page() {
		...
		const factory = appyframe.utils.to_title_case(route[0]);

		if (route[1] && appyframe.views[factory + "Factory"]) {
			route[0] = factory;
			// has a view generator, generate!
			if (!appyframe.view_factory[factory]) {
				appyframe.view_factory[factory] = new appyframe.views[factory + "Factory"]();
			}

			appyframe.view_factory[factory].show();
		} else {
			// show page
			const route_name = appyframe.utils.xss_sanitise(route[0]);
			if (appyframe.views.pageview) {
				appyframe.views.pageview.show(route_name);
			}
		}
	},

	re_route(sub_path) {
		if (appyframe.re_route[sub_path] !== undefined) {
			...
			const re_route_val = this.get_sub_path(appyframe.re_route[sub_path]);
			if (re_route_val === this.current_sub_path) {
				window.history.back();
			} else {
				appyframe.set_route(re_route_val);
			}
	...
	set_title(sub_path) {
		if (appyframe.route_titles[sub_path]) {
			appyframe.utils.set_title(appyframe.route_titles[sub_path]);
		}
	},

	set_route() {
		// set the route (push state) with given arguments
		// example 1: appyframe.set_route('a', 'b', 'c');
		// example 2: appyframe.set_route(['a', 'b', 'c']);
		// example 3: appyframe.set_route('a/b/c');
		...
		return new Promise((resolve) => {
			...
			sub_path += appyframe.route_hash || "";
			appyframe.route_hash = null;
			if (appyframe.open_in_new_tab) {
				localStorage["route_options"] = JSON.stringify(appyframe.route_options);
		...
		}).finally(() => (appyframe.route_flags = {}));
	},

	get_route_from_arguments(route) {
		if (route.length === 1 && $.isArray(route[0])) {
			// called as appyframe.set_route(['a', 'b', 'c']);
			route = route[0];
		}
		...
		if (view === "list") {
			if (route[2] && route[2] !== "list" && !$.isPlainObject(route[2])) {
				...
				if ($.isPlainObject(route[2])) {
					appyframe.route_options = route[2];
				}
	...
	make_url(params) {
		...
		let private_home = `home-${appyframe.user.name.toLowerCase()}`;
		let default_workspace = appyframe.router.slug(appyframe.boot.user.default_workspace?.name || "");

		let workspace =
			appyframe.workspaces[default_workspace] ||
			appyframe.workspaces[private_home] ||
			appyframe.workspaces["home"] ||
			Object.values(appyframe.workspaces)[0];

		if (workspace) {
			return (
				"/app/" +
				(workspace.public ? "" : "private/") +
				appyframe.router.slug(workspace.title)
			);
		}
	...
	push_state(url) {
		...
		if (window.location.pathname !== url) {
			...
			const method = appyframe.route_flags.replace_route ? "replaceState" : "pushState";
	...
	set_route_options_from_url() {
		// set query parameters as appyframe.route_options
		let query_string = window.location.search;

		if (!appyframe.route_options) {
			appyframe.route_options = {};
		}

		if (localStorage.getItem("route_options")) {
			appyframe.route_options = JSON.parse(localStorage.getItem("route_options"));
...
appyframe.get_route = () => appyframe.router.current_route;
appyframe.get_route_str = () => appyframe.router.current_route.join("/");
appyframe.set_route = function () {
	return appyframe.router.set_route.apply(appyframe.router, arguments);
};

appyframe.get_prev_route = function () {
	if (appyframe.route_history && appyframe.route_history.length > 1) {
		return appyframe.route_history[appyframe.route_history.length - 2];
	} else {
		return [];
	}
};

appyframe.set_re_route = function () {
	var tmp = appyframe.router.get_sub_path();
	appyframe.set_route.apply(null, arguments);
	appyframe.re_route[tmp] = appyframe.router.get_sub_path();
};

appyframe.has_route_options = function () {
	return Boolean(Object.keys(appyframe.route_options || {}).length);
};

appyframe.utils.make_event_emitter(appyframe.router);
