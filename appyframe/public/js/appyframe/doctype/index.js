...
*/
appyframe.model.DocTypeController = class DocTypeController extends appyframe.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		appyframe.meta.docfield_map[
	...
	show_db_utilization() {
		...
		appyframe
			.xcall("appyframe.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = appyframe.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			appyframe.model.with_doctype(link_doctype, () => {
				let fields = appyframe.meta
