...
$(document).ready(function () {
	if (
		appyframe.boot.is_fc_site &&
		appyframe.boot.setup_complete === 1 &&
		!appyframe.is_mobile() &&
		appyframe.user.has_role("System Manager")
	) {
		appyframe.call({
			method: "appyframe.integrations.frappe_providers.frappecloud_billing.current_site_info",
			callback: (r) => {
				if (!r?.message) return;
...
function openFrappeCloudDashboard() {
	window.open(`${frappeCloudBaseEndpoint}/dashboard/sites/${appyframe.boot.sitename}`, "_blank");
}
