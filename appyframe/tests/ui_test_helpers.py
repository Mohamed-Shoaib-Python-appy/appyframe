...
UI_TEST_USER = "appyframe@example.com"


def whitelist_for_tests(fn):
	if appyframe.request and not (appyframe.flags.in_test or getattr(appyframe.local, "dev_server", 0)):
		appyframe.throw("Cannot run UI tests. Use a development server with `bench start`")

	return appyframe.whitelist()(fn)
...
def create_if_not_exists(doc):
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = appyframe.db.exists(doc.doctype, filters)
		if not name:
			d = appyframe.get_doc(doc)
...
def prepare_webform_test():
	for note in appyframe.get_all("Note", pluck="name"):
		appyframe.delete_doc("Note", note, force=True)

	appyframe.delete_doc_if_exists("Web Form", "note")
	...
	doc = appyframe.get_doc(
		{
			...
			"communication_date": appyframe.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@whitelist_for_tests
def setup_workflow():
	from appyframe.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if appyframe.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = appyframe.new_doc("Contact")
...
def create_contact_records():
	if appyframe.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if appyframe.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [(f"100{i}", f"Multiple ToDo {i}") for i in range(1, 1002)]

	appyframe.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = appyframe.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if appyframe.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return

	tour = appyframe.get_doc(
		{
			"doctype": "Form Tour",
...
def create_web_page(title, route, single_thread):
	web_page = appyframe.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	web_page = appyframe.get_doc({"doctype": "Web Page", "title": title, "route": route, "published": True})
	web_page.save()
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": appyframe.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not appyframe.db.exists("Web Form", "update-profile-duplicate"):
		doc = appyframe.get_doc("Web Form", "edit-profile")
		_doc = appyframe.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if appyframe.get_all(name, {"title": "Test Grid Search"}):
		return
...
def insert_translations():
	...
	for doc in translation:
		if not appyframe.db.exists("doc"):
			appyframe.get_doc(doc).insert()


@whitelist_for_tests
def create_blog_post():
	blog_category = appyframe.get_doc(
		{"name": "general", "doctype": "Blog Category", "title": "general"}
	).insert(ignore_if_duplicate=True)

	blogger = appyframe.get_doc(
		{
...
def create_test_user(username=None):
	...
	if appyframe.db.exists("User", name):
		return

	user = appyframe.new_doc("User")
	user.email = name
	user.first_name = "Appyframe"
	user.new_password = appyframe.local.conf.admin_password
	...
	all_roles = set(appyframe.get_all("Role", pluck="name"))
...
def setup_tree_doctype():
	appyframe.delete_doc_if_exists("DocType", "Custom Tree", force=True)
	...
	if not appyframe.db.exists("Custom Tree", "All Trees"):
		appyframe.get_doc({"doctype": "Custom Tree", "tree": "All Trees"}).insert()


@whitelist_for_tests
def setup_image_doctype():
	appyframe.delete_doc_if_exists("DocType", "Custom Image", force=True)
...
@whitelist_for_tests
def setup_inbox():
	appyframe.db.delete("User Email")
...
def setup_default_view(view, force_reroute=None):
	appyframe.delete_doc_if_exists("Property Setter", "Event-main-default_view")
	appyframe.delete_doc_if_exists("Property Setter", "Event-main-force_re_route_to_default_view")
...
@whitelist_for_tests
def create_kanban():
	if not appyframe.db.exists("Custom Field", "Note-kanban"):
		appyframe.get_doc(
			{
				"is_system_generated": 0,
				"dt": "Note",
	...
	if not appyframe.db.exists("Kanban Board", "_Note _Kanban"):
		appyframe.get_doc(
			{
...
def create_todo(description):
	return appyframe.get_doc({"doctype": "ToDo", "description": description}).insert()
...
def create_admin_kanban():
	if not appyframe.db.exists("Kanban Board", "Admin Kanban"):
		appyframe.get_doc(
			{
				"doctype": "Kanban Board",
				"name": "Admin Kanban",
...
def publish_progress(duration=3, title=None, doctype=None, docname=None):
	...
	appyframe.enqueue(slow_task, duration=duration, title=title, doctype=doctype, docname=docname)
...
def slow_task(duration, title, doctype, docname):
	...
	for i in range(steps + 1):
		appyframe.publish_progress(i * 10, title=title, doctype=doctype, docname=docname)
