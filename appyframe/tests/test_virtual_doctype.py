...
import appyframe
import appyframe.modules.utils
...
class VirtualDoctypeTest(Document):
	...
	def get_list(args):
		...
		return [appyframe._dict(doc) for name, doc in data.items()]
...
class TestVirtualDoctypes(FrappeTestCase):
	@classmethod
	def setUpClass(cls):
		appyframe.flags.allow_doctype_export = True
		cls.addClassCleanup(appyframe.flags.pop, "allow_doctype_export", None)
		...
		patch_virtual_doc = patch(
			"appyframe.controllers", new={appyframe.local.site: {TEST_DOCTYPE_NAME: VirtualDoctypeTest}}
		)
	...
	def test_insert_update_and_load_from_desk(self):
		...
		docname = appyframe.response.docs[0]["name"]

		doc = appyframe.get_doc(TEST_DOCTYPE_NAME, docname)
	...
	def test_multiple_doc_insert_and_get_list(self):
		doc1 = appyframe.new_doc(doctype=TEST_DOCTYPE_NAME)
		doc1.append("child_table", {"name": "first", "some_fieldname": "first-value"})
		doc1.insert()
		doc2 = appyframe.new_doc(doctype=TEST_DOCTYPE_NAME)
	...
	def test_delete_doc(self):
		doc = appyframe.get_doc(doctype=TEST_DOCTYPE_NAME).insert()

		appyframe.delete_doc(doc.doctype, doc.name)
