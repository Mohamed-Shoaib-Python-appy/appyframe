...
from appyframe.utils.password import check_password, decrypt, encrypt, passlibctx, update_password


class TestPassword(FrappeTestCase):
	def setUp(self):
		appyframe.delete_doc("Email Account", "Test Email Account Password")
		appyframe.delete_doc("Email Account", "Test Email Account Password-new")
	...
	def make_email_account(self, name="Test Email Account Password"):
		if not appyframe.db.exists("Email Account", name):
			...
			return appyframe.get_doc("Email Account", name)
	...
	def test_hashed_password(self, user="test@example.com"):
		...
		self.assertRaises(appyframe.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		appyframe.rename_doc(doc.doctype, old_name, new_name)

		new_doc = appyframe.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not get_password_list(doc))

		appyframe.rename_doc(doc.doctype, new_name, old_name)
