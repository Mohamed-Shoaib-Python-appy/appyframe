...
from appyframe.modules import export_customizations, export_module_json, get_module_path
from appyframe.modules.utils import export_doc, sync_customizations
...
class TestUtils(FrappeTestCase):
	def setUp(self):
		self._dev_mode = appyframe.local.conf.developer_mode
		appyframe.local.conf.developer_mode = True

	def tearDown(self):
		appyframe.db.rollback()
		appyframe.local.conf.developer_mode = self._dev_mode
		appyframe.local.flags.pop("in_import", None)

	def test_export_module_json_no_export(self):
		appyframe.local.flags.in_import = True
		doc = appyframe.get_last_doc("DocType")
	...
	def test_export_module_json(self):
		doc = appyframe.get_last_doc("DocType", {"issingle": 0, "custom": 0})
		...
		with open(export_doc_path) as f:
			export_doc_before = appyframe.parse_json(f.read())

		last_modified_before = os.path.getmtime(export_doc_path)
		self.addCleanup(write_file, path=export_doc_path, content=appyframe.as_json(export_doc_before))

		appyframe.flags.in_import = False
		appyframe.conf.developer_mode = True
		...
		with open(f"{export_path}.json") as f:
			appyframe.parse_json(f.read())  # export_doc_after
	...
	def test_sync_customizations(self):
		with note_customizations() as (custom_field, property_setter):
			...
			sync_customizations(app="appyframe")
	...
	def test_reload_doc(self):
		appyframe.db.set_value("DocType", "Note", "migration_hash", "", update_modified=False)
		self.assertFalse(appyframe.db.get_value("DocType", "Note", "migration_hash"))
		...
		self.assertEqual(
			appyframe.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Text",
		)
		appyframe.reload_doctype("Note")
		self.assertEqual(
			appyframe.db.get_value("DocField", {"parent": "Note", "fieldname": "title"}, "fieldtype"),
			"Data",
		)
		self.assertTrue(appyframe.db.get_value("DocType", "Note", "migration_hash"))
	...
	def test_export_doc(self):
		...
		note.title = appyframe.generate_hash(length=10)
		note.save()
		export_doc(doctype="Note", name=note.name)
		exported_doc_path = Path(
			appyframe.get_app_path("appyframe", "desk", "note", note.name, f"{note.name}.json")
		)
	...
	def test_make_boilerplate(self):
		with temp_doctype() as doctype:
			scrubbed = appyframe.scrub(doctype.name)
			path = appyframe.get_app_path("appyframe", "core", "doctype", scrubbed, f"{scrubbed}.json")
...
def temp_doctype():
	try:
		...
		appyframe.db.sql_ddl(f"DROP TABLE `tab{doctype.name}`")
...
def note_customizations():
	try:
		...
		delete_path(appyframe.get_module_path("Desk", "Note"))
