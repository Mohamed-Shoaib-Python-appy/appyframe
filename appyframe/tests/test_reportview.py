...
import appyframe
from appyframe.desk.reportview import export_query, extract_fieldnames
...
class TestReportview(FrappeTestCase):
	def test_csv(self):
		...
		for delimiter in (",", ";", "\t", "|"):
			appyframe.local.form_dict.csv_delimiter = delimiter
			for quoting in (QUOTE_ALL, QUOTE_MINIMAL, QUOTE_NONE, QUOTE_NONNUMERIC):
				appyframe.local.form_dict.csv_quoting = quoting
