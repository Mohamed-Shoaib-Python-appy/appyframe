...
import appyframe
from appyframe.integrations.oauth2 import encode_params
from appyframe.test_runner import make_test_records
from appyframe.tests.test_api import get_test_client, make_request, suppress_stdout
from appyframe.tests.utils import FrappeTestCase

if TYPE_CHECKING:
	from appyframe.integrations.doctype.social_login_key.social_login_key import SocialLoginKey
...
class FrappeRequestTestCase(FrappeTestCase):
	...
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			appyframe.local.login_manager.login_as("test@example.com")
			self._sid = appyframe.session.sid
...
class TestOAuth20(FrappeRequestTestCase):
	...
	def setUpClass(cls):
		...
		# Set Appyframe server URL reqired for id_token generation
		frappe_login_key: "SocialLoginKey" = appyframe.new_doc("Social Login Key")
		frappe_login_key.get_social_login_provider("Appyframe", initialize=True)
		frappe_login_key.base_url = appyframe.utils.get_url()
	...
	def setUp(self):
		...
		self.oauth_client = appyframe.new_doc("OAuth Client")
	...
	def test_login_using_authorization_code(self):
		...
		resp = self.get(
			"/api/method/appyframe.integrations.oauth2.authorize",
			{
		...
		token_response = self.post(
			"/api/method/appyframe.integrations.oauth2.get_token",
	...
	def test_revoke_token(self):
		client = appyframe.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = self.post(
			"/api/method/appyframe.integrations.oauth2.revoke_token",
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = appyframe.get_doc("OAuth Client", self.client_id)
		...
		try:
			session.get(
				get_full_url("/api/method/appyframe.integrations.oauth2.authorize"),
...
def check_valid_openid_response(access_token=None, client: "FrappeRequestTestCase" = None):
	...
	# Use token in header
	headers = {}
	URL = "/api/method/appyframe.integrations.oauth2.openid_profile"
...
def get_full_url(endpoint):
	...
	return urljoin(appyframe.utils.get_url(), endpoint)
