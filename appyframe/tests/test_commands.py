...
import appyframe
import appyframe.commands.scheduler
import appyframe.commands.site
import appyframe.commands.utils
import appyframe.recorder
from appyframe.installer import add_to_installed_apps, remove_app
...
from appyframe.utils import add_to_date, get_bench_path, get_bench_relative_path, now
from appyframe.utils.backups import BackupGenerator, fetch_latest_backups
from appyframe.utils.jinja_globals import bundled_asset
from appyframe.utils.scheduler import enable_scheduler, is_scheduler_inactive

_result: Result | None = None
TEST_SITE = "commands-site-O4PN2QK.test"  # added random string tag to avoid collisions
CLI_CONTEXT = appyframe._dict(sites=[TEST_SITE])
...
def missing_in_backup(doctypes: list, file: os.PathLike) -> list:
	...
	"""
	predicate = 'COPY public."tab{}"' if appyframe.conf.db_type == "postgres" else "CREATE TABLE `tab{}`"
...
@contextmanager
def maintain_locals():
	pre_site = appyframe.local.site
	pre_flags = appyframe.local.flags.copy()
	pre_db = appyframe.local.db

	try:
		yield
	finally:
		post_site = getattr(appyframe.local, "site", None)
		if not post_site or post_site != pre_site:
			appyframe.init(site=pre_site)
			appyframe.local.db = pre_db
			appyframe.local.flags.update(pre_flags)
...
def cli(cmd: Command, args: list | None = None):
	with maintain_locals():
		...
		patch_ctx = patch("appyframe.commands.pass_context", pass_test_context)
...
class BaseTestCommands(FrappeTestCase):
	...
	def execute(self, command, kwargs=None):
		...
		site = {"site": appyframe.local.site}
		cmd_input = None
		if kwargs:
	...
	def setup_test_site(cls):
		cmd_config = {
			"test_site": TEST_SITE,
			"admin_password": appyframe.conf.admin_password,
			"db_type": appyframe.conf.db_type,
			"db_root_username": appyframe.conf.root_login,
			"db_root_password": appyframe.conf.root_password,
		}
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("bench --site {site} execute appyframe.db.get_database_size")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command expecting an errored output as local won't exist
		self.execute("bench --site {site} execute appyframe.local.site")
		...
		self.execute("""bench --site {site} execute appyframe.bold --kwargs '{{"text": "DocType"}}'""")
		self.assertEqual(self.returncode, 0)
		self.assertEqual(self.stdout[1:-1], appyframe.bold(text="DocType"))
	...
	def test_restore(self):
		...
		global_config = {
			"admin_password": appyframe.conf.admin_password,
			"root_login": appyframe.conf.root_login,
			"root_password": appyframe.conf.root_password,
			"db_type": appyframe.conf.db_type,
		}
		...
		with self.switch_site(TEST_SITE):
			public_file = appyframe.new_doc(
				"File", file_name=f"test_{appyframe.generate_hash()}", content=appyframe.generate_hash()
			).insert()
			private_file = appyframe.new_doc(
				"File", file_name=f"test_{appyframe.generate_hash()}", content=appyframe.generate_hash()
			).insert()
		...
		self.execute(
			"bench --site {test_site} execute appyframe.utils.backups.fetch_latest_backups",
			site_data,
		)
		...
		with self.switch_site(TEST_SITE):
			...
			appyframe.db.sql_ddl("DROP TABLE IF EXISTS `tabToDo`")
		...
		with self.switch_site(TEST_SITE):
			self.assertTrue(appyframe.db.table_exists("ToDo", cached=False))
		...
		self.execute(
			"bench --site {test_site} execute" " appyframe.utils.backups.fetch_latest_backups --kwargs {kw}",
			site_data,
		)
	...
	def test_partial_restore(self):
			...
			appyframe.get_doc(
				{
					...
					"description": appyframe.mock("paragraph"),
				}
			).insert()
		appyframe.db.commit()
		todo_count = appyframe.db.count("ToDo")
		...
		self.assertEqual(self.returncode, 0)
		self.assertEqual(appyframe.db.count("ToDo"), todo_count)

	def test_recorder(self):
		appyframe.recorder.stop()

		self.execute("bench --site {site} start-recording")
		appyframe.local.cache = {}
		self.assertEqual(appyframe.recorder.status(), True)

		self.execute("bench --site {site} stop-recording")
		appyframe.local.cache = {}
		self.assertEqual(appyframe.recorder.status(), False)
	...
	def test_list_apps(self):
		...
		doctype = appyframe.get_single("Installed Applications").installed_applications
		if doctype:
	...
	def test_frappe_site_env(self):
		os.putenv("FRAPPE_SITE", appyframe.local.site)
		self.execute("bench execute appyframe.ping")
		self.assertEqual(self.returncode, 0)
	...
	def test_set_password(self):
		from appyframe.utils.password import check_password
		...
		original_password = appyframe.conf.admin_password or "admin"
	...
	@skipIf(
		not (appyframe.conf.root_password and appyframe.conf.admin_password and appyframe.conf.db_type == "mariadb"),
	...
	def test_bench_drop_site_should_archive_site(self):
		...
		self.execute(
			...
			f"--admin-password {appyframe.conf.admin_password} "
			f"--mariadb-root-password {appyframe.conf.root_password} "
			f"--db-type {appyframe.conf.db_type} "
		)
		self.assertEqual(self.returncode, 0)

		self.execute(f"bench drop-site {site} --force --root-password {appyframe.conf.root_password}")
		self.assertEqual(self.returncode, 0)
	...
	def test_force_install_app(self):
		...
		# set admin password in site_config as when appyframe force installs, we don't have the conf
		self.execute(f"bench --site {TEST_SITE} set-config admin_password {appyframe.conf.admin_password}")
	...
	def test_set_global_conf(self):
		...
		value = "42"
		self.execute(f"bench set-config {key} {value} -g")
		conf = appyframe.get_site_config()
...
class TestBackups(BaseTestCommands):
	...
	site_backup_path = appyframe.utils.get_site_path("private", "backups")
	...
	@skipIf(
		not (appyframe.conf.db_type == "mariadb"),
		"Only for MariaDB",
	)
	...
	def test_backup_fails_with_exit_code(self):
		...
		odb = BackupGenerator(
			appyframe.conf.db_name,
			appyframe.conf.db_name,
			appyframe.conf.db_password + "INCORRECT PASSWORD",
			db_socket=appyframe.conf.db_socket,
			db_host=appyframe.conf.db_host,
			db_port=appyframe.conf.db_port,
			db_type=appyframe.conf.db_type,
		)
	...
	def test_clear_log_table(self):
		d = appyframe.get_doc(doctype="Error Log", title="Something").insert()
		d.db_set("modified", "2010-01-01", update_modified=False)
		appyframe.db.commit()

		tables_before = appyframe.db.get_tables(cached=False)

		self.execute("bench --site {site} clear-log-table --days=30 --doctype='Error Log'")
		self.assertEqual(self.returncode, 0)
		appyframe.db.commit()

		self.assertFalse(appyframe.db.exists("Error Log", d.name))
		tables_after = appyframe.db.get_tables(cached=False)
	...
	def test_backup_only_specific_doctypes(self):
		"""Take a backup with (include) backup options set in the site config `appyframe.conf.backup.includes`"""
		self.execute(
	...
	def test_backup_excluding_specific_doctypes(self):
		"""Take a backup with (exclude) backup options set (`appyframe.conf.backup.excludes`, `--exclude`)"""
		# test 1: take a backup with appyframe.conf.backup.excludes
		self.execute(
	...
	def test_selective_backup_priority_resolution(self):
		"""Take a backup with conflicting backup options set (`appyframe.conf.excludes`, `--include`)"""
		self.execute(
	...
	def test_dont_backup_conf(self):
		"""Take a backup ignoring appyframe.conf.backup settings (with --ignore-backup-conf option)"""
...
class TestRemoveApp(FrappeTestCase):
	def test_delete_modules(self):
		from appyframe.installer import (
			_delete_doctypes,
			_delete_modules,
			_get_module_linked_doctype_field_map,
		)

		test_module = appyframe.new_doc("Module Def")

		test_module.update({"module_name": "RemoveThis", "app_name": "appyframe"})
		test_module.save()

		module_def_linked_doctype = appyframe.get_doc(
			{
				"doctype": "DocType",
		...
		self.assertFalse(appyframe.db.exists("Module Def", test_module.module_name))
		self.assertFalse(appyframe.db.exists("DocType", module_def_linked_doctype.name))

	def test_dry_run(self):
		"""Check if dry run in not destructive."""

		# nothing to assert, if this fails rest of the test suite will crumble.
		remove_app("appyframe", dry_run=True, yes=True, no_backup=True)


class TestSiteMigration(BaseTestCommands):
	def test_migrate_cli(self):
		with cli(appyframe.commands.site.migrate) as result:
...
class TestAddNewUser(BaseTestCommands):
	def test_create_user(self):
		...
		self.assertEqual(self.returncode, 0)
		user = appyframe.get_doc("User", "test@gmail.com")
...
class TestBenchBuild(BaseTestCommands):
	def test_build_assets_size_check(self):
		with cli(appyframe.commands.utils.build, "--force --production") as result:
			self.assertEqual(result.exit_code, 0)
			self.assertEqual(result.exception, None)
		...
		for chunk in default_bundle:
			abs_path = Path.cwd() / appyframe.local.sites_path / bundled_asset(chunk)[1:]
...
class TestDBUtils(BaseTestCommands):
	...
	def test_db_add_index(self):
		...
		appyframe.db.rollback()
		index_name = appyframe.db.get_index_name((field,))
		self.assertTrue(appyframe.db.has_index("tabUser", index_name))
		meta = appyframe.get_meta("User", cached=False)
...
class TestSchedulerUtils(BaseTestCommands):
	...
	def test_ready_for_migrate(self):
		with cli(appyframe.commands.scheduler.ready_for_migration) as result:
			self.assertEqual(result.exit_code, 0)


class TestCommandUtils(FrappeTestCase):
	def test_bench_helper(self):
		from appyframe.utils.bench_helper import get_app_groups

		app_groups = get_app_groups()
		self.assertIn("appyframe", app_groups)
		self.assertIsInstance(app_groups["appyframe"], click.Group)
...
class TestSchedulerCLI(BaseTestCommands):
	...
	def test_scheduler_status(self):
		...
		parsed_output = appyframe.parse_json(self.stdout)
		self.assertEqual(self.returncode, 0)
