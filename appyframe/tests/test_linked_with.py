...
from appyframe.desk.form import linked_with
...
class TestLinkedWith(FrappeTestCase):
	...
	def test_get_doctype_references_by_dlink_field(self):
		...
		parent_record = appyframe.get_doc({"doctype": "Parent DocType"}).insert()
	...
	def test_check_delete_integrity(self):
		...
		doc = appyframe.get_doc({"doctype": "Parent DocType"}).insert()
		doc.submit()
		doc.cancel()

		amendment = appyframe.copy_doc(doc)
		amendment.amended_from = doc.name
		amendment.docstatus = 0
		amendment.insert()
		amendment.submit()

		self.assertRaises(appyframe.LinkExistsError, doc.delete)
	...
	def test_reserved_keywords(self):
		...
		linked_doc = appyframe.new_doc(dt_name).insert().submit()

		second_doc = (
			appyframe.new_doc(dt_name, **{"from": linked_doc.doctype, "order": linked_doc.name})
			.insert()
			.submit()
		)

		with savepoint(appyframe.LinkExistsError):
