...
from appyframe.model import default_fields
from appyframe.tests.utils import FrappeTestCase
from appyframe.utils.data import get_url


class TestFrappeClient(FrappeTestCase):
	PASSWORD = appyframe.conf.admin_password or "admin"
	...
	def test_get_single(self):
		...
		appyframe.db.set_single_value("Website Settings", "title_prefix", "")
	...
	def test_update_child_doc(self):
		server = FrappeClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		appyframe.db.delete("Contact", {"first_name": "George", "last_name": "Steevens"})
		appyframe.db.delete("Contact", {"first_name": "William", "last_name": "Shakespeare"})
		appyframe.db.delete("Communication", {"reference_doctype": "Event"})
		appyframe.db.delete("Communication Link", {"link_doctype": "Contact"})
		appyframe.db.delete("Event", {"subject": "Sing a song of sixpence"})
		appyframe.db.delete("Event Participants", {"reference_doctype": "Contact"})
		appyframe.db.commit()
		...
		self.assertTrue(appyframe.db.exists("Communication Link", {"link_name": "William Shakespeare"}))

	def test_delete_doc(self):
		server = FrappeClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		NAME_TO_DELETE = server.insert({"doctype": "Note", "title": "Sing"}).get("name")
		server.delete("Note", NAME_TO_DELETE)
		self.assertFalse(appyframe.db.get_value("Note", NAME_TO_DELETE))

	def test_auth_via_api_key_secret(self):
		# generate API key and API secret for administrator
		keys = generate_keys("Administrator")
		appyframe.db.commit()
		generated_secret = appyframe.utils.password.get_decrypted_password(
			"User", "Administrator", fieldname="api_secret"
		)

		api_key = appyframe.db.get_value("User", "Administrator", "api_key")
		header = {"Authorization": f"token {api_key}:{generated_secret}"}
		res = requests.post(get_url() + "/api/method/appyframe.auth.get_logged_user", headers=header)
		...
		header = {
			"Authorization": "Basic {}".format(
				base64.b64encode(appyframe.safe_encode(f"{api_key}:{generated_secret}")).decode()
			)
		}
		res = requests.post(get_url() + "/api/method/appyframe.auth.get_logged_user", headers=header)
		self.assertEqual(res.status_code, 200)
