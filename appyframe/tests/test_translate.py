...
from appyframe.translate import (
...
from appyframe.utils import get_bench_path, set_request
...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	appyframe.get_all("Language", pluck="name", filters=[["name", "not like", "en%"], ["enabled", "=", 1]]),
	k=5,
)
...
class TestTranslate(FrappeTestCase):
	...
	def tearDown(self):
		appyframe.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
	...
	def test_clear_cache(self):
		...
		self.assertIsNotNone(appyframe.cache.hget(USER_TRANSLATION_KEY, appyframe.local.lang))
		self.assertIsNotNone(appyframe.cache.hget(MERGED_TRANSLATION_KEY, appyframe.local.lang))

		clear_cache()

		self.assertIsNone(appyframe.cache.hget(USER_TRANSLATION_KEY, appyframe.local.lang))
		self.assertIsNone(appyframe.cache.hget(MERGED_TRANSLATION_KEY, appyframe.local.lang))

	def test_extract_message_from_file(self):
		data = appyframe.translate.get_messages_from_file(translation_string_file)
		bench_path = get_bench_path()
		file_path = appyframe.get_app_path("appyframe", "tests", "translation_test_file.txt")
	...
	def test_read_language_variant(self):
		...
		try:
			appyframe.local.lang = "pt-BR"
			self.assertEqual(_("Mobile No"), "Telefone Celular")
			appyframe.local.lang = "pt"
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for appyframe.translate.get_language

		Case 1: appyframe.form_dict._lang is set
		"""

		appyframe.form_dict._lang = first_lang

		with patch.object(appyframe.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: appyframe.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(appyframe.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: appyframe.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: appyframe.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: appyframe.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_load_all_translate_files(self):
		...
		verify_translation_files("appyframe")
	...
	def test_python_extractor(self):
		code = textwrap.dedent(
			...
			appyframe._("attr")
			_("name")
			appyframe._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(appyframe.get_app_path(app)) / "translations"
