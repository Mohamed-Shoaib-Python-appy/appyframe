...
from appyframe.utils.caching import redis_cache, request_cache, site_cache
...
@site_cache
def ping() -> str:
	register_with_external_service(appyframe.local.site)
	return appyframe.local.site
...
class TestCachingUtils(FrappeTestCase):
	def test_request_cache(self):
		...
		acceptable_args = [
			...
			range(10),
			{"abc": "test-key"},
			appyframe.get_last_doc("DocType"),
			appyframe._dict(),
		]
...
class TestRedisCache(FrappeAPITestCase):
	...
	def test_user_cache(self):
		...
		def calculate_area(radius: float) -> float:
			nonlocal function_call_count
			PI_APPROX = ENGINEERING_PI if appyframe.session.user == "Engineer" else PI
			function_call_count += 1
...
class TestDocumentCache(FrappeAPITestCase):
	...
	def setUp(self) -> None:
		self.test_value = appyframe.generate_hash()

	def test_caching(self):
		doc = appyframe.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		with self.assertQueryCount(0):
			doc = appyframe.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		doc.db_set(self.TEST_FIELD, self.test_value)
		new_doc = appyframe.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		self.assertIsNot(doc, new_doc)  # Shouldn't be same object from appyframe.local
		self.assertEqual(new_doc.get(self.TEST_FIELD), self.test_value)  # Cache invalidated and fetched
		appyframe.db.rollback()

		doc_after_rollback = appyframe.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)
		...
		with self.assertQueryCount(0):
			appyframe.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)
...
class TestRedisWrapper(FrappeAPITestCase):
	def test_delete_keys(self):
		...
		for i in range(5):
			appyframe.cache.set_value(f"{prefix}{i}", 1)

		self.assertEqual(len(appyframe.cache.get_keys(prefix)), 5)
		appyframe.cache.delete_keys(prefix)
		self.assertEqual(len(appyframe.cache.get_keys(prefix)), 0)

	def test_backward_compat_cache(self):
		self.assertEqual(appyframe.cache, appyframe.cache())
