...
import appyframe
from appyframe.modules import patch_handler
...
execute:appyframe.db.updatedb("Item")
execute:appyframe.function(arg="1")
...
class TestPatches(FrappeTestCase):
	def test_patch_module_names(self):
		appyframe.flags.final_patches = []
		appyframe.flags.in_install = True
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(appyframe.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))

		appyframe.flags.in_install = False

	def test_get_patch_list(self):
		pre = patch_handler.get_patches_from_app("appyframe", patch_handler.PatchType.pre_model_sync)
		post = patch_handler.get_patches_from_app("appyframe", patch_handler.PatchType.post_model_sync)
		all_patches = patch_handler.get_patches_from_app("appyframe")
	...
	def test_all_patches_are_marked_completed(self):
		all_patches = patch_handler.get_patches_from_app("appyframe")
		finished_patches = appyframe.db.count("Patch Log")
...
class TestPatchReader(FrappeTestCase):
	def get_patches(self):
		return (
			patch_handler.get_patches_from_app("appyframe"),
			patch_handler.get_patches_from_app("appyframe", patch_handler.PatchType.pre_model_sync),
			patch_handler.get_patches_from_app("appyframe", patch_handler.PatchType.post_model_sync),
		)
...
def check_patch_files(app):
	...
	patch_dir = Path(appyframe.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = appyframe.get_module(module)
...
def _get_dotted_path(file: Path, app) -> str:
	app_path = Path(appyframe.get_app_path(app))
