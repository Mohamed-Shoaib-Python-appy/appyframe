...
from appyframe.database.utils import DefaultOrderBy
from appyframe.desk.reportview import get_filters_cond
from appyframe.handler import execute_cmd
from appyframe.model.db_query import DatabaseQuery, get_between_date_filter
from appyframe.permissions import add_user_permission, clear_user_permissions_for_doctype
from appyframe.query_builder import Column
...
class TestDBQuery(FrappeTestCase):
	...
	def test_child_table_field_syntax(self):
		note = appyframe.get_doc(
			doctype="Note",
			title=f"Test {appyframe.utils.random_string(8)}",
			content="test",
	...
	def test_child_table_join(self):
		appyframe.delete_doc_if_exists("DocType", "Parent DocType 1")
		appyframe.delete_doc_if_exists("DocType", "Parent DocType 2")
		appyframe.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		appyframe.get_doc(
			{
				"doctype": "DocType",
		...
		# clear records
		appyframe.db.delete("Parent DocType 1")
		appyframe.db.delete("Parent DocType 2")
		appyframe.db.delete("Child DocType")
		...
		# test query
		results1 = appyframe.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = appyframe.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_link_field_syntax(self):
		todo = appyframe.get_doc(doctype="ToDo", description="Test ToDo", allocated_to="Administrator").insert()
		result = appyframe.get_all(
			"ToDo",
	...
	def test_build_match_conditions(self):
		...
		test2user = appyframe.get_doc("User", "test2@example.com")
	...
	def test_none_filter(self):
		query = appyframe.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_between_filters_date_bounds(self):
		date_df = appyframe._dict(fieldtype="Date")
		datetime_df = appyframe._dict(fieldtype="Datetime")
		today = appyframe.utils.nowdate()
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		appyframe.set_user("test2@example.com")
		self.assertRaises(appyframe.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(appyframe.DataError):
			DatabaseQuery("DocType").execute(
				fields=[
					"name",
					"issingle",
	...
	def test_nested_permission(self):
		...
		from appyframe.core.page.permission_manager.permission_manager import update
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = appyframe.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = appyframe.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = appyframe.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = appyframe.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = appyframe.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = appyframe.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = appyframe.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = appyframe.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = appyframe.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(appyframe.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(appyframe.get_all("Nested DocType"))
		)
	...
	def test_is_set_is_not_set(self):
		...
		appyframe.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_virtual_field_get_list(self):
		try:
			appyframe.get_list("Prepared Report", ["*"])
			appyframe.get_list("Scheduled Job Type", ["*"])
		except Exception:
	...
	def test_prepare_select_args(self):
		# appyframe.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = appyframe.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = appyframe.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_cast_name(self):
		...
		appyframe.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		appyframe.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		appyframe.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = appyframe.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from appyframe.desk.doctype.dashboard_settings.dashboard_settings import (
		...
		dashboard_settings = appyframe.db.sql(
			f"""
				SELECT name
	...
	def test_virtual_doctype(self):
		...
		with patch(
			"appyframe.controllers",
			new={appyframe.local.site: {"Virtual DocType": VirtualDocType}},
		):
			VirtualDocType.get_list = MagicMock()

			appyframe.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)
	...
	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", appyframe.get_all("User", {"first_name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", appyframe.get_all("User", {"first_name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", appyframe.get_all("User", {"first_name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", appyframe.get_all("User", {"first_name": ("in", [])}, run=0))
		self.assertIn("ifnull", appyframe.get_all("User", {"first_name": ("not in", ["a"])}, run=0))
		self.assertIn("ifnull", appyframe.get_all("User", {"first_name": ("not in", [])}, run=0))
		self.assertIn("ifnull", appyframe.get_all("User", {"first_name": ("not in", [""])}, run=0))

		# primary key is never nullable
		self.assertNotIn("ifnull", appyframe.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertNotIn("ifnull", appyframe.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertNotIn("ifnull", appyframe.get_all("User", {"name": ("in", (""))}, run=0))
		self.assertNotIn("ifnull", appyframe.get_all("User", {"name": ("in", ())}, run=0))

	def test_coalesce_with_datetime_ops(self):
		self.assertNotIn("ifnull", appyframe.get_all("User", {"last_active": (">", "2022-01-01")}, run=0))
		self.assertNotIn("ifnull", appyframe.get_all("User", {"creation": ("<", "2022-01-01")}, run=0))
		self.assertNotIn(
			"ifnull",
			appyframe.get_all("User", {"last_active": ("between", ("2022-01-01", "2023-01-01"))}, run=0),
		)
		self.assertIn("ifnull", appyframe.get_all("User", {"last_active": ("<", "2022-01-01")}, run=0))

	def test_ambiguous_linked_tables(self):
		from appyframe.desk.reportview import get

		if not appyframe.db.exists("DocType", "Related Todos"):
			appyframe.get_doc(
				{
					"doctype": "DocType",
					"custom": 1,
			...
			appyframe.db.delete("Related Todos")

		todo_one = appyframe.get_doc(
			{
				"doctype": "ToDo",
		...
		appyframe.form_dict.doctype = "Related Todos"
	...
	def test_select_star_expansion(self):
		count = appyframe.get_list("Language", ["SUM(1)", "COUNT(*)"], as_list=1, order_by=None)[0]
		self.assertEqual(count[0], appyframe.db.count("Language"))
		self.assertEqual(count[1], appyframe.db.count("Language"))
...
class TestReportView(FrappeTestCase):
	...
	def test_get_count(self):
		...
		count = execute_cmd("appyframe.desk.reportview.get_count")
		appyframe.local.form_dict = appyframe._dict(
			{
				"doctype": "DocType",
		...
		dict_filter_response = execute_cmd("appyframe.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("appyframe.desk.reportview.get_count")
		current_value = appyframe.db.sql(
	...
	def test_reportview_get(self):
		...
		response = execute_cmd("appyframe.desk.reportview.get")
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			self.assertEqual(appyframe.get_list("Blog Post", "published"), [])
...
def create_nested_doctype():
	if appyframe.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = appyframe.new_doc("Nested DocType")
		d.update(r)
