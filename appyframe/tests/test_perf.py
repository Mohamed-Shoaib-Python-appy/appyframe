...
from appyframe.website.path_resolver import PathResolver
...
class TestPerformance(FrappeTestCase):
	def reset_request_specific_caches(self):
		...
		appyframe.destroy()  # releases everything on appyframe.local
		appyframe.init(site=self.TEST_SITE)
		appyframe.connect()
		appyframe.clear_cache()

	def setUp(self) -> None:
		self.HOST = appyframe.utils.get_site_url(appyframe.local.site)

		self.reset_request_specific_caches()

	def test_meta_caching(self):
		appyframe.clear_cache()
		appyframe.get_meta("User")
	...
	def test_permitted_fieldnames(self):
		...
		doc = appyframe.new_doc("Prepared Report")
		# load permitted fieldnames once
		doc.permitted_fieldnames

		with patch("appyframe.model.base_document.get_permitted_fields") as mocked:
			doc.as_dict()
			# get_permitted_fields should not be called again
			mocked.assert_not_called()

	def test_set_value_query_count(self):
		appyframe.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			appyframe.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			appyframe.db.set_value("User", {"user_type": "System User"}, "interest", "Nothing")
	...
	def test_get_value_limits(self):
		...
		# Warm up code
		appyframe.db.get_values("User", filters=filters[0], limit=1)
		for filter in filters:
			with self.assertRowsRead(1):
				self.assertEqual(1, len(appyframe.db.get_values("User", filters=filter, limit=1)))
			with self.assertRowsRead(2):
				self.assertEqual(2, len(appyframe.db.get_values("User", filters=filter, limit=2)))

			self.assertEqual(
				len(appyframe.db.get_values("User", filters=filter)), appyframe.db.count("User", filter)
			)

			with self.assertRowsRead(1):
				appyframe.db.get_value("User", filters=filter)

			with self.assertRowsRead(1):
				appyframe.db.exists("User", filter)
	...
	def test_consistent_build_version(self):
		from appyframe.utils import get_build_version
...
class TestOverheadCalls(FrappeAPITestCase):
	...
	def test_get_doc_overheads(self):
		sid = self.sid
		tables = len(appyframe.get_meta("User").get_table_fields())
