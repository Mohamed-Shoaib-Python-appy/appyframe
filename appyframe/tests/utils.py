...
from appyframe.utils.data import convert_utc_to_timezone, get_datetime, get_system_timezone
...
class FrappeTestCase(unittest.TestCase):
	"""Base test class for Appyframe tests.
	...
	def setUpClass(cls) -> None:
		cls.TEST_SITE = getattr(appyframe.local, "site", None) or cls.TEST_SITE
		cls.ADMIN_PASSWORD = appyframe.get_conf(cls.TEST_SITE).admin_password
		cls._primary_connection = appyframe.local.db
		cls._secondary_connection = None
		# flush changes done so far to avoid flake
		appyframe.db.commit()
		if cls.SHOW_TRANSACTION_COMMIT_WARNINGS:
			appyframe.db.before_commit.add(_commit_watcher)

		# enqueue teardown actions (executed in LIFO order)
		cls.addClassCleanup(_restore_thread_locals, copy.deepcopy(appyframe.local.flags))
	...
	# --- Appyframe Framework specific assertions
	...
	def primary_connection(self):
		...
		try:
			current_conn = appyframe.local.db
			appyframe.local.db = self._primary_connection
			yield
		finally:
			appyframe.local.db = current_conn

	@contextmanager
	def secondary_connection(self):
		"""Switch to secondary DB connection."""
		if self._secondary_connection is None:
			appyframe.connect()  # get second connection
			self._secondary_connection = appyframe.local.db

		try:
			current_conn = appyframe.local.db
			appyframe.local.db = self._secondary_connection
			yield
		finally:
			appyframe.local.db = current_conn
	...
	def assertQueryCount(self, count):
		...
		try:
			orig_sql = appyframe.db.__class__.sql
			appyframe.db.__class__.sql = _sql_with_count
			yield
			self.assertLessEqual(len(queries), count, msg="Queries executed: \n" + "\n\n".join(queries))
		finally:
			appyframe.db.__class__.sql = orig_sql
	...
	def assertRedisCallCounts(self, count):
		...
		try:
			orig_execute = appyframe.cache.execute_command
			appyframe.cache.execute_command = execute_command_and_count
			yield
			self.assertLessEqual(
				len(commands), count, msg="commands executed: \n" + "\n".join(str(c) for c in commands)
			)
		finally:
			appyframe.cache.execute_command = orig_execute
	...
	def assertRowsRead(self, count):
		...
		def _sql_with_count(*args, **kwargs):
			...
			rows_read += cint(appyframe.db._cursor.rowcount)
			return ret

		try:
			orig_sql = appyframe.db.sql
			appyframe.db.sql = _sql_with_count
			yield
			self.assertLessEqual(rows_read, count, msg="Queries read more rows than expected")
		finally:
			appyframe.db.sql = orig_sql
	...
	def enable_safe_exec(cls) -> None:
		...
		from appyframe.utils.safe_exec import SAFE_EXEC_CONFIG_KEY

		cls._common_conf = os.path.join(appyframe.local.sites_path, "common_site_config.json")
	...
	def switch_site(self, site: str):
		...
		try:
			old_site = appyframe.local.site
			appyframe.init(site, force=True)
			appyframe.connect()
			yield
		finally:
			appyframe.init(old_site, force=True)
			appyframe.connect()
...
def _rollback_db():
	appyframe.db.value_cache = {}
	appyframe.db.rollback()


def _restore_thread_locals(flags):
	appyframe.local.flags = flags
	appyframe.local.error_log = []
	appyframe.local.message_log = []
	appyframe.local.debug_log = []
	appyframe.local.conf = appyframe._dict(appyframe.get_site_config())
	appyframe.local.cache = {}
	appyframe.local.lang = "en"
	appyframe.local.preload_assets = {"style": [], "script": [], "icons": []}
...
def change_settings(doctype, settings_dict=None, /, commit=False, **settings):
	...
	try:
		settings = appyframe.get_doc(doctype)
		# remember setting
		...
		appyframe.db.value_cache[settings] = {}
		if commit:
			appyframe.db.commit()
...
def patch_hooks(overridden_hoooks):
	...
	with patch.object(appyframe, "get_hooks", patched_hooks):
		yield
...
def check_orpahned_doctypes():
	...
	doctypes = appyframe.get_all("DocType", {"custom": 0}, pluck="name")
	orpahned_doctypes = []
