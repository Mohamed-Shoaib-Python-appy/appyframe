...
from appyframe.utils import cint, get_site_url, get_test_client, get_url

try:
	_site = appyframe.local.site
except Exception:
	_site = None
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("appyframe.app.get_site_name", return_value=self.site):
				header_patch = patch("appyframe.get_request_header", new=patch_request_header)
...
class TestResourceAPI(FrappeAPITestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(20):
			doc = appyframe.get_doc({"doctype": "ToDo", "description": appyframe.mock("paragraph")}).insert()
	...
	def tearDownClass(cls):
		for name in cls.GENERATED_DOCUMENTS:
			appyframe.delete_doc_if_exists(cls.DOCTYPE, name)
		appyframe.db.commit()
	...
	def test_get_list_dict(self):
		...
		json = appyframe._dict(response.json)
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		data = {"description": appyframe.mock("paragraph"), "sid": self.sid}
	...
	def test_update_document(self):
		generated_desc = appyframe.mock("paragraph")
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "appyframe.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(FrappeAPITestCase):
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/appyframe.realtime.get_user_info
		response = self.get(self.method("appyframe.realtime.get_user_info"))
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get(self.method("appyframe.auth.get_logged_user"))
	...
	def test_logs(self):
		method = "appyframe.tests.test_api.test"

		def get_message(resp, msg_type):
			return appyframe.parse_json(appyframe.parse_json(appyframe.parse_json(resp.json)[msg_type])[0])
	...
	def test_array_response(self):
		method = "appyframe.tests.test_api.test_array"
...
class TestReadOnlyMode(FrappeAPITestCase):
	...
	def test_blocked_writes(self):
		with suppress_stdout():
			response = self.post(
				self.resource("ToDo"), {"description": appyframe.mock("paragraph"), "sid": self.sid}
			)
...
class TestWSGIApp(FrappeAPITestCase):
	def test_request_hooks(self):
		...
		with patch_hooks(
			{
				"before_request": ["appyframe.tests.test_api.before_request"],
				"after_request": ["appyframe.tests.test_api.after_request"],
			}
		):
...
class TestResponse(FrappeAPITestCase):
	def test_generate_pdf(self):
		response = self.get(
			"/api/method/appyframe.utils.print_format.download_pdf",
	...
	def test_binary_and_csv_response(self):
		def download_template(file_type):
			...
			return self.post(
				"/api/method/appyframe.core.doctype.data_import.data_import.download_template",
				{
					"sid": self.sid,
					"doctype": "User",
		...
		from appyframe.desk.utils import provide_binary_file
	...
	def test_download_private_file_with_unique_url(self):
		test_content = appyframe.generate_hash()
...
def generate_admin_keys():
	from appyframe.core.doctype.user.user import generate_keys
...
def test(*, fail=False, handled=True, message="Failed"):
	if fail:
		if handled:
			appyframe.throw(message)
		else:
			1 / 0
	else:
		appyframe.msgprint(message)
