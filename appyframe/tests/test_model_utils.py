...
import appyframe
from appyframe.model import core_doctypes_list, get_permitted_fields, is_default_field
from appyframe.model.utils import get_fetch_values
...
class TestModelUtils(FrappeTestCase):
	def test_get_fetch_values(self):
		...
		full_name = appyframe.db.get_value("User", user, "full_name")
	...
	def test_get_permitted_fields(self):
		...
		todo_all_columns = appyframe.get_meta("ToDo").get_valid_columns()
		...
		with set_user("Guest"):
			...
			picked_doctype_all_columns = appyframe.get_meta(picked_doctype).get_valid_columns()
		...
		with set_user("Administrator"):
			...
			child_all_fields = appyframe.get_meta("Installed Application").get_valid_columns()
...
def set_user(user: str):
	past_user = appyframe.session.user or "Administrator"
	appyframe.set_user(user)
	yield
	appyframe.set_user(past_user)
