...
import appyframe
from appyframe.auth import LoginAttemptTracker
from appyframe.frappeclient import AuthError, FrappeClient
from appyframe.sessions import Session, get_expired_sessions, get_expiry_in_seconds
from appyframe.tests.test_api import FrappeAPITestCase
from appyframe.tests.utils import FrappeTestCase
from appyframe.utils import get_datetime, get_site_url, now
from appyframe.utils.data import add_to_date
from appyframe.www.login import _generate_temporary_login_link
...
class TestAuth(FrappeTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.HOST_NAME = appyframe.get_site_config().host_name or get_site_url(appyframe.local.site)
	...
	def tearDownClass(cls):
		appyframe.delete_doc("User", cls.test_user_email, force=True)
		appyframe.local.request_ip = None
		appyframe.form_dict.email = None
		appyframe.local.response["http_status_code"] = None

	def set_system_settings(self, k, v):
		appyframe.db.set_single_value("System Settings", k, v)
...
class TestSessionExpirty(FrappeAPITestCase):
	def test_session_expires(self):
		...
		s: Session = appyframe.local.session_obj
