...
import appyframe
from appyframe.app import process_response
...
class TestCORS(FrappeTestCase):
	def make_request_and_test(self, origin="http://example.com", absent=False):
		...
		appyframe.utils.set_request(method="OPTIONS", headers=headers)
	...
	def test_valid_origin(self):
		...
		appyframe.conf.allow_cors = "*"
		self.make_request_and_test()

		appyframe.conf.allow_cors = ["http://example.com", "https://example.com"]
		self.make_request_and_test()

	def test_invalid_origin(self):
		appyframe.conf.allow_cors = "http://example1.com"
		self.make_request_and_test(absent=True)

		appyframe.conf.allow_cors = ["http://example1.com", "https://example.com"]
		self.make_request_and_test(absent=True)
