...
from appyframe.tests.test_api import FrappeAPITestCase, suppress_stdout
...
class TestResourceAPIV2(FrappeAPITestCase):
	...
	def test_copy_document(self):
		doc = appyframe.get_doc(self.DOCTYPE, self.GENERATED_DOCUMENTS[0])
	...
	def test_execute_doc_method(self):
		...
		self.assertEqual(response.json["data"][0]["name"], "appyframe")
	...
	def test_delete_document_non_existing(self):
		non_existent_doc = appyframe.generate_hash(length=12)
...
class TestMethodAPIV2(FrappeAPITestCase):
	...
	def test_shorthand_controller_methods(self):
		...
		expanded_response = self.get(
			self.method("appyframe.core.doctype.user.user.get_all_roles"), {"sid": self.sid}
		)
	...
	def test_add_comment(self):
		comment_txt = appyframe.generate_hash()
		response = self.post(
