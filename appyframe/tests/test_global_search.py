...
from appyframe.utils import global_search, now_datetime
...
class TestGlobalSearch(FrappeTestCase):
	def setUp(self):
		...
		self.assertTrue("__global_search" in appyframe.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		appyframe.db.delete("Property Setter", {"doc_type": "Event"})
	...
	def test_update_doc(self):
		...
		event = appyframe.get_doc("Event", appyframe.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		self.insert_test_events()
		event_name = appyframe.get_all("Event")[0].name
		event = appyframe.get_doc("Event", event_name)
		...
		appyframe.delete_doc("Event", event_name)
		global_search.sync_global_search()
		appyframe.db.commit()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = appyframe.get_doc({"doctype": "Event", "subject": text, "starts_on": now_datetime()})
			doc.insert()
