...
import appyframe
from appyframe import get_hooks
from appyframe.tests.utils import FrappeTestCase
from appyframe.utils import set_request
from appyframe.website.page_renderers.static_page import StaticPage
from appyframe.website.serve import get_response, get_response_content
from appyframe.website.utils import build_response, clear_website_cache, get_home_page
...
class TestWebsite(FrappeTestCase):
	...
	def test_home_page(self):
		...
		appyframe.db.set_value("Role", "home-page-test", "home_page", "home-page-test")
		appyframe.set_user("test-user-for-home-page@example.com")
		self.assertEqual(get_home_page(), "home-page-test")

		appyframe.set_user("Administrator")
		appyframe.db.set_value("Role", "home-page-test", "home_page", "")

		# home page via portal settings
		appyframe.db.set_single_value("Portal Settings", "default_portal_home", "test-portal-home")

		appyframe.set_user("test-user-for-home-page@example.com")
		appyframe.cache.hdel("home_page", appyframe.session.user)
		self.assertEqual(get_home_page(), "test-portal-home")

		appyframe.db.set_single_value("Portal Settings", "default_portal_home", "")
		clear_website_cache()

		# home page via website settings
		appyframe.db.set_single_value("Website Settings", "home_page", "contact")
		self.assertEqual(get_home_page(), "contact")

		appyframe.db.set_single_value("Website Settings", "home_page", None)
		clear_website_cache()
		...
		with patch.object(
			...
			"get_hooks",
			patched_get_hooks(
				"get_website_user_home_page", ["appyframe.www._test._test_home_page.get_website_user_home_page"]
			),
		):
			self.assertEqual(get_home_page(), "_test/_test_folder")

		clear_website_cache()
		with patch.object(appyframe, "get_hooks", patched_get_hooks("website_user_home_page", ["login"])):
			self.assertEqual(get_home_page(), "login")

		clear_website_cache()
		with patch.object(appyframe, "get_hooks", patched_get_hooks("home_page", ["about"])):
			self.assertEqual(get_home_page(), "about")

		clear_website_cache()
		with patch.object(
			appyframe, "get_hooks", patched_get_hooks("role_home_page", {"home-page-test": ["home-page-test"]})
		):
			self.assertEqual(get_home_page(), "home-page-test")
	...
	def test_page_load(self):
		...
		html = appyframe.safe_decode(response.get_data())
	...
	def test_app(self):
		...
		appyframe.local.session_obj = None
	...
	def test_redirect(self):
		import appyframe.hooks

		appyframe.set_user("Administrator")

		appyframe.hooks.website_redirects = [
		...
		website_settings = appyframe.get_doc("Website Settings")
		...
		delattr(appyframe.hooks, "website_redirects")
		appyframe.cache.delete_key("app_hooks")
	...
	def test_custom_page_renderer(self):
		...
		def patched_get_hooks(*args, **kwargs):
			...
			if args and args[0] == "page_renderer":
				return_value = ["appyframe.tests.test_website.CustomPageRenderer"]
			return return_value

		with patch.object(appyframe, "get_hooks", patched_get_hooks):
	...
	def test_printview_page(self):
		appyframe.db.value_cache[("DocType", "Language", "name")] = (("Language",),)
		appyframe.set_user("Administrator")
	...
	def test_json_sidebar_data(self):
		appyframe.flags.look_for_sidebar = False
		content = get_response_content("/_test/_test_folder/_test_page")
		self.assertNotIn("Test Sidebar", content)
		clear_website_cache()
		appyframe.flags.look_for_sidebar = True
		content = get_response_content("/_test/_test_folder/_test_page")
	...
	def test_caching(self):
		# to enable caching
		appyframe.flags.force_website_cache = True
	...
	def test_safe_render(self):
		...
		self.assertNotIn("appyframe.exceptions.ValidationError: Illegal template", content)

	def test_never_render(self):
		from pathlib import Path
		from random import choices

		WWW = Path(appyframe.get_app_path("appyframe")) / "www"
	...
	def test_resolve_class(self):
		from appyframe.utils.jinja_globals import resolve_class

		context = appyframe._dict(primary=True)
		...
		self.assertEqual(
			appyframe.render_template(content, context), '<a class="btn btn-default btn-primary">Test</a>'
		)
	...
	def test_app_include(self):
		...
		with patch.object(appyframe, "get_hooks", patched_get_hooks):
			appyframe.set_user("Administrator")
			appyframe.hooks.app_include_js.append("test_app_include.js")
			appyframe.hooks.app_include_css.append("test_app_include.css")
			appyframe.conf.update({"app_include_js": ["test_app_include_via_site_config.js"]})
			appyframe.conf.update({"app_include_css": ["test_app_include_via_site_config.css"]})
