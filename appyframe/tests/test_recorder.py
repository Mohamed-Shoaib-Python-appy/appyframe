...
from appyframe.recorder import normalize_query
...
class TestRecorder(FrappeTestCase):
	...
	def wait_for_background_jobs(self):
		while any_job_pending(appyframe.local.site):
			time.sleep(1)
	...
	def test_start(self):
		self.stop_recording()
		requests = appyframe.recorder.get()
		self.assertEqual(len(requests), 1)

	def test_do_not_record(self):
		appyframe.recorder.do_not_record(appyframe.get_all)("DocType")
		self.stop_recording()
		requests = appyframe.recorder.get()
	...
	def test_get(self):
		...
		request = appyframe.recorder.get(requests[0]["uuid"])
	...
	def test_explain(self):
		appyframe.db.sql("SELECT * FROM tabDocType")
		appyframe.db.sql("COMMIT")
		appyframe.db.sql("select 1", run=0)
		self.stop_recording()
	...
	def test_multiple_queries(self):
		...
		sql_dialect = appyframe.db.db_type or "mariadb"
		for query in queries:
			appyframe.db.sql(query[sql_dialect])
...
class TestRecorderDeco(FrappeTestCase):
	def test_recorder_flag(self):
		...
		@appyframe.recorder.record_queries
		def test():
			appyframe.get_all("User")

		test()
		self.assertTrue(appyframe.recorder.get())
