...
from appyframe.utils import get_bench_id
from appyframe.utils.background_jobs import get_redis_conn
...
class TestRedisAuth(FrappeTestCase):
	...
	@patch.dict(appyframe.conf, {"bench_id": "test_bench", "use_rq_auth": False})
	...
	def test_rq_namespace(self):
		...
		# Current bench ID
		bench_id = appyframe.conf.get("bench_id")
		conn = get_redis_conn()
		...
		conn.acl_deluser(username)
		appyframe.conf.update({"bench_id": tmp_bench_id})
		_ = RedisQueue(conn).add_user(username, password)
		...
		appyframe.conf.update({"bench_id": bench_id})
		conn.acl_deluser(username)
