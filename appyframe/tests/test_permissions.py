...
from appyframe.core.doctype.user_permission.user_permission import clear_user_permissions
from appyframe.core.page.permission_manager.permission_manager import add, remove, reset, update
...
from appyframe.test_runner import make_test_records_for_doctype
...
class TestPermissions(FrappeTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		appyframe.clear_cache(doctype="Blog Post")
		user = appyframe.get_doc("User", "test1@example.com")
		...
		user = appyframe.get_doc("User", "test3@example.com")
		user.add_roles("Sales User")

		user = appyframe.get_doc("User", "testperm@example.com")
		user.add_roles("Website Manager")
	...
	def setUp(self):
		...
		appyframe.db.delete("User Permission")

		appyframe.set_user("test1@example.com")

	def tearDown(self):
		appyframe.set_user("Administrator")
		appyframe.db.set_value("Blogger", "_Test Blogger 1", "user", None)
	...
	def set_strict_user_permissions(ignore):
		ss = appyframe.get_doc("System Settings")
		ss.apply_strict_user_permissions = ignore
		ss.flags.ignore_mandatory = 1
		ss.save()

	def test_basic_permission(self):
		post = appyframe.get_doc("Blog Post", "-test-blog-post")
	...
	def test_select_permission(self):
		...
		appyframe.clear_cache(doctype="Blog Post")
		appyframe.set_user("test3@example.com")
		...
		self.assertRaises(appyframe.PermissionError, post.save)

		permitted_record = appyframe.get_list("Blog Post", fields="*", limit=1)[0]
		full_record = appyframe.get_all("Blog Post", fields="*", limit=1)[0]
	...
	def test_user_permissions_in_doc(self):
		...
		post1 = appyframe.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_user_permissions_in_report(self):
		...
		appyframe.set_user("test2@example.com")
		names = [d.name for d in appyframe.get_list("Blog Post", fields=["name", "blog_category"])]

		self.assertTrue("-test-blog-post-1" in names)
		self.assertFalse("-test-blog-post" in names)

	def test_default_values(self):
		doc = appyframe.new_doc("Blog Post")
	...
	def test_user_link_match_doc(self):
		blogger = appyframe.get_doc("Blogger", "_Test Blogger 1")
		blogger.user = "test2@example.com"
		blogger.save()

		appyframe.set_user("test2@example.com")

		post = appyframe.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(post.has_permission("read"))
	...
	def test_user_link_match_report(self):
		...
		names = [d.name for d in appyframe.get_list("Blog Post", fields=["name", "owner"])]
	...
	def test_not_allowed_to_set_user_permissions(self):
		...
		self.assertRaises(
			appyframe.PermissionError, add_user_permission, "Blog Post", "-test-blog-post", "test2@example.com"
		)
	...
	def test_read_if_explicit_user_permissions_are_set(self):
		...
		doc = appyframe.get_doc("Blog Post", "-test-blog-post")
		self.assertTrue(doc.has_permission("read"))

		# and not this one
		doc = appyframe.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_standard_fields_manually(self):
		...
		fake_owner = appyframe.db.get_value("User", {"name": ("!=", appyframe.session.user)})

		d = appyframe.new_doc("ToDo")
	...
	def test_dont_change_standard_constants(self):
		...
		self.assertRaises(appyframe.CannotChangeConstantError, user.save)
	...
	def test_set_only_once(self):
		blog_post = appyframe.get_meta("Blog Post")
		doc = appyframe.get_doc("Blog Post", "-test-blog-post-1")
		doc.db_set("title", "Old")
		blog_post.get_field("title").set_only_once = 1
		doc.title = "New"
		self.assertRaises(appyframe.CannotChangeConstantError, doc.save)
	...
	def test_set_only_once_child_table_rows(self):
		...
		doc = appyframe.get_doc("DocType", "Blog Post")
	...
	def test_user_permission_doctypes(self):
		...
		doc = appyframe.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(doc.has_permission("read"))
	...
	def test_insert_if_owner_with_user_permissions(self):
		...
		appyframe.delete_doc("Blog Post", "-test-blog-post-title")
		...
		self.assertRaises(appyframe.PermissionError, doc.insert)
		...
		appyframe.set_user("test2@example.com")
		doc = appyframe.get_doc(doc.doctype, doc.name)
		self.assertTrue(doc.has_permission("read"))
	...
	def test_strict_user_permissions(self):
		...
		appyframe.set_user("Administrator")
		appyframe.db.delete("Contact")
		appyframe.db.delete("Contact Email")
		appyframe.db.delete("Contact Phone")
		...
		allowed_contact = appyframe.get_doc("Contact", "_Test Contact For _Test Customer")
		other_contact = appyframe.get_doc("Contact", "_Test Contact For _Test Supplier")

		appyframe.set_user("test3@example.com")
		self.assertTrue(allowed_contact.has_permission("read"))
		self.assertTrue(other_contact.has_permission("read"))
		self.assertEqual(len(appyframe.get_list("Contact")), 2)
		...
		self.assertTrue(len(appyframe.get_list("Contact")), 1)
	...
	def test_contextual_user_permission(self):
		...
		posts = appyframe.get_all("Blog Post", fields=["name", "blogger"])
		...
		posts = appyframe.get_list("Blog Post", fields=["name", "blogger"])
	...
	def test_if_owner_permission_overrides_properly(self):
		...
		appyframe.set_user("test2@example.com")
		self.assertEqual(appyframe.get_list("Blog Post"), [])
	...
	def test_if_owner_permission_on_getdoc(self):
		...
		doclist = [d.name for d in appyframe.response.docs]
		self.assertTrue(doc.name in doclist)

		appyframe.set_user("test2@example.com")
		self.assertRaises(appyframe.PermissionError, getdoc, "Blog Post", doc.name)
	...
	def test_if_owner_permission_on_get_list(self):
		...
		appyframe.set_user("test2@example.com")
		self.assertIn(doc.name, appyframe.get_list("Blog Post", pluck="name"))
		...
		appyframe.set_user("test2@example.com")
		self.assertNotIn(doc.name, appyframe.get_list("Blog Post", pluck="name"))
	...
	def test_if_owner_permission_on_delete(self):
		...
		self.assertRaises(appyframe.PermissionError, appyframe.delete_doc, "Blog Post", doc.name)

		appyframe.set_user("test2@example.com")
		appyframe.delete_doc("Blog Post", "-test-blog-post-title-new-1")
	...
	def test_clear_user_permissions(self):
		...
		# reset the user
		appyframe.set_user(current_user)

	def test_child_permissions(self):
		appyframe.set_user("test3@example.com")
		self.assertIsInstance(appyframe.get_list("DefaultValue", parent_doctype="User", limit=1), list)

		# appyframe.get_list
		self.assertRaises(appyframe.PermissionError, appyframe.get_list, "DefaultValue")
		self.assertRaises(appyframe.PermissionError, appyframe.get_list, "DefaultValue", parent_doctype="ToDo")
		self.assertRaises(
			appyframe.PermissionError, appyframe.get_list, "DefaultValue", parent_doctype="DefaultValue"
		)
		...
		self.assertRaises(appyframe.PermissionError, doc.check_permission)
		...
		user = appyframe.get_doc("User", "Administrator")
		doc = user.append("defaults")
		self.assertRaises(appyframe.PermissionError, doc.check_permission)
	...
	def test_select_user(self):
		...
		for gender in ("Male", "Female"):
			if appyframe.db.exists("Gender", gender):
				continue

			appyframe.get_doc({"doctype": "Gender", "gender": gender}).insert()

		# asssign gender to test users
		appyframe.db.set_value("User", "test1@example.com", "gender", "Male")
		appyframe.db.set_value("User", "test2@example.com", "gender", "Female")
		appyframe.db.set_value("User", "test3@example.com", "gender", "Female")
		...
		appyframe.set_user("test3@example.com")
		users = appyframe.get_list("User", pluck="name")
	...
	def test_automatic_permissions(self):
		def assertHasRole(*roles: str | tuple[str, ...]):
			for role in roles:
				self.assertIn(role, appyframe.get_roles())
	...
	def test_overrides_work_as_expected(self):
		...
		custom_role = appyframe.new_doc("Role", role_name=appyframe.generate_hash()).insert().name
		with self.set_user("Administrator"):
