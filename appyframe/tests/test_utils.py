...
import appyframe
from appyframe.installer import parse_app_name
from appyframe.model.document import Document
from appyframe.tests.utils import FrappeTestCase, MockedRequestTestCase, change_settings
...
from appyframe.utils.change_log import (
	get_source_url,
	parse_github_url,
)
from appyframe.utils.data import (
	add_to_date,
	add_years,
	cast,
	cint,
...
from appyframe.utils.dateutils import get_dates_from_timegrain
from appyframe.utils.diff import _get_value_from_version, get_version_diff, version_query
from appyframe.utils.identicon import Identicon
from appyframe.utils.image import optimize_image, strip_exif_data
from appyframe.utils.make_random import can_make, get_random, how_many
from appyframe.utils.response import json_handler
from appyframe.utils.synchronization import LockTimeoutError, filelock
	...
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/appyframe/test.jpg">
			</div>
			<div style="background-image: url('/assets/appyframe/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/appyframe/test.jpg">' in html)
		self.assertTrue(f"style=\"background-image: url('{url}/assets/appyframe/bg.jpg') !important\"" in html)
...
class TestHTMLUtils(FrappeTestCase):
	def test_clean_email_html(self):
		from appyframe.utils.html_utils import clean_email_html
...
class TestValidationUtils(FrappeTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://appyframe.io", throw=True))
		...
		self.assertRaises(appyframe.ValidationError, validate_url, "appyframe", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://appyframe.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(validate_url("bolo://appyframe.io", valid_schemes=("http", "https", "ftp", "ftps")))
		self.assertRaises(
			appyframe.ValidationError, validate_url, "gopher://appyframe.io", valid_schemes="https", throw=True
		)
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@appyframe.com"))
		self.assertTrue(validate_email_address("someone@appyframe.com, anyone@appyframe.io"))
		self.assertTrue(validate_email_address("test%201@appyframe.com"))

		# Invalid address
		self.assertFalse(validate_email_address("someone"))
		self.assertFalse(validate_email_address("someone@----.com"))
		self.assertFalse(validate_email_address("test 1@appyframe.com"))
		self.assertFalse(validate_email_address("test@example.com test2@example.com,undisclosed-recipient"))

		# Invalid with throw
		self.assertRaises(appyframe.InvalidEmailAddressError, validate_email_address, "someone.com", throw=True)

		self.assertEqual(validate_email_address("Some%20One@appyframe.com"), "Some%20One@appyframe.com")
		self.assertEqual(
			validate_email_address("erp+Job%20Applicant=JA00004@appyframe.com"),
			"erp+Job%20Applicant=JA00004@appyframe.com",
		)
	...
	def test_valid_phone(self):
		...
		self.assertRaises(
			appyframe.InvalidPhoneNumberError,
			validate_phone_number_with_country_code,
			"+420 1234567890",
			"field",
		)
	...
	def test_validate_name(self):
		...
		for name in invalid_names:
			self.assertRaises(appyframe.InvalidNameError, validate_name, name, True)


class TestImage(FrappeTestCase):
	def test_strip_exif_data(self):
		original_image = Image.open(appyframe.get_app_path("appyframe", "tests", "data", "exif_sample_image.jpg"))
		original_image_content = open(
			appyframe.get_app_path("appyframe", "tests", "data", "exif_sample_image.jpg"), mode="rb"
		).read()
	...
	def test_optimize_image(self):
		image_file_path = appyframe.get_app_path("appyframe", "tests", "data", "sample_image_for_optimization.jpg")
...
class TestPythonExpressions(FrappeTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(appyframe.ValidationError, validate_python_code, expr)
...
class TestDiffUtils(FrappeTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = appyframe.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(FrappeTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(appyframe.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				appyframe.utils.get_first_day_of_week("2020-12-25"), appyframe.utils.getdate("2020-12-21")
			)
			self.assertEqual(
				appyframe.utils.get_first_day_of_week("2020-12-20"), appyframe.utils.getdate("2020-12-14")
			)

		# Sunday as start of the week
		self.assertEqual(appyframe.utils.get_first_day_of_week("2020-12-25"), appyframe.utils.getdate("2020-12-20"))
		self.assertEqual(appyframe.utils.get_first_day_of_week("2020-12-21"), appyframe.utils.getdate("2020-12-20"))

	def test_last_day_of_week(self):
		self.assertEqual(appyframe.utils.get_last_day_of_week("2020-12-24"), appyframe.utils.getdate("2020-12-26"))
		self.assertEqual(appyframe.utils.get_last_day_of_week("2020-12-28"), appyframe.utils.getdate("2021-01-02"))

	def test_is_last_day_of_the_month(self):
		self.assertEqual(appyframe.utils.is_last_day_of_the_month("2020-12-24"), False)
		self.assertEqual(appyframe.utils.is_last_day_of_the_month("2020-12-31"), True)
...
class TestResponse(FrappeTestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				appyframe.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
...
class TestXlsxUtils(FrappeTestCase):
	def test_unescape(self):
		from appyframe.utils.xlsxutils import handle_html
...
class TestLinkTitle(FrappeTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		"""
		custom_doctype = appyframe.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		from appyframe.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = appyframe.local.response["_link_titles"]
...
class TestAppParser(MockedRequestTestCase):
	def test_app_name_parser(self):
		self.responses.add(
			"HEAD",
			"https://api.github.com/repos/appyframe/healthcare",
			status=200,
			json={},
		)
		bench_path = get_bench_path()
		frappe_app = os.path.join(bench_path, "apps", "appyframe")
		self.assertEqual("appyframe", parse_app_name(frappe_app))
		self.assertEqual("healthcare", parse_app_name("healthcare"))
		self.assertEqual("healthcare", parse_app_name("https://github.com/appyframe/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("git@github.com:appyframe/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("appyframe/healthcare@develop"))
...
class TestIntrospectionMagic(FrappeTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(appyframe.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(appyframe.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(appyframe.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(appyframe.get_newargs(lambda: None, args), {})
...
class TestLazyLoader(FrappeTestCase):
	def test_lazy_import_module(self):
		from appyframe.utils.lazy_loader import lazy_import

		with Capturing() as output:
			ls = lazy_import("appyframe.tests.data.load_sleep")
		self.assertEqual(output, [])

		with Capturing() as output:
			ls.time
		self.assertEqual(["Module `appyframe.tests.data.load_sleep` loaded"], output)


class TestIdenticon(FrappeTestCase):
	def test_get_gravatar(self):
		# developers@appyframe.io has a gravatar linked so str URL will be returned
		appyframe.flags.in_test = False
		gravatar_url = get_gravatar("developers@appyframe.io")
		appyframe.flags.in_test = True
		...
		gravatar_url = get_gravatar(f"developers{random_string(6)}@appyframe.io")
		self.assertIsInstance(gravatar_url, str)
...
class TestMiscUtils(FrappeTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(appyframe.local.site, get_sites())

	def test_get_site_info(self):
		info = get_site_info()

		installed_apps = [app["app_name"] for app in info["installed_apps"]]
		self.assertIn("appyframe", installed_apps)
...
class TestTypingValidations(FrappeTestCase):
	...
	def test_validate_whitelisted_api(self):
		...
		for fn, args, kwargs in test_cases:
			with self.assertRaisesRegex(appyframe.FrappeTypeError, self.ERR_REGEX):
				fn(*args, **kwargs)

	def test_validate_whitelisted_doc_method(self):
		report = appyframe.get_last_doc("Report")
...
class TestTBSanitization(FrappeTestCase):
	def test_traceback_sanitzation(self):
		try:
			password = appyframe.generate_hash()
			args = {"password": "42", "pwd": password, "safe": "safe_value"}
			args = appyframe._dict({"password": "42", "pwd": password, "safe": "safe_value"})  # noqa: F841
			raise Exception
		except Exception:
			traceback = appyframe.get_traceback(with_context=True)
...
class TestRounding(FrappeTestCase):
	...
	def test_default_rounding(self):
		self.assertEqual(appyframe.get_system_settings("rounding_method"), "Banker's Rounding")
...
class TestArgumentTypingValidations(FrappeTestCase):
	def test_validate_argument_types(self):
		from appyframe.core.doctype.doctype.doctype import DocType
		from appyframe.utils.typing_validations import FrappeTypeError, validate_argument_types
		...
		doctype = appyframe.get_last_doc("DocType")
...
class TestChangeLog(FrappeTestCase):
	def test_get_remote_url(self):
		self.assertIsInstance(get_source_url("appyframe"), str)

	def test_parse_github_url(self):
		# using erpnext as repo in order to be different from the owner
		owner, repo = parse_github_url("https://github.com/appyframe/erpnext.git")
		self.assertEqual(owner, "appyframe")
		self.assertEqual(repo, "erpnext")

		owner, repo = parse_github_url("https://github.com/appyframe/erpnext")
		self.assertEqual(owner, "appyframe")
		self.assertEqual(repo, "erpnext")

		owner, repo = parse_github_url("git@github.com:appyframe/erpnext.git")
		self.assertEqual(owner, "appyframe")
		self.assertEqual(repo, "erpnext")
