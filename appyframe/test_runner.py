...
from appyframe.modules import get_module_name, load_doctype_module
...
def main(
	...
	if doctype_list_path:
		...
		with open(appyframe.get_app_path(app, doctype_list_path)) as f:
	...
	try:
		appyframe.flags.print_messages = verbose
		appyframe.flags.in_test = True

		# workaround! since there is no separate test db
		appyframe.clear_cache()
		scheduler_disabled_by_user = appyframe.utils.scheduler.is_scheduler_disabled(verbose=False)
		if not scheduler_disabled_by_user:
			appyframe.utils.scheduler.disable_scheduler()

		if not appyframe.flags.skip_before_tests:
			if verbose:
				print('Running "before_tests" hooks')
			for fn in appyframe.get_hooks("before_tests", app_name=app):
		...
		if doctype:
			...
			doctypes = appyframe.db.get_list(
...
def run_all_tests(app=None, verbose=False, profile=False, failfast=False, junit_xml_output=False):
	...
	apps = [app] if app else appyframe.get_installed_apps()

	test_suite = unittest.TestSuite()
	for app in apps:
		for path, folders, files in os.walk(appyframe.get_app_path(app)):
...
def run_tests_for_doctype(
	...
	for doctype in doctypes:
		module = appyframe.db.get_value("DocType", doctype, "module")
		if not module:
		...
		if force:
			for name in appyframe.db.sql_list("select name from `tab%s`" % doctype):
				appyframe.delete_doc(doctype, name, force=True)
...
def _run_unittest(
	...
	appyframe.flags.tests_verbose = verbose
...
def _add_test(app, path, filename, verbose, test_suite=None):
	...
	app_path = appyframe.get_app_path(app)
...
def make_test_records(doctype, verbose=0, force=False, commit=False):
	if appyframe.flags.skip_test_records:
		return
	...
	for options in get_dependencies(doctype):
		...
		if options not in appyframe.local.test_objects:
			appyframe.local.test_objects[options] = []
...
def get_dependencies(doctype):
	...
	for df in meta.get_table_fields():
		link_fields.extend(appyframe.get_meta(df.options).get_link_fields())
...
def make_test_records_for_doctype(doctype, verbose=0, force=False, commit=False):
	...
	if hasattr(test_module, "_make_test_records"):
		appyframe.local.test_objects[doctype] += test_module._make_test_records(verbose)

	elif hasattr(test_module, "test_records"):
		if doctype in appyframe.local.test_objects:
			appyframe.local.test_objects[doctype] += make_test_objects(
		...
		test_records = appyframe.get_test_records(doctype)
		if test_records:
			appyframe.local.test_objects[doctype] += make_test_objects(
...
def make_test_objects(doctype, test_records=None, verbose=None, reset=False, commit=False):
	...
	for doc in test_records:
		...
		if appyframe.db.exists(d.doctype, d.name) and not reset:
...
def add_to_test_record_log(doctype):
	...
	if doctype not in test_record_log:
		appyframe.flags.test_record_log.append(doctype)
		with open(appyframe.get_site_path(".test_log"), "w") as f:
			f.write("\n".join(filter(None, appyframe.flags.test_record_log)))


def get_test_record_log():
	"""Return the list of doctypes for which test records have been created"""
	if "test_record_log" not in appyframe.flags:
		if os.path.exists(appyframe.get_site_path(".test_log")):
			with open(appyframe.get_site_path(".test_log")) as f:
				appyframe.flags.test_record_log = f.read().splitlines()
		else:
			appyframe.flags.test_record_log = []

	return appyframe.flags.test_record_log
