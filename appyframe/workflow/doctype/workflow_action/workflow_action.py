...
from appyframe.desk.form.utils import get_pdf_link
from appyframe.desk.notifications import clear_doctype_notifications
...
from appyframe.utils import get_datetime, get_url
from appyframe.utils.background_jobs import enqueue
from appyframe.utils.data import get_link_to_form
from appyframe.utils.user import get_users_with_role
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from appyframe.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	appyframe.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def get_permission_query_conditions(user):
	...
	permitted_workflow_actions = (
		appyframe.qb.from_(WorkflowAction)
	...
	return f"""(`tabWorkflow Action`.`name` in ({permitted_workflow_actions})
		or `tabWorkflow Action`.`user`={appyframe.db.escape(user)})
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(appyframe.get_roles(user))
...
def process_workflow_actions(doc, state):
	...
	if send_email_alert(workflow) and appyframe.db.get_value(
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = appyframe.session.user
	...
	if logged_in_user == "Guest":
		appyframe.set_user(logged_in_user)
...
def return_success_page(doc):
	appyframe.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), appyframe.bold(doc.get("name")), appyframe.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	appyframe.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			appyframe.bold(doc.get("name")),
			appyframe.bold(doc_workflow_state),
			appyframe.bold(appyframe.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else appyframe.session.user

	allowed_roles = appyframe.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(appyframe.get_roles(user))
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		appyframe.qb.update(WorkflowAction)
...
def clear_old_workflow_actions_using_user(doc, user=None):
	...
	if appyframe.db.has_column("Workflow Action", "user"):
		appyframe.db.delete(
			"Workflow Action",
			{
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = appyframe._dict(
					{
						"possible_actions": [],
						"email": appyframe.db.get_value("User", user, "email"),
					}
				)
...
def create_workflow_actions_for_roles(roles, doc):
	if not roles:
		return
	workflow_action = appyframe.get_doc(
		{
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = "/api/method/appyframe.workflow.doctype.workflow_action.workflow_action.apply_action"
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/appyframe.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	lang = doc.get("language") or (
		appyframe.get_cached_value("Print Format", print_format, "default_print_language")
		if print_format
		else None
	)
...
def get_email_template_from_workflow(doc):
	...
	template_name = appyframe.db.get_value(
		"Workflow Document State",
...
def get_state_optional_field_value(workflow_name, state):
	return appyframe.get_cached_value(
