...
import appyframe
from appyframe.desk.form.assign_to import add as assign_to
from appyframe.desk.page.user_profile.user_profile import get_energy_points_heatmap_data
...
class TestEnergyPointLog(FrappeTestCase):
	...
	def setUp(self):
		appyframe.cache.delete_value("energy_point_rule_map")

	def tearDown(self):
		appyframe.set_user("Administrator")
		appyframe.db.delete("Energy Point Log")
		appyframe.db.delete("Energy Point Rule")
		appyframe.cache.delete_value("energy_point_rule_map")
	...
	def test_revert_points_on_cancelled_doc(self):
		...
		energy_point_logs = appyframe.get_all("Energy Point Log")
		...
		energy_point_logs = appyframe.get_all("Energy Point Log", fields=["reference_name", "type", "reverted"])
	...
	def test_allow_creation_of_new_log_if_the_previous_log_was_reverted(self):
		...
		log_name = appyframe.db.exists("Energy Point Log", {"reference_name": created_todo.name})
		appyframe.get_doc("Energy Point Log", log_name).revert("Just for test")
...
def create_energy_point_rule_for_todo(
	...
	name = "ToDo Closed"
	point_rule_exists = appyframe.db.exists("Energy Point Rule", name)

	if point_rule_exists:
		return appyframe.get_doc("Energy Point Rule", name)
