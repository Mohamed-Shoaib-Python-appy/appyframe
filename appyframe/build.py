...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "appyframe", "dist", type)
	...
	assets_json = appyframe.read_file("assets/assets.json")
	if assets_json:
		assets_json = appyframe.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/appyframe"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["appyframe/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="appyframe")
...
def get_assets_link(frappe_head) -> str:
	...
	tag = getoutput(
		r"cd ../apps/appyframe && git show-ref --tags -d | grep %s | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{}//'" % frappe_head
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/appyframe/appyframe/releases/download/{tag}/assets.tar.gz"
	else:
...
def fetch_assets(url, frappe_head):
	...
	prefix = mkdtemp(prefix="appyframe-assets-", suffix=frappe_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded Appyframe assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./appyframe-bench/sites", "")
...
def download_frappe_assets(verbose=True):
	"""Downloads and sets up Appyframe assets if they exist based on the current
	commit HEAD.
	Returns True if correctly setup else returns False.
	"""
	frappe_head = getoutput("cd ../apps/appyframe && git rev-parse HEAD")
...
def symlink(target, link_name, overwrite=False):
	...
	while True:
		temp_link_name = f"tmp{appyframe.generate_hash()}"
...
def setup():
	...
	for app in appyframe.get_all_apps(True):
		try:
			pymodules.append(appyframe.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(appyframe.local.sites_path, "assets")
...
def bundle(
	...
	frappe_app_path = appyframe.get_app_source_path("appyframe")
	appyframe.commands.popen(command, cwd=frappe_app_path, env=get_node_env(), raise_err=True)

	with suppress(Exception):
		appyframe.cache.flushall()
...
def watch(apps=None):
	...
	live_reload = appyframe.utils.cint(os.environ.get("LIVE_RELOAD", appyframe.conf.live_reload))
	...
	check_node_executable()
	frappe_app_path = appyframe.get_app_source_path("appyframe")
	appyframe.commands.popen(command, cwd=frappe_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in appyframe.get_all_apps():
		app_doc_path = None

		pymodule = appyframe.get_module(app_name)
...
def html_to_js_template(path, content):
	"""returns HTML template content as Javascript code, adding it to `appyframe.templates`"""
	return """appyframe.templates["{key}"] = '{content}';\n""".format(
