appyframe.pages["print-format-builder-beta"].on_page_load = function (wrapper) {
	...
	if (appyframe.boot.developer_mode) {
		...
		appyframe.hot_update.push(() => load_print_format_builder_beta(wrapper));
	}
};

appyframe.pages["print-format-builder-beta"].on_page_show = function (wrapper) {
...
function load_print_format_builder_beta(wrapper) {
	...
	if (route.length > 1) {
		appyframe.require("print_format_builder.bundle.js").then(() => {
			appyframe.print_format_builder = new appyframe.ui.PrintFormatBuilder({
				wrapper: $parent,
				page: wrapper.page,
		...
		let d = new appyframe.ui.Dialog({
			...
			primary_action({ action, doctype, print_format, print_format_name }) {
				if (action === "Edit") {
					appyframe.set_route("print-format-builder-beta", print_format);
					...
					appyframe.db
						...
						.then((doc) => {
							appyframe.set_route("print-format-builder-beta", doc.name);
						})
						.finally(() => {
