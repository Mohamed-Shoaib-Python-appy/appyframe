appyframe.pages["print-format-builder"].on_page_load = function (wrapper) {
	appyframe.print_format_builder = new appyframe.PrintFormatBuilder(wrapper);
	appyframe.breadcrumbs.add("Setup", "Print Format");
};

appyframe.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = appyframe.get_route();
	if (route.length > 1) {
		appyframe.model.with_doc("Print Format", route[1], function () {
			appyframe.print_format_builder.print_format = appyframe.get_doc("Print Format", route[1]);
			appyframe.print_format_builder.refresh();
		});
	} else if (appyframe.route_options) {
		if (appyframe.route_options.make_new) {
			let { doctype, name, based_on, beta } = appyframe.route_options;
			appyframe.route_options = null;
			appyframe.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			appyframe.print_format_builder.print_format = appyframe.route_options.doc;
			appyframe.route_options = null;
			appyframe.print_format_builder.refresh();
		}
	}
...
appyframe.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = appyframe.ui.make_app_page({
			parent: this.parent,
		...
		this.page.main = $(
			'<div class="col-md-12 border print-format-builder-main appyframe-card"></div>'
	...
	show_start() {
		this.page.main.html(appyframe.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = appyframe.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			appyframe.model.with_doc("Print Format", name, function (doc) {
				appyframe.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = appyframe.ui.form.make_control({
		...
		this.name_input = appyframe.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				appyframe.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		appyframe.call({
			method: "appyframe.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		appyframe.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = appyframe.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				appyframe.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(appyframe.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			appyframe.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						appyframe.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!appyframe.model.layout_fields.includes(f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(appyframe.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						field = appyframe.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = appyframe.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				appyframe.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = appyframe.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				appyframe.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		appyframe.call({
			...
			callback: function (r) {
				...
				appyframe.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
