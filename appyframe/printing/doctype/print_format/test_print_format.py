...
test_records = appyframe.get_test_records("Print Format")


class TestPrintFormat(FrappeTestCase):
	def test_print_user(self, style=None):
		print_html = appyframe.get_print("User", "Administrator", style=style)
	...
	def test_export_doc(self):
		doc: "PrintFormat" = appyframe.get_doc("Print Format", test_records[0]["name"])

		# this is only to make export_doc happy
		doc.standard = "Yes"
		_before = appyframe.conf.developer_mode
		appyframe.conf.developer_mode = True
		export_path = doc.export_doc()
		appyframe.conf.developer_mode = _before
		...
		with open(exported_doc_path) as f:
			exported_doc = appyframe.parse_json(f.read())
