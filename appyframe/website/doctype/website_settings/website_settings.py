...
from appyframe.utils import encode, get_request_site_address
from appyframe.website.utils import get_boot_data
...
class WebsiteSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from appyframe.website.doctype.top_bar_item.top_bar_item import TopBarItem
		from appyframe.website.doctype.website_route_redirect.website_route_redirect import (
	...
	def validate_google_settings(self):
		if self.enable_google_indexing and not appyframe.get_cached_value("Google Settings", None, "enable"):
		...
		for idx, row in enumerate(self.route_redirects):
			try:
				...
				if not appyframe.flags.in_migrate:
					appyframe.throw(_("Invalid redirect regex in row #{}: {}").format(idx, str(e)))
	...
	def get_access_token(self):
		...
		if not self.indexing_refresh_token:
			button_label = appyframe.bold(_("Allow API Indexing Access"))
...
def get_website_settings(context=None):
	...
	context = appyframe._dict(context or {})
	settings: "WebsiteSettings" = appyframe.get_cached_doc("Website Settings")
	...
	if context.disable_website_theme:
		context.theme = appyframe._dict()

	else:
		from appyframe.website.doctype.website_theme.website_theme import get_active_theme

		context.theme = get_active_theme() or appyframe._dict()

	if not context.get("favicon"):
		context["favicon"] = "/assets/appyframe/images/appyframe-favicon.svg"
	...
	context.read_only_mode = appyframe.flags.read_only
...
def get_auto_account_deletion():
	return appyframe.db.get_single_value("Website Settings", "auto_account_deletion")
