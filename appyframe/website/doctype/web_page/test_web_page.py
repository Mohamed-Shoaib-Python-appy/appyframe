...
test_records = appyframe.get_test_records("Web Page")


class TestWebPage(FrappeTestCase):
	def setUp(self):
		appyframe.db.delete("Web Page")
		for t in test_records:
			appyframe.get_doc(t).insert()
	...
	def test_dynamic_route(self):
		web_page = appyframe.get_doc(
			dict(
				...
				content_type="HTML",
				dynamic_template=1,
				main_section_html="<div>{{ appyframe.form_dict.doctype }}</div>",
			)
		).insert()
		try:
