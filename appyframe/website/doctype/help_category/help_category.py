...
import appyframe
from appyframe.website.doctype.help_article.help_article import clear_cache
...
class HelpCategory(WebsiteGenerator):
	...
	website = appyframe._dict(condition_field="published", page_title_field="category_name")
	...
	def validate(self):
		...
		if not self.published:
			for d in appyframe.get_all("Help Article", dict(category=self.name)):
				appyframe.db.set_value("Help Article", d.name, "published", 0)
