...
from appyframe.website.utils import (
	clear_cache,
	find_first_image,
	get_comment_list,
...
class BlogPost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return (
				appyframe.db.get_value("Blog Category", self.blog_category, "route")
				+ "/"
	...
	def validate(self):
		...
		if self.featured:
			if not self.meta_image:
				appyframe.throw(_("A featured post must have a cover image"))
			self.reset_featured_for_other_blogs()

		self.set_read_time()

	def reset_featured_for_other_blogs(self):
		all_posts = appyframe.get_all("Blog Post", {"featured": 1})
		for post in all_posts:
			appyframe.db.set_value("Blog Post", post.name, "featured", 0)
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and appyframe.db.get_single_value(
			"Blog Settings", "show_cta_in_blog", cache=True
		)

		if self.blogger:
			context.blogger_info = appyframe.get_doc("Blogger", self.blogger).as_dict()
		...
		context.category = appyframe.db.get_value(
		...
		context.guest_allowed = appyframe.db.get_single_value("Blog Settings", "allow_guest_to_comment")

	def fetch_cta(self):
		if appyframe.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
			blog_settings = appyframe.get_cached_doc("Blog Settings")
	...
	def fetch_social_links_info(self):
		if not appyframe.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
			return []

		url = appyframe.local.site + "/" + self.route
	...
	def load_likes(self, context):
		...
		context.like_count = appyframe.db.count("Comment", filters) or 0
		...
		context.like = appyframe.db.count("Comment", filters) or 0
...
def get_list_context(context=None):
	...
	blog_settings = appyframe.get_doc("Blog Settings").as_dict(no_default_fields=True)
	list_context.update(blog_settings)

	category_name = appyframe.utils.escape_html(
		appyframe.local.form_dict.blog_category or appyframe.local.form_dict.category
	)
	if category_name:
		category = appyframe.get_doc("Blog Category", category_name)
	...
	elif appyframe.local.form_dict.blogger:
		blogger = appyframe.db.get_value("Blogger", {"name": appyframe.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif appyframe.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(appyframe.local.form_dict.txt))
...
def get_blog_categories():
	...
	post, category = appyframe.qb.DocType("Blog Post"), appyframe.qb.DocType("Blog Category")
	return (
		appyframe.qb.from_(category)
		.select(category.name, category.route, category.title)
		.where(
			(category.published == 1)
			& ExistsCriterion(
				appyframe.qb.from_(post)
				.select("name")
...
def clear_blog_cache():
	for blog in appyframe.db.get_list("Blog Post", fields=["route"], pluck="route", filters={"published": True}):
		clear_cache(blog)
...
def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
	...
	if filters and filters.get("blog_category"):
		...
		category = appyframe.utils.escape_html(
			appyframe.local.form_dict.blog_category or appyframe.local.form_dict.category
		)

	if filters and filters.get("blogger"):
		conditions.append("t1.blogger=%s" % appyframe.db.escape(filters.get("blogger")))

	if category:
		conditions.append("t1.blog_category=%s" % appyframe.db.escape(category))

	if txt:
		conditions.append(
			'(t1.content like {0} or t1.title like {0}")'.format(appyframe.db.escape("%" + txt + "%"))
		)

	if conditions:
		appyframe.local.no_cache = 1
	...
	posts = appyframe.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = appyframe.db.get_value(
