...
appyframe.provide("website");
appyframe.provide("appyframe.awesome_bar_path");
window.cur_frm = null;
...
$.extend(appyframe, {
	...
	require: async function (links, callback) {
		...
		links = links.map((link) => appyframe.bundled_asset(link));
	...
	bundled_asset(path, is_rtl = null) {
		if (!path.startsWith("/assets") && path.includes(".bundle.")) {
			...
			path = appyframe.boot.assets_json[path] || path;
			return path;
		}
		return path;
	},
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (appyframe._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				appyframe._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					appyframe.msgprint(__("Not found"));
				},
				403: function () {
					appyframe.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			appyframe.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				appyframe.msgprint(server_messages);
			}
		}
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return appyframe.call({
			type: "GET",
			method: "appyframe.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (appyframe.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", appyframe.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				appyframe.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				appyframe.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				appyframe.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	freeze: function (msg) {
		...
		appyframe.freeze_count++;
	},
	unfreeze: function () {
		if (!appyframe.freeze_count) return; // anything open?
		appyframe.freeze_count--;
		if (!appyframe.freeze_count) {
	...
	trigger_ready: function () {
		appyframe.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = appyframe.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		appyframe.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				appyframe.do_search(val);
				return false;
			}
		});
	},
	do_search: function (val) {
		var path =
			(appyframe.awesome_bar_path && appyframe.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		appyframe.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return appyframe.get_cookie("user_id") !== "Guest" && appyframe.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (appyframe.session.user === "Guest" && window.show_language_picker) {
			appyframe
				.call("appyframe.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = appyframe.get_cookie("preferred_language");
					let language_codes = [];
...
appyframe.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		appyframe.utils.debounce(() => {
			...
			appyframe
				.call({
					method: "appyframe.search.web_search",
					args: {
...
window.valid_email = function (id) {
	// copied regex from appyframe/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = appyframe.get_cookie("full_name");
	var logged_in = appyframe.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	appyframe.bind_navbar_search();

	// add apps link
	let apps = appyframe.boot?.apps_data?.apps;
	let obj = {
		label: __("Apps"),
		route: "/apps",
	};
	if (apps?.length) {
		...
		let is_desk_apps = appyframe.boot?.apps_data?.is_desk_apps;
		!is_desk_apps && appyframe.add_apps(obj);
	}

	// switch to app link
	if (appyframe.get_cookie("system_user") === "yes" && logged_in) {
		appyframe.add_switch_to_desk();
	}

	appyframe.render_user();
...
$(document).on("page-change", function () {
	...
	$.extend(appyframe, appyframe.get_cookies());
	appyframe.session = { user: appyframe.user_id };

	appyframe.datetime.refresh_when();
	appyframe.trigger_ready();
	appyframe.bind_filters();
	appyframe.highlight_code_blocks();
	appyframe.add_link_to_headings();
	appyframe.make_navbar_active();
	// scroll to hash
...
appyframe.ready(function () {
	appyframe.show_language_picker();
	appyframe.setup_videos();
	appyframe.realtime.init(window.socketio_port, true); // lazy connection
});
