...
import appyframe
from appyframe.website.page_renderers.base_template_page import BaseTemplatePage
from appyframe.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def set_template_path(self):
		...
		for app in reversed(appyframe.get_installed_apps()):
	...
	def get_index_path_options(search_path):
		return (
			appyframe.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = appyframe.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = appyframe.get_module(self.pymodule_name)
		...
		self.context.developer_mode = appyframe.conf.developer_mode
	...
	def render_template(self):
		if self.template_path.endswith("min.js"):
			...
			html = appyframe.render_template(self.source, self.context, safe_render=safe_render)
	...
	def get_raw_template(self):
		return appyframe.get_jloader().get_source(appyframe.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = appyframe.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "appyframe"
		self.app_path = appyframe.get_app_path("appyframe")
		self.path = path
	...
	def set_user_info(self):
		from appyframe.utils.user import get_fullname_and_avatar

		info = get_fullname_and_avatar(appyframe.session.user)
