...
def delete_page_cache(path):
	appyframe.cache.delete_value("full_index")
	groups = ("website_page", "page_context")
	if path:
		for name in groups:
			appyframe.cache.hdel(name, path)
	else:
		for name in groups:
...
def can_cache(no_cache=False):
	if appyframe.flags.force_website_cache:
		return True
	if appyframe.conf.disable_website_cache or appyframe.conf.developer_mode:
		return False
	if getattr(appyframe.local, "no_cache", False):
		return False
	return not no_cache
...
def get_comment_list(doctype, name):
	comments = appyframe.get_all(
		...
		or_filters=[["owner", "=", appyframe.session.user], ["published", "=", 1]],
	)

	communications = appyframe.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{appyframe.session.user}%"],
			["cc", "like", f"%{appyframe.session.user}%"],
			["bcc", "like", f"%{appyframe.session.user}%"],
		],
	)
...
def get_home_page():
	if appyframe.local.flags.home_page and not appyframe.flags.in_test:
		return appyframe.local.flags.home_page
	...
	def _get_home_page():
		...
		if appyframe.session.user != "Guest":
			# by role
			for role in appyframe.get_roles():
				home_page = appyframe.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = appyframe.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = appyframe.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if appyframe.session.user == "Guest" else "me"
	...
	return appyframe.cache.hget("home_page", appyframe.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = appyframe.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = appyframe.get_attr(home_page_method[-1])(appyframe.session.user)
	elif appyframe.get_hooks("website_user_home_page"):
		home_page = appyframe.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = appyframe.get_hooks("role_home_page")
		if role_home_page:
			for role in appyframe.get_roles():
	...
	if not home_page:
		home_page = appyframe.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	...
	apps = get_apps() or []
	return {
		"lang": appyframe.local.lang or "en",
		"apps_data": {
			"apps": apps,
		...
		"sysdefaults": {
			"float_precision": cint(appyframe.get_system_settings("float_precision")) or 3,
			"date_format": appyframe.get_system_settings("date_format") or "yyyy-mm-dd",
			"time_format": appyframe.get_system_settings("time_format") or "HH:mm:ss",
			"first_day_of_the_week": appyframe.get_system_settings("first_day_of_the_week") or "Sunday",
			"number_format": appyframe.get_system_settings("number_format") or "#,###.##",
			"currency": appyframe.get_system_settings("currency"),
		},
		"time_zone": {
			"system": get_system_timezone(),
			"user": appyframe.db.get_value("User", appyframe.session.user, "time_zone") or get_system_timezone(),
		},
		"assets_json": get_assets_json(),
		"sitename": appyframe.local.site,
		"is_fc_site": 1 if is_fc_site() else 0,
	}


def is_signup_disabled():
	return appyframe.get_website_settings("disable_signup")
...
def get_toc(route, url_prefix=None, app=None):
	...
	return appyframe.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from appyframe.website.router import get_pages

	if not appyframe.local.flags.children_map:
		...
		children_map = appyframe.cache.get_value("website_full_index", _build)

		appyframe.local.flags.children_map = children_map

	return appyframe.local.flags.children_map
...
def clear_cache(path=None):
	...
	appyframe.cache.delete_value("website_404")
	if path:
		appyframe.cache.hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		appyframe.clear_cache("Guest")
		for key in (
			"portal_menu_items",
			"home_page",
			"website_route_rules",
	...
	for method in appyframe.get_hooks("website_clear_cache"):
		appyframe.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = appyframe.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else appyframe.flags.look_for_sidebar
	...
	if not sidebar_items and parent_sidebar:
		sidebar_items = appyframe.get_all(
...
def get_portal_sidebar_items():
	sidebar_items = appyframe.cache.hget("portal_menu_items", appyframe.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = appyframe.get_roles()
		portal_settings = appyframe.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = appyframe.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		appyframe.cache.hset("portal_menu_items", appyframe.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			appyframe.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		if can_cache():
			html = None
			page_cache = appyframe.cache.hget("website_page", args[0].path)
			if page_cache and appyframe.local.lang in page_cache:
				html = page_cache[appyframe.local.lang]
			if html:
				appyframe.local.response.from_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = appyframe.cache.hget("website_page", args[0].path) or {}
			page_cache[appyframe.local.lang] = html
			appyframe.cache.hset("website_page", args[0].path, page_cache)
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = appyframe.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	links = [f"<{css}>; rel=preload; as=style" for css in appyframe.local.preload_assets["style"]]
	links.extend(f"<{js}>; rel=preload; as=script" for js in appyframe.local.preload_assets["script"])

	version = get_build_version()
	# include_icons = appyframe.get_hooks().get("app_include_icons", [])
	links.extend(
		f"</assets/{svg}?v={version}>; rel=preload; as=fetch; crossorigin"
		for svg in appyframe.local.preload_assets["icons"]
	)
