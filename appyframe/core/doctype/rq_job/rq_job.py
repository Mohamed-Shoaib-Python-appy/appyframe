...
from appyframe.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			appyframe.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> appyframe._dict:
	...
	if job_name == "appyframe.utils.background_jobs.run_doc_method":
	...
	return appyframe._dict(
		...
		arguments=appyframe.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == appyframe.local.site
...
@appyframe.whitelist()
def stop_job(job_id):
	appyframe.get_doc("RQ Job", job_id).stop_job()
