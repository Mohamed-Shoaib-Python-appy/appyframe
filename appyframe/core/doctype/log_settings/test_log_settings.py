...
import appyframe
from appyframe.core.doctype.log_settings.log_settings import _supports_log_clearing, run_log_clean_up
...
class TestLogSettings(FrappeTestCase):
	...
	def test_delete_logs(self):
		...
		activity_log_count = appyframe.db.count("Activity Log", {"creation": ("<=", self.datetime.past)})
		error_log_count = appyframe.db.count("Error Log", {"creation": ("<=", self.datetime.past)})
		email_queue_count = appyframe.db.count("Email Queue", {"creation": ("<=", self.datetime.past)})
...
def setup_test_logs(past: datetime) -> None:
	activity_log = appyframe.get_doc(
		{
