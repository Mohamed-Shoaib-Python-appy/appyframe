...
class TestUserType(FrappeTestCase):
	...
	def test_add_select_perm_doctypes(self):
		...
		doc = appyframe.get_meta("Contact")
		link_fields = doc.get_link_fields()
		select_doctypes = appyframe.get_all(
	...
	def test_print_share_email_default(self):
		...
		perm = appyframe.get_all("Custom DocPerm", filters={"role": "_Test User Type"}, fields=["*"])[0]
...
def create_user_type(user_type):
	if appyframe.db.exists("User Type", user_type):
		appyframe.delete_doc("User Type", user_type)

	user_type_limit = {appyframe.scrub(user_type): 1}
...
def create_role():
	if not appyframe.db.exists("Role", "_Test User Type"):
		appyframe.get_doc(
