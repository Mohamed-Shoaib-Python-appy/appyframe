...
from appyframe.core.api.file import (
	create_new_folder,
	get_attached_images,
	get_files_in_folder,
	move_file,
...
from appyframe.core.doctype.file.exceptions import FileTypeNotAllowed
from appyframe.core.doctype.file.utils import get_corrupted_image_msg, get_extension
from appyframe.desk.form.utils import add_comment
from appyframe.exceptions import ValidationError
from appyframe.tests.utils import FrappeTestCase, change_settings
from appyframe.utils import get_files_path, set_request
...
def make_test_doc(ignore_permissions=False):
	...
	d.description = "Test"
	d.assigned_by = appyframe.session.user
...
def make_test_image_file(private=False):
	file_path = appyframe.get_app_path("appyframe", "tests/data/sample_image_for_optimization.jpg")
	with open(file_path, "rb") as f:
		file_content = f.read()

	test_file = appyframe.get_doc(
		{
			"doctype": "File",
	...
	# remove those flags
	_test_file: "File" = appyframe.get_doc("File", test_file.name)
...
class TestSimpleFile(FrappeTestCase):
	...
	def test_save(self):
		_file = appyframe.get_doc("File", {"file_url": self.saved_file_url})
...
class TestFSRollbacks(FrappeTestCase):
	def test_rollback_from_file_system(self):
		file_name = content = appyframe.generate_hash()
		file = appyframe.new_doc("File", file_name=file_name, content=content).insert()
...
class TestExtensionValidations(FrappeTestCase):
	...
	def test_allowed_extension(self):
		...
		file_name = content = appyframe.generate_hash()
		bad_file = appyframe.new_doc("File", file_name=f"{file_name}.png", content=content)
		self.assertRaises(FileTypeNotAllowed, bad_file.insert)

		bad_file = appyframe.new_doc("File", file_name=f"{file_name}.csv", content=content).insert()
		appyframe.db.rollback()
...
class TestSameFileName(FrappeTestCase):
	def test_saved_content(self):
		...
		_file = appyframe.get_doc("File", {"file_url": self.saved_file_url1})
		content1 = _file.get_content()
		self.assertEqual(content1, self.test_content1)
		_file = appyframe.get_doc("File", {"file_url": self.saved_file_url2})
	...
	def test_saved_content_private(self):
		...
		_file = appyframe.get_doc("File", {"file_url": _file1.file_url})
		self.assertEqual(_file.get_content(), test_content1)

		_file = appyframe.get_doc("File", {"file_url": _file2.file_url})
...
class TestSameContent(FrappeTestCase):
	...
	def test_attachment_limit(self):
		...
		self.assertRaises(appyframe.exceptions.AttachmentLimitReached, file2.insert)
...
class TestFile(FrappeTestCase):
	...
	def tearDown(self):
		try:
			appyframe.get_doc("File", {"file_name": "file_copy.txt"}).delete()
		except appyframe.DoesNotExistError:
			pass

	def delete_test_data(self):
		test_file_data = appyframe.get_all(
			"File",
			pluck="name",
	...
	def tests_after_upload(self):
		...
		file_folder = appyframe.db.get_value("File", self.saved_name, "folder")
		self.assertEqual(file_folder, _("Home/Test Folder 1"))

	def test_file_copy(self):
		folder = self.get_folder("Test Folder 2", "Home")

		file = appyframe.get_doc("File", {"file_name": "file_copy.txt"})
	...
	def test_folder_copy(self):
		...
		file = appyframe.get_doc("File", {"file_name": "folder_copy.txt"})
		file_copy_txt = appyframe.get_value("File", {"file_name": "file_copy.txt"})
		if file_copy_txt:
			appyframe.get_doc("File", file_copy_txt).delete()

		self.assertEqual(_("Home/Test Folder 1/Test Folder 3"), file.folder)

	def test_default_folder(self):
		d = appyframe.get_doc({"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1})
		d.save()
	...
	def test_on_delete(self):
		...
		self.assertEqual(appyframe.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0)
		...
		folder = appyframe.get_doc("File", "Home/Test Folder 1/Test Folder 3")
	...
	def test_same_file_url_update(self):
		...
		file2 = appyframe.get_doc("File", file2.name)
	...
	def test_file_url_validation(self):
		test_file: "File" = appyframe.new_doc("File")
		test_file.update({"file_name": "logo", "file_url": "https://appyframe.io/files/appyframe.png"})
	...
	def test_make_thumbnail(self):
		# test web image
		test_file: "File" = appyframe.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": appyframe.utils.get_url("/_test/assets/image.jpg"),
			}
		...
		test_file = appyframe.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": appyframe.utils.get_url("/_test/assets/image"),
			}
		).insert(ignore_permissions=True)
		...
		test_file.db_set("thumbnail_url", None)
		test_file.reload()
		test_file.file_url = appyframe.utils.get_url("unknown.jpg")
		test_file.make_thumbnail(suffix="xs")
		self.assertEqual(
			appyframe.message_log[0].get("message"),
			f"File '{appyframe.utils.get_url('unknown.jpg')}' not found",
		)
		self.assertEqual(test_file.thumbnail_url, None)

	def test_file_unzip(self):
		file_path = appyframe.get_app_path("appyframe", "www/_test/assets/file.zip")
		public_file_path = appyframe.get_site_path("public", "files")
		try:
			import shutil
...
class TestAttachment(FrappeTestCase):
	...
	def tearDownClass(cls):
		appyframe.db.rollback()
		appyframe.delete_doc("DocType", cls.test_doctype)

	def test_file_attachment_on_update(self):
		doc = appyframe.get_doc(doctype=self.test_doctype, title="test for attachment on update").insert()
...
class TestAttachmentsAccess(FrappeTestCase):
	def setUp(self) -> None:
		appyframe.db.delete("File", {"is_folder": 0})
...
class TestFileUtils(FrappeTestCase):
	def test_extract_images_from_doc(self):
		is_private = not appyframe.get_meta("ToDo").make_attachments_public
		...
		self.assertTrue(appyframe.db.exists("File", {"attached_to_name": todo.name, "is_private": is_private}))
		...
		filename = appyframe.db.exists("File", {"attached_to_name": todo.name})
		self.assertIn(f'<img src="{appyframe.get_doc("File", filename).file_url}', todo.description)
	...
	def test_extract_images_from_comment(self):
		...
		"""
		is_private = not appyframe.get_meta("ToDo").make_attachments_public
		test_doc = appyframe.get_doc(doctype="ToDo", description="comment test").insert()
		comment = add_comment(
			"ToDo",
			test_doc.name,
		...
		self.assertTrue(
			appyframe.db.exists("File", {"attached_to_name": test_doc.name, "is_private": is_private})
		)
	...
	def test_extract_images_from_communication(self):
		...
		"""
		is_private = not appyframe.get_meta("Communication").make_attachments_public
		communication = appyframe.get_doc(
		...
		self.assertTrue(
			appyframe.db.exists("File", {"attached_to_name": communication.name, "is_private": is_private})
		)
...
class TestFileOptimization(FrappeTestCase):
	...
	def test_optimize_svg(self):
		file_path = appyframe.get_app_path("appyframe", "tests/data/sample_svg.svg")
		with open(file_path, "rb") as f:
			file_content = f.read()
		test_file = appyframe.get_doc(
	...
	def test_optimize_folder(self):
		test_folder = appyframe.get_doc("File", "Home/Attachments")
...
class TestGuestFileAndAttachments(FrappeTestCase):
	...
	def tearDown(self) -> None:
		appyframe.set_user("Administrator")
		appyframe.db.rollback()
		appyframe.delete_doc("DocType", "Test For Attachment")
	...
	def test_attach_unattached_guest_file(self):
		...
		d = appyframe.new_doc("Test For Attachment")
	...
	def test_private_remains_private_even_if_same_hash(self):
		file_name = "test" + appyframe.generate_hash()
		content = file_name.encode()

		doc_pub: "File" = appyframe.new_doc("File")  # type: ignore
		doc_pub.file_url = f"/files/{file_name}.txt"
		doc_pub.content = content
		doc_pub.save()

		doc_pri: "File" = appyframe.new_doc("File")  # type: ignore
