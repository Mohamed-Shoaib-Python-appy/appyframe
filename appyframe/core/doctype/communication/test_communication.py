...
import appyframe
from appyframe.core.doctype.communication.communication import Communication, get_emails, parse_email
from appyframe.core.doctype.communication.email import add_attachments, make
from appyframe.email.doctype.email_queue.email_queue import EmailQueue
from appyframe.tests.utils import FrappeTestCase

if TYPE_CHECKING:
	from appyframe.contacts.doctype.contact.contact import Contact
	from appyframe.email.doctype.email_account.email_account import EmailAccount

test_records = appyframe.get_test_records("Communication")
...
class TestCommunication(FrappeTestCase):
	def test_email(self):
		...
		for i, x in enumerate(valid_email_list):
			with self.subTest(i=i, x=x):
				self.assertTrue(appyframe.utils.parse_addr(x)[1])

		for i, x in enumerate(invalid_email_list):
			with self.subTest(i=i, x=x):
				self.assertFalse(appyframe.utils.parse_addr(x)[0])
	...
	def test_name(self):
		...
		for x in valid_email_list:
			self.assertTrue(appyframe.utils.parse_addr(x)[0])
	...
	def test_circular_linking(self):
		...
		a = appyframe.get_doc("Communication", a.name)
		a.reference_doctype = "Communication"
		a.reference_name = c.name

		self.assertRaises(appyframe.CircularLinkingError, a.save)

	def test_deduplication_timeline_links(self):
		appyframe.delete_doc_if_exists("Note", "deduplication timeline links")
	...
	def test_contacts_attached(self):
		contact_sender: "Contact" = appyframe.get_doc(
			{
				"doctype": "Contact",
		...
		contact_recipient: "Contact" = appyframe.get_doc(
			{
				"doctype": "Contact",
		...
		contact_cc: "Contact" = appyframe.get_doc(
			{
				"doctype": "Contact",
		...
		comm: Communication = appyframe.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
	...
	def test_get_communication_data(self):
		from appyframe.desk.form.load import get_communication_data

		appyframe.delete_doc_if_exists("Note", "get communication data")

		note = appyframe.get_doc(
			{"doctype": "Note", "title": "get communication data", "content": "get communication data"}
		).insert(ignore_permissions=True)

		comm_note_1 = appyframe.get_doc(
			{
				"doctype": "Communication",
				"communication_type": "Communication",
	...
	def test_signature_in_email_content(self):
		...
		comm_with_signature = appyframe.get_doc(
			base_communication
			| {
		...
		comm_without_signature = appyframe.get_doc(
			base_communication
			| {
	...
	def test_mark_as_spam(self):
		...
		spam_comm: Communication = appyframe.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
		...
		normal_comm: Communication = appyframe.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
				"subject": "This is spam",
...
class TestCommunicationEmailMixin(FrappeTestCase):
	...
	def test_cc(self):
		def test(assertion, cc_list=None, set_user_as=None, include_sender=False, thread_notify=False):
			if set_user_as:
				appyframe.set_user(set_user_as)
	...
	def test_add_attachments_by_filename(self):
		...
		attached_file_name, attached_content_hash = appyframe.db.get_value(
			"File",
	...
	def test_add_attachments_by_file_content(self):
		...
		attached_file_name = appyframe.db.get_value(
			"File",
			{"attached_to_name": comm.name, "attached_to_doctype": comm.doctype},
		)
		attached_file = appyframe.get_doc("File", attached_file_name)
...
def create_email_account() -> "EmailAccount":
	appyframe.delete_doc_if_exists("Email Account", "_Test Comm Account 1")
