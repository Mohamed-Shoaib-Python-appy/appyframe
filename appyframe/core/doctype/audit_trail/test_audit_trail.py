...
class TestAuditTrail(FrappeTestCase):
	...
	def test_compare_changed_fields(self):
		doc = appyframe.new_doc("Test Custom Doctype for Doc Comparator")
		doc.test_field = "first value"
		doc.submit()
		doc.cancel()

		changed_fields = appyframe._dict(test_field="second value")
		amended_doc = amend_document(doc, changed_fields, {}, 1)
		amended_doc.cancel()

		changed_fields = appyframe._dict(test_field="third value")
	...
	def test_compare_rows(self):
		...
		rows_updated = appyframe._dict(child_table_field=child_table_new)
		...
		results = appyframe._dict(results)
...
def create_custom_child_doctype():
	child_doctype = appyframe.get_doc(
		{
			"doctype": "DocType",
			"module": "Core",
...
def amend_document(amend_from, changed_fields, rows_updated, submit=False):
	amended_doc = appyframe.copy_doc(amend_from)
...
def create_comparator_doc(doctype_name, document):
	comparator = appyframe.new_doc("Audit Trail")
