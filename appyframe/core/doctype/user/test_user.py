...
from appyframe.core.doctype.user.user import (
	User,
...
from appyframe.desk.notifications import extract_mentions
...
user_module = appyframe.core.doctype.user.user
test_records = appyframe.get_test_records("User")


class TestUser(FrappeTestCase):
	def tearDown(self):
		# disable password strength test
		appyframe.db.set_single_value("System Settings", "enable_password_policy", 0)
		appyframe.db.set_single_value("System Settings", "minimum_password_score", "")
	...
	def test_user_type(self):
		user_id = appyframe.generate_hash() + "@example.com"
		new_user = appyframe.get_doc(doctype="User", email=user_id, first_name="Tester").insert()
		self.assertEqual(new_user.user_type, "Website User")

		# social login userid for appyframe
		self.assertTrue(new_user.social_logins[0].userid)
		self.assertEqual(new_user.social_logins[0].provider, "appyframe")
		...
		appyframe.delete_doc("User", new_user.name)

	def test_delete(self):
		appyframe.get_doc("User", "test@example.com").add_roles("_Test Role 2")
		self.assertRaises(appyframe.LinkExistsError, delete_doc, "Role", "_Test Role 2")
		appyframe.db.delete("Has Role", {"role": "_Test Role 2"})
		delete_doc("Role", "_Test Role 2")

		if appyframe.db.exists("User", "_test@example.com"):
			delete_contact("_test@example.com")
			delete_doc("User", "_test@example.com")

		user = appyframe.copy_doc(test_records[1])
		user.email = "_test@example.com"
		user.insert()

		appyframe.get_doc({"doctype": "ToDo", "description": "_Test"}).insert()

		delete_contact("_test@example.com")
		delete_doc("User", "_test@example.com")

		self.assertTrue(
			not appyframe.db.sql("""select * from `tabToDo` where allocated_to=%s""", ("_test@example.com",))
		)

		from appyframe.core.doctype.role.test_role import test_records as role_records

		appyframe.copy_doc(role_records[1]).insert()

	def test_get_value(self):
		self.assertEqual(appyframe.db.get_value("User", "test@example.com"), "test@example.com")
		self.assertEqual(appyframe.db.get_value("User", {"email": "test@example.com"}), "test@example.com")
		self.assertEqual(
			appyframe.db.get_value("User", {"email": "test@example.com"}, "email"), "test@example.com"
		)
		self.assertEqual(
			appyframe.db.get_value("User", {"email": "test@example.com"}, ["first_name", "email"]),
		...
		test_user = appyframe.db.sql("select * from tabUser where name='test@example.com'", as_dict=True)[0]
		self.assertEqual(
			appyframe.db.get_value("User", {"email": "test@example.com"}, "*", as_dict=True), test_user
		)

		self.assertEqual(appyframe.db.get_value("User", "xxxtest@example.com"), None)

		appyframe.db.set_single_value("Website Settings", "_test", "_test_val")
		self.assertEqual(appyframe.db.get_value("Website Settings", None, "_test"), "_test_val")
		self.assertEqual(appyframe.db.get_value("Website Settings", "Website Settings", "_test"), "_test_val")

	def test_high_permlevel_validations(self):
		user = appyframe.get_meta("User")
		self.assertTrue("roles" in [d.fieldname for d in user.get_high_permlevel_fields()])

		me = appyframe.get_doc("User", "testperm@example.com")
	...
	def test_delete_user(self):
		...
		delete_contact(new_user.name)
		appyframe.delete_doc("User", new_user.name)
		self.assertFalse(appyframe.db.exists("User", new_user.name))
	...
	def test_password_strength(self):
		...
		appyframe.db.set_single_value("System Settings", "enable_password_policy", 1)
		appyframe.db.set_single_value("System Settings", "minimum_password_score", 2)
		...
		self.assertRaises(appyframe.exceptions.ValidationError, handle_password_test_fail, result["feedback"])
		self.assertRaises(
			appyframe.exceptions.ValidationError, handle_password_test_fail, result
		...
		self.assertRaises(appyframe.exceptions.ValidationError, user.save)
		user.reload()
	...
	def test_comment_mentions(self):
		...
		appyframe.delete_doc("User Group", "Team")
	...
	def test_rate_limiting_for_reset_password(self):
		url = get_url()
		data = {"cmd": "appyframe.core.doctype.user.user.reset_password", "user": "test@test.com"}

		# Clear rate limit tracker to start fresh
		key = f"rl:{data['cmd']}:{data['user']}"
		appyframe.cache.delete(key)
	...
	def test_user_rename(self):
		...
		appyframe.rename_doc("User", user.name, new_name)
		self.assertTrue(appyframe.db.exists("Notification Settings", new_name))

		appyframe.delete_doc("User", new_name)

	def test_signup(self):
		import appyframe.website.utils

		random_user = appyframe.mock("email")
		random_user_name = appyframe.mock("name")
		# disabled signup
		with patch.object(user_module, "is_signup_disabled", return_value=True):
			self.assertRaisesRegex(
				appyframe.exceptions.ValidationError,
				"Sign Up is disabled",
				sign_up,
				random_user,
				random_user_name,
		...
		self.assertEqual(appyframe.cache.hget("redirect_after_login", random_user), "/welcome")

		# re-register
		self.assertTupleEqual(sign_up(random_user, random_user_name, "/welcome"), (0, "Already Registered"))

		# disabled user
		user = appyframe.get_doc("User", random_user)
		user.enabled = 0
		user.save()
		...
		with patch.object(user_module.appyframe.db, "get_creation_count", return_value=301):
			self.assertRaisesRegex(
				appyframe.exceptions.ValidationError,
				"Throttled",
				sign_up,
				appyframe.mock("email"),
				random_user_name,
				"/signup",
			)
	...
	def test_reset_password(self):
		...
		appyframe.local.request_ip = "127.0.0.69"

		appyframe.set_user("testpassword@example.com")
		test_user = appyframe.get_doc("User", "testpassword@example.com")
		...
		self.assertRaises(appyframe.exceptions.AuthenticationError, verify_password, old_password)
		...
		# test API endpoint
		with patch.object(user_module.appyframe, "sendmail") as sendmail:
			appyframe.clear_messages()
			test_user = appyframe.get_doc("User", "test2@example.com")
			...
			update_password(old_password, old_password=new_password)
			self.assertEqual(
				appyframe.message_log[0].get("message"),
...
class TestImpersonation(FrappeAPITestCase):
	def test_impersonation(self):
		with test_user(roles=["System Manager"], commit=True) as user:
			self.post(
				self.method("appyframe.core.doctype.user.user.impersonate"),
				{"user": user.name, "reason": "test", "sid": self.sid},
			)
			resp = self.get(self.method("appyframe.auth.get_logged_user"))
...
def test_user(
	*, first_name: str | None = None, email: str | None = None, roles: list[str], commit=False, **kwargs
	...
	try:
		first_name = first_name or appyframe.generate_hash()
		email = email or (first_name + "@example.com")
		user: User = appyframe.new_doc(
			"User",
		...
		commit and appyframe.db.commit()


def delete_contact(user):
	appyframe.db.delete("Contact", {"email_id": user})
	appyframe.db.delete("Contact Email", {"email_id": user})
