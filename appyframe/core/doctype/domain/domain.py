...
class Domain(Document):
	...
	def setup_domain(self):
		...
		if self.data.on_setup:
			...
			appyframe.get_attr(self.data.on_setup)()
	...
	def remove_domain(self):
		...
		if self.data.restricted_roles:
			for role_name in self.data.restricted_roles:
				if appyframe.db.exists("Role", role_name):
					role = appyframe.get_doc("Role", role_name)
					role.disabled = 1
					role.save()
		...
		if self.data.custom_fields:
			for doctype in self.data.custom_fields:
				...
				for custom_field_detail in custom_fields:
					custom_field_name = appyframe.db.get_value(
	...
	def setup_roles(self):
		...
		if self.data.restricted_roles:
			...
			for role_name in self.data.restricted_roles:
				...
				if not appyframe.db.get_value("Role", role_name):
					appyframe.get_doc(dict(doctype="Role", role_name=role_name)).insert()
					continue
	...
	def setup_data(self, domain=None):
		...
		self.data = appyframe.get_domain_data(self.name)

	def get_domain_data(self, module):
		return appyframe.get_attr(appyframe.get_hooks("domains")[self.name] + ".data")
	...
	def setup_properties(self):
		if self.data.properties:
			for args in self.data.properties:
				appyframe.make_property_setter(args)

	def set_values(self):
		"""set values based on `data.set_value`"""
		if self.data.set_value:
			for args in self.data.set_value:
				appyframe.reload_doctype(args[0])
				doc = appyframe.get_doc(args[0], args[1] or args[0])
	...
	def setup_sidebar_items(self):
		...
		if self.data.allow_sidebar_items:
			# disable all
			appyframe.db.sql("update `tabPortal Menu Item` set enabled=0")
		...
		if self.data.remove_sidebar_items:
			# disable all
			appyframe.db.sql("update `tabPortal Menu Item` set enabled=1")
