...
from appyframe.core.doctype.data_import.exporter import Exporter
from appyframe.core.doctype.data_import.importer import Importer
from appyframe.model import CORE_DOCTYPES
from appyframe.model.document import Document
from appyframe.modules.import_file import import_file_by_path
from appyframe.utils.background_jobs import enqueue, is_job_enqueued
from appyframe.utils.csvutils import validate_google_sheets_url
...
class DataImport(Document):
	...
	def validate_doctype(self):
		if self.reference_doctype in BLOCKED_DOCTYPES:
			appyframe.throw(_("Importing {0} is not allowed.").format(self.reference_doctype))
	...
	def start_import(self):
		...
		run_now = appyframe.flags.in_test or appyframe.conf.developer_mode
		if is_scheduler_inactive() and not run_now:
			appyframe.throw(_("Scheduler is inactive. Cannot import data."), title=_("Scheduler Inactive"))
	...
	def on_trash(self):
		appyframe.db.delete("Data Import Log", {"data_import": self.name})
...
def get_preview_from_template(
	...
	di: DataImport = appyframe.get_doc("Data Import", data_import)
...
def start_import(data_import):
	...
	data_import = appyframe.get_doc("Data Import", data_import)
	try:
	...
	appyframe.publish_realtime("data_import_refresh", {"data_import": data_import.name})
...
def download_template(doctype, export_fields=None, export_records=None, export_filters=None, file_type="CSV"):
	...
	"""
	appyframe.has_permission(doctype, "read", throw=True)

	export_fields = appyframe.parse_json(export_fields)
	export_filters = appyframe.parse_json(export_filters)
...
def download_errored_template(data_import_name: str):
	data_import: DataImport = appyframe.get_doc("Data Import", data_import_name)
...
def get_import_logs(data_import: str):
	doc = appyframe.get_doc("Data Import", data_import)
...
def import_doc(path, pre_process=None):
	...
	for f in files:
		if f.endswith(".json"):
			appyframe.flags.mute_emails = True
			import_file_by_path(
...
def export_json(doctype, path, filters=None, or_filters=None, name=None, order_by="creation asc"):
	...
	out = []
	if name:
		out.append(appyframe.get_doc(doctype, name).as_dict())
	elif appyframe.db.get_value("DocType", doctype, "issingle"):
		out.append(appyframe.get_doc(doctype).as_dict())
	else:
		for doc in appyframe.get_all(
			doctype,
			fields=["name"],
			filters=filters,
			...
			out.append(appyframe.get_doc(doctype, doc.name).as_dict())
	post_process(out)
	...
	with open(path, "w") as outfile:
		outfile.write(appyframe.as_json(out, ensure_ascii=False))


def export_csv(doctype, path):
	from appyframe.core.doctype.data_export.exporter import export_data

	with open(path, "wb") as csvfile:
		export_data(doctype=doctype, all_doctypes=True, template=True, with_data=True)
		csvfile.write(appyframe.response.result.encode("utf-8"))
