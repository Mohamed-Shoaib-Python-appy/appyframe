...
from appyframe.core.doctype.data_import.test_importer import create_doctype_if_not_exists
...
class TestExporter(FrappeTestCase):
	...
	def test_exports_specified_fields(self):
		if not appyframe.db.exists(doctype_name, "Test"):
			...
			doc = appyframe.get_doc(doctype_name, "Test")
	...
	def test_export_csv_response(self):
		...
		self.assertEqual(appyframe.response["doctype"], doctype_name)
