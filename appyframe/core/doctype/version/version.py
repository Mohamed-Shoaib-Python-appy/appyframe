...
class Version(Document):
	...
	def set_impersonator(data):
		if not appyframe.session:
			return
		if impersonator := appyframe.session.data.get("impersonated_by"):
			data["impersonated_by"] = impersonator

		if audit_user := appyframe.session.data.get("audit_user"):
	...
	def set_diff(self, old: Document, new: Document) -> bool:
		...
		if diff:
			...
			self.data = appyframe.as_json(diff, indent=None, separators=(",", ":"))
			return True
		else:
			return False
	...
	def for_insert(self, doc: Document) -> bool:
		...
		self.data = appyframe.as_json(data, indent=None, separators=(",", ":"))
		return True
...
def on_doctype_update():
	appyframe.db.add_index("Version", ["ref_doctype", "docname"])
