appyframe.ui.form.on("System Settings", {
	refresh: function (frm) {
		appyframe.call({
			method: "appyframe.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				appyframe.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", appyframe.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					appyframe.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					appyframe.app.setup_moment();
	...
	after_save: function (frm) {
		...
		const attr_tuples = [
			[frm.doc.language, appyframe.boot.sysdefaults.language, appyframe.boot.user.language],
			[frm.doc.rounding_method, appyframe.boot.sysdefaults.rounding_method], // no user override.
		];
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == appyframe.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		appyframe.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", appyframe.boot.sysdefaults.rounding_method);
			}
		);
	},
