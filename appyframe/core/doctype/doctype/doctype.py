...
from appyframe.cache_manager import clear_controller_cache, clear_user_cache
...
from appyframe.database import savepoint
from appyframe.database.schema import validate_column_length, validate_column_name
from appyframe.desk.notifications import delete_notification_count_for, get_filters_for
from appyframe.desk.utils import validate_route_conflict
from appyframe.model import (
	child_table_fields,
	data_field_options,
	default_fields,
	no_value_fields,
...
from appyframe.model.docfield import supports_translation
from appyframe.model.document import Document
from appyframe.model.meta import Meta
from appyframe.modules import get_doc_path, make_boilerplate
from appyframe.modules.import_file import get_file_path
from appyframe.permissions import ALL_USER_ROLE, AUTOMATIC_ROLES, SYSTEM_USER_ROLE
from appyframe.query_builder.functions import Concat
from appyframe.utils import cint, flt, get_datetime, is_a_property, random_string
from appyframe.website.utils import clear_cache

if TYPE_CHECKING:
	from appyframe.custom.doctype.customize_form.customize_form import CustomizeForm
...
class InvalidFieldNameError(appyframe.ValidationError):
	pass


class UniqueFieldnameError(appyframe.ValidationError):
	pass


class IllegalMandatoryError(appyframe.ValidationError):
	pass


class DoctypeLinkError(appyframe.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(appyframe.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(appyframe.ValidationError):
	pass


class NonUniqueError(appyframe.ValidationError):
	pass


class CannotIndexedError(appyframe.ValidationError):
	pass


class CannotCreateStandardDoctypeError(appyframe.ValidationError):
	pass
...
class DocType(Document):
	...
	if TYPE_CHECKING:
		from appyframe.core.doctype.docfield.docfield import DocField
		from appyframe.core.doctype.docperm.docperm import DocPerm
		from appyframe.core.doctype.doctype_action.doctype_action import DocTypeAction
		from appyframe.core.doctype.doctype_link.doctype_link import DocTypeLink
		from appyframe.core.doctype.doctype_state.doctype_state import DocTypeState
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = appyframe.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			appyframe.throw(_("Standard DocType cannot have default print format, use Customize Form"))
	...
	def check_indexing_for_dashboard_links(self):
		...
		for d in self.fields:
			if d.fieldtype == "Link" and not (d.unique or d.search_index):
				referred_as_link = appyframe.db.exists(
					"DocType Link",
	...
	def check_developer_mode(self):
		...
		if appyframe.flags.in_patch or appyframe.flags.in_test:
			return

		if not appyframe.conf.get("developer_mode") and not self.custom:
			appyframe.throw(
		...
		if self.is_virtual and self.custom:
			appyframe.throw(_("Not allowed to create custom Virtual DocType."), CannotCreateStandardDoctypeError)

		if appyframe.conf.developer_mode and not self.owner:
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = appyframe.get_meta(appyframe.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except appyframe.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = appyframe.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if "route" not in [d.fieldname for d in self.fields]:
				appyframe.throw(_('Field "route" is mandatory for Web Views'), title="Missing Field")
	...
	def validate_virtual_doctype_methods(self):
		...
		from appyframe.model.virtual_doctype import validate_controller
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from appyframe.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = appyframe.bold(self.meta.get_field("max_attachments").label)
			appyframe.msgprint(
	...
	def change_modified_of_parent(self):
		...
		if appyframe.flags.in_import:
			return
		parent_list = appyframe.get_all(
			"DocField", "parent", dict(fieldtype=["in", appyframe.model.table_fields], options=self.name)
		)
		for p in parent_list:
			appyframe.db.set_value("DocType", p.parent, {})
	...
	def on_update(self):
		...
		try:
			appyframe.db.updatedb(self.name, Meta(self))
		...
		allow_doctype_export = (
			not self.custom
			and not appyframe.flags.in_import
			and (appyframe.conf.developer_mode or appyframe.flags.allow_doctype_export)
		)
		...
		appyframe.clear_cache(doctype=self.name)

		# clear user cache so that on the next reload this doctype is included in boot
		clear_user_cache(appyframe.session.user)

		if not appyframe.flags.in_install and hasattr(self, "before_update"):
	...
	def setup_autoincrement_and_sequence(self):
		...
		name_type = f"varchar({appyframe.db.VARCHAR_LEN})"

		if self.autoname == "autoincrement":
			name_type = "bigint"
			appyframe.db.create_sequence(self.name, check_not_exists=True)
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not appyframe.request) or appyframe.flags.in_test or appyframe.flags.in_install
			appyframe.enqueue("appyframe.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = appyframe.scrub(self.name)
		scrubbed_module = appyframe.scrub(self.module)
		controller_path = appyframe.get_module_path(
			scrubbed_module, "doctype", scrubbed_name, f"{scrubbed_name}.py"
		)

		document_cls_tag = f"class {despaced_name}(Document)"
		document_import_tag = "from appyframe.model.document import Document"
		website_generator_cls_tag = f"class {despaced_name}(WebsiteGenerator)"
		website_generator_import_tag = "from appyframe.website.website_generator import WebsiteGenerator"
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and appyframe.session.user != "Administrator":
			appyframe.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			appyframe.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			appyframe.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			appyframe.db.sql(
			...
			appyframe.db.rename_table(old, new)
			appyframe.db.commit()

		# Do not rename and move files and folders for custom doctype
		if not self.custom:
			if not appyframe.flags.in_patch:
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if appyframe.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(appyframe.scrub(old), appyframe.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		appyframe.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(appyframe.scrub(new)))
				...
				with open(fname, "w") as f:
					if fname.endswith(".js"):
						...
						file_content = code.replace(
							appyframe.scrub(old), appyframe.scrub(new)
		...
		doctype_json_path = os.path.join(new_path, f"{appyframe.scrub(new)}.json")
		current_data = appyframe.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
			appyframe.db.table_exists(self.name)
			and appyframe.get_all(self.name, fields=["name"], limit=1, as_list=True)
		):
			return

		existing_property_setter = appyframe.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = appyframe.get_module_path(
				appyframe.scrub(self.module), "doctype", appyframe.scrub(self.name), "templates"
			)
	...
	def export_types_to_controller(self):
		from appyframe.modules.utils import get_module_app
		from appyframe.types.exporter import TypeExporter
		...
		if any(appyframe.get_hooks("export_python_type_annotations", app_name=app)):
			TypeExporter(self).export_types()

	def make_amendable(self):
		"""If is_submittable is set, add amended_from docfields."""
		if self.is_submittable:
			docfield_exists = appyframe.get_all(
	...
	def make_repeatable(self):
		...
		if self.allow_auto_repeat:
			if not appyframe.db.exists(
				"Custom Field", {"fieldname": "auto_repeat", "dt": self.name}
			) and not appyframe.db.exists("DocField", {"fieldname": "auto_repeat", "parent": self.name}):
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = appyframe.bold(_("Parent Field (Tree)"))
			appyframe.throw(_("{0} is a mandatory field").format(field_label), appyframe.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			appyframe.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = appyframe.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = appyframe.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = appyframe.db.MAX_COLUMN_LENGTH - 3
		...
		if WHITESPACE_PADDING_PATTERN.search(name):
			appyframe.throw(_("DocType's name should not start or end with whitespace"), appyframe.NameError)
	...
	def check_pending_migration(self) -> bool:
		...
		file = Path(get_file_path(appyframe.scrub(self.module), self.doctype, self.name))
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		if not dt.get("fields", {"fieldname": fieldname}):
			appyframe.throw(
				_("Fieldname called {0} must exist to enable autonaming").format(appyframe.bold(fieldname)),
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			appyframe.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if (
		...
		doctype = appyframe.qb.DocType("DocType")
		used_in = (
			appyframe.qb.from_(doctype)
			.select(doctype.name)
			.where(doctype.autoname.like(Concat(prefix, ".%")))
			.where(doctype.name != name)
		).run()
		if used_in:
			appyframe.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
...
def validate_autoincrement_autoname(dt: Union[DocType, "CustomizeForm"]) -> bool:
	...
	def get_autoname_before_save(dt: Union[DocType, "CustomizeForm"]) -> str:
		if dt.doctype == "Customize Form":
			property_value = appyframe.db.get_value(
			...
			if not property_value:
				return appyframe.db.get_value("DocType", dt.doc_type, "autoname") or ""
	...
	if not dt.is_new():
		...
		if (
			...
			if dt.doctype == "Customize Form":
				appyframe.throw(_("Cannot change to/from autoincrement autoname in Customize Form"))

			if appyframe.get_meta(dt.name).issingle:
				return False

			if not appyframe.get_all(dt.name, limit=1):
...
def change_name_column_type(doctype_name: str, type: str) -> None:
	...
	appyframe.db.change_column_type(*args)


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or appyframe.flags.in_patch or appyframe.flags.in_fixtures or appyframe.flags.in_migrate:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			appyframe.throw(message, appyframe.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			appyframe.throw(message, appyframe.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = appyframe.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, appyframe.bold(link.table_fieldname), appyframe.bold(meta.name)
			)
			appyframe.throw(message, appyframe.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		appyframe.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		appyframe.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		appyframe.throw(msg, InvalidFieldNameError)


def validate_fields_for_doctype(doctype):
	meta = appyframe.get_meta(doctype, cached=False)
...
def validate_fields(meta: Meta):
	...
	:param meta: `appyframe.model.meta.Meta` object to check."""
		...
		if fieldname in Document._reserved_keywords:
			appyframe.throw(
				_("{0}: fieldname cannot be set to reserved keyword {1}").format(
					appyframe.bold(docname),
					appyframe.bold(fieldname),
				),
	...
	def check_link_table_options(docname, d):
		if appyframe.flags.in_patch or appyframe.flags.in_fixtures:
			return
		...
		if d.fieldtype in ("Link", *table_fields):
			...
			if d.options != d.parent:
				options = appyframe.db.get_value("DocType", d.options, "name")
				if not options:
					appyframe.throw(
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default and not appyframe.flags.in_migrate:
			appyframe.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			appyframe.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_precision(d):
		if (
			...
			appyframe.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and appyframe.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = appyframe.db.sql(
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					appyframe.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						appyframe.throw(_("Fold must come before a Section Break"))
				else:
					appyframe.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				appyframe.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			appyframe.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			appyframe.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			appyframe.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			appyframe.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			appyframe.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			appyframe.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			appyframe.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and DEPENDS_ON_PATTERN.match(depends_on):
				appyframe.throw(_("Invalid {0} condition").format(appyframe.unscrub(field)), appyframe.ValidationError)
	...
	def validate_fetch_from(field):
		...
		if source_field == field.fieldname:
			...
			appyframe.throw(msg, title=_("Recursive Fetch From"))
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (docfield.oldfieldtype and docfield.oldfieldtype != "Data"):
			if docfield.options and (docfield.options not in data_field_options):
				df_str = appyframe.bold(_(docfield.label, context=docfield.parent))
				text_str = (
				...
				appyframe.msgprint(text_str + df_options_str, title="Invalid Data Field", alert=True)

	def check_child_table_option(docfield):
		if appyframe.flags.in_fixtures:
			return
		if docfield.fieldtype not in ["Table MultiSelect", "Table"]:
			return

		doctype = docfield.options
		child_doctype_meta = appyframe.get_meta(doctype)

		if not child_doctype_meta.istable:
			appyframe.throw(
				_("Option {0} for field {1} is not a child table").format(
					appyframe.bold(doctype), appyframe.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			appyframe.throw(f"Max for {appyframe.bold(docfield.fieldname)} height must be in px, em, rem")
	...
	def check_no_of_ratings(docfield):
		if docfield.fieldtype == "Rating":
			if docfield.options and (int(docfield.options) > 10 or int(docfield.options) < 3):
				appyframe.throw(_("Options for Rating field can range from 3 to 10"))
	...
	for d in fields:
		...
		if not appyframe.flags.in_migrate:
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = appyframe.get_doc("DocType", doctype)
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		appyframe.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_atleast_one_set(d):
		if not d.select and not d.read and not d.write and not d.submit and not d.cancel and not d.create:
			appyframe.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			appyframe.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			appyframe.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			appyframe.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			appyframe.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			appyframe.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
	...
	def check_if_submittable(d):
		if d.submit and not issubmittable:
			appyframe.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			appyframe.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			appyframe.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
		...
		if doctype.custom:
			if d.role in AUTOMATIC_ROLES:
				appyframe.throw(
					...
					).format(d.idx, appyframe.bold(_(d.role))),
					title=_("Permissions Error"),
				)

			roles = [row.name for row in appyframe.get_all("Role", filters={"is_custom": 1})]
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not appyframe.db.exists("Domain", doc.restrict_to_domain)
		):
			appyframe.get_doc(dict(doctype="Domain", domain=doc.restrict_to_domain)).insert()

		if "tabModule Def" in appyframe.db.get_tables() and not appyframe.db.exists("Module Def", doc.module):
			m = appyframe.get_doc({"doctype": "Module Def", "module_name": doc.module})
			if appyframe.scrub(doc.module) in appyframe.local.module_app:
				m.app_name = appyframe.local.module_app[appyframe.scrub(doc.module)]
			else:
				m.app_name = "appyframe"
			m.flags.ignore_mandatory = m.flags.ignore_permissions = True
			if appyframe.flags.package:
				m.package = appyframe.flags.package.name
				m.custom = 1
			m.insert()
		...
		for role in list(set(roles)):
			if appyframe.db.table_exists("Role", cached=False) and not appyframe.db.exists("Role", role):
				r = appyframe.new_doc("Role")
				r.role_name = role
				r.desk_access = 1
		...
		if appyframe.db.is_table_missing(e):
			pass
		else:
			raise
...
def check_fieldname_conflicts(docfield):
	...
	doc = appyframe.get_doc({"doctype": docfield.dt})
	...
	if docfield.fieldname in method_list + property_list:
		appyframe.msgprint(msg, raise_exception=not docfield.is_virtual)


def clear_linked_doctype_cache():
	appyframe.cache.delete_value("linked_doctypes_without_ignore_user_permissions_enabled")
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		appyframe.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		appyframe.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		appyframe.throw(_("Select a valid Sender Field for creating documents from Email"))

	if not sender_field.options == "Email":
		appyframe.throw(_("Sender Field should have Email in options"))
...
def get_row_size_utilization(doctype: str) -> float:
	...
	appyframe.has_permission("DocType", throw=True)
	try:
		return flt(appyframe.db.get_row_size(doctype) / appyframe.db.MAX_ROW_SIZE_LIMIT * 100, 2)
	except Exception:
		return 0.0
