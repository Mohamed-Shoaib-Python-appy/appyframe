appyframe.listview_settings["DocType"] = {
	primary_action: function () {
		this.new_doctype_dialog();
	},

	new_doctype_dialog() {
		let non_developer = appyframe.session.user !== "Administrator" || !appyframe.boot.developer_mode;
		let fields = [
			{
				label: __("Name"),
				fieldname: "name",
		...
		let new_d = new appyframe.ui.Dialog({
			...
			primary_action(values) {
				...
				appyframe.db
					...
					.then((doc) => {
						appyframe.set_route("Form", "DocType", doc.name);
					});
			},
			secondary_action_label: __("Cancel"),
			secondary_action() {
				new_d.hide();
				if (appyframe.get_route()[0] === "Form") {
