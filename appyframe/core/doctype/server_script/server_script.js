...
appyframe.ui.form.on("Server Script", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Compare Versions"), () => {
				new appyframe.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}
	...
	check_safe_exec(frm) {
		appyframe.xcall("appyframe.core.doctype.server_script.server_script.enabled").then((enabled) => {
			if (enabled === false) {
				let docs_link =
...
if appyframe.form_dict.message == "ping":
	appyframe.response['message'] = "pong"
else:
	appyframe.response['message'] = "ok"
</code></pre>
...
tenant_id = appyframe.db.get_value(...)
