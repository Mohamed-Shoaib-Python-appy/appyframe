...
# this is a separate file since it is imported in appyframe.model.document
...
def run_server_script_for_doc_event(doc, event):
	...
	if scripts:
		...
		for script_name in scripts:
			appyframe.get_doc("Server Script", script_name).execute_doc(doc)
...
def get_server_script_map():
	...
	# 	}
	# }
	if appyframe.flags.in_patch and not appyframe.db.table_exists("Server Script"):
		return {}

	script_map = appyframe.cache.get_value("server_script_map")
	if script_map is None:
		script_map = {"permission_query": {}}
		enabled_server_scripts = appyframe.get_all(
			"Server Script",
		...
		appyframe.cache.set_value("server_script_map", script_map)
