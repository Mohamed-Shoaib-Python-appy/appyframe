...
from appyframe.utils.safe_exec import (
	FrappeTransformer,
...
class ServerScript(Document):
	...
	def on_trash(self):
		...
		if self.script_type == "Scheduler Event":
			for job in self.scheduled_jobs:
				appyframe.delete_doc("Scheduled Job Type", job.name)
	...
	def sync_scheduled_jobs(self):
		...
		for scheduled_job in self.scheduled_jobs:
			if bool(scheduled_job.stopped) != bool(self.disabled):
				job = appyframe.get_doc("Scheduled Job Type", scheduled_job.name)
	...
	def clear_scheduled_events(self):
		...
		if (
			...
			for scheduled_job in self.scheduled_jobs:
				appyframe.delete_doc("Scheduled Job Type", scheduled_job.name, delete_permanently=1)
	...
	def check_if_compilable_in_restricted_context(self):
		...
		try:
			...
			appyframe.msgprint(str(e), title=_("Compilation warning"))

	def execute_method(self) -> dict:
		"""Specific to API endpoint Server Scripts

		Raises:
		        appyframe.DoesNotExistError: If self.script_type is not API
		        appyframe.PermissionError: If self.allow_guest is unset for API accessed by Guest user

		Returns:
		        dict: Evaluates self.script with appyframe.utils.safe_exec.safe_exec and returns the flags set in it's safe globals
		"""
	...
	def execute_scheduled_method(self):
		...
		Raises:
		        appyframe.DoesNotExistError: If script type is not a scheduler event
		"""
		if self.script_type != "Scheduler Event":
	...
	def get_autocompletion_items(self):
		...
		Returns:
		        ...
		        For e.g., ["appyframe.utils.cint", "appyframe.get_all", ...]
		"""

		return appyframe.cache.get_value(
...
def setup_scheduler_events(script_name: str, frequency: str, cron_format: str | None = None):
	...
	Args:
	        ...
	        frequency (str): Event label compatible with the Appyframe scheduler
	"""
	method = appyframe.scrub(f"{script_name}-{frequency}")
	scheduled_script = appyframe.db.get_value("Scheduled Job Type", {"method": method})
	...
	if not scheduled_script:
		...
		appyframe.msgprint(_("Enabled scheduled execution for script {0}").format(script_name))

	else:
		doc = appyframe.get_doc("Scheduled Job Type", scheduled_script)
		...
		appyframe.msgprint(_("Scheduled execution for script {0} has updated").format(script_name))
...
def execute_api_server_script(script=None, *args, **kwargs):
	...
	if appyframe.session.user == "Guest" and not script.allow_guest:
	...
	return _globals.appyframe.flags


@appyframe.whitelist()
def enabled() -> bool | None:
	if appyframe.has_permission("Server Script"):
