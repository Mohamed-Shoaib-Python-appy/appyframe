...
import appyframe
import appyframe.desk.query_report
from appyframe import _, scrub
from appyframe.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from appyframe.core.doctype.page.page import delete_custom_role
from appyframe.desk.reportview import append_totals_row
from appyframe.model.document import Document
from appyframe.modules import make_boilerplate
from appyframe.modules.export_file import export_to_files
from appyframe.utils import cint, cstr
from appyframe.utils.safe_exec import check_safe_sql_query, safe_exec
...
class Report(Document):
	...
	if TYPE_CHECKING:
		...
		from appyframe.core.doctype.report_column.report_column import ReportColumn
		from appyframe.core.doctype.report_filter.report_filter import ReportFilter
	...
	def validate(self):
		...
		if not self.module:
			self.module = appyframe.db.get_value("DocType", self.ref_doctype, "module")

		if not self.is_standard:
			self.is_standard = "No"
			if (
				appyframe.session.user == "Administrator"
				and getattr(appyframe.local.conf, "developer_mode", 0) == 1
			):
		...
		if self.is_standard == "No":
			...
			if self.report_type != "Report Builder":
				appyframe.only_for("Script Manager", True)

			if appyframe.db.get_value("Report", self.name, "is_standard") == "Yes":
				appyframe.throw(_("Cannot edit a standard report. Please duplicate and create a new report"))

		if self.is_standard == "Yes" and appyframe.session.user != "Administrator":
			appyframe.throw(_("Only Administrator can save a standard report. Please rename and save."))
	...
	def on_trash(self):
		if (
			self.is_standard == "Yes"
			and not cint(getattr(appyframe.local.conf, "developer_mode", 0))
			and not (appyframe.flags.in_migrate or appyframe.flags.in_patch)
		):
			appyframe.throw(_("You are not allowed to delete Standard Report"))
	...
	def set_doctype_roles(self):
		if not self.get("roles") and self.is_standard == "No":
			meta = appyframe.get_meta(self.ref_doctype)
			if not meta.istable:
	...
	def is_permitted(self):
		...
		if has_common(appyframe.get_roles(), allowed):
			return True
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and (appyframe.local.conf.get("developer_mode") or 0) == 1:
	...
	def execute_query_report(self, filters):
		if not self.query:
			appyframe.throw(_("Must specify a Query to run"), title=_("Report Document Error"))

		check_safe_sql_query(self.query)

		result = [list(t) for t in appyframe.db.sql(self.query, filters)]
		columns = self.get_columns() or [cstr(c[0]) for c in appyframe.db.get_description()]
	...
	def execute_script_report(self, filters):
		...
		if not self.prepared_report:
			prepared_report_watcher = threading.Timer(
				...
				kwargs={"report": self.name, "site": appyframe.local.site},
			)
		...
		appyframe.cache.hset("report_execution_time", self.name, execution_time)

		return res

	def execute_module(self, filters):
		# report in python module
		module = self.module or appyframe.db.get_value("DocType", self.ref_doctype, "module")
		method_name = get_report_module_dotted_path(module, self.name) + ".execute"
		return appyframe.get_attr(method_name)(appyframe._dict(filters))

	def execute_script(self, filters):
		# server script
		loc = {"filters": appyframe._dict(filters), "data": None, "result": None}
	...
	def run_query_report(
		...
		columns, result = [], []
		data = appyframe.desk.query_report.run(
			self.name,
			filters=filters,
			user=user,
		...
		for d in data.get("columns"):
			if isinstance(d, dict):
				...
				columns.append(
					appyframe._dict(label=parts[0], fieldtype=fieldtype, fieldname=parts[0], options=options)
				)
	...
	def get_standard_report_columns(self, params):
		if params.get("fields"):
			...
			columns.extend(
				...
				for df in appyframe.get_meta(self.ref_doctype).fields
				if df.in_list_view
			)
		return columns
	...
	def get_standard_report_order_by(self, params):
		...
		if params.get("group_by"):
			group_by_args = appyframe._dict(params["group_by"])
	...
	def build_standard_report_columns(self, columns, group_by_args):
		...
		for fieldname, doctype in columns:
			...
			if meta.get_field(fieldname):
				...
				field = appyframe._dict(fieldname=fieldname, label=label)
	...
	def toggle_disable(self, disable: bool):
		if not self.has_permission("write"):
			appyframe.throw(_("You are not allowed to edit the report."))

		self.db_set("disabled", cint(disable))


def is_prepared_report_enabled(report):
	return cint(appyframe.db.get_value("Report", report, "prepared_report")) or 0


def get_report_module_dotted_path(module, report_name):
	return (
		appyframe.local.module_app[scrub(module)]
		+ "."
		+ scrub(module)
		+ ".report."
		+ scrub(report_name)
...
def enable_prepared_report(report: str, site: str):
	appyframe.init(site)
	appyframe.connect()
	appyframe.db.set_value("Report", report, "prepared_report", 1)
	appyframe.db.commit()
	appyframe.destroy()
