...
from appyframe.custom.doctype.customize_form.customize_form import reset_customization
from appyframe.desk.query_report import add_total_row, run, save_report
from appyframe.desk.reportview import delete_report
from appyframe.desk.reportview import save_report as _save_report
from appyframe.tests.utils import FrappeTestCase

test_records = appyframe.get_test_records("Report")
...
class TestReport(FrappeTestCase):
	...
	def test_report_builder(self):
		if appyframe.db.exists("Report", "User Activity Report"):
			appyframe.delete_doc("Report", "User Activity Report")

		with open(os.path.join(os.path.dirname(__file__), "user_activity_report.json")) as f:
			appyframe.get_doc(json.loads(f.read())).insert()

		report = appyframe.get_doc("Report", "User Activity Report")
	...
	def test_query_report(self):
		report = appyframe.get_doc("Report", "Permitted Documents For User")
	...
	def test_save_or_delete_report(self):
		...
		try:
			...
			appyframe.set_user("test_report_owner@example.com")
			self.assertRaises(appyframe.PermissionError, delete_report, report.name)
	...
	def test_custom_report(self):
		...
		custom_report = appyframe.get_doc("Report", custom_report_name)
		columns, result = custom_report.run_query_report(user=appyframe.session.user)

		self.assertListEqual(["email"], [column.get("fieldname") for column in columns])
		admin_dict = appyframe.core.utils.find(result, lambda d: d["name"] == "Administrator")
	...
	def test_report_permissions(self):
		appyframe.set_user("test@example.com")
		appyframe.db.delete("Has Role", {"parent": appyframe.session.user, "role": "Test Has Role"})
		appyframe.db.commit()
		if not appyframe.db.exists("Role", "Test Has Role"):
			appyframe.get_doc({"doctype": "Role", "role_name": "Test Has Role"}).insert(ignore_permissions=True)

		if not appyframe.db.exists("Report", "Test Report"):
			report = appyframe.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = appyframe.get_doc("Report", "Test Report")
	...
	def test_report_custom_permissions(self):
		appyframe.set_user("test@example.com")
		appyframe.db.delete("Custom Role", {"report": "Test Custom Role Report"})
		appyframe.db.commit()  # nosemgrep
		if not appyframe.db.exists("Report", "Test Custom Role Report"):
			report = appyframe.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = appyframe.get_doc("Report", "Test Custom Role Report")
	...
	def test_format_method(self):
		if appyframe.db.exists("Report", "User Activity Report Without Sort"):
			appyframe.delete_doc("Report", "User Activity Report Without Sort")
		with open(os.path.join(os.path.dirname(__file__), "user_activity_report_without_sort.json")) as f:
			appyframe.get_doc(json.loads(f.read())).insert()

		report = appyframe.get_doc("Report", "User Activity Report Without Sort")
		columns, data = report.get_data()
	...
	def test_non_standard_script_report(self):
		...
		if not appyframe.db.exists("Report", report_name):
			report = appyframe.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
...
totals = {}
for user in appyframe.get_all('User', fields = ['name', 'user_type', 'creation']):
...
"""
	...
	def test_script_report_with_columns(self):
		...
		if appyframe.db.exists("Report", report_name):
			appyframe.delete_doc("Report", report_name)
...
"""
	...
	def test_toggle_disabled(self):
		...
		reports = appyframe.get_all(doctype="Report", limit=1)
		...
		doc = appyframe.get_doc("Report", report_name)
		with self.assertRaises(appyframe.exceptions.ValidationError):
	...
	def test_cte_in_query_report(self):
		...
		if appyframe.db.db_type == "mariadb":
			...
			self.assertRaises(appyframe.PermissionError, report.execute_query_report, filters={})
