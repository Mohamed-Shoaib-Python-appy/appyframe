appyframe.listview_settings["Recorder"] = {
	...
	onload(listview) {
		...
		listview.page.add_button(__("Clear"), () => {
			appyframe.xcall("appyframe.recorder.delete").then(listview.refresh);
		});
		...
		listview.page.add_menu_item(__("Import"), () => {
			new appyframe.ui.FileUploader({
				...
				on_success: (file) => {
					...
					appyframe.call({
						method: "appyframe.recorder.import_data",
						args: {
							file: file.file_url,
						},
						callback: function () {
		...
		listview.page.add_menu_item(__("Export"), () => {
			appyframe.call({
				method: "appyframe.recorder.export_data",
				callback: function (r) {
	...
	fetch_recorder_status(listview) {
		return appyframe.xcall("appyframe.recorder.status").then((status) => {
	...
	stop_recorder(listview) {
		let me = this;
		appyframe.xcall("appyframe.recorder.stop", {}).then(() => {
			listview.refresh();
	...
	start_recorder(listview) {
		...
		appyframe.prompt(
			...
			(values) => {
				appyframe.xcall("appyframe.recorder.start", values).then(() => {
					listview.refresh();
