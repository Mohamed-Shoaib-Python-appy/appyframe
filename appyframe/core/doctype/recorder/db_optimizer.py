"""Basic DB optimizer for Appyframe Framework based app.
...
import appyframe
from appyframe.utils import flt
...
class DBColumn:
	...
	def from_frappe_ouput(cls, data) -> "DBColumn":
		"Parse DBColumn from output of describe-database-table command in Appyframe"
		return cls(
			name=data["column"],
...
class DBIndex:
	...
	def from_frappe_ouput(cls, data, table) -> "DBIndex":
		"Parse DBIndex from output of describe-database-table command in Appyframe"
		return cls(
			name=data["name"],
			table=table,
			unique=data["unique"],
...
class DBTable:
	...
	def from_frappe_ouput(cls, data) -> "DBTable":
		"Parse DBTable from output of describe-database-table command in Appyframe"
...
class DBOptimizer:
	...
	def suggest_index(self) -> DBIndex | None:
		...
		if missing_tables := (set(self.tables_examined()) - set(self.tables.keys())):
			appyframe.throw("DBTable infomation missing for: " + ", ".join(missing_tables))
