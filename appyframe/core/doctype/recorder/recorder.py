...
from appyframe.core.doctype.recorder.db_optimizer import DBOptimizer, DBTable
from appyframe.custom.doctype.property_setter.property_setter import make_property_setter
from appyframe.model.document import Document
from appyframe.recorder import RECORDER_REQUEST_HASH
from appyframe.recorder import get as get_recorder_data
from appyframe.utils import cint, cstr, evaluate_filters, get_table_name
...
class Recorder(Document):
	...
	if TYPE_CHECKING:
		from appyframe.core.doctype.recorder_query.recorder_query import RecorderQuery
		from appyframe.core.doctype.recorder_suggested_index.recorder_suggested_index import (
	...
	def load_from_db(self):
		...
		if not request_data:
			raise appyframe.DoesNotExistError
...
def serialize_request(request):
	request = appyframe._dict(request)
	if request.get("calls"):
		for i in request.calls:
			i["stack"] = appyframe.as_json(i["stack"])
			i["explain_result"] = appyframe.as_json(i["explain_result"])
	request.update(
		name=request.get("uuid"),
		number_of_queries=request.get("queries"),
		time_in_queries=request.get("time_queries"),
		request_headers=appyframe.as_json(request.get("headers"), indent=4),
		form_dict=appyframe.as_json(request.get("form_dict"), indent=4),
...
def add_indexes(indexes):
	...
	for index in indexes:
		appyframe.enqueue(_add_index, table=index["table"], column=index["column"])
	appyframe.msgprint(_("Enqueued creation of indexes"), alert=True)


def _add_index(table, column):
	doctype = get_doctype_name(table)
	appyframe.db.add_index(doctype, [column])
	appyframe.msgprint(
...
def optimize(recorder_id: str):
	appyframe.only_for("Administrator")
	appyframe.enqueue(_optimize, recorder_id=recorder_id, queue="long")


def _optimize(recorder_id):
	record: Recorder = appyframe.get_doc("Recorder", recorder_id)
	...
	data = appyframe.cache.hget(RECORDER_REQUEST_HASH, record.name)
	data["suggested_indexes"] = [{"table": idx[0][0], "column": idx[0][1]} for idx in suggested_indexes]
	appyframe.cache.hset(RECORDER_REQUEST_HASH, record.name, data)
	appyframe.publish_realtime("recorder-analysis-complete", user=appyframe.session.user)
	appyframe.msgprint(_("Query analysis complete. Check suggested indexes."), realtime=True, alert=True)
...
def _fetch_table_stats(doctype: str, columns: list[str]) -> dict | None:
	...
	if not appyframe.db.table_exists(doctype):
		return

	table = get_table_name(doctype, wrap_in_backticks=True)

	schema = []
	for field in appyframe.db.sql(f"describe {table}", as_dict=True):
		schema.append(
			{
				"column": field["Field"],
				"type": field["Type"],
	...
	indexes = []
	for idx in appyframe.db.sql(f"show index from {table}", as_dict=True):
		indexes.append(
			{
...
def _get_column_cardinality(table, column):
	return appyframe.db.sql(f"select count(distinct {column}) from {table}")[0][0]
