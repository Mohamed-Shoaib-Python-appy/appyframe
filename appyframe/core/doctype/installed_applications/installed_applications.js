...
appyframe.ui.form.on("Installed Applications", {
	...
	show_update_order_dialog() {
		const dialog = new appyframe.ui.Dialog({
			...
			primary_action: function () {
				...
				appyframe.call({
					method: "appyframe.core.doctype.installed_applications.installed_applications.update_installed_apps_order",
					freeze: true,
					args: {
						new_order: new_order,
					},
		...
		appyframe
			.xcall(
				"appyframe.core.doctype.installed_applications.installed_applications.get_installed_app_order"
			)
			.then((data) => {
				data.forEach((app) => {
				...
				grid.find(".grid-row-check:first").remove() &&
					grid.find(".grid-row-check").replaceWith(appyframe.utils.icon("menu"));
				dialog.show();
			});
	},
});
