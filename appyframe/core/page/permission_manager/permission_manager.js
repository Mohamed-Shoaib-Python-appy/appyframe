appyframe.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = appyframe.ui.make_app_page({
		parent: wrapper,
	...
	$(appyframe.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new appyframe.PermissionEngine(wrapper);
};

appyframe.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

appyframe.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			options: "DocType",
			change: function () {
				appyframe.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return appyframe.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (appyframe.get_route()[1]) {
			this.doctype_select.set_value(appyframe.get_route()[1]);
		} else if (appyframe.route_options) {
			if (appyframe.route_options.doctype) {
				this.doctype_select.set_value(appyframe.route_options.doctype);
			}
			if (appyframe.route_options.role) {
				this.role_select.set_value(appyframe.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = appyframe.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return appyframe
				.call({
					module: "appyframe.core",
					page: "permission_manager",
		...
		let $d = $(d.wrapper)
			.find(".appyframe-confirm-message")
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(appyframe.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${appyframe.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return appyframe.call({
					...
					callback: (r) => {
						if (r.exc) {
							appyframe.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			appyframe.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					appyframe.call({
						...
						callback: (r) => {
							if (r.exc) {
								appyframe.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						},
	...
	get_link_fields(doctype) {
		return appyframe.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
