...
from appyframe import _, is_whitelisted, validate_and_sanitize_search_inputs
from appyframe.database.schema import SPECIAL_CHAR_PATTERN
from appyframe.model.db_query import get_order_by
from appyframe.permissions import has_permission
from appyframe.utils import cint, cstr, unique
from appyframe.utils.data import make_filter_tuple
...
def sanitize_searchfield(searchfield: str):
	...
	if SPECIAL_CHAR_PATTERN.search(searchfield):
		appyframe.throw(_("Invalid Search Field {0}").format(searchfield), appyframe.DataError)
...
def search_widget(
	...
	standard_queries = appyframe.get_hooks().standard_queries or {}
	...
	if query:  # Query = custom search query i.e. python function
		try:
			is_whitelisted(appyframe.get_attr(query))
		...
		except (appyframe.PermissionError, appyframe.AppNotInstalledError, ImportError):
			if appyframe.local.conf.developer_mode:
				raise
			else:
	...
	if not meta.translated_doctype:
		_txt = appyframe.db.escape((txt or "").replace("%", "").replace("@", ""))
...
def get_names_for_mentions(search_term):
	users_for_mentions = appyframe.cache.get_value("users_for_mentions", get_users_for_mentions)
	user_groups = appyframe.cache.get_value("user_groups", get_user_groups)
	...
	for mention_data in users_for_mentions + user_groups:
		...
		mention_data["link"] = appyframe.utils.get_url_to_form(
...
def get_user_groups():
	return appyframe.get_all("User Group", fields=["name as id", "name as value"], update={"is_group": True})
...
def get_link_title(doctype, docname):
	...
	if meta.show_title_field_in_link:
		return appyframe.db.get_value(doctype, docname, meta.title_field)
