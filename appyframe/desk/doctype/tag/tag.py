...
from appyframe.utils import unique
...
def check_user_tags(dt):
	...
	try:
		doctype = DocType(dt)
		appyframe.qb.from_(doctype).select(doctype._user_tags).limit(1).run()
...
def add_tags(tags, dt, docs, color=None):
	"adds a new tag to a record, and creates the Tag master"
	tags = appyframe.parse_json(tags)
...
def get_tagged_docs(doctype, tag):
	...
	return (appyframe.qb.from_(doctype).where(doctype._user_tags.like(tag)).select(doctype.name)).run()


@appyframe.whitelist()
def get_tags(doctype, txt):
	tag = appyframe.get_list("Tag", filters=[["name", "like", f"%{txt}%"]])
...
class DocTags:
	...
	def get_tag_fields(self):
		...
		return appyframe.db.get_value("DocType", self.dt, "tag_fields")

	def get_tags(self, dn):
		"""returns tag for a particular item"""
		return (appyframe.db.get_value(self.dt, dn, "_user_tags", ignore=1) or "").strip()
	...
	def add(self, dn, tag):
		...
		if tag not in tl:
			tl.append(tag)
			if not appyframe.db.exists("Tag", tag):
				appyframe.get_doc({"doctype": "Tag", "name": tag}).insert(ignore_permissions=True)
			self.update(dn, tl)
	...
	def update(self, dn, tl):
		...
		try:
			...
			doc = appyframe.get_doc(self.dt, dn)
...
def delete_tags_for_document(doc):
	...
	"""
	if not appyframe.db.table_exists("Tag Link"):
		return

	appyframe.db.delete("Tag Link", {"document_type": doc.doctype, "document_name": doc.name})
...
def update_tags(doc, tags):
	...
	for tag in deleted_tags:
		appyframe.db.delete("Tag Link", {"document_type": doc.doctype, "document_name": doc.name, "tag": tag})
...
def get_tags_list_for_awesomebar():
	return appyframe.get_list("Tag", pluck="name", order_by=None)
