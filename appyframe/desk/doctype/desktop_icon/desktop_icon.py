...
def after_doctype_insert():
	appyframe.db.add_unique("Desktop Icon", ("module_name", "owner", "standard"))
...
def get_desktop_icons(user=None):
	...
	user_icons = appyframe.cache.hget("desktop_icons", user)
	...
	if not user_icons:
		...
		blocked_doctypes = appyframe.get_all(
			"DocType",
		...
		standard_icons = appyframe.get_all("Desktop Icon", fields=fields, filters={"standard": 1})
		...
		user_icons = appyframe.get_all("Desktop Icon", fields=fields, filters={"standard": 0, "owner": user})
		...
		user_blocked_modules = appyframe.get_doc("User", user).get_blocked_modules()
		for icon in user_icons:
		...
		appyframe.cache.hset("desktop_icons", user, user_icons)
...
def add_user_icon(_doctype, _report=None, label=None, link=None, type="link", standard=0):
	...
	icon_name = appyframe.db.exists(
		"Desktop Icon", {"standard": standard, "link": link, "owner": appyframe.session.user}
	)

	if icon_name:
		if appyframe.db.get_value("Desktop Icon", icon_name, "hidden"):
			# if it is hidden, unhide it
			appyframe.db.set_value("Desktop Icon", icon_name, "hidden", 0)
			clear_desktop_icons_cache()

	else:
		idx = (
			appyframe.db.sql("select max(idx) from `tabDesktop Icon` where owner=%s", appyframe.session.user)[0][0]
			or appyframe.db.sql("select count(*) from `tabDesktop Icon` where standard=1")[0][0]
		)

		if not appyframe.db.get_value("Report", _report):
			_report = None
			userdefined_icon = appyframe.db.get_value(
		...
		module_icon = appyframe.get_value(
			"Desktop Icon",
		...
		except appyframe.UniqueValidationError:
			appyframe.throw(_("Desktop Icon already exists"))
	...
	for i, module_name in enumerate(new_order):
		if module_name not in ("Explore",):
			if user:
				...
				name = appyframe.db.get_value("Desktop Icon", {"standard": 1, "module_name": module_name})
				if name:
					icon = appyframe.get_doc("Desktop Icon", name)
				else:
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
	...
	if not int(appyframe.defaults.get_defaults().setup_complete or 0):
		appyframe.db.delete("Desktop Icon", {"standard": 0})

	# set standard as blocked and hidden if setting first active domain
	if not appyframe.flags.keep_desktop_icons:
		appyframe.db.sql("update `tabDesktop Icon` set blocked=0, hidden=1 where standard=1")

	# set as visible if present, or add icon
	for module_name in list(visible_list):
		name = appyframe.db.get_value("Desktop Icon", {"module_name": module_name})
		if name:
			appyframe.db.set_value("Desktop Icon", name, "hidden", 0)
		else:
			if appyframe.db.exists("DocType", module_name):
				try:
					add_user_icon(module_name, standard=1)
				except appyframe.UniqueValidationError as e:
...
def set_hidden(module_name, user=None, hidden=1):
	...
	if user:
		...
		if hidden and icon.custom:
			appyframe.delete_doc(icon.doctype, icon.name, ignore_permissions=True)
			return

		# hidden by user
		icon.db_set("hidden", hidden)
	else:
		icon = appyframe.get_doc("Desktop Icon", {"standard": 1, "module_name": module_name})
...
def get_all_icons():
	return [
		d.module_name for d in appyframe.get_all("Desktop Icon", filters={"standard": 1}, fields=["module_name"])
	]


def clear_desktop_icons_cache(user=None):
	appyframe.cache.hdel("desktop_icons", user or appyframe.session.user)
	appyframe.cache.hdel("bootinfo", user or appyframe.session.user)
...
def get_user_copy(module_name, user=None):
	...
	desktop_icon_name = appyframe.db.get_value(
		"Desktop Icon", {"module_name": module_name, "owner": user, "standard": 0}
	)

	if desktop_icon_name:
		return appyframe.get_doc("Desktop Icon", desktop_icon_name)
	else:
...
def make_user_copy(module_name, user):
	...
	standard_name = appyframe.db.get_value("Desktop Icon", {"module_name": module_name, "standard": 1})

	if not standard_name:
		appyframe.throw(_("{0} not found").format(module_name), appyframe.DoesNotExistError)

	original = appyframe.get_doc("Desktop Icon", standard_name)

	desktop_icon = appyframe.get_doc(
...
def sync_from_app(app):
	...
	try:
		modules = appyframe.get_attr(app + ".config.desktop.get_data")() or {}
	except ImportError:
		return []
	...
	for i, m in enumerate(modules_list):
		...
		if desktop_icon_name:
			desktop_icon = appyframe.get_doc("Desktop Icon", desktop_icon_name)
		else:
			# new icon
			desktop_icon = appyframe.get_doc(
		...
		try:
			desktop_icon.save()
		except appyframe.exceptions.UniqueValidationError:
			pass
...
def update_icons(hidden_list, user=None):
	...
	appyframe.msgprint(appyframe._("Updated"), indicator="green", title=_("Success"), alert=True)


def get_context(context):
	context.icons = get_user_icons(appyframe.session.user)
	context.user = appyframe.session.user

	if "System Manager" in appyframe.get_roles():
		context.users = appyframe.get_all(
			"User",
...
def get_module_icons(user=None):
	...
	if not user:
		icons = appyframe.get_all("Desktop Icon", fields="*", filters={"standard": 1}, order_by="idx")
	else:
		appyframe.cache.hdel("desktop_icons", user)
		icons = get_user_icons(user)

	for icon in icons:
		icon.value = appyframe.db.escape(_(icon.label or icon.module_name))
...
def get_user_icons(user):
	...
	from appyframe.boot import get_allowed_pages
