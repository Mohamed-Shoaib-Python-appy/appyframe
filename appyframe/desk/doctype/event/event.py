...
from appyframe.contacts.doctype.contact.contact import get_default_contact
...
from appyframe.utils.user import get_enabled_system_users
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from appyframe.desk.doctype.event_participants.event_participants import EventParticipants
	...
	def validate(self):
		...
		if self.repeat_on == "Daily" and self.ends_on and getdate(self.starts_on) != getdate(self.ends_on):
			appyframe.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			appyframe.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		for communication in communications:
			appyframe.delete_doc("Communication", communication, force=True)
	...
	def sync_communication(self):
		...
		for participant in self.event_participants:
			if communications := appyframe.get_all(
				"Communication",
				filters=[
				...
				for comm in communications:
					communication = appyframe.get_doc("Communication", comm)
					self.update_communication(participant, communication)
			else:
				meta = appyframe.get_meta(participant.reference_doctype)
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				appyframe.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)
...
def delete_communication(event, reference_doctype, reference_docname):
	...
	deleted_participant = appyframe.get_doc(reference_doctype, reference_docname)

	comms = appyframe.get_list(
		"Communication",
		filters=[
	...
	for comm in comms:
		appyframe.delete_doc("Communication", comm)


def get_permission_query_conditions(user):
	if not user:
		user = appyframe.session.user
	return f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={appyframe.db.escape(user)})"""
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			appyframe.set_user_lang(user.name, user.language)
			...
			appyframe.sendmail(
				...
				subject=appyframe._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[appyframe._("Events in Today's Calendar"), "blue"],
			)


@appyframe.whitelist()
def get_events(start, end, user=None, for_reminder=False, filters=None) -> list[appyframe._dict]:
	if not user:
		user = appyframe.session.user
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = appyframe.get_all(
		"Event Participants",
	...
	if participations:
		for participation in participations:
			if delete_event:
				appyframe.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = appyframe.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					appyframe.db.delete("Event", {"name": participation.parent})
					appyframe.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = appyframe.get_list("Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			appyframe.db.set_value("Event", event.name, "status", "Closed")
