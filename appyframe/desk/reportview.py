...
from appyframe.model import child_table_fields, default_fields, get_permitted_fields, optional_fields
...
from appyframe.utils import add_user_info, cint, format_duration
...
def get_list():
	...
	if is_virtual_doctype(args.doctype):
		...
		# uncompressed (refactored from appyframe.model.db_query.get_list)
		data = execute(**args)
...
def get_count() -> int:
	...
	if is_virtual_doctype(args.doctype):
		...
		if args.limit:
			...
			count = appyframe.db.sql(f"""select count(*) from ( {partial_query} ) p""")[0][0]
		else:
...
def get_form_params():
	...
	data = appyframe._dict(appyframe.local.form_dict)
	clean_params(data)
	validate_args(data)
	return data
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = appyframe.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			appyframe.throw(_("Invalid aggregate function"))

		if appyframe.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
			data.fields.append(
...
def raise_invalid_field(fieldname):
	appyframe.throw(_("Field not permitted in query") + f": {fieldname}", appyframe.DataError)
...
def get_meta_and_docfield(fieldname, data):
	...
	meta = appyframe.get_meta(parenttype)
...
def get_parenttype_and_fieldname(field, data):
	if "." in field:
		...
		parenttype = parts[0]
		fieldname = parts[1]
		df = appyframe.get_meta(data.doctype).get_field(parenttype)
...
def compress(data, args=None):
	...
	from appyframe.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = appyframe.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if appyframe.db.exists("Report", name):
		report = appyframe.get_doc("Report", name)
		if report.is_standard == "Yes":
			appyframe.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			appyframe.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != appyframe.session.user and not report.has_permission("write"):
			appyframe.throw(_("Insufficient Permissions for editing Report"), appyframe.PermissionError)
	else:
	...
	appyframe.msgprint(
		_("Report {0} saved").format(appyframe.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
def delete_report(name):
	...
	report = appyframe.get_doc("Report", name)
	if report.is_standard == "Yes":
		appyframe.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		appyframe.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != appyframe.session.user and not report.has_permission("delete"):
		appyframe.throw(_("Insufficient Permissions for deleting Report"), appyframe.PermissionError)

	report.delete(ignore_permissions=True)
	appyframe.msgprint(
		_("Report {0} deleted").format(appyframe.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	from appyframe.desk.utils import get_csv_bytes, pop_csv_params, provide_binary_file
	...
	if not appyframe.permissions.can_export(doctype):
		if appyframe.permissions.can_export(doctype, is_owner=True):
			for row in ret:
				if row[-1] != appyframe.session.user:
	...
	if appyframe.local.lang == "en" or not translate_values:
	...
	if file_format_type == "CSV":
		...
		content = get_csv_bytes(
			[[handle_html(appyframe.as_unicode(v)) if isinstance(v, str) else v for v in r] for r in data],
			csv_params,
		)
	elif file_format_type == "Excel":
		from appyframe.utils.xlsxutils import make_xlsx
...
def get_field_info(fields, doctype):
	...
	for key in fields:
		...
		if parenttype == doctype and fieldname == "name":
			...
			df = appyframe.get_meta(parenttype).get_field(fieldname)
			if df and df.fieldtype in ("Data", "Select", "Small Text", "Text"):
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = getattr(df, "translatable", False)
			elif df and df.fieldtype == "Link" and appyframe.get_meta(df.options).translated_doctype:
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = True
...
def delete_items():
	...
	items = sorted(json.loads(appyframe.form_dict.get("items")), reverse=True)
	doctype = appyframe.form_dict.get("doctype")

	if len(items) > 10:
		appyframe.enqueue("appyframe.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
...
def delete_bulk(doctype, items):
	...
	for i, d in enumerate(items):
		try:
			appyframe.delete_doc(doctype, d)
			if len(items) >= 5:
...
def get_stats(stats, doctype, filters=None):
	...
	try:
		db_columns = appyframe.db.get_table_columns(doctype)
	except (appyframe.db.InternalError, appyframe.db.ProgrammingError):
	...
	for column in columns:
		...
		try:
			...
			if column == "_user_tags":
				...
				no_tag_count = appyframe.get_list(
					doctype,
					fields=[column, "count(*)"],
