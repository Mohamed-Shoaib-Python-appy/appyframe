...
from appyframe.desk.doctype.notification_log.notification_log import (
	enqueue_create_notification,
	get_title,
	get_title_html,
)
from appyframe.desk.form.document_follow import follow_document
from appyframe.utils.data import strip_html


class DuplicateToDoError(appyframe.ValidationError):
	pass
...
def add(args=None, *, ignore_permissions=False):
	...
	for assign_to in appyframe.parse_json(args.get("assign_to")):
		filters = {
		...
		if not ignore_permissions:
			appyframe.get_doc(args["doctype"], args["name"]).check_permission()

		if appyframe.get_all("ToDo", filters=filters):
			...
			d = appyframe.get_doc(
				{
					...
					"assigned_by": args.get("assigned_by", appyframe.session.user),
					"assignment_rule": args.get("assignment_rule"),
				}
			).insert(ignore_permissions=True)

			# set assigned_to if field exists
			if appyframe.get_meta(args["doctype"]).get_field("assigned_to"):
				appyframe.db.set_value(args["doctype"], args["name"], "assigned_to", assign_to)

			doc = appyframe.get_doc(args["doctype"], args["name"])

			# if assignee does not have permissions, share or inform
			if not appyframe.has_permission(doc=doc, user=assign_to):
				if appyframe.get_system_settings("disable_document_sharing"):
					msg = _("User {0} is not permitted to access this document.").format(
						appyframe.bold(assign_to)
					)
					msg += "<br>" + _(
						"As document sharing is disabled, please give them the required permissions before assigning."
					)
					appyframe.throw(msg, title=_("Missing Permission"))
				else:
					appyframe.share.add(doc.doctype, doc.name, assign_to)
					shared_with_users.append(assign_to)

			# make this document followed by assigned user
			if appyframe.get_cached_value("User", assign_to, "follow_assigned_documents"):
	...
	if users_with_duplicate_todo:
		...
		appyframe.msgprint(_("Already in the following Users ToDo list:{0}").format(user_list, alert=True))
...
def close(doctype: str, name: str, assign_to: str, ignore_permissions=False):
	if assign_to != appyframe.session.user:
		appyframe.throw(_("Only the assignee can complete this to-do."))
...
def set_status(doctype, name, todo=None, assign_to=None, status="Cancelled", ignore_permissions=False):
	...
	if not ignore_permissions:
		appyframe.get_doc(doctype, name).check_permission()
	try:
		if not todo:
			todo = appyframe.db.get_value(
				"ToDo",
				{
		...
		if todo:
			todo = appyframe.get_doc("ToDo", todo)
			todo.status = status
	...
	if appyframe.get_meta(doctype).get_field("assigned_to") and status in ("Cancelled", "Closed"):
		appyframe.db.set_value(doctype, name, "assigned_to", None)
...
def notify_assignment(assigned_by, allocated_to, doc_type, doc_name, action="CLOSE", description=None):
	...
	assigned_user = appyframe.db.get_value("User", allocated_to, ["language", "enabled"], as_dict=True)
	...
	user_name = appyframe.get_cached_value("User", appyframe.session.user, "full_name")
	...
	if action == "CLOSE":
		subject = _("Your assignment on {0} {1} has been removed by {2}", lang=assigned_user.language).format(
			appyframe.bold(_(doc_type)), get_title_html(title), appyframe.bold(user_name)
		)
	else:
		user_name = appyframe.bold(user_name)
		document_type = appyframe.bold(_(doc_type, lang=assigned_user.language))
