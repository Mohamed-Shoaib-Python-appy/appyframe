...
from appyframe.utils import get_url_to_form


@appyframe.whitelist()
def update_follow(doctype: str, doc_name: str, following: bool):
	if following:
		return follow_document(doctype, doc_name, appyframe.session.user)
	else:
		return unfollow_document(doctype, doc_name, appyframe.session.user)
...
def follow_document(doctype, doc_name, user):
	...
	if (not appyframe.get_meta(doctype).track_changes) or user == "Administrator":
		return

	if not appyframe.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		return

	if not is_document_followed(doctype, doc_name, user):
		doc = appyframe.new_doc("Document Follow")
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		appyframe.delete_doc("Document Follow", doc[0].name)
		return 1
	return 0
...
def get_user_list(frequency):
	...
	User = DocType("User")
	return (
		appyframe.qb.from_(DocumentFollow)
		.join(User)
...
def get_version(doctype, doc_name, frequency, user):
	...
	if version:
		for v in version:
			change = appyframe.parse_json(v.data)
			time = appyframe.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = appyframe.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append(
			{
...
def _get_filters(frequency, user):
	...
	if frequency == "Weekly":
		filters += [
			["modified", ">", appyframe.utils.add_days(appyframe.utils.nowdate(), -7)],
			["modified", "<", appyframe.utils.nowdate()],
		]

	elif frequency == "Daily":
		filters += [
			["modified", ">", appyframe.utils.add_days(appyframe.utils.nowdate(), -1)],
			["modified", "<", appyframe.utils.nowdate()],
		]

	elif frequency == "Hourly":
		filters += [
			["modified", ">", appyframe.utils.add_to_date(appyframe.utils.now_datetime(), hours=-1)],
			["modified", "<", appyframe.utils.now_datetime()],
		]
