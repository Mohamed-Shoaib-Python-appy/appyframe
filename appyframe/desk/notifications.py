...
def get_notifications():
	...
	if appyframe.flags.in_install or not appyframe.db.get_single_value("System Settings", "setup_complete"):
		return out
	...
	for name in groups:
		count = appyframe.cache.hget("notification_count:" + name, appyframe.session.user)
...
def get_notifications_for_doctypes(config, notification_count):
	...
	can_read = appyframe.get_user().get_can_read()
	open_count_doctype = {}
			...
			if d in notification_count:
				...
				try:
					if isinstance(condition, dict):
						result = appyframe.get_list(
							d, fields=["count(*) as count"], filters=condition, ignore_ifnull=True
						)[0].count
					else:
						result = appyframe.get_attr(condition)()

				except appyframe.PermissionError:
					appyframe.clear_messages()
					pass
					# appyframe.msgprint("Permission Error in notifications for {0}".format(d))
					...
					appyframe.cache.hset("notification_count:" + d, appyframe.session.user, result)
...
def clear_notifications(user=None):
	...
	for name in groups:
		if user:
			appyframe.cache.hdel("notification_count:" + name, user)
		else:
			appyframe.cache.delete_key("notification_count:" + name)


def clear_notification_config(user):
	appyframe.cache.hdel("notification_config", user)


def delete_notification_count_for(doctype):
	appyframe.cache.delete_key("notification_count:" + doctype)
...
def get_notification_info():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(appyframe.db.sql("""select name, module from tabDocType"""))
...
def get_notification_config():
	...
	return appyframe.cache.hget("notification_config", user, _get)
...
def get_open_count(doctype: str, name: str, items=None):
	...
	if appyframe.flags.in_migrate or appyframe.flags.in_install:
		return {"count": []}
	...
	if not meta.custom:
		module = appyframe.get_meta_module(doctype)
...
def notify_mentions(ref_doctype, ref_name, content):
	if ref_doctype and ref_name and content:
		...
		sender_fullname = get_fullname(appyframe.session.user)
		...
		notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
			appyframe.bold(sender_fullname), appyframe.bold(ref_doctype), get_title_html(title)
		)
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = appyframe.get_cached_doc("User Group", mention["data-id"])
