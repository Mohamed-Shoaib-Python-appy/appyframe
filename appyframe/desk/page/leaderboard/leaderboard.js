appyframe.pages["leaderboard"].on_page_load = (wrapper) => {
	appyframe.leaderboard = new Leaderboard(wrapper);

	$(wrapper).bind("show", () => {
		// Get which leaderboard to show
		let doctype = appyframe.get_route()[1];
		appyframe.leaderboard.show_leaderboard(doctype);
	});
};
...
class Leaderboard {
	...
	get_leaderboard_config() {
		...
		appyframe
			.xcall("appyframe.desk.page.leaderboard.leaderboard.get_leaderboard_config")
			.then((config) => {
				...
				const _initial_doctype = appyframe.get_route()[1] || this.doctypes[0];
	...
	setup_leaderboard_fields() {
		this.company_select = this.page.add_field({
			...
			fieldtype: "Link",
			options: "Company",
			default: appyframe.defaults.get_default("company"),
			reqd: 1,
			change: (e) => {
				this.make_request();
			},
		...
		this.type_select = this.page.add_select(
			...
			this.options.selected_filter.map((d) => {
				return { label: __(appyframe.model.unscrub(d)), value: d };
			})
		);
	...
	create_date_range_field() {
		let timespan_field = $(this.parent).find(
			`.appyframe-control[data-original-title="${__("Timespan")}"]`
		);
		this.date_range_field = $(`<div class="from-date-field"></div>`)
			.insertAfter(timespan_field)
			.hide();

		let date_field = appyframe.ui.form.make_control({
			df: {
				fieldtype: "DateRange",
				fieldname: "selected_date_range",
				placeholder: __("Date Range"),
				default: [appyframe.datetime.month_start(), appyframe.datetime.now_date()],
	...
	render_selected_doctype() {
		this.$sidebar_list.on("click", "li", (e) => {
			...
			this.company_select.set_value(
				appyframe.defaults.get_default("company") || this.company_select.get_value()
			);
			...
			appyframe.set_route("leaderboard", this.options.selected_doctype);
			this.make_request();
		});
	}
	...
	make_request() {
		appyframe.model.with_doctype(this.options.selected_doctype, () => {
	...
	get_leaderboard(notify) {
		...
		if (!company && !this.leaderboard_config[this.options.selected_doctype].company_disabled) {
			notify(this, null);
			appyframe.show_alert(__("Please select Company"));
			return;
		}
		appyframe
			...
			.then((r) => {
				...
				appyframe.utils.make_chart(".leaderboard-graph", custom_options);
	...
	get_leaderboard_data(me, res) {
		if (res && res.message.length) {
			...
			appyframe.utils.setup_search($(me.parent), ".list-item-container", ".list-id");
		} else {
			me.$graph_area.hide();
	...
	render_list_header() {
		const _selected_filter = this.options.selected_filter.map((i) => appyframe.model.unscrub(i));
		const fields = ["rank", "name", this.options.selected_filter_item];
		const filters = fields
			.map((filter) => {
				const col = __(appyframe.model.unscrub(filter));
	...
	render_message() {
		...
		return `<div class="leaderboard-empty-state ${display_class}">
  			<div class="no-result text-center">
  				<img src="/assets/appyframe/images/ui-states/search-empty-state.svg"
  					alt="Empty State"
  					class="null-state"
  				>
	...
	get_item_html(item, index) {
		...
		const link = `/app/${appyframe.router.slug(this.options.selected_doctype)}/${item.name}`;
	...
	get_sidebar_item(item, icon) {
		let icon_html = icon ? appyframe.utils.icon(icon, "md") : "";
	...
	get_date_range() {
		...
		let current_date = appyframe.datetime.now_date();
		let date_range_map = {
			"this week": [appyframe.datetime.week_start(), appyframe.datetime.week_end()],
			"this month": [appyframe.datetime.month_start(), appyframe.datetime.month_end()],
			"this quarter": [appyframe.datetime.quarter_start(), appyframe.datetime.quarter_end()],
			"this year": [appyframe.datetime.year_start(), appyframe.datetime.year_end()],
			"last week": [appyframe.datetime.add_days(current_date, -7), current_date],
			"last month": [appyframe.datetime.add_months(current_date, -1), current_date],
			"last quarter": [appyframe.datetime.add_months(current_date, -3), current_date],
			"last year": [appyframe.datetime.add_months(current_date, -12), current_date],
			"all time": null,
			"select date range": this.selected_date_range || [
				appyframe.datetime.month_start(),
				current_date,
			],
		};
