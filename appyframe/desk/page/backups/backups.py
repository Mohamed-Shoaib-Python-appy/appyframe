...
from appyframe.utils import get_site_path, get_url
from appyframe.utils.data import convert_utc_to_system_timezone
...
def get_context(context):
	...
	backup_limit = appyframe.get_system_settings("backup_limit")
...
def delete_downloadable_backups():
	...
	for x in path.iterdir():
		...
		# Based on the naming convention of the backup files defined in appyframe.utils.backups
		backup_name = x.name.rsplit("-" + appyframe.local.site.replace(".", "_"), maxsplit=1)[0]
...
def schedule_files_backup(user_email: str):
	...
	queued_jobs = get_jobs(site=appyframe.local.site, queue="long")
	method = "appyframe.desk.page.backups.backups.backup_files_and_notify_user"

	if method not in queued_jobs[appyframe.local.site]:
		enqueue(
			"appyframe.desk.page.backups.backups.backup_files_and_notify_user",
			queue="long",
			user_email=user_email,
		)
		appyframe.msgprint(_("Queued for backup. You will receive an email with the download link"))
	else:
		appyframe.msgprint(_("Backup job is already queued. You will receive an email with the download link"))


def backup_files_and_notify_user(user_email=None):
	from appyframe.utils.backups import backup
