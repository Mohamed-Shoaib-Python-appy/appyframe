...
from appyframe.geo.country_info import get_country_info
from appyframe.permissions import AUTOMATIC_ROLES
from appyframe.translate import send_translations, set_default_language
from appyframe.utils import cint, now, strip
...
def get_setup_stages(args):  # nosemgrep
	# App setup stage functions should not include appyframe.db.commit
	# That is done by appyframe after successful completion of all stages
	stages = [
		{
...
def setup_complete(args):
	...
	if cint(appyframe.db.get_single_value("System Settings", "setup_complete")):
		return {"status": "ok"}

	args = parse_args(sanitize_input(args))
	stages = get_setup_stages(args)
	is_background_task = appyframe.conf.get("trigger_site_setup_in_background")
...
def process_setup_stages(stages, user_input, is_background_task=False):
	from appyframe.utils.telemetry import capture

	capture("initated_server_side", "setup")
	try:
		appyframe.flags.in_setup_wizard = True
		current_task = None
		...
		appyframe.log_error(title=f"Setup failed: {message}")
		if not is_background_task:
			appyframe.response["setup_wizard_failure_message"] = message
			raise
		appyframe.publish_realtime(
			"setup_task",
		...
		if not is_background_task:
			return {"status": "ok"}
		appyframe.publish_realtime("setup_task", {"status": "ok"}, user=appyframe.session.user)
	finally:
		appyframe.flags.in_setup_wizard = False
...
def run_post_setup_complete(args):  # nosemgrep
	...
	appyframe.get_cached_doc("System Settings") and appyframe.get_doc("System Settings")


def run_setup_success(args):  # nosemgrep
	for hook in appyframe.get_hooks("setup_wizard_success"):
		appyframe.get_attr(hook)(args)
	install_fixtures.install()


def get_stages_hooks(args):  # nosemgrep
	stages = []
	for method in appyframe.get_hooks("setup_wizard_stages"):
		stages += appyframe.get_attr(method)(args)
	return stages
	...
	return [
		{
			...
			"tasks": [
				{
					"fn": appyframe.get_attr(method),
					"args": args,
					"fail_msg": "Failed to execute method",
				}
			],
		}
		for method in appyframe.get_hooks("setup_wizard_complete")
	]
...
def handle_setup_exception(args):  # nosemgrep
	...
	if args:
		...
		print(traceback)
		for hook in appyframe.get_hooks("setup_wizard_exception"):
			appyframe.get_attr(hook)(traceback, args)
...
def update_system_settings(args):  # nosemgrep
	...
	system_settings = appyframe.get_doc("System Settings", "System Settings")
	system_settings.update(
		{
			...
			"date_format": appyframe.db.get_value("Country", args.get("country"), "date_format"),
			"time_format": appyframe.db.get_value("Country", args.get("country"), "time_format"),
			"number_format": number_format,
			"enable_scheduler": 1 if not appyframe.flags.in_test else 0,
	...
	if args.get("enable_telemetry"):
		appyframe.db.set_default("session_recording_start", now())
...
def create_or_update_user(args):  # nosemgrep
	...
	if user := appyframe.db.get_value("User", email, ["first_name", "last_name"], as_dict=True):
		if user.first_name != first_name or user.last_name != last_name:
			(
				appyframe.qb.update("User")
				.set("first_name", first_name)
				.set("last_name", last_name)
				.set("full_name", args.get("full_name"))
			).run()
	else:
		_mute_emails, appyframe.flags.mute_emails = appyframe.flags.mute_emails, True
		...
		appyframe.flags.mute_emails = _mute_emails
...
def set_timezone(args):  # nosemgrep
	if args.get("timezone"):
		for name in appyframe.STANDARD_USERS:
			appyframe.db.set_value("User", name, "time_zone", args.get("timezone"))


def parse_args(args):  # nosemgrep
	if not args:
		args = appyframe.local.form_dict
...
def add_all_roles_to(name):
	user = appyframe.get_doc("User", name)
	user.append_roles(*_get_default_roles())
	user.save()
...
def _get_default_roles() -> set[str]:
	...
	return set(appyframe.get_all("Role", pluck="name")) - skip_roles


def disable_future_access():
	appyframe.db.set_default("desktop:home_page", "workspace")
	# Enable onboarding after install
	appyframe.db.set_single_value("System Settings", "enable_onboarding", 1)

	appyframe.db.set_single_value("System Settings", "setup_complete", 1)
...
@appyframe.whitelist()
def load_languages():
	language_codes = appyframe.db.sql(
	...
	return {
		"default_language": appyframe.db.get_value("Language", appyframe.local.lang, "language_name")
		or appyframe.local.lang,
		"languages": sorted(appyframe.db.sql_list("select language_name from tabLanguage order by name")),
...
def load_country():
	...
	return get_geo_ip_country(appyframe.local.request_ip) if appyframe.local.request_ip else None


@appyframe.whitelist()
def load_user_details():
	return {
		"full_name": appyframe.cache.hget("full_name", "signup"),
		"email": appyframe.cache.hget("email", "signup"),
	}
...
def email_setup_wizard_exception(traceback, args):  # nosemgrep
	if not appyframe.conf.setup_wizard_exception_email:
		return
...
- **User:** {user}""".format(
		...
		args="\n".join(pretty_args),
		user=appyframe.session.user,
		headers=appyframe.request.headers if appyframe.request else "[no request]",
	)

	appyframe.sendmail(
		recipients=appyframe.conf.setup_wizard_exception_email,
		sender=appyframe.session.user,
		subject=f"Setup failed: {appyframe.local.site}",
		message=message,
		delayed=False,
	)
...
def get_language_code(lang):
	return appyframe.db.get_value("Language", {"language_name": lang})


def enable_twofactor_all_roles():
	all_role = appyframe.get_doc("Role", {"role_name": "All"})
	all_role.two_factor_auth = True
	all_role.save(ignore_permissions=True)


def make_records(records, debug=False):
	from appyframe import _dict
...
def show_document_insert_error():
	...
	appyframe.log_error("Exception during Setup")
