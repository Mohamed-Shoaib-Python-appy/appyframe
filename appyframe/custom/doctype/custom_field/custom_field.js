...
appyframe.ui.form.on("Custom Field", {
	setup: function (frm) {
	...
	dt: function (frm) {
		...
		return appyframe.call({
			method: "appyframe.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && appyframe.utils.has_special_chars(frm.doc.label)) {
			...
			frm.add_custom_button(__("Rename Fieldname"), () => {
				appyframe.prompt(
					...
					function (data) {
						appyframe
							.xcall(
								"appyframe.custom.doctype.custom_field.custom_field.rename_fieldname",
								{
...
appyframe.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";
