...
from appyframe.model import core_doctypes_list
from appyframe.model.docfield import supports_translation
from appyframe.model.document import Document
from appyframe.query_builder.functions import IfNull
from appyframe.utils import cstr, random_string
...
class CustomField(Document):
		...
		if not self.fieldname:
			...
			if not label:
				if self.fieldtype in ["Section Break", "Column Break", "Tab Break"]:
					...
					appyframe.throw(_("Label is mandatory"))
	...
	def validate(self):
		...
		from appyframe.core.doctype.doctype.doctype import check_fieldname_conflicts
		...
		if not self.fieldname:
			appyframe.throw(_("Fieldname not set for Custom Field"))
	...
	def on_update(self):
		...
		if not appyframe.flags.in_create_custom_fields:
			appyframe.clear_cache(doctype=self.dt)
			appyframe.db.updatedb(self.dt)

	def on_trash(self):
		# check if Admin owned field
		if self.owner == "Administrator" and appyframe.session.user != "Administrator":
			appyframe.throw(
				_(
					"Custom Field {0} is created by the Administrator and can only be deleted through the Administrator account."
				).format(appyframe.bold(self.label))
			)

		# delete property setter entries
		appyframe.db.delete("Property Setter", {"doc_type": self.dt, "field_name": self.fieldname})

		# update doctype layouts
		doctype_layouts = appyframe.get_all("DocType Layout", filters={"document_type": self.dt}, pluck="name")
	...
	def validate_insert_after(self, meta):
		if not meta.get_field(self.insert_after):
			appyframe.throw(
				...
				appyframe.DoesNotExistError,
			)

		if self.fieldname == self.insert_after:
			appyframe.throw(_("Insert After cannot be set as {0}").format(meta.get_label(self.insert_after)))
...
def get_fields_label(doctype=None):
	...
	if doctype in core_doctypes_list:
		return appyframe.msgprint(_("Custom Fields cannot be added to core DocTypes."))

	if meta.custom:
		return appyframe.msgprint(_("Custom Fields can only be added to a standard DocType."))

	return [
		{"value": df.fieldname or "", "label": _(df.label, context=df.parent) if df.label else ""}
		for df in appyframe.get_meta(doctype).get("fields")
	]


def create_custom_field_if_values_exist(doctype, df):
	df = appyframe._dict(df)
	if df.fieldname in appyframe.db.get_table_columns(doctype) and appyframe.db.count(
...
def create_custom_field(doctype, df, ignore_validate=False, is_system_generated=True):
	...
	if not df.fieldname and df.label:
		df.fieldname = appyframe.scrub(df.label)
	if not appyframe.db.get_value("Custom Field", {"dt": doctype, "fieldname": df.fieldname}):
...
def create_custom_fields(custom_fields: dict, ignore_validate=False, update=True):
	...
	try:
		appyframe.flags.in_create_custom_fields = True
		doctypes_to_update = set()

		if appyframe.flags.in_setup_wizard:
			...
			for doctype in doctypes:
				...
				for df in fields:
					...
					if not field:
						...
						except appyframe.exceptions.DuplicateEntryError:
							pass

					elif update:
						custom_field = appyframe.get_doc({"doctype": "Custom Field", **field})
		...
		appyframe.flags.in_create_custom_fields = False
...
def get_existing_custom_fields(custom_fields):
	...
	existing_fields = appyframe.get_all("Custom Field", filters={"dt": ("in", doctypes_to_fetch)}, fields="*")
...
def rename_fieldname(custom_field: str, fieldname: str):
	...
	field: CustomField = appyframe.get_doc("Custom Field", custom_field)
	...
	if field.is_system_generated:
		appyframe.throw(_("System Generated Fields can not be renamed"))
	if appyframe.db.has_column(parent_doctype, fieldname):
		appyframe.throw(_("Can not rename as column {0} is already present on DocType.").format(fieldname))
	if old_fieldname == new_fieldname:
		appyframe.msgprint(_("Old and new fieldnames are same."), alert=True)
		return

	if appyframe.db.has_column(field.dt, old_fieldname):
		appyframe.db.rename_column(parent_doctype, old_fieldname, new_fieldname)
	...
	appyframe.msgprint(_("Custom field renamed to {0} successfully.").format(fieldname), alert=True)
	appyframe.db.commit()
...
def _update_fieldname_references(field: CustomField, old_fieldname: str, new_fieldname: str) -> None:
	...
	if field.fieldtype == "Password":
		Auth = appyframe.qb.Table("__Auth")
