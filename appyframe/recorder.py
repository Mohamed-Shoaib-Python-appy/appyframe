...
from appyframe.database.database import is_query_type
...
class RecorderConfig:
	...
	def __post_init__(self):
		if not (self.record_jobs or self.record_requests):
			appyframe.throw("You must record one of jobs or requests")

	def store(self):
		appyframe.cache.set_value(RECORDER_CONFIG_FLAG, self, expires_in_sec=RECORDER_AUTO_DISABLE)

	@classmethod
	def retrieve(cls):
		return appyframe.cache.get_value(RECORDER_CONFIG_FLAG) or cls()

	@staticmethod
	def delete():
		appyframe.cache.delete_value(RECORDER_CONFIG_FLAG)


def record_sql(*args, **kwargs):
	start_time = time.monotonic()
	result = appyframe.db._sql(*args, **kwargs)
	end_time = time.monotonic()

	query = getattr(appyframe.db, "last_query", None)
	if not query or isinstance(result, str):
		# run=0, doesn't actually run the query so last_query won't be present
		return result

	stack = []
	if appyframe.local._recorder.config.capture_stack:
	...
	appyframe.local._recorder.register(data)
	return result


def get_current_stack_frames():
	from appyframe.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX
...
def post_process():
	...
	appyframe.db.begin(read_only=True)  # Explicitly start read only transaction

	config = RecorderConfig.retrieve()
	result = list(appyframe.cache.hgetall(RECORDER_REQUEST_HASH).values())
	...
	for request in result:
		for call in request["calls"]:
			...
			if config.explain and is_query_type(formatted_query, ("select", "update", "delete")):
				...
				try:
					call["explain_result"] = appyframe.db.sql(f"EXPLAIN {formatted_query}", as_dict=True)
				except Exception:
					pass
		mark_duplicates(request)
		appyframe.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
...
def record(force=False):
	if __debug__:
		if appyframe.cache.get_value(RECORDER_INTERCEPT_FLAG) or force:
			appyframe.local._recorder = Recorder(force=force)


def dump():
	if __debug__:
		if hasattr(appyframe.local, "_recorder"):
			appyframe.local._recorder.dump()
...
class Recorder:
	def __init__(self, force=False):
		...
		if (
			...
			and appyframe.request
			and self.config.request_filter in appyframe.request.path
		):
			self.path = appyframe.request.path
			self.cmd = appyframe.local.form_dict.cmd or ""
			self.method = appyframe.request.method
			self.headers = dict(appyframe.local.request.headers)
			self.form_dict = appyframe.local.form_dict
			self.event_type = "HTTP Request"
		elif self.config.record_jobs and appyframe.job and self.config.jobs_filter in appyframe.job.method:
			self.event_type = "Background Job"
			self.path = appyframe.job.method
			self.cmd = None
			self.method = None
			self.headers = None
			self.form_dict = None
		...
		self.uuid = appyframe.generate_hash(length=10)
	...
	def dump(self):
		...
		appyframe.cache.hset(RECORDER_REQUEST_SPARSE_HASH, self.uuid, request_data)

		request_data["calls"] = self.calls
		request_data["headers"] = self.headers
		request_data["form_dict"] = self.form_dict
		request_data["profile"] = "".join(profiler_output.splitlines(keepends=True)[:200])
		appyframe.cache.hset(RECORDER_REQUEST_HASH, self.uuid, request_data)
	...
	@staticmethod
	def _patch_sql():
		appyframe.db._sql = appyframe.db.sql
		appyframe.db.sql = record_sql

	@staticmethod
	def _unpatch_sql():
		appyframe.db.sql = appyframe.db._sql
...
def do_not_record(function):
	...
	def wrapper(*args, **kwargs):
		if hasattr(appyframe.local, "_recorder"):
			appyframe.local._recorder.cleanup()
			del appyframe.local._recorder
...
def administrator_only(function):
	...
	def wrapper(*args, **kwargs):
		if appyframe.session.user != "Administrator":
			appyframe.throw(_("Only Administrator is allowed to use Recorder"))
		return function(*args, **kwargs)
...
def status(*args, **kwargs):
	return bool(appyframe.cache.get_value(RECORDER_INTERCEPT_FLAG))
...
def start(
	...
	**kwargs,
	...
	appyframe.cache.set_value(RECORDER_INTERCEPT_FLAG, 1, expires_in_sec=RECORDER_AUTO_DISABLE)
...
def stop(*args, **kwargs):
	appyframe.cache.delete_value(RECORDER_INTERCEPT_FLAG)
	appyframe.enqueue(post_process, now=appyframe.flags.in_test)
...
def get(uuid=None, *args, **kwargs):
	if uuid:
		result = appyframe.cache.hget(RECORDER_REQUEST_HASH, uuid)
	else:
		result = list(appyframe.cache.hgetall(RECORDER_REQUEST_SPARSE_HASH).values())
	return result
...
def export_data(*args, **kwargs):
	return list(appyframe.cache.hgetall(RECORDER_REQUEST_HASH).values())
...
def delete(*args, **kwargs):
	appyframe.cache.delete_value(RECORDER_REQUEST_SPARSE_HASH)
	appyframe.cache.delete_value(RECORDER_REQUEST_HASH)
...
def record_queries(func: Callable):
	...
	def wrapped(*args, **kwargs):
		record(force=True)
		appyframe.local._recorder.path = f"Function call: {func.__module__}.{func.__qualname__}"
...
def import_data(file: str) -> None:
	file_doc = appyframe.get_doc("File", {"file_url": file})
	file_content = json.loads(file_doc.get_content())
	for request in file_content:
		appyframe.cache.hset(RECORDER_REQUEST_SPARSE_HASH, request["uuid"], request)
		appyframe.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
