...
from appyframe.utils.background_jobs import get_queues_timeout
from appyframe.utils.jinja import validate_template
from appyframe.utils.safe_exec import get_safe_globals

WEBHOOK_SECRET_HEADER = "X-Appyframe-Webhook-Signature"
...
class Webhook(Document):
	...
	if TYPE_CHECKING:
		from appyframe.integrations.doctype.webhook_data.webhook_data import WebhookData
		from appyframe.integrations.doctype.webhook_header.webhook_header import WebhookHeader
	...
	def on_update(self):
		appyframe.cache.delete_value("webhooks")

	def validate_docevent(self):
		if self.webhook_doctype:
			is_submittable = appyframe.get_value("DocType", self.webhook_doctype, "is_submittable")
			if not is_submittable and self.webhook_docevent in [
				"on_submit",
				"on_cancel",
				"on_update_after_submit",
			]:
				appyframe.throw(_("DocType must be Submittable for the selected Doc Event"))

	def validate_condition(self):
		temp_doc = appyframe.new_doc(self.webhook_doctype)
		if self.condition:
			try:
				appyframe.safe_eval(self.condition, eval_locals=get_context(temp_doc))
			except Exception as e:
				appyframe.throw(_("Invalid Condition: {}").format(e))
	...
	def validate_request_url(self):
		try:
			...
			appyframe.throw(_("Check Request URL"), exc=e)
	...
	def validate_repeating_fields(self):
		...
		if len(webhook_data) != len(set(webhook_data)):
			appyframe.throw(_("Same Field is entered more than once"))
	...
	def validate_secret(self):
		if self.enable_security:
			try:
				...
				appyframe.throw(_("Invalid Webhook Secret"))
	...
	def meets_condition(self):
		...
		try:
			doc = appyframe.get_cached_doc(self.webhook_doctype, self.preview_document)
			met_condition = appyframe.safe_eval(self.condition, eval_locals=get_context(doc))
		except Exception as e:
	...
	def preview_request_body(self):
		...
		try:
			doc = appyframe.get_cached_doc(self.webhook_doctype, self.preview_document)
			return appyframe.as_json(get_webhook_data(doc, self))
		except Exception as e:
			return _("Failed to compute request body: {}").format(e)


def get_context(doc):
	return {"doc": doc, "utils": get_safe_globals().get("appyframe").get("utils")}


def enqueue_webhook(doc, webhook) -> None:
	request_url = headers = data = r = None
	try:
		webhook: Webhook = appyframe.get_doc("Webhook", webhook.get("name"))
		request_url = webhook.request_url
		if webhook.is_dynamic_url:
			request_url = appyframe.render_template(webhook.request_url, get_context(doc))
		headers = get_webhook_headers(doc, webhook)
		data = get_webhook_data(doc, webhook)

	except Exception as e:
		appyframe.logger().debug({"enqueue_webhook_error": e})
	...
	for i in range(3):
		try:
			...
			r.raise_for_status()
			appyframe.logger().debug({"webhook_success": r.text})
			log_request(webhook.name, doc.name, request_url, headers, data, r)
			break

		except requests.exceptions.ReadTimeout as e:
			appyframe.logger().debug({"webhook_error": e, "try": i + 1})
			log_request(webhook.name, doc.name, request_url, headers, data)
...
def log_request(
	...
	request_log = appyframe.get_doc(
		{
			"doctype": "Webhook Request Log",
			"webhook": webhook,
			"reference_document": docname,
			"user": appyframe.session.user if appyframe.session.user else None,
			"url": url,
			"headers": appyframe.as_json(headers) if headers else None,
			"data": appyframe.as_json(data) if data else None,
			"response": res.text if res is not None else None,
			"error": appyframe.get_traceback(),
		}
	)
...
def get_webhook_data(doc, webhook):
	...
	if webhook.webhook_data:
		...
		data = appyframe.render_template(webhook.webhook_json, get_context(doc))
