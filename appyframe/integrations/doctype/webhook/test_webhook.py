...
import appyframe
from appyframe.integrations.doctype.webhook import flush_webhook_execution_queue
from appyframe.integrations.doctype.webhook.webhook import (
	enqueue_webhook,
	get_webhook_data,
	get_webhook_headers,
)
...
def get_test_webhook(config):
	wh = appyframe.get_doc(config)
	if not wh.name:
		wh.name = appyframe.generate_hash()
	wh.insert()
	wh.reload()
	try:
		yield wh
...
class TestWebhook(FrappeTestCase):
	...
	def setUpClass(cls):
		...
		appyframe.db.delete("Webhook")
		# Delete existing logs if any
		appyframe.db.delete("Webhook Request Log")
		super().setUpClass()
	...
	def setUp(self):
		...
		if appyframe.db.exists("Webhook", webhook_fields):
			self.webhook = appyframe.get_doc("Webhook", webhook_fields)
		else:
			self.webhook = appyframe.new_doc("Webhook")
			self.webhook.update(webhook_fields)

		# create a User document
		self.user = appyframe.new_doc("User")
		self.user.first_name = appyframe.mock("name")
		self.user.email = appyframe.mock("email")
		self.user.save()

		# Create another test user specific to this test
		self.test_user = appyframe.new_doc("User")
	...
	def test_webhook_trigger_with_enabled_webhooks(self):
		...
		webhooks = appyframe.cache.get_value("webhooks")
		self.assertTrue("User" in webhooks)
		self.assertEqual(len(webhooks.get("User")), 1)

		# only 1 hook (enabled) must be queued
		self.assertEqual(len(appyframe.local._webhook_queue), 1)
		execution = appyframe.local._webhook_queue[0]
	...
	def test_validate_doc_events(self):
		...
		self.assertRaises(appyframe.ValidationError, self.webhook.save)
	...
	def test_webhook_req_log_creation(self):
		...
		if not appyframe.db.get_value("User", "user2@integration.webhooks.test.com"):
			user = appyframe.get_doc(
				{"doctype": "User", "email": "user2@integration.webhooks.test.com", "first_name": "user2"}
			).insert()
		else:
			user = appyframe.get_doc("User", "user2@integration.webhooks.test.com")

		webhook = appyframe.get_doc("Webhook", {"webhook_doctype": "User"})
		enqueue_webhook(user, webhook)

		self.assertTrue(appyframe.get_all("Webhook Request Log", pluck="name"))
	...
	def test_webhook_with_array_body(self):
		...
		doc = appyframe.new_doc("Note")
		doc.title = "Test Webhook Note"
		final_title = appyframe.generate_hash()
		...
		with get_test_webhook(wh_config):
			...
			log = appyframe.get_last_doc("Webhook Request Log")
