...
import appyframe
from appyframe.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = appyframe.new_doc(doctype)
	connected_app.provider_name = "appyframe"
...
def get_oauth_client():
	oauth_client = appyframe.new_doc("OAuth Client")
...
class TestConnectedApp(FrappeTestCase):
	def setUp(self):
		...
		Appyframe comes with it's own oAuth2 provider that we can test against. The
		...
		Appyframe's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/appyframe.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		if getattr(self, "oauth_client", None):
			tokens = appyframe.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = appyframe.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = appyframe.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = appyframe.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
