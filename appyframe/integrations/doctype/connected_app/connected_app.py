...
if any((os.getenv("CI"), appyframe.conf.developer_mode, appyframe.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	if TYPE_CHECKING:
		from appyframe.integrations.doctype.oauth_scope.oauth_scope import OAuthScope
		from appyframe.integrations.doctype.query_parameters.query_parameters import QueryParameters
	...
	def validate(self):
		base_url = appyframe.utils.get_url()
		callback_path = (
			"/api/method/appyframe.integrations.doctype.connected_app.connected_app.callback/" + self.name
		)
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = appyframe.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		appyframe.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if appyframe.db.exists("Token Cache", token_cache_name):
			token_cache = appyframe.get_doc("Token Cache", token_cache_name)
...
@appyframe.whitelist(methods=["GET"], allow_guest=True)
def callback(code=None, state=None):
	...
	if appyframe.session.user == "Guest":
		appyframe.local.response["type"] = "redirect"
		appyframe.local.response["location"] = "/login?" + urlencode({"redirect-to": appyframe.request.url})
		return

	path = appyframe.request.path[1:].split("/")
	if len(path) != 4 or not path[3]:
		appyframe.throw(_("Invalid Parameters."))

	connected_app = appyframe.get_doc("Connected App", path[3])
	token_cache = appyframe.get_doc("Token Cache", connected_app.name + "-" + appyframe.session.user)

	if state != token_cache.state:
		appyframe.throw(_("Invalid token state! Check if the token has been created by the OAuth user."))
	...
	appyframe.local.response["type"] = "redirect"
	appyframe.local.response["location"] = token_cache.get("success_uri") or connected_app.get_url()


@appyframe.whitelist()
def has_token(connected_app, connected_user=None):
	app = appyframe.get_doc("Connected App", connected_app)
	token_cache = app.get_token_cache(connected_user or appyframe.session.user)
