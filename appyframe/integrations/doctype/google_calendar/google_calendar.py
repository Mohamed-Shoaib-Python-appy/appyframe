...
from appyframe.integrations.google_oauth import GoogleOAuth
...
from appyframe.utils.password import set_encrypted_password

if TYPE_CHECKING:
	from appyframe.desk.doctype.event.event import Event
...
class GoogleCalendar(Document):
	...
	def validate(self):
		google_settings = appyframe.get_cached_doc("Google Settings")
		if not google_settings.enable:
			appyframe.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			appyframe.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			raise appyframe.ValidationError(
				_("Click on {0} to generate Refresh Token.").format(appyframe.bold(allow_google_calendar_label))
			)
		...
		try:
			...
			appyframe.throw(
				...
				).format(appyframe.bold(allow_google_calendar_label))
			)
...
def authorize_access(g_calendar: str, reauthorize: bool = False):
	...
	"""
	google_calendar = appyframe.get_doc("Google Calendar", g_calendar)
	...
	if not google_calendar.authorization_code or reauthorize:
		appyframe.cache.hset("google_calendar", "google_calendar", google_calendar.name)
	...
	if "refresh_token" in r:
		appyframe.db.set_value("Google Calendar", google_calendar.name, "refresh_token", r["refresh_token"])
		appyframe.db.commit()

	appyframe.local.response["type"] = "redirect"
	appyframe.local.response["location"] = google_calendar.get_url()

	appyframe.msgprint(_("Google Calendar has been configured."), indicator="green")
...
def google_callback(code=None):
	...
	"""
	google_calendar = appyframe.cache.hget("google_calendar", "google_calendar")
	appyframe.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	appyframe.db.commit()
...
def sync(g_calendar: str | None = None):
	...
	for g in appyframe.get_list("Google Calendar", filters=filters, pluck="name"):
...
def get_google_calendar_object(g_calendar):
	...
	google_settings = appyframe.get_cached_doc("Google Settings")
	account: GoogleCalendar = appyframe.get_doc("Google Calendar", g_calendar)
...
def check_google_calendar(account: GoogleCalendar, google_calendar):
	...
	calendar = {
		...
		"timeZone": appyframe.get_system_settings("time_zone"),
	}
	try:
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				appyframe.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				appyframe.throw(msg)
	...
	for idx, event in enumerate(results):
		appyframe.publish_realtime(
			"import_google_calendar", {"progress": idx + 1, "total": len(results)}, user=appyframe.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not appyframe.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
		...
		# If any synced Google Calendar Event is cancelled, then close the Event
			...
			event_name = appyframe.db.get_value(
				"Event",
				{
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in Appyframe Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	e: Event = appyframe.get_doc(calendar_event)
...
def update_participants_in_event(calendar_event: "Event", google_event: dict):
	...
	in_system_participants = appyframe.get_all(
...
def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in Appyframe Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = appyframe.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	if (
		...
		or not appyframe.db.exists("Google Calendar", {"name": doc.google_calendar})
	):
		return
	...
	try:
		...
		appyframe.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		appyframe.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in Appyframe Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if Appyframe Event is deleted.
	"""

	if not appyframe.db.exists("Google Calendar", {"name": doc.google_calendar, "push_to_google_calendar": 1}):
		return
