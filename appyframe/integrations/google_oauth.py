...
import appyframe
from appyframe.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/appyframe.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "appyframe.email.oauth.authorize_google_access",
	"contacts": "appyframe.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"drive": "appyframe.integrations.doctype.google_drive.google_drive.authorize_access",
	"indexing": "appyframe.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = appyframe.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			appyframe.throw(appyframe._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			appyframe.throw(appyframe._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		appyframe.log_error(appyframe._(error_title), appyframe._(response.get("error_description", error_message)))

		if raise_err:
			appyframe.throw(appyframe._(error_title), GoogleAuthenticationError, appyframe._(error_message))
...
@appyframe.whitelist(methods=["GET"])
def callback(state: str, code: str | None = None, error: str | None = None) -> None:
	"""Common callback for google integrations.
	Invokes functions using `appyframe.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to appyframe site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			appyframe.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
	...
	appyframe.local.response["location"] = f"{redirect}?{failure_query_param if error else success_query_param}"
