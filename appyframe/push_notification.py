...
import appyframe
from appyframe import sbool
from appyframe.utils.response import Response
...
class PushNotification:
	...
	def send_notification_to_user(
		...
		if strip_html:
			body = appyframe.utils.strip_html(body)
	...
	def is_enabled(self) -> bool:
		...
		"""
		return sbool(
			appyframe.db.get_single_value("Push Notification Settings", "enable_push_notification_relay")
		)
	...
	def _get_credential(self) -> tuple[str, str]:
		...
		"""
		notification_settings = appyframe.get_doc("Push Notification Settings")
		...
		token = appyframe.generate_hash(length=48)
		...
		body = {
			...
			"token": token,
			"webhook_route": "/api/method/appyframe.push_notification.auth_webhook",
		}
	...
	def _send_post_request(self, method: str, params: dict, use_authentication: bool = True):
		...
		relay_server_endpoint = appyframe.conf.get("push_relay_server_url")
	...
	def _site_name(self) -> str:
		return urlparse(appyframe.utils.get_url()).hostname

	@property
	def _site_protocol(self) -> str:
		return urlparse(appyframe.utils.get_url()).scheme

	@property
	def _site_port(self) -> str:
		site_uri = urlparse(appyframe.utils.get_url())
...
def auth_webhook():
	url = urlparse(appyframe.utils.get_url()).hostname
	token = appyframe.cache().get_value(f"{url}:push_relay_registration_token")
	response = Response()
...
def subscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).add_token(appyframe.session.user, fcm_token)
	return {"success": success, "message": message}


@appyframe.whitelist(methods=["GET"])
def unsubscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).remove_token(appyframe.session.user, fcm_token)
	return {"success": success, "message": message}
