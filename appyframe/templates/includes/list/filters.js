function setup_list_filters() {
	...
	appyframe.call('appyframe.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = appyframe.utils.get_query_params()[df.fieldname];
			const f = appyframe.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(appyframe.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = appyframe.utils.make_query_string(query_params);
...
appyframe.ready(setup_list_filters);
