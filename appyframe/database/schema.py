...
class DBTable:
	def __init__(self, doctype, meta=None):
		...
		self.meta = meta or appyframe.get_meta(doctype, False)
	...
	def sync(self):
		...
		if self.is_new():
			...
			appyframe.cache.hdel("table_columns", self.table_name)
			self.alter()
	...
	def get_column_definitions(self):
		column_list = [*appyframe.db.DEFAULT_COLUMNS]
		ret = []
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			for fieldname in appyframe.db.OPTIONAL_COLUMNS:
	...
	def validate(self):
		...
		columns = [
			appyframe._dict({"fieldname": f, "fieldtype": "Data"}) for f in appyframe.db.STANDARD_VARCHAR_COLUMNS
		]
		if self.meta.get("istable"):
			columns += [
				appyframe._dict({"fieldname": f, "fieldtype": "Data"}) for f in appyframe.db.CHILD_TABLE_COLUMNS
			]
		...
		for col in columns:
			if len(col.fieldname) >= 64:
				appyframe.throw(
					_("Fieldname is limited to 64 characters ({0})").format(appyframe.bold(col.fieldname))
				)

			if "varchar" in appyframe.db.type_map.get(col.fieldtype, ()):
				# validate length range
				new_length = cint(col.length) or cint(appyframe.db.VARCHAR_LEN)
				if not (1 <= new_length <= 1000):
					appyframe.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
				...
				if cint(current_length) != cint(new_length):
					try:
						...
						max_length = appyframe.db.sql(
							f"""SELECT MAX(CHAR_LENGTH(`{col.fieldname}`)) FROM `tab{self.doctype}`"""
						)

					except appyframe.db.InternalError as e:
					...
					if max_length and max_length[0][0] and max_length[0][0] > new_length:
						...
						appyframe.msgprint(info_message)

	def is_new(self):
		return self.table_name not in appyframe.db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in appyframe.db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, for_modification=False):
		...
		if self.fieldtype in ("Check", "Int"):
			...
			self.default
			and (self.default not in appyframe.db.DEFAULT_SHORTCUTS)
			and not cstr(self.default).startswith(":")
		):
			column_def += f" default {appyframe.db.escape(self.default)}"
	...
	if special_characters := SPECIAL_CHAR_PATTERN.findall(n):
		...
		appyframe.throw(
			_("Fieldname {0} cannot have special characters like {1}").format(
				appyframe.bold(cstr(n)), special_characters
			),
			appyframe.db.InvalidColumnName,
		)
	return n


def validate_column_length(fieldname):
	if len(fieldname) > appyframe.db.MAX_COLUMN_LENGTH:
		appyframe.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))


def get_definition(fieldtype, precision=None, length=None):
	d = appyframe.db.type_map.get(fieldtype)
	...
	if fieldtype == "Int" and length and length > 11:
		...
		d = appyframe.db.type_map.get("Long Int")
	...
	if size:
		...
		if length:
			if coltype == "varchar":
				...
				# NOTE: this will only be applicable for mariadb as appyframe implements int
