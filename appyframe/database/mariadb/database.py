...
from appyframe.database.mariadb.schema import MariaDBTable
from appyframe.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if appyframe.conf.local_infile:
			conn_settings["local_infile"] = appyframe.conf.local_infile

		if appyframe.conf.db_ssl_ca and appyframe.conf.db_ssl_cert and appyframe.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": appyframe.conf.db_ssl_ca,
				"cert": appyframe.conf.db_ssl_cert,
				"key": appyframe.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via appyframe.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = appyframe.as_unicode(escape_string(appyframe.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_column_index(self, table_name: str, fieldname: str, unique: bool = False) -> appyframe._dict | None:
	...
	def add_index(self, doctype: str, fields: list, index_name: str | None = None):
		...
		if not self.has_index(table_name, index_name):
			...
			# via code or console.
			if len(fields) == 1 and not (appyframe.flags.in_install or appyframe.flags.in_migrate):
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = appyframe.cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			tables = (
				...
				.where(information_schema.tables.table_schema == appyframe.db.cur_db_name)
				.run(pluck=True)
			)
			appyframe.cache.set_value("db_tables", tables)
