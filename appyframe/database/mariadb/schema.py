...
from appyframe.database.schema import DBTable
...
class MariaDBTable(DBTable):
	...
	def alter(self):
		...
		add_index_query = [
			...
			if not appyframe.db.get_column_index(self.table_name, col.fieldname, unique=False)
		]

		if self.meta.sort_field == "creation" and not appyframe.db.get_column_index(
		...
		for col in {*self.drop_index, *self.drop_unique}:
			...
			if unique_constraint_changed and not col.unique:
				if unique_index := appyframe.db.get_column_index(self.table_name, col.fieldname, unique=True):
			...
			if index_constraint_changed and not col.set_index:
				if index_record := appyframe.db.get_column_index(self.table_name, col.fieldname, unique=False):
