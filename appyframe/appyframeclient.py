"""
FrappeClient is a library that helps you connect with other appyframe systems
"""
import base64
import json
...
class FrappeClient:
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.frappe_authorization_source:
				auth_source = {"Appyframe-Authorization-Source": self.frappe_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": appyframe.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return appyframe._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "appyframe.client.insert_many", "docs": appyframe.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": appyframe.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return appyframe._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "appyframe.client.bulk_update", "docs": appyframe.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "appyframe.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "appyframe.client.submit", "doc": appyframe.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "appyframe.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": appyframe.as_json(filters),
			}
		)
	...
	def set_value(self, doctype, docname, fieldname, value):
		...
		return self.post_request(
			{
				"cmd": "appyframe.client.set_value",
				"doctype": doctype,
				"name": docname,
				"fieldname": fieldname,
				"value": value,
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "appyframe.client.cancel", "doctype": doctype, "name": name})
	...
	def rename_doc(self, doctype, old_name, new_name):
		...
		params = {
			"cmd": "appyframe.client.rename_doc",
			"doctype": doctype,
			"old_name": old_name,
			"new_name": new_name,
		}
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not appyframe.db.exists("User", doc.get("owner")):
				appyframe.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = appyframe.db.get_single_value(doctype, "modified")
