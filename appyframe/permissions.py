...
from appyframe import _, msgprint
...
def print_has_permission_check_logs(func):
	...
	def inner(*args, **kwargs):
		...
		self_perm_check = True if not kwargs.get("user") else kwargs.get("user") == appyframe.session.user

		if raise_exception:
			appyframe.flags["has_permission_check_logs"] = []
		...
		if not result and self_perm_check and raise_exception:
			msgprint(("<br>").join(appyframe.flags.get("has_permission_check_logs", [])))

		if raise_exception:
			appyframe.flags.pop("has_permission_check_logs", None)
		return result
...
def _debug_log(log: str):
	if not hasattr(appyframe.local, "permission_debug_log"):
		appyframe.local.permission_debug_log = []
	appyframe.local.permission_debug_log.append(log)


def _pop_debug_log() -> list[str]:
	if log := getattr(appyframe.local, "permission_debug_log", None):
		del appyframe.local.permission_debug_log
		return log
	return []
...
def has_permission(
	...
	*,
	...
	:param raise_exception:
	        ...
	        If not False, will display a message using appyframe.msgprint
	...
	if ptype == "share" and appyframe.get_system_settings("disable_document_sharing"):
	...
	if doc:
		if isinstance(doc, str | int):
			doc = appyframe.get_doc(meta.name, doc)
		...
		if not perm:
			...
			msg = _("User {0} does not have access to this document").format(appyframe.bold(user))
			if appyframe.has_permission(doc.doctype):
		...
		debug and _debug_log(
			...
			+ appyframe.as_json(role_permissions, indent=8)
		)
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					appyframe.bold(user), appyframe.bold(_(doctype))
				),
				debug=debug,
			)
	...
	def false_if_not_shared():
		...
		if doc:
			...
			shared = appyframe.share.get_shared(
				doctype,
				user,
				rights=rights,
		...
		elif appyframe.share.get_shared(doctype, user, rights=rights, limit=1):
...
def get_doc_permissions(doc, user=None, ptype=None, debug=False):
	...
	debug and _debug_log(
		"User has following permissions using role permission system: "
		+ appyframe.as_json(permissions, indent=8)
	)
	...
	if permissions.get("has_if_owner_enabled"):
		...
		debug and _debug_log(
			"User is owner of document, so permissions are updated to: " + appyframe.as_json(permissions)
		)
...
def get_role_permissions(doctype_meta, user=None, is_owner=None, debug=False):
	...
	if isinstance(doctype_meta, str):
		doctype_meta = appyframe.get_meta(doctype_meta)  # assuming doctype name was passed
	...
	if not appyframe.local.role_permissions.get(cache_key) or debug:
		perms = appyframe._dict(if_owner={})
		...
		appyframe.local.role_permissions[cache_key] = perms

	return appyframe.local.role_permissions[cache_key]


def get_user_permissions(user):
	from appyframe.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None, debug=False):
	...
	apply_strict_user_permissions = (
		False if doc.meta.issingle else appyframe.get_system_settings("apply_strict_user_permissions")
	)
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = appyframe.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None, debug=False) -> bool:
	...
	hooks = appyframe.get_hooks("has_permission")
	methods = hooks.get(doc.doctype, []) + hooks.get("*", [])

	for method in reversed(methods):
		controller_permission = appyframe.call(method, doc=doc, ptype=ptype, user=user, debug=debug)
...
def get_all_perms(role):
	...
	perms = appyframe.get_all("DocPerm", fields="*", filters=dict(role=role))
	custom_perms = appyframe.get_all("Custom DocPerm", fields="*", filters=dict(role=role))
	doctypes_with_custom_perms = appyframe.get_all("Custom DocPerm", pluck="parent", distinct=True)
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return appyframe.get_all("Role", pluck="name")  # return all available roles
		else:
	...
	roles = appyframe.cache.hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype="DocPerm"):
	...
	return appyframe.get_all(perm_doctype, fields=["*"], filters=filters)


def get_doctypes_with_custom_docperms():
	"""Returns all the doctypes with Custom Docperms"""

	doctypes = appyframe.get_all("Custom DocPerm", fields=["parent"], distinct=1)
...
def add_user_permission(
	...
	"""Add user permission"""
	from appyframe.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not appyframe.db.exists(doctype, name):
			appyframe.throw(_("{0} {1} not found").format(_(doctype), name), appyframe.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = appyframe.db.get_value(
		"User Permission", dict(user=user, allow=doctype, for_value=name)
	)
	appyframe.delete_doc("User Permission", user_permission_name)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	if user:
		filters["user"] = user
	user_permissions_for_doctype = appyframe.get_all("User Permission", filters=filters)
	for d in user_permissions_for_doctype:
		appyframe.delete_doc("User Permission", d.name)


def can_import(doctype, raise_exception=False):
	if not ("System Manager" in appyframe.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise appyframe.PermissionError(f"You are not allowed to import: {doctype}")
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False, is_owner=False):
	if "System Manager" in appyframe.get_roles():
		return True
	else:
		role_permissions = appyframe.permissions.get_role_permissions(doctype, is_owner=is_owner)
		has_access = role_permissions.get("export") or role_permissions.get("if_owner").get("export")
		if not has_access and raise_exception:
			raise appyframe.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access
...
def update_permission_property(
	...
	from appyframe.core.doctype.doctype.doctype import validate_permissions_for_doctype

	out = setup_custom_perms(doctype)

	name = appyframe.db.get_value(
		"Custom DocPerm", dict(parent=doctype, role=role, permlevel=permlevel, if_owner=if_owner)
	)
	table = DocType("Custom DocPerm")
	appyframe.qb.update(table).set(ptype, value).where(table.name == name).run()
...
def setup_custom_perms(parent):
	...
	if not appyframe.db.exists("Custom DocPerm", dict(parent=parent)):
		copy_perms(parent)
		return True
...
def add_permission(doctype, role, permlevel=0, ptype=None):
	...
	custom_docperm = appyframe.get_doc(
		{
...
def copy_perms(parent):
	...
	for d in appyframe.get_all("DocPerm", fields="*", filters=dict(parent=parent)):
		custom_perm = appyframe.new_doc("Custom DocPerm")
...
def reset_perms(doctype):
	...
	appyframe.db.delete("Custom DocPerm", {"parent": doctype})


def get_linked_doctypes(dt: str) -> list:
	meta = appyframe.get_meta(dt)
	linked_doctypes = [dt] + [
		d.options
		for d in meta.get(
			"fields",
...
def push_perm_check_log(log, debug=False):
	...
	if appyframe.flags.get("has_permission_check_logs") is None:
		return

	appyframe.flags.get("has_permission_check_logs").append(log)
...
def has_child_permission(
	...
	*,
	...
	if isinstance(child_doc, str):
		child_doc = appyframe.db.get_value(
			child_doctype,
			child_doc,
	...
	if not parent_doctype:
		push_perm_check_log(
			_("Please specify a valid parent DocType for {0}").format(appyframe.bold(child_doctype)),
			debug=debug,
		)
		return False

	parent_meta = appyframe.get_meta(parent_doctype)
	...
	if parent_meta.istable or not (
		...
		push_perm_check_log(
			_("{0} is not a valid parent DocType for {1}").format(
				appyframe.bold(parent_doctype), appyframe.bold(child_doctype)
			),
			debug=debug,
		)
		return False
	...
	if child_doc:
		...
		if not parentfield:
			push_perm_check_log(
				_("Parentfield not specified in {0}: {1}").format(
					appyframe.bold(child_doctype), appyframe.bold(child_doc.name)
				),
				debug=debug,
			)
			return False
		...
		if parentfield not in valid_parentfields:
			push_perm_check_log(
				_("{0} is not a valid parentfield for {1}").format(
					appyframe.bold(parentfield), appyframe.bold(child_doctype)
				),
				debug=debug,
			)
			return False
		...
		if permlevel > 0 and permlevel not in accessible_permlevels:
			push_perm_check_log(
				_("Insufficient Permission Level for {0}").format(appyframe.bold(parent_doctype)), debug=debug
			)
			debug and _debug_log(
...
def is_system_user(user: str | None = None) -> bool:
	return appyframe.get_cached_value("User", user or appyframe.session.user, "user_type") == "System User"
...
def check_doctype_permission(doctype: str, ptype: str = "read") -> None:
	...
	_message_log = appyframe.local.message_log
	appyframe.local.message_log = []
	try:
		appyframe.has_permission(doctype, ptype, throw=True, ignore_share_permissions=True)
	except appyframe.PermissionError:
		appyframe.flags.disable_traceback = True
		raise

	appyframe.local.message_log = _message_log
...
def handle_does_not_exist_error(fn):
	...
	def wrapper(e, *args, **kwargs):
		if isinstance(e, appyframe.DoesNotExistError) and (doctype := getattr(e, "doctype", None)):
			try:
				check_doctype_permission(doctype)
			except appyframe.PermissionError as _e:
