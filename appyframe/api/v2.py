...
from appyframe import _, get_newargs, is_whitelisted
from appyframe.core.doctype.server_script.server_script_utils import get_server_script_map
from appyframe.handler import is_valid_http_method, run_server_script, upload_file
...
def handle_rpc_call(method: str, doctype: str | None = None):
	from appyframe.modules.utils import load_doctype_module
	...
	method = appyframe.override_whitelisted_method(method)
	...
	try:
		...
		appyframe.throw(_("Failed to get method {0} with {1}").format(method, e))
...
def document_list(doctype: str):
	...
	appyframe.form_dict.limit_page_length = appyframe.form_dict.limit or 20
...
def count(doctype: str) -> int:
	from appyframe.desk.reportview import get_count
...
def copy_doc(doctype: str, name: str, ignore_no_copy: bool = True):
	...
	copy = appyframe.copy_doc(doc, ignore_no_copy=ignore_no_copy)
...
def delete_doc(doctype: str, name: str):
	appyframe.client.delete_doc(doctype, name)
	appyframe.response.http_status_code = 202
	return "ok"


def get_meta(doctype: str):
	appyframe.only_for("All")
	return appyframe.get_meta(doctype)
...
def execute_doc_method(doctype: str, name: str, method: str | None = None):
	...
	doc.check_permission(PERMISSION_MAP[appyframe.request.method])
...
def run_doc_method(method: str, document: dict[str, Any] | str, kwargs=None):
	...
	if isinstance(document, str):
		document = appyframe.parse_json(document)

	if kwargs is None:
		kwargs = {}

	doc = appyframe.get_doc(document)
	...
	appyframe.response.docs.append(doc)  # send modified document and result both.
	return response
...
url_rules = [
	...
	Rule("/method/ping", endpoint=appyframe.ping),
	Rule("/method/upload_file", endpoint=upload_file),
	Rule("/method/<method>", endpoint=handle_rpc_call),
	Rule(
		"/method/run_doc_method",
		methods=["GET", "POST"],
		endpoint=lambda: appyframe.call(run_doc_method, **appyframe.form_dict),
	),
