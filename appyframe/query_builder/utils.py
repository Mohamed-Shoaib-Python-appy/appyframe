...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(appyframe.conf.db_type)
...
def get_query(*args, **kwargs) -> QueryBuilder:
	from appyframe.database.query import Engine
...
def DocType(*args, **kwargs):
	return appyframe.qb.DocType(*args, **kwargs)


def Table(*args, **kwargs):
	return appyframe.qb.Table(*args, **kwargs)


def patch_query_execute():
	"""Patch the Query Builder with helper execute method
	This excludes the use of `appyframe.db.sql` method while
	...
	def execute_query(query, *args, **kwargs):
		...
		result = appyframe.db.sql(query, params, *args, **kwargs)  # nosemgrep
	...
	def prepare_query(query):
		...
		if appyframe.flags.in_safe_exec:
			from appyframe.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX, check_safe_sql_query
			...
			if not check_safe_sql_query(query, throw=False):
				...
				if len(callstack) >= 3 and SERVER_SCRIPT_FILE_PREFIX in callstack[2].filename:
					raise appyframe.PermissionError("Only SELECT SQL allowed in scripting")

		return query, param_collector.get_parameters()

	builder_class = appyframe.qb._BuilderClasss
...
def patch_query_aggregation():
	"""Patch aggregation functions to appyframe.qb"""
	from appyframe.query_builder.functions import _avg, _max, _min, _sum

	appyframe.qb.max = _max
	appyframe.qb.min = _min
	appyframe.qb.avg = _avg
	appyframe.qb.sum = _sum
