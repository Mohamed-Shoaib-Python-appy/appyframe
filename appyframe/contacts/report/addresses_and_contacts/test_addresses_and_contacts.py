...
from appyframe.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from appyframe.tests.utils import FrappeTestCase


def get_custom_linked_doctype():
	if bool(appyframe.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if appyframe.flags.test_address_created:
		return

	address = appyframe.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	appyframe.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if appyframe.flags.test_contact_created:
		return
	...
	appyframe.flags.test_contact_created = True
