...
from appyframe.contacts.address_and_contact import set_link_title
from appyframe.core.doctype.access_log.access_log import make_access_log
from appyframe.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from appyframe.model.document import Document
from appyframe.model.naming import append_number_if_name_exists
from appyframe.utils import cstr, has_gravatar
...
class Contact(Document):
	...
	if TYPE_CHECKING:
		from appyframe.contacts.doctype.contact_email.contact_email import ContactEmail
		from appyframe.contacts.doctype.contact_phone.contact_phone import ContactPhone
		from appyframe.core.doctype.dynamic_link.dynamic_link import DynamicLink
	...
	def autoname(self):
		...
		if appyframe.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			appyframe.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = appyframe.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not appyframe.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not appyframe.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			appyframe.throw(_("Only one {0} can be set as primary.").format(appyframe.bold(_("Email ID"))))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			appyframe.throw(
				_("Only one {0} can be set as primary.").format(appyframe.bold(appyframe.unscrub(fieldname)))
			)
...
def download_vcard(contact: str):
	...
	contact = appyframe.get_doc("Contact", contact)
	contact.check_permission()

	vcard = contact.get_vcard()
	make_access_log(doctype="Contact", document=contact.name, file_type="vcf")

	appyframe.response["filename"] = f"{contact.name}.vcf"
	appyframe.response["filecontent"] = vcard.serialize().encode("utf-8")
...
def download_vcards(contacts: str):
	...
	from appyframe.utils.data import now

	contact_ids = appyframe.parse_json(contacts)

	vcards = []
	for contact_id in contact_ids:
		contact = appyframe.get_doc("Contact", contact_id)
		contact.check_permission()
		vcard = contact.get_vcard()
	...
	appyframe.response["filename"] = f"{timestamp} Contacts.vcf"
	appyframe.response["filecontent"] = "\n".join(vcards).encode("utf-8")
	appyframe.response["type"] = "binary"


def get_default_contact(doctype, name):
	"""Returns default contact for the given doctype, name"""
	out = appyframe.db.sql(
		"""select parent,
...
def invite_user(contact: str):
	...
	if not contact.email_id:
		appyframe.throw(_("Please set Email Address"))
...
def update_contact(doc, method):
	...
	contact_name = appyframe.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	from appyframe.desk.reportview import get_match_cond

	doctype = "Contact"
	if not appyframe.get_meta(doctype).get_field(searchfield) and searchfield not in appyframe.db.DEFAULT_COLUMNS:
		return []
...
def get_contact_name(email_id: str) -> str | None:
	...
	for contact_id in appyframe.get_all(
		"Contact Email", filters={"email_id": email_id, "parenttype": "Contact"}, pluck="parent"
	):
		if appyframe.db.exists("Contact", contact_id):
			return contact_id
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = appyframe.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = appyframe.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return appyframe.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
...
def get_contact_display_list(doctype: str, name: str) -> list[dict]:
	from appyframe.contacts.doctype.address.address import get_condensed_address

	if not appyframe.has_permission("Contact", "read"):
		return []

	contact_list = appyframe.get_list(
		"Contact",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
	...
	for contact in contact_list:
		contact["email_ids"] = appyframe.get_all(
			"Contact Email",
			filters={"parenttype": "Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)

		contact["phone_nos"] = appyframe.get_all(
			"Contact Phone",
			filters={
				"parenttype": "Contact",
				"parent": contact.name,
		...
		if contact.address and appyframe.has_permission("Address", "read"):
			address = appyframe.get_doc("Address", contact.address)
