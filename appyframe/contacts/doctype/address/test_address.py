...
import appyframe
from appyframe.contacts.doctype.address.address import address_query, get_address_display
from appyframe.tests.utils import FrappeTestCase


class TestAddress(FrappeTestCase):
	def test_template_works(self):
		if not appyframe.db.exists("Address Template", "India"):
			appyframe.get_doc({"doctype": "Address Template", "country": "India", "is_default": 1}).insert()

		if not appyframe.db.exists("Address", "_Test Address-Office"):
			appyframe.get_doc(
				{
		...
		address = appyframe.get_list("Address")[0].name
		display = get_address_display(appyframe.get_doc("Address", address).as_dict())
