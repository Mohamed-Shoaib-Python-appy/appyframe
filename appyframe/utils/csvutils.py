...
from appyframe.utils import cint, comma_or, cstr, flt


def read_csv_content_from_attached_file(doc):
	fileid = appyframe.get_all(
		"File",
		fields=["name"],
	...
	try:
		_file = appyframe.get_doc("File", fileid)
...
def read_csv_content(fcontent):
	...
	content = [appyframe.safe_decode(line) for line in fcontent.splitlines(True)]
	...
	try:
		...
		appyframe.msgprint(_("Not a valid Comma Separated Value (CSV File)"))
		raise
...
def send_csv_to_client(args):
	...
	appyframe.response["result"] = cstr(to_csv(args.data))
	appyframe.response["doctype"] = args.filename
...
def build_csv_response(data, filename):
	appyframe.response["result"] = cstr(to_csv(data))
	appyframe.response["doctype"] = filename
	appyframe.response["type"] = "csv"
...
def check_record(d):
	...
	from appyframe.utils.dateutils import parse_date
	...
	for key in d:
		...
		if docfield:
			if docfield.reqd and (val == "" or val is None):
				appyframe.msgprint(_("{0} is required").format(docfield.label), raise_exception=1)
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
	...
	if d.get("name") and appyframe.db.exists(doctype, d["name"]):
		if overwrite:
			doc = appyframe.get_doc(doctype, d["name"])
