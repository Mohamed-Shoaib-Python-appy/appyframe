...
def main():
	...
	commands.update({"get-appyframe-commands": get_frappe_commands, "get-appyframe-help": get_frappe_help})
	click.Group(commands=commands)(prog_name="bench")


def get_app_groups() -> dict[str, click.Group]:
	"""Get all app groups, put them in main group "appyframe" since bench is
	designed to only handle that"""
	commands = {}
	for app in get_apps():
		if app_commands := get_app_commands(app):
			commands |= app_commands
	return dict(appyframe=click.group(name="appyframe", commands=commands)(app_group))
...
def app_group(ctx, site=False, force=False, verbose=False, profile=False):
	...
	if ctx.info_name == "appyframe":
		ctx.info_name = ""


def get_sites(site_arg: str) -> list[str]:
	if site_arg == "all":
		return appyframe.utils.get_sites()
	elif site_arg:
		return [site_arg]
	elif os.environ.get("FRAPPE_SITE"):
		return [os.environ.get("FRAPPE_SITE")]
	elif default_site := appyframe.get_conf().default_site:
		return [default_site]
	# This is not supported, just added here for warning.
	elif (site := appyframe.read_file("currentsite.txt")) and site.strip():
		click.secho(
			dedent(
				f"""
...
@click.command("get-appyframe-commands")
def get_frappe_commands():
	commands = list(get_app_commands("appyframe"))
...
@click.command("get-appyframe-help")
def get_frappe_help():
	print(click.Context(get_app_groups()["appyframe"]).get_help())


def get_apps():
	return appyframe.get_all_apps(with_internal_apps=False, sites_path=".")


if __name__ == "__main__":
	if not appyframe._dev_server:
