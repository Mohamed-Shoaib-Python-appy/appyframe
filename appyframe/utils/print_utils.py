...
def get_print(
	...
	from appyframe.website.serve import get_response_without_exception_handling

	local = appyframe.local
	if "pdf_generator" not in local.form_dict:
		# if arg is passed, use that, else get setting from print format
		if pdf_generator is None:
			pdf_generator = (
				appyframe.get_cached_value("Print Format", print_format, "pdf_generator") or "wkhtmltopdf"
			)
	...
	if local.form_dict.pdf_generator != "wkhtmltopdf":
		hook_func = appyframe.get_hooks("pdf_generator")
