...
from appyframe.database.database import Database, EmptyQueryValues


class FrappeIntegration(Integration):
	identifier = "appyframe"
...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = appyframe._dict({"scheduled": False, "wait": 0})
		...
		if appyframe.form_dict.cmd:
			path = f"/api/method/{appyframe.form_dict.cmd}"
		else:
	...
	scope.set_user({"id": appyframe.local.site})
	user = getattr(appyframe.session, "user", "Unidentified")
	scope.set_tag("frappe_user", user)
	# Extract `X-Appyframe-Request-ID` to store as a separate field if its present
	...
	try:
		...
		with hub.configure_scope() as scope:
			...
			if appyframe.request:
				evt_processor = _make_wsgi_event_processor(appyframe.request.environ, False)
				scope.add_event_processor(evt_processor)
				if appyframe.request.is_json:
					scope.set_context("JSON Body", appyframe.request.json)
				elif appyframe.request.form:
					scope.set_context("Form Data", appyframe.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], appyframe.ValidationError | appyframe.PermissionError):
					return
		...
		appyframe.logger().error("Failed to capture exception", exc_info=True)
