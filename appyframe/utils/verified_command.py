...
def get_secret():
	from appyframe.utils.password import get_encryption_key

	return appyframe.local.conf.get("secret") or get_encryption_key()


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = appyframe.safe_decode(
		appyframe.local.flags.signed_query_string or getattr(appyframe.request, "query_string", None)
	)
	...
	if signature_string in query_string:
		...
		valid_method = appyframe.request.method == "GET"
		valid_request_data = not (appyframe.request.form or appyframe.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return appyframe.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(appyframe.utils.cstr(p) for p in params.values())
	if not secret:
		secret = appyframe.local.conf.get("secret") or "secret"
