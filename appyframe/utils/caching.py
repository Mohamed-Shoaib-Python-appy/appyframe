...
def request_cache(func: Callable) -> Callable:
	...
	appyframe.local.request_cache. The cache only persists for the current request
	...
	def wrapper(*args, **kwargs):
		if not getattr(appyframe.local, "initialised", None):
			return func(*args, **kwargs)
		if not hasattr(appyframe.local, "request_cache"):
			appyframe.local.request_cache = defaultdict(dict)
		...
		try:
			return appyframe.local.request_cache[func][args_key]
		except KeyError:
			return_val = func(*args, **kwargs)
			appyframe.local.request_cache[func][args_key] = return_val
			return return_val
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	appyframe.utils.caching._SITE_CACHE. The cache persists on the parent process.
	...
	workers, use redis (appyframe.cache API) instead.
	...
	def time_cache_wrapper(func: Callable | None = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			if getattr(appyframe.local, "initialised", None):
				...
				if hasattr(func, "maxsize") and len(_SITE_CACHE[func_key][appyframe.local.site]) >= func.maxsize:
					_SITE_CACHE[func_key][appyframe.local.site].pop(
						next(iter(_SITE_CACHE[func_key][appyframe.local.site])), None
					)

				if func_call_key not in _SITE_CACHE[func_key][appyframe.local.site]:
					_SITE_CACHE[func_key][appyframe.local.site][func_call_key] = func(*args, **kwargs)

				return _SITE_CACHE[func_key][appyframe.local.site][func_call_key]
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None, shared: bool = False) -> Callable:
	...
	def wrapper(func: Callable | None = None) -> Callable:
		...
		def clear_cache():
			appyframe.cache.delete_keys(func_key)
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			if appyframe.cache.exists(func_call_key, user=user, shared=shared):
				return appyframe.cache.get_value(func_call_key, user=user, shared=shared)
			val = func(*args, **kwargs)
			ttl = getattr(func, "ttl", 3600)
			appyframe.cache.set_value(func_call_key, val, expires_in_sec=ttl, user=user, shared=shared)
			return val
