...
from appyframe.utils import CallbackManager, cint, get_bench_id
from appyframe.utils.commands import log
from appyframe.utils.deprecations import deprecation_warning
from appyframe.utils.redis_queue import RedisQueue
...
def get_queues_timeout():
	common_site_config = appyframe.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	:param now: if now=True, the method is executed via appyframe.call
	...
	if deduplicate:
		if not job_id:
			appyframe.throw(_("`job_id` paramater is required for deduplication."))
		job = get_job(job_id)
		if job and job.get_status() in (JobStatus.QUEUED, JobStatus.STARTED):
			appyframe.logger().error(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not appyframe.flags.in_test:
		deprecation_warning(
			"Using enqueue with is_async=False outside of tests is not recommended, use now=True instead."
		)

	call_directly = now or (not is_async and not appyframe.flags.in_test)
	if call_directly:
		return appyframe.call(method, **kwargs)

	try:
		q = get_queue(queue, is_async=is_async)
	except ConnectionError:
		if appyframe.local.flags.in_migrate:
	...
	def enqueue_call():
		return q.enqueue_call(
			...
			kwargs=queue_args,
			at_front=at_front,
			failure_ttl=appyframe.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
			result_ttl=appyframe.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
			job_id=job_id,
		)

	if enqueue_after_commit:
		appyframe.db.after_commit.add(enqueue_call)
		return
...
def enqueue_doc(doctype, name=None, method=None, queue="default", timeout=300, now=False, **kwargs):
	...
	return enqueue(
		"appyframe.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(appyframe.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	appyframe.local.job = appyframe._dict(
		site=site,
		method=method_name,
		job_name=job_name,
		kwargs=kwargs,
	...
	for before_job_task in appyframe.get_hooks("before_job"):
		appyframe.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (appyframe.db.InternalError, appyframe.RetryBackgroundJobError) as e:
		appyframe.db.rollback()

		if retry < 5 and (
			isinstance(e, appyframe.RetryBackgroundJobError)
			or (appyframe.db.is_deadlocked(e) or appyframe.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			# 1205 = lock wait timeout
			# or RetryBackgroundJobError is explicitly raised
			appyframe.job.after_job.reset()
			appyframe.destroy()
			time.sleep(retry + 1)
			...
			appyframe.log_error(title=method_name)
			raise
		...
		if not hasattr(appyframe.local, "site"):
			appyframe.init(site)
			appyframe.connect()
		for after_job_task in appyframe.get_hooks("after_job"):
			appyframe.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
		appyframe.local.job.after_job.run()
...
def start_worker_pool(
	...
	import appyframe.database.query  # sqlparse and indirect imports
	import appyframe.query_builder  # pypika
	import appyframe.utils  # common utils
...
def validate_queue(queue, default_queue_list=None):
	...
	if queue not in default_queue_list:
		appyframe.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(appyframe.local, "conf"):
		raise Exception("You need to call appyframe.init")

	elif not appyframe.local.conf.redis_queue:
		raise Exception("redis_queue missing in common_site_config.json")

	global _redis_queue_conn

	cred = appyframe._dict()
	if appyframe.conf.get("use_rq_auth"):
		if username:
			cred["username"] = username
			cred["password"] = password
		else:
			cred["username"] = appyframe.get_site_config().rq_username or get_bench_id()
			cred["password"] = appyframe.get_site_config().rq_password
	...
	try:
		...
		log(
			f"Please make sure that Redis Queue runs @ {appyframe.get_conf().redis_queue}. Redis reported error: {e!s}",
			colour="red",
		)
		raise
...
def enqueue_test_job():
	enqueue("appyframe.utils.background_jobs.test_job", s=100)
...
def create_job_id(job_id: str) -> str:
	...
	if not job_id:
		job_id = str(uuid4())
	return f"{appyframe.local.site}::{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from appyframe.utils import create_batch

	conf = appyframe.conf if appyframe.conf else appyframe.get_conf(site=job.kwargs.get("site"))
