...
from appyframe.utils import cstr, scrub_urls
from appyframe.utils.caching import redis_cache
from appyframe.utils.jinja_globals import bundled_asset, is_rtl
...
def pdf_body_html(template, args, **kwargs):
	try:
		...
		appyframe.throw(
			...
			exc=appyframe.PrintFormatError,
...
def get_pdf(html, options=None, output: PdfWriter | None = None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				print(html, options)
				appyframe.throw(_("PDF generation failed because of broken image links"))
...
def prepare_options(html, options):
	...
	# page size
	pdf_page_size = (
		options.get("page-size") or appyframe.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
	)

	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or appyframe.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or appyframe.db.get_single_value(
...
def get_cookie_options():
	options = {}
	if appyframe.session and appyframe.session.sid and hasattr(appyframe.local, "request"):
		# Use wkhtmltopdf's cookie-jar feature to set cookies and restrict them to host domain
		cookiejar = f"/tmp/{appyframe.generate_hash()}.jar"

		# Remove port from request.host
		# https://werkzeug.palletsprojects.com/en/0.16.x/wrappers/#werkzeug.wrappers.BaseRequest.host
		domain = appyframe.utils.get_host_name().split(":", 1)[0]
		with open(cookiejar, "w") as f:
			f.write(f"sid={appyframe.session.sid}; Domain={domain};\n")
...
def _get_base64_image(src):
	...
	try:
		...
		appyframe.logger("pdf").error("Failed to convert inline images to base64", exc_info=True)
...
def prepare_header_footer(soup: BeautifulSoup):
	...
	css = appyframe.read_file(os.path.join(appyframe.local.sites_path, print_css))
	...
	for html_id in ("header-html", "footer-html"):
		if content := soup.find(id=html_id):
			...
			hook_func = appyframe.get_hooks(id_map.get(html_id))
			html = appyframe.call(
				hook_func[-1],
				soup=soup,
				head=head,
			...
			# create temp file
			fname = os.path.join("/tmp", f"appyframe-pdf-{appyframe.generate_hash()}.html")
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/appyframe-pdf-random.html"}
...
def get_wkhtmltopdf_version():
	wkhtmltopdf_version = appyframe.cache.hget("wkhtmltopdf_version", None)

	if not wkhtmltopdf_version:
		try:
			res = subprocess.check_output(["wkhtmltopdf", "--version"])
			wkhtmltopdf_version = res.decode("utf-8").split(" ")[1]
			appyframe.cache.hset("wkhtmltopdf_version", None, wkhtmltopdf_version)
		except Exception:
			pass
