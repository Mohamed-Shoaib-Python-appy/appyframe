...
from appyframe.utils import cint, strip_html_tags
...
def reset():
	...
	:return:
	"""
	appyframe.db.delete("__global_search")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in appyframe.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = appyframe.get_meta(d.name)
		...
		doctypes = [
			...
			if module_app.get(appyframe.scrub(d.module)) and module_app[appyframe.scrub(d.module)] in installed_apps
		]

		return doctypes

	return appyframe.cache.get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if appyframe.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = appyframe._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = appyframe.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", appyframe.model.table_fields], "options": doctype},
		)
	...
	all_records = appyframe.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = appyframe.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": appyframe.db.escape(doctype),
					"name": appyframe.db.escape(doc.name),
					"content": appyframe.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": appyframe.db.escape((title or "")[: int(appyframe.db.VARCHAR_LEN)]),
					"route": appyframe.db.escape((route or "")[: int(appyframe.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def delete_global_search_records_for_doctype(doctype):
	appyframe.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = appyframe._dict()
	child_search_fields = appyframe._dict()

	for child in meta.get_table_fields():
		child_meta = appyframe.get_meta(child.options)
		...
		if search_fields:
			...
			child_records = appyframe.get_all(
				child.options,
			...
			for record in child_records:
				all_children.setdefault(record.parent, appyframe._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	content = [
		...
		if doc.get(field.fieldname) and field.fieldtype not in appyframe.model.table_fields
	]
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(appyframe.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if appyframe.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	...
	routes_to_index = []
	for app in apps:
		base = appyframe.get_app_path(app, "www")
		path_to_index = appyframe.get_app_path(app, "www")
...
def add_route_to_global_search(route):
	...
	from appyframe.website.serve import get_response_content

	appyframe.set_user("Guest")
	appyframe.local.no_cache = True
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(appyframe.safe_decode(value))
...
def sync_global_search():
	...
	def get_search_queue_item_generator():
		while value := appyframe.cache.rpop("global_search_queue"):
			yield value
...
def sync_values(values: list):
	...
	GlobalSearch = appyframe.qb.Table("__global_search")
	conflict_fields = ["content", "published", "title", "route"]

	query = appyframe.qb.into(GlobalSearch).columns(["doctype", "name", *conflict_fields]).insert(*values)
...
def sync_value_in_queue(value):
	try:
		...
		appyframe.cache.lpush("global_search_queue", json.dumps(value))
...
def delete_for_document(doc):
	...
	"""
	appyframe.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	"""
	from appyframe.desk.doctype.global_search_settings.global_search_settings import (
		get_doctypes_for_global_search,
	)
	from appyframe.query_builder.functions import Match

	results = []
	sorted_results = []

	allowed_doctypes = set(get_doctypes_for_global_search()) & set(appyframe.get_user().get_can_read())
	...
	for doctype in allowed_doctypes:
		for r in results:
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = appyframe.get_meta(r.doctype)
					if meta.image_field:
						r.image = appyframe.db.get_value(r.doctype, r.name, meta.image_field)
				except Exception:
...
def web_search(text: str, scope: str | None = None, start: int = 0, limit: int = 20):
	...
	for text in texts:
		...
		mariadb_conditions += "MATCH(`content`) AGAINST ({} IN BOOLEAN MODE)".format(
			appyframe.db.escape("+" + text + "*")
		)
		postgres_conditions += f'TO_TSVECTOR("content") @@ PLAINTO_TSQUERY({appyframe.db.escape(text)})'

		values = {"scope": "".join([scope, "%"]) if scope else "", "limit": limit, "start": start}

		result = appyframe.db.multisql(
			{
