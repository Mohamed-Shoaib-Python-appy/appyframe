...
from appyframe.www.printview import validate_print_permission
...
def download_multi_pdf_async(
	...
	appyframe.local.response["http_status_code"] = http.HTTPStatus.CREATED
...
def _download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		for idx, ss in enumerate(result):
			try:
				pdf_writer = appyframe.get_print(
					doctype,
					ss,
					format,
					as_pdf=True,
				...
				if task_id:
					appyframe.publish_realtime(task_id=task_id, message={"message": "Failed"})
		...
		if task_id is None:
			appyframe.local.response.filename = "{doctype}.pdf".format(
		...
		for doctype_name in doctype:
			...
			for doc_name in doctype[doctype_name]:
				try:
					...
					if task_id:
						appyframe.publish_realtime(task_id=task_id, message="Failed")
					appyframe.log_error(
		...
		if task_id is None:
			appyframe.local.response.filename = f"{name}.pdf"
	...
	with BytesIO() as merged_pdf:
		...
		if task_id:
			...
			_file.save()
			appyframe.publish_realtime(f"task_complete:{task_id}", message={"file_url": _file.unique_url})
		else:
			appyframe.local.response.filecontent = merged_pdf.getvalue()
...
def download_pdf(
	...
	doc = doc or appyframe.get_doc(doctype, name)
	validate_print_permission(doc)

	with print_language(language):
		pdf_file = appyframe.get_print(
			doctype,
			name,
			format,
			doc=doc,
	...
	appyframe.local.response.filecontent = pdf_file
	appyframe.local.response.type = "pdf"
...
def report_to_pdf(html, orientation="Landscape"):
	...
	appyframe.local.response.filename = "report.pdf"
	appyframe.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	appyframe.local.response.type = "pdf"
...
def print_by_server(
	...
	print_settings = appyframe.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
		appyframe.throw(_("You need to install pycups to use this feature!"))
	...
	try:
		...
		output = PdfWriter()
		output = appyframe.get_print(
			doctype, name, print_format, doc=doc, no_letterhead=no_letterhead, as_pdf=True, output=output
		)
		if not file_path:
			file_path = os.path.join("/", "tmp", f"appyframe-pdf-{appyframe.generate_hash()}.pdf")
		...
		if (
			...
			appyframe.throw(_("PDF generation failed"))
	except cups.IPPError:
		appyframe.throw(_("Printing failed"))
