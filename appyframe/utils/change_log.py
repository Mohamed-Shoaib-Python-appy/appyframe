...
import appyframe
from appyframe import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = appyframe._dict(
		json.loads(appyframe.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "appyframe" in current_versions:
		set_in_change_log("appyframe", current_versions["appyframe"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(appyframe.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = appyframe.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "appyframe": {
	                        "title": "Appyframe Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in appyframe.get_installed_apps(_ensure_on_bench=True):
		app_hooks = appyframe.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = appyframe.get_attr(app + ".__version__")
...
def check_for_update():
	if appyframe.get_system_settings("disable_system_update_notification"):
		return

	updates = appyframe._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def has_app_update_notifications() -> bool:
	return bool(appyframe.cache.sismember("changelog-update-user-set", appyframe.session.user))
		...
		for vuln in advisory["vulnerabilities"]:
			with suppress(Exception):
				...
				if (
					...
					# XXX: this is not 100% correct, but works for appyframe
...
def add_message_to_redis(update_json):
	...
	appyframe.cache.set_value("changelog-update-info", json.dumps(update_json))
	user_list = [x.name for x in appyframe.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in appyframe.get_roles(user)]
	appyframe.cache.sadd("changelog-update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = appyframe.cache.get_value("changelog-update-info")
	if not update_info:
		return
	...
	update_message = ""
	if appyframe.cache.sismember("changelog-update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				app = appyframe._dict(app)
				security_msg = ""
				if app.security_issues:
					...
					security_msg = security_msg.format(appyframe.bold(app.security_issues))
	...
	if on_frappecloud():
		primary_action = {
			"label": _("Update from Appyframe Cloud"),
			"client_action": "window.open",
			"args": f"https://frappecloud.com/dashboard/sites/{appyframe.local.site}",
		}
	...
	if update_message:
		...
		appyframe.cache.srem("changelog-update-user-set", user)


def get_pyproject(app: str) -> dict | None:
	from tomli import load

	pyproject_path = appyframe.get_app_path(app, "..", "pyproject.toml")
