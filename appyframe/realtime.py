...
def publish_progress(percent, title=None, doctype=None, docname=None, description=None, task_id=None):
	publish_realtime(
		...
		user=None if doctype and docname else appyframe.session.user,
		doctype=doctype,
		docname=docname,
		task_id=task_id,
	)
...
def publish_realtime(
	...
	if not task_id and hasattr(appyframe.local, "task_id"):
		task_id = appyframe.local.task_id
	...
	if after_commit:
		if not hasattr(appyframe.local, "_realtime_log"):
			appyframe.local._realtime_log = []
			appyframe.db.after_commit.add(flush_realtime_log)
			appyframe.db.after_rollback.add(clear_realtime_log)

		params = [event, message, room]
		if params not in appyframe.local._realtime_log:
			appyframe.local._realtime_log.append(params)
	else:
		emit_via_redis(event, message, room)


def flush_realtime_log():
	for args in appyframe.local._realtime_log:
		appyframe.realtime.emit_via_redis(*args)

	clear_realtime_log()


def clear_realtime_log():
	if hasattr(appyframe.local, "_realtime_log"):
		del appyframe.local._realtime_log
...
def emit_via_redis(event, message, room):
	...
	from appyframe.utils.background_jobs import get_redis_connection_without_auth
	...
	with suppress(redis.exceptions.ConnectionError):
		...
		r.publish(
			"events",
			appyframe.as_json(
				{"event": event, "message": message, "room": room, "namespace": appyframe.local.site}
			),
		)
...
def can_subscribe_doc(doctype: str, docname: str) -> bool:
	appyframe.has_permission(doctype, doc=docname, throw=True)
	return True


@appyframe.whitelist(allow_guest=True)
def can_subscribe_doctype(doctype: str) -> bool:
	from appyframe.exceptions import PermissionError

	if not appyframe.has_permission(doctype=doctype, ptype="read"):
...
@appyframe.whitelist(allow_guest=True)
def get_user_info():
	user_type = appyframe.session.data.user_type
	# For requests with Bearer tokens, user_type is not set in the session data
	if not user_type:
		user_type = appyframe.get_cached_value("User", appyframe.session.user, "user_type")
	return {
		"user": appyframe.session.user,
		"user_type": user_type,
	}
