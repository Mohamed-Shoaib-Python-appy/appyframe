...
Translation tools for appyframe
"""
...
import appyframe
from appyframe.query_builder import DocType, Field
from appyframe.utils import cstr, get_bench_path, is_html, strip, strip_html_tags, unique
...
def get_language(lang_list: list | None = None) -> str:
	"""Set `appyframe.local.lang` from HTTP headers at beginning of request
	...
	"""
	is_logged_in = appyframe.session.user != "Guest"

	# fetch language from form_dict
	if appyframe.form_dict._lang:
		language = get_lang_code(appyframe.form_dict._lang or get_parent_language(appyframe.form_dict._lang))
		if language:
			return language

	# use language set in User or System Settings if user is logged in
	if is_logged_in:
		return appyframe.local.lang
	...
	accept_language = list(appyframe.request.accept_languages.values())
	...
	return appyframe.db.get_default("lang") or "en"
...
def get_user_lang(user: str | None = None) -> str:
	"""Set appyframe.local.lang from user preferences on session beginning or resumption"""
	user = user or appyframe.session.user

	# User.language => Session Defaults => appyframe.local.lang => 'en'
	return (
		appyframe.get_cached_value("User", user, "language")
		or appyframe.db.get_default("lang")
		or appyframe.local.lang
		or "en"
	)


def get_lang_code(lang: str) -> str | None:
	return appyframe.db.get_value("Language", {"name": lang}) or appyframe.db.get_value(
...
def set_default_language(lang):
	...
	if appyframe.db.get_default("lang") != lang:
		appyframe.db.set_default("lang", lang)
	appyframe.local.lang = lang


def get_lang_dict():
	"""Return all languages in dict format, full name is the key e.g. `{"english":"en"}`."""
	return dict(
		appyframe.get_all("Language", fields=["language_name", "name"], order_by="creation", as_list=True)
	)
...
def get_messages_for_boot():
	...
	return get_all_translations(appyframe.local.lang)


@appyframe.whitelist(allow_guest=True)
def get_app_translations():
	if appyframe.session.user != "Guest":
		language = appyframe.db.get_value("User", appyframe.session.user, "language")
	else:
		language = appyframe.db.get_single_value("System Settings", "language")
...
def get_all_translations(lang: str) -> dict[str, str]:
	...
	def _merge_translations():
		from appyframe.geo.country_info import get_translated_countries
	...
	try:
		return appyframe.cache.hget(MERGED_TRANSLATION_KEY, lang, generator=_merge_translations)
	except Exception:
		if appyframe.flags and appyframe.flags.in_test:
			raise
		...
		appyframe.logger().error("Unable to load translations", exc_info=True)
		return {}
...
def get_translations_from_apps(lang, apps=None):
	...
	translations = {}
	from appyframe.gettext.translate import get_translations_from_mo

	for app in apps or appyframe.get_installed_apps(_ensure_on_bench=True):
...
def get_translations_from_csv(lang, app):
	return get_translation_dict_from_file(
		os.path.join(appyframe.get_app_path(app, "translations"), lang + ".csv"), lang, app
	)
	...
	if os.path.exists(path):
		...
		for item in csv_content:
			if len(item) == 3 and item[2]:
				...
				appyframe.log_error(message=msg, title="Error in translation file")
				if throw:
					appyframe.throw(msg, title="Error in translation file")
...
def get_user_translations(lang):
	def _read_from_db():
		user_translations = {}
		translations = appyframe.get_all(
	...
	return appyframe.cache.hget(USER_TRANSLATION_KEY, lang, generator=_read_from_db)


def clear_cache():
	"""Clear all translation assets from :meth:`appyframe.cache`"""
	appyframe.cache.delete_value(
...
def get_messages_for_app(app, deduplicate=True):
	...
	messages = []
	modules = [appyframe.unscrub(m) for m in appyframe.local.app_modules[app]]
	...
	if modules:
		if isinstance(modules, str):
			modules = [modules]
		filtered_doctypes = (
			appyframe.qb.from_("DocType").where(Field("module").isin(modules)).select("name").run(pluck=True)
		)
		...
		# pages
		filtered_pages = (
			appyframe.qb.from_("Page").where(Field("module").isin(modules)).select("name", "title").run()
		)
...
def get_messages_from_navbar():
	...
	labels = appyframe.get_all("Navbar Item", filters={"item_label": ("is", "set")}, pluck="item_label")
...
def get_messages_from_doctype(name):
	...
	from appyframe.gettext.extractors.utils import is_translatable

	messages = []
	meta = appyframe.get_meta(name)
	...
	if not meta.custom:
		doctype_file_path = appyframe.get_module_path(meta.module, "doctype", meta.name, meta.name)
...
def get_messages_from_workflow(doctype=None, app_name=None):
	...
	workflows = []
	if doctype:
		workflows = appyframe.get_all("Workflow", filters={"document_type": doctype})
	else:
		fixtures = appyframe.get_hooks("fixtures", app_name=app_name) or []
		for fixture in fixtures:
			if isinstance(fixture, str) and fixture == "Worflow":
				workflows = appyframe.get_all("Workflow")
				break
			elif isinstance(fixture, dict) and fixture.get("dt", fixture.get("doctype")) == "Workflow":
				workflows.extend(appyframe.get_all("Workflow", filters=fixture.get("filters")))

	messages = []
	document_state = DocType("Workflow Document State")
	for w in workflows:
		states = appyframe.db.get_values(
			document_state,
		...
		actions = appyframe.db.get_values(
			"Workflow Transition",
...
def get_messages_from_custom_fields(app_name):
	...
	for fixture in fixtures:
		if isinstance(fixture, str) and fixture == "Custom Field":
			custom_fields = appyframe.get_all(
...
def get_messages_from_page(name):
	"""Return all translatable strings from a :class:`appyframe.core.doctype.Page`."""
	return _get_messages_from_page_or_report("Page", name)


def get_messages_from_report(name):
	"""Return all translatable strings from a :class:`appyframe.core.doctype.Report`."""
	from appyframe.gettext.extractors.utils import is_translatable

	report = appyframe.get_doc("Report", name)
	messages = _get_messages_from_page_or_report(
		"Report", name, appyframe.db.get_value("DocType", report.ref_doctype, "module")
	)
...
def _get_messages_from_page_or_report(doctype, name, module=None):
	if not module:
		module = appyframe.db.get_value(doctype, name, "module")

	doc_path = appyframe.get_module_path(module, doctype, name)

	messages = get_messages_from_file(os.path.join(doc_path, appyframe.scrub(name) + ".py"))
...
def get_server_messages(app):
	"""Extracts all translatable strings (tagged with :func:`appyframe._`) from Python modules
	inside an app"""
	messages = []
	file_extensions = (".py", ".html", ".js", ".vue")
	app_walk = os.walk(appyframe.get_app_path(app))
...
def get_messages_from_include_files(app_name=None):
	...
	from appyframe.utils.jinja_globals import bundled_asset

	messages = []
	app_include_js = appyframe.get_hooks("app_include_js", app_name=app_name) or []
	web_include_js = appyframe.get_hooks("web_include_js", app_name=app_name) or []
	include_js = app_include_js + web_include_js

	for js_path in include_js:
		file_path = bundled_asset(js_path)
		relative_path = os.path.join(appyframe.local.sites_path, file_path.lstrip("/"))
...
def get_all_messages_from_js_files(app_name=None):
	...
	messages = []
	for app in [app_name] if app_name else appyframe.get_installed_apps(_ensure_on_bench=True):
		if os.path.exists(appyframe.get_app_path(app, "public")):
			for basepath, folders, files in os.walk(appyframe.get_app_path(app, "public")):  # noqa: B007
				if "appyframe/public/js/lib" in basepath:
					continue
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	from appyframe.gettext.extractors.utils import extract_messages_from_code

	appyframe.flags.setdefault("scanned_files", set())
	# TODO: Find better alternative
	# To avoid duplicate scan
	if path in appyframe.flags.scanned_files:
		return []

	appyframe.flags.scanned_files.add(path)
...
def extract_messages_from_javascript_code(code: str) -> list[tuple[int, str, str | None]]:
	...
	messages = []
	from appyframe.gettext.extractors.javascript import extract_javascript
...
def get_untranslated(lang, untranslated_file, get_all=False, app="_ALL_APPS"):
	...
	clear_cache()
	apps = appyframe.get_all_apps(True)
	if app != "_ALL_APPS":
		if app not in apps:
...
def update_translations(lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	for key, value in zip(
		appyframe.get_file_items(untranslated_file, ignore_empty_lines=False),
		appyframe.get_file_items(translated_file, ignore_empty_lines=False),
		strict=False,
	):
...
def import_translations(lang, path):
	...
	for app in appyframe.get_all_apps(True):
...
def migrate_translations(source_app, target_app):
	...
	strings_in_source_app = [m[1] for m in appyframe.translate.get_messages_for_app(source_app)]
	strings_in_target_app = [m[1] for m in appyframe.translate.get_messages_for_app(target_app)]

	strings_in_target_app_but_not_in_source_app = list(
		set(strings_in_target_app) - set(strings_in_source_app)
	)

	languages = appyframe.translate.get_all_languages()

	source_app_translations_dir = appyframe.get_app_path(source_app, "translations")
	target_app_translations_dir = appyframe.get_app_path(target_app, "translations")
...
def rebuild_all_translation_files():
	...
	for lang in get_all_languages():
		for app in appyframe.get_all_apps():
...
def write_translations_file(app, lang, full_dict=None, app_messages=None):
	...
	tpath = appyframe.get_app_path(app, "translations")
	appyframe.create_folder(tpath)
	write_csv_file(os.path.join(tpath, lang + ".csv"), app_messages, full_dict or get_all_translations(lang))


def send_translations(translation_dict):
	"""Append translated dict in `appyframe.local.response`"""
	if "__messages" not in appyframe.local.response:
		appyframe.local.response["__messages"] = {}

	appyframe.local.response["__messages"].update(translation_dict)
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = appyframe.db.get_values(
...
def get_all_languages(with_language_name: bool = False) -> list:
	...
	def get_language_codes():
		return appyframe.get_all("Language", filters={"enabled": 1}, pluck="name")

	def get_all_language_with_name():
		return appyframe.get_all("Language", ["language_code", "language_name"], {"enabled": 1})

	if with_language_name:
		return appyframe.cache.get_value("languages_with_name", get_all_language_with_name)
	else:
		return appyframe.cache.get_value("languages", get_language_codes)


def get_preferred_language_cookie():
	return appyframe.request.cookies.get("preferred_language")


def get_translated_doctypes():
	dts = appyframe.get_all("DocType", {"translated_doctype": 1}, pluck="name")
...
def print_language(language: str):
	...
	with print_language("de"):
	    html = appyframe.get_print(...)
	```
	"""
	if not language or language == appyframe.local.lang:
		# do nothing
		yield
		return

	# remember original values
	_lang = appyframe.local.lang
	_jenv = appyframe.local.jenv

	# set language, empty any existing lang_full_dict and jenv
	appyframe.local.lang = language
	appyframe.local.jenv = None

	yield

	# restore original values
	appyframe.local.lang = _lang
	appyframe.local.jenv = _jenv
