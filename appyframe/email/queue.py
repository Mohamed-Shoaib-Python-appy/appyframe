...
from appyframe.utils import cint, cstr, get_url, now_datetime
from appyframe.utils.data import getdate
...
def get_emails_sent_this_month(email_account=None):
	...
	return appyframe.db.count("Email Queue", filters=filters)
...
def get_emails_sent_today(email_account=None):
	...
	return appyframe.db.sql(q, q_args)[0][0]


def get_unsubscribe_message(unsubscribe_message: str, expose_recipients: str) -> "appyframe._dict[str, str]":
	...
	return appyframe._dict(html=html, text=text)
...
def unsubscribe(doctype, name, email):
	...
	if not appyframe.flags.in_test and not verify_request():
		return
...
def flush():
	...
	if appyframe.are_emails_muted():
		msgprint(_("Emails are muted"))

	if cint(appyframe.db.get_default("suspend_email_queue")) == 1:
		return
	...
	for row in email_queue_batch:
		try:
			email_queue: EmailQueue = appyframe.get_doc("Email Queue", row.name)
			email_queue.send()
		except Exception:
			appyframe.get_doc("Email Queue", row.name).log_error()
			...
			if (
				...
				appyframe.throw(_("Email Queue flushing aborted due to too many failures."))


def get_queue():
	batch_size = cint(appyframe.conf.email_queue_batch_size) or 500
