...
class EmailDomain(Document):
	...
	def validate(self):
		...
		if appyframe.local.flags.in_patch or appyframe.local.flags.in_test or appyframe.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in appyframe.get_all("Email Account", filters={"domain": self.name}):
			try:
