...
import appyframe
from appyframe.email.doctype.newsletter.exceptions import (
	NewsletterAlreadySentError,
	NoRecipientFoundError,
)
from appyframe.email.doctype.newsletter.newsletter import (
	Newsletter,
...
from appyframe.utils import add_days, getdate
...
class TestNewsletterMixin:
	...
	def tearDown(self):
		...
		for newsletter in newsletters:
			...
			appyframe.delete_doc("Newsletter", newsletter)
			appyframe.db.delete("Newsletter Email Group", {"parent": newsletter})
			newsletters.remove(newsletter)

	def setup_email_group(self):
		if not appyframe.db.exists("Email Group", "_Test Email Group"):
			appyframe.get_doc({"doctype": "Email Group", "title": "_Test Email Group"}).insert()
		...
		for email in emails:
			...
			try:
				...
				appyframe.db.set_value(doctype, email_filters, "unsubscribed", 0)
	...
	def send_newsletter(self, published=0, schedule_send=None) -> str | None:
		...
		appyframe.db.delete("Newsletter")
	...
	def get_newsletter(**kwargs) -> "Newsletter":
		...
		similar_newsletters = appyframe.get_all(doctype, newsletter_content, pluck="name")

		for similar_newsletter in similar_newsletters:
			appyframe.delete_doc(doctype, similar_newsletter)

		newsletter = appyframe.get_doc({"doctype": doctype, **newsletter_content, **kwargs})
		...
		attached_files = appyframe.get_all(
			"File",
			{
		...
		for file in attached_files:
			appyframe.delete_doc("File", file)
...
class TestNewsletter(TestNewsletterMixin, FrappeTestCase):
	def test_send(self):
		...
		email_queue_list = [appyframe.get_doc("Email Queue", e.name) for e in appyframe.get_all("Email Queue")]
	...
	def test_unsubscribe(self):
		...
		group = appyframe.get_all("Newsletter Email Group", filters={"parent": name}, fields=["email_group"])
	...
	def test_send_scheduled_email_error_handling(self):
		...
		job_path = "appyframe.email.doctype.newsletter.newsletter.Newsletter.queue_all"
		m = MagicMock(side_effect=appyframe.OutgoingEmailError)

		with self.assertRaises(appyframe.OutgoingEmailError):
	...
	def test_retry_partially_sent_newsletter(self):
		...
		# retry
		newsletter.send_emails()
		self.assertEqual(appyframe.db.count("Email Queue"), 4)
