...
from appyframe.email.doctype.email_group.email_group import add_subscribers
from appyframe.rate_limiter import rate_limit
from appyframe.utils.safe_exec import is_job_queued
...
class Newsletter(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		from appyframe.email.doctype.newsletter_attachment.newsletter_attachment import NewsletterAttachment
		from appyframe.email.doctype.newsletter_email_group.newsletter_email_group import (
	...
	def get_sending_status(self):
		count_by_status = appyframe.get_all(
			"Email Queue",
		...
		emails_queued = is_job_queued(
			job_name=appyframe.utils.get_job_name("send_bulk_emails_for", self.doctype, self.name),
			queue="long",
		)
	...
	def send_test_email(self, email):
		test_emails = appyframe.utils.validate_email_address(email, throw=True)
		self.send_newsletter(emails=test_emails, test_email=True)
		appyframe.msgprint(_("Test email sent to {0}").format(email), alert=True)
	...
	def validate_newsletter_status(self):
		if self.email_sent:
			appyframe.throw(_("Newsletter has already been sent"), exc=NewsletterAlreadySentError)

		if self.get("__islocal"):
			appyframe.throw(_("Please save the Newsletter before sending"), exc=NewsletterNotSavedError)

	def validate_newsletter_recipients(self):
		if not self.newsletter_recipients:
			appyframe.throw(_("Newsletter should have atleast one recipient"), exc=NoRecipientFoundError)

	def validate_sender_address(self):
		"""Validate self.send_from is a valid email address or not."""
		if self.sender_email:
			appyframe.utils.validate_email_address(self.sender_email, throw=True)
			self.send_from = (
	...
	def validate_publishing(self):
		if self.send_webview_link and not self.published:
			appyframe.throw(_("Newsletter must be published to send webview link in email"))

	def validate_scheduling_date(self):
		if getattr(appyframe.flags, "is_scheduler_running", False):
			return

		if (
			self.schedule_sending
			and appyframe.utils.get_datetime(self.schedule_send) < appyframe.utils.now_datetime()
		):
			appyframe.throw(_("Past dates are not allowed for Scheduling."))
	...
	def queue_all(self):
		...
		self.email_sent_at = appyframe.utils.now()
	...
	def send_newsletter(self, emails: list[str], test_email: bool = False):
		...
		sender = self.send_from or appyframe.utils.get_formatted_email(self.owner)
		args = self.as_dict()
		args["message"] = self.get_message(medium="email")

		is_auto_commit_set = bool(appyframe.db.auto_commit_on_many_writes)
		appyframe.db.auto_commit_on_many_writes = not appyframe.flags.in_test
		...
		appyframe.sendmail(
			...
			if test_email
			else "/api/method/appyframe.email.doctype.newsletter.newsletter.newsletter_email_read",
		)

		appyframe.db.auto_commit_on_many_writes = is_auto_commit_set

	def get_message(self, medium=None) -> str:
		message = self.message
		if self.content_type == "Markdown":
			message = appyframe.utils.md_to_html(self.message_md)
		if self.content_type == "HTML":
			message = self.message_html

		html = appyframe.render_template(message, {"doc": self.as_dict()})
	...
	def add_source(self, html: str, medium="None") -> str:
		...
		for link in links:
			...
			if href and not href.startswith("#"):
				if not appyframe.utils.is_site_link(href):
					continue
				new_href = appyframe.utils.add_trackers_to_url(
	...
	def get_email_groups(self) -> list[str]:
		...
		return [x.email_group for x in self.email_group] or appyframe.get_all(
...
def confirmed_unsubscribe(email, group):
	...
	doc = appyframe.get_doc("Email Group Member", {"email": email, "email_group": group})
...
def subscribe(email, email_group=None):
	...
	api_endpoint = appyframe.utils.get_url(
		"/api/method/appyframe.email.doctype.newsletter.newsletter.confirm_subscription"
	)
	...
	email_confirmation_template = appyframe.db.get_value(
	...
	if email_confirmation_template:
		...
		email_template = appyframe.get_doc("Email Template", email_confirmation_template)
		email_subject = email_template.subject
		content = appyframe.render_template(email_template.response, args)
	else:
...
def confirm_subscription(email, email_group=None):
	...
	try:
		group = appyframe.get_doc("Email Group", email_group)
	except appyframe.DoesNotExistError:
		group = appyframe.get_doc({"doctype": "Email Group", "title": email_group}).insert(
	...
	if welcome_url:
		...
		appyframe.local.response["location"] = welcome_url
	else:
...
def send_scheduled_email():
	...
	appyframe.flags.is_scheduler_running = True

	scheduled_newsletter = appyframe.get_all(
		"Newsletter",
		filters={
			"schedule_send": ("<=", appyframe.utils.now_datetime()),
			"email_sent": False,
	...
	for newsletter_name in scheduled_newsletter:
		try:
			newsletter = appyframe.get_doc("Newsletter", newsletter_name)
			...
			appyframe.db.set_value("Newsletter", newsletter_name, "email_sent", 0)
			newsletter.log_error("Failed to send newsletter")

		if not appyframe.flags.in_test:
			appyframe.db.commit()

	appyframe.flags.is_scheduler_running = False
...
def newsletter_email_read(recipient_email=None, reference_doctype=None, reference_name=None):
	...
	verify_request()
	try:
		doc = appyframe.get_cached_doc("Newsletter", reference_name)
		if doc.add_viewed(recipient_email, force=True, unique_views=True):
			newsletter = appyframe.qb.DocType("Newsletter")
			(
				appyframe.qb.update(newsletter)
		...
		appyframe.response.update(appyframe.utils.get_imaginary_pixel_response())


def get_default_email_group():
	return _("Website", lang=appyframe.db.get_default("language"))
