...
from appyframe.utils import parse_addr, validate_email_address
...
class EmailGroup(Document):
	...
	def onload(self):
		singles = [d.name for d in appyframe.get_all("DocType", "name", {"issingle": 1})]
		self.get("__onload").import_types = [
			{"value": d.parent, "label": f"{d.parent} ({d.label})"}
			for d in appyframe.get_all("DocField", ("parent", "label"), {"options": "Email"})
	...
	def import_from(self, doctype):
		...
		for user in appyframe.get_all(doctype, [email_field, unsubscribed_field or "name"]):
			with contextlib.suppress(appyframe.UniqueValidationError, appyframe.InvalidEmailAddressError):
		...
		appyframe.msgprint(_("{0} subscribers added").format(added))
	...
	def on_trash(self):
		for d in appyframe.get_all("Email Group Member", "name", {"email_group": self.name}):
			appyframe.delete_doc("Email Group Member", d.name)


@appyframe.whitelist()
def import_from(name, doctype):
	nlist = appyframe.get_doc("Email Group", name)
...
def add_subscribers(name, email_list):
	...
	template = appyframe.db.get_value("Email Group", name, "welcome_email_template")
	welcome_email = appyframe.get_doc("Email Template", template) if template else None
	...
	for email in email_list:
		...
		if parsed_email:
			if not appyframe.db.get_value("Email Group Member", {"email_group": name, "email": parsed_email}):
				appyframe.get_doc(
					{"doctype": "Email Group Member", "email_group": name, "email": parsed_email}
				).insert(ignore_permissions=appyframe.flags.ignore_permissions)
			...
			appyframe.msgprint(_("{0} is not a valid Email Address").format(email))

	appyframe.msgprint(_("{0} subscribers added").format(count))

	return appyframe.get_doc("Email Group", name).update_total_subscribers()
...
def send_welcome_email(welcome_email, email, email_group):
	...
	message = appyframe.render_template(welcome_email.response_, args)
	appyframe.sendmail(email, subject=welcome_email.subject, message=message)
