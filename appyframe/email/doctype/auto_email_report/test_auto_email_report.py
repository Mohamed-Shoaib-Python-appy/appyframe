...
from appyframe.utils import add_to_date, get_link_to_form, today
from appyframe.utils.data import is_html

# test_records = appyframe.get_test_records('Auto Email Report')


class TestAutoEmailReport(FrappeTestCase):
	def test_auto_email(self):
		appyframe.delete_doc("Auto Email Report", "Permitted Documents For User")
...
def get_auto_email_report():
	if not appyframe.db.exists("Auto Email Report", "Permitted Documents For User"):
		auto_email_report = appyframe.get_doc(
			dict(
		...
		auto_email_report = appyframe.get_doc("Auto Email Report", "Permitted Documents For User")
