...
function oauth_access(frm) {
	appyframe.model.with_doc("Connected App", frm.doc.connected_app, () => {
		const connected_app = appyframe.get_doc("Connected App", frm.doc.connected_app);
...
function set_default_max_attachment_size(frm) {
	if (frm.doc.__islocal && !frm.doc["attachment_limit"]) {
		appyframe.call({
			method: "appyframe.core.api.file.get_max_file_size",
			callback: function (r) {
				if (!r.exc) {
...
appyframe.ui.form.on("Email Account", {
	service: function (frm) {
		$.each(appyframe.email_defaults[frm.doc.service], function (key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(appyframe.email_defaults_pop[frm.doc.service], function (key, value) {
				frm.set_value(key, value);
			});
		}
	},
	...
	onload: function (frm) {
		...
		frm.set_query(
			"append_to",
			"appyframe.email.doctype.email_account.email_account.get_append_to"
		);
		frm.set_query("append_to", "imap_folder", function () {
			return {
				query: "appyframe.email.doctype.email_account.email_account.get_append_to",
			};
		});
	...
	refresh: function (frm) {
		...
		if (appyframe.route_flags.delete_user_from_locals && appyframe.route_flags.linked_user) {
			delete appyframe.route_flags.delete_user_from_locals;
			delete locals["User"][appyframe.route_flags.linked_user];
		}
	},
	...
	show_oauth_authorization_message(frm) {
		if (
			...
			appyframe.call({
				method: "appyframe.integrations.doctype.connected_app.connected_app.has_token",
				args: {
	...
	domain: appyframe.utils.debounce((frm) => {
		if (frm.doc.domain) {
			appyframe.call({
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			...
			appyframe.confirm(msg, null, function () {
	...
	warn_autoreply_on_incoming: function (frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			appyframe.confirm(msg, null, function () {
				...
				appyframe.show_alert({ message: __("Disabled Auto Reply"), indicator: "blue" });
			});
		}
	},
});
