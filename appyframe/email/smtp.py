...
from appyframe.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(appyframe.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			appyframe.msgprint(
				...
				raise_exception=appyframe.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				appyframe.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=appyframe.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					appyframe.msgprint(res[1], raise_exception=appyframe.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if appyframe.request and hasattr(appyframe.request, "after_response"):
			appyframe.request.after_response.add(self.quit)
		elif appyframe.job:
			appyframe.job.after_job.add(self.quit)
		elif not appyframe.flags.in_test:
			# Console?
			import atexit
