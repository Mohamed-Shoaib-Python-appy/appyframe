...
import appyframe
from appyframe import _, safe_decode, safe_encode
from appyframe.core.doctype.file.exceptions import MaxFileSizeReachedError
from appyframe.core.doctype.file.utils import get_random_filename
from appyframe.email.oauth import Oauth
...
from appyframe.utils.user import is_system_user
...
class EmailSizeExceededError(appyframe.ValidationError):
	pass


class LoginLimitExceeded(appyframe.ValidationError):
	pass
...
class EmailServer:
	...
	def __init__(self, args=None):
		self.settings = args or appyframe._dict()
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = imaplib.IMAP4_SSL(
					...
					timeout=appyframe.conf.pop_timeout,
					ssl_context=ssl.create_default_context(),
				)
			else:
				self.imap = imaplib.IMAP4(
					self.settings.host, self.settings.incoming_port, timeout=appyframe.conf.pop_timeout
				)
			...
			appyframe.msgprint(_("Invalid Mail Server. Please rectify and try again."))
			raise
	...
	def connect_pop(self):
		...
		try:
			...
			appyframe.log_error("POP: Unable to connect")
			...
			if self.is_temporary_system_problem(e):
				...
				appyframe.msgprint(_("Invalid User Name or Support Password. Please rectify and try again."))
				raise
	...
	def check_imap_uidvalidity(self, folder):
		...
		appyframe.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)
		...
		if not uid_validity or uid_validity != current_uid_validity:
			...
			appyframe.qb.update(Communication).set(Communication.uid, -1).where(
			...
			if self.settings.use_imap:
				...
				IMAPFolder = appyframe.qb.DocType("IMAP Folder")
				appyframe.qb.update(IMAPFolder).set(IMAPFolder.uidvalidity, current_uid_validity).set(
					IMAPFolder.uidnext, uidnext
				).where(IMAPFolder.parent == self.settings.email_account_name).where(
					IMAPFolder.folder_name == folder
				).run()
			else:
				EmailAccount = appyframe.qb.DocType("Email Account")
				appyframe.qb.update(EmailAccount).set(EmailAccount.uidvalidity, current_uid_validity).set(
	...
	def retrieve_message(self, uid, msg_num):
		try:
			...
			appyframe.log_error("Unable to fetch email", self.make_error_msg(uid, msg_num))
	...
	def get_email_seen_status(self, uid, flag_string):
		...
		for flag in imaplib.ParseFlags(flag_string) or []:
			match = WORDS_PATTERN.search(appyframe.as_unicode(flag))
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not appyframe.db.get_value("Email Account", {"email_id": _reply_to}, "email_id"):
	...
	def decode_email(email):
		if not email:
			return
		decoded = ""
		for part, encoding in decode_header(appyframe.as_unicode(email).replace('"', " ").replace("'", " ")):
			if encoding:
	...
	def save_attachments_in_doc(self, doc):
		...
		for attachment in self.attachments:
			try:
				...
				pass
			except appyframe.FileAlreadyAttachedException:
				pass
...
class InboundMail(Email):
	...
	def _build_communication_doc(self):
		...
		communication = appyframe.get_doc(data)
	...
	def is_exist_in_system(self):
		...
		from appyframe.core.doctype.communication.communication import Communication
	...
	def is_reply_to_system_sent_mail(self):
		...
		return self.is_reply() and appyframe.local.site in self.in_reply_to
	...
	def match_record_by_subject_and_sender(self, doctype):
		...
		3. Find by matching subject alone (Special case)
		        ...
		        the system user is replying to via the common email account in Appyframe. This fix bypasses
		...
		if not record:
			...
			name = appyframe.db.get_value(self.email_account.append_to, filters=filters)
			record = self.get_doc(doctype, name, ignore_error=True) if name else None
		return record

	def _create_reference_document(self, doctype):
		"""Create reference document if it does not exist in the system."""
		parent = appyframe.new_doc(doctype)
		email_fields = self.get_email_fields(doctype)

		if email_fields.subject_field:
			parent.set(email_fields.subject_field, appyframe.as_unicode(self.subject)[:140])

		if email_fields.sender_field:
			parent.set(email_fields.sender_field, appyframe.as_unicode(self.from_email))

		if email_fields.sender_name_field:
			parent.set(email_fields.sender_name_field, appyframe.as_unicode(self.from_real_name))
		...
		try:
			...
			return appyframe.db.get_value(doctype, {email_fields.sender_field: self.from_email})

	@staticmethod
	def get_doc(doctype, docname, ignore_error=False):
		try:
			return appyframe.get_doc(doctype, docname)
	...
	def get_users_linked_to_account(email_account):
		...
		users = appyframe.get_all("User Email", filters={"email_account": email_account.name}, fields=["parent"])
	...
	def clean_subject(subject):
		...
		return appyframe.as_unicode(strip(re.sub(regex, "", subject, count=0, flags=re.IGNORECASE)))

	@staticmethod
	def get_email_fields(doctype):
		"""Returns Email related fields of a doctype."""
		fields = appyframe._dict()
	...
	def get_document(self, doctype, name):
		"""Is same as appyframe.get_doc but suppresses the DoesNotExist error."""
		try:
			return appyframe.get_doc(doctype, name)
		except appyframe.DoesNotExistError:
			return None
