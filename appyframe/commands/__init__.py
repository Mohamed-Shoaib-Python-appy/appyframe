...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			ret = f(appyframe._dict(ctx.obj), *args, **kwargs)
		except appyframe.exceptions.SiteNotSpecifiedError as e:
			click.secho(str(e), fg="yellow")
			sys.exit(1)
		except appyframe.exceptions.IncorrectSitePath:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise appyframe.SiteNotSpecifiedError
		return None
