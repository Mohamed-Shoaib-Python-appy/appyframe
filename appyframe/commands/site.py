...
from appyframe.utils import CallbackManager
...
def new_site(
	...
	"Create a new site"
	from appyframe.installer import _new_site

	appyframe.init(site=site, new_site=True)

	if site in appyframe.get_all_apps():
		click.secho(
...
def _restore(
	*,
	...
	from appyframe.installer import extract_files
	from appyframe.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key

	err, out = appyframe.utils.execute_in_shell(f"file {sql_file_path}", check_exit_code=True)
	if err:
...
def restore_backup(
	...
	from appyframe.installer import _new_site, is_downgrade, is_partial, validate_database_sql
	...
	if is_partial(sql_file_path):
		click.secho(
			"Partial Backup file detected. You cannot use a partial file to restore a Appyframe site.",
			fg="red",
		)
		click.secho(
	...
	# Check if the backup is of an older version of appyframe and the user hasn't specified force
...
def partial_restore(context, sql_file_path, verbose, encryption_key=None):
	from appyframe.installer import is_partial, partial_restore
	from appyframe.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
	...
	if "cipher" in out.decode().split(":")[-1].strip():
		...
		with decrypt_backup(sql_file_path, key):
			if not is_partial(sql_file_path):
				click.secho(
					"Full backup file detected. Use `bench restore` to restore a Appyframe Site.",
					fg="red",
				)
				sys.exit(1)
...
def _reinstall(
	...
	try:
		...
		installed = appyframe.get_installed_apps()
		appyframe.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context, apps, force=False):
	...
	from appyframe.installer import install_app as _install_app
	from appyframe.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except appyframe.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {err!s}\n{appyframe.get_traceback(with_context=True)}"
...
def list_apps(context, format):
	...
	for site in context.sites:
		...
		apps = appyframe.get_single("Installed Applications").installed_applications
		if apps:
			installed_apps_info.extend(format_app(app) for app in apps)
		else:
			installed_apps_info.extend(appyframe.get_installed_apps())
	...
	if format == "json":
		click.echo(appyframe.as_json(summary_dict))
...
def add_db_index(context, doctype, column):
	...
	for site in context.sites:
		...
		appyframe.connect()
		try:
			appyframe.db.add_index(doctype, columns)
			appyframe.db.commit()
		finally:
			appyframe.destroy()
...
def describe_database_table(context, doctype, column):
	...
	from appyframe.core.doctype.recorder.recorder import _fetch_table_stats
...
def add_system_manager(context, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		...
		appyframe.connect()
		try:
			appyframe.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			appyframe.db.commit()
		finally:
			appyframe.destroy()
	if not context.sites:
...
def disable_user(context, email):
	...
	with appyframe.init_site(site):
		appyframe.connect()
		user = appyframe.get_doc("User", email)
		user.enabled = 0
...
def migrate(context, skip_failing=False, skip_search_index=False):
	...
	from appyframe.migrate import SiteMigration
...
def migrate_to():
	...
	from appyframe.integrations.frappe_providers import migrate_to
...
def run_patch(context, module, force):
	...
	for site in context.sites:
		...
		try:
			appyframe.connect()
			appyframe.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			appyframe.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context, module, doctype, docname):
	...
	for site in context.sites:
		try:
			appyframe.init(site=site)
			appyframe.connect()
			appyframe.reload_doc(module, doctype, docname, force=context.force)
			appyframe.db.commit()
		finally:
			appyframe.destroy()
	if not context.sites:
...
def reload_doctype(context, doctype):
	...
	for site in context.sites:
		try:
			appyframe.init(site=site)
			appyframe.connect()
			appyframe.reload_doctype(doctype, force=context.force)
			appyframe.db.commit()
		finally:
			appyframe.destroy()
	if not context.sites:
...
def add_to_hosts(context):
	"Add site to hosts"
	for site in context.sites:
		appyframe.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
	...
	for site in context.sites:
		try:
			...
			if rollback_callback:
				...
				rollback_callback = None
			if verbose:
				print(appyframe.get_traceback(with_context=True))
			exit_code = 1
			continue
		if appyframe.get_system_settings("encrypt_backup") and appyframe.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context, app):
	...
	from appyframe.installer import remove_from_installed_apps
...
def uninstall(context, app, dry_run, yes, no_backup, force):
	...
	ensure_app_not_frappe(app)
	from appyframe.installer import remove_app
	from appyframe.utils.synchronization import filelock
...
def _drop_site(
	...
	from appyframe.database import drop_user_and_database
	from appyframe.utils.backups import scheduled_backup
	...
	drop_user_and_database(appyframe.conf.db_name, db_root_username, db_root_password)

	archived_sites_path = archived_sites_path or os.path.join(
		appyframe.utils.get_bench_path(), "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	appyframe.init(site)
	old_path = appyframe.utils.get_site_path()
...
def set_last_active_for_user(context, user=None):
	...
	with appyframe.init_site(site):
		...
		appyframe.db.set_value("User", user, "last_active", now_datetime())
		appyframe.db.commit()
...
def publish_realtime(context, event, message, room, user, doctype, docname, after_commit):
	...
	from appyframe import publish_realtime
...
def browse(
	...
	if site not in appyframe.utils.get_sites():
	...
	if user:
		...
		if appyframe.conf.developer_mode or user == "Administrator":
			appyframe.utils.set_request(path="/")
			appyframe.local.cookie_manager = CookieManager()
			appyframe.local.login_manager = LoginManager()
			appyframe.local.login_manager.login_as(user, session_end, user_for_audit)
			sid = f"/app?sid={appyframe.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{appyframe.utils.get_site_url(site)}{sid}"
...
def start_ngrok(context, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = appyframe.conf.ngrok_authtoken
...
def clear_log_table(context, doctype, days, no_backup):
	...
	"""
	from appyframe.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from appyframe.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from appyframe.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise appyframe.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		database_tables: list[str] = (
			...
			.select(table_name)
			.where(information_schema.tables.table_schema == appyframe.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run(pluck=True)
		)
		doctype_tables = appyframe.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			...
			for table in TABLES_TO_DROP:
				...
				appyframe.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[appyframe.local.site] = TABLES_TO_DROP
		appyframe.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"appyframe",
		"database",
		appyframe.conf.db_type,
		f"framework_{appyframe.conf.db_type}.sql",
	)
...
def trim_tables(context, dry_run, format, no_backup):
	...
	from appyframe.model.meta import trim_tables
	from appyframe.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {appyframe.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {appyframe.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({appyframe.local.site: data}, indent=1, sort_keys=True))
	else:
...
def ensure_app_not_frappe(app: str) -> None:
	"""
	Ensure that the app name passed is not 'appyframe'

	:param app: Name of the app
	:return: Nothing
	"""
	if app == "appyframe":
		click.secho("You cannot remove or uninstall the app `appyframe`", fg="red")
		sys.exit(1)
