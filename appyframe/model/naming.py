...
from appyframe.utils import cint, cstr, now_datetime
...
class InvalidNamingSeriesError(appyframe.ValidationError):
	pass
...
class NamingSeries:
	...
	def validate(self):
		if "." not in self.series:
			appyframe.throw(
				_("Invalid naming series {}: dot (.) missing").format(appyframe.bold(self.series)),
		...
		if not NAMING_SERIES_PATTERN.match(self.series):
			appyframe.throw(
				...
				).format(appyframe.bold(self.series)),
				exc=InvalidNamingSeriesError,
			)
	...
	def get_prefix(self) -> str:
		...
		if prefix is None:
			appyframe.throw(_("Invalid Naming Series: {}").format(self.series))
	...
	def update_counter(self, new_count: int) -> None:
		...
		Series = appyframe.qb.DocType("Series")
		prefix = self.get_prefix()

		# Initialize if not present in DB
		if appyframe.db.get_value("Series", prefix, "name", order_by="name") is None:
			appyframe.qb.into(Series).insert(prefix, 0).columns("name", "current").run()

		(appyframe.qb.update(Series).set(Series.current, cint(new_count)).where(Series.name == prefix)).run()

	def get_current_value(self) -> int:
		prefix = self.get_prefix()
		return cint(appyframe.db.get_value("Series", prefix, "current", order_by="name"))
...
def set_new_name(doc):
	...
	if autoname.lower() != "prompt" and not appyframe.flags.in_import:
		doc.name = None

	if is_autoincremented(doc.doctype, meta):
		doc.name = appyframe.db.get_next_sequence_val(doc.doctype)
		return
...
def set_name_from_naming_options(autoname, doc):
	...
	if _autoname.startswith("field:"):
		...
		if not doc.name:
			...
			appyframe.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
...
def set_naming_from_document_naming_rule(doc):
	...
	"""
	from appyframe.model.base_document import DOCTYPES_FOR_DOCTYPE
	...
	document_naming_rules = appyframe.cache_manager.get_doctype_map(
	...
	for d in document_naming_rules:
		appyframe.get_cached_doc("Document Naming Rule", d.name).apply(doc)
		if doc.name:
			break
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		appyframe.throw(appyframe._("Naming Series mandatory"))
...
def _generate_random_string(length=10):
	"""Better version of appyframe.generate_hash for naming.
...
def parse_naming_series(
	...
	for e in parts:
		...
		if e.startswith("#"):
			...
			part = appyframe.get_attr(method[0])(doc, e)
		else:
			part = e
...
def has_custom_parser(e):
	...
	return appyframe.get_hooks("naming_series_variables", {}).get(e)
...
def getseries(key, digits):
	# series created ?
	# Using appyframe.qb as appyframe.get_values does not allow order_by=None
	series = DocType("Series")
	current = (appyframe.qb.from_(series).where(series.name == key).for_update().select("current")).run()

	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		appyframe.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		appyframe.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name, doc=None):
	...
	*Example:*
	...
	series = DocType("Series")
	current = (appyframe.qb.from_(series).where(series.name == prefix).for_update().select("current")).run()

	if current and current[0][0] == count:
		appyframe.db.sql("UPDATE `tabSeries` SET `current` = `current` - 1 WHERE `name`=%s", prefix)


def get_default_naming_series(doctype: str) -> str | None:
	"""get default value for `naming_series` property"""
	naming_series_options = appyframe.get_meta(doctype).get_naming_series_options()
...
def validate_name(doctype: str, name: int | str):
	if not name:
		appyframe.throw(_("No Name Specified for {0}").format(doctype))
	...
	if isinstance(name, int):
		if is_autoincremented(doctype):
			...
			appyframe.db.set_next_sequence_val(doctype, name, is_val_used=True)
			return name

		appyframe.throw(_("Invalid name type (integer) for varchar name column"), appyframe.NameError)

	if name.startswith("New " + doctype):
		appyframe.throw(
			_("There were some errors setting the name, please contact the administrator"), appyframe.NameError
		)
	name = name.strip()

	if not appyframe.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		appyframe.throw(_("Name of {0} cannot be {1}").format(doctype, name), appyframe.NameError)

	special_characters = "<>"
	if re.findall(f"[{special_characters}]+", name):
		message = ", ".join(f"'{c}'" for c in special_characters)
		appyframe.throw(_("Name cannot contain special characters like {0}").format(message), appyframe.NameError)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = appyframe.db.exists(doctype, filters)

	regex = f"^{re.escape(value)}{separator}\\d+$"

	if exists:
		last = appyframe.db.sql(
			f"""SELECT `{fieldname}` FROM `tab{doctype}`
			WHERE `{fieldname}` {appyframe.db.REGEX_CHARACTER} %s
...
def _set_amended_name(doc):
	...
	if appyframe.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		appyframe.throw(_("Please set the document name"))
