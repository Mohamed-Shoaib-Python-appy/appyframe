...
def get_user_settings(doctype, for_update=False):
	user_settings = appyframe.cache.hget("_user_settings", f"{doctype}::{appyframe.session.user}")

	if user_settings is None:
		user_settings = appyframe.db.sql(
			"""select data from `__UserSettings`
			where `user`=%s and `doctype`=%s""",
			(appyframe.session.user, doctype),
		)
...
def update_user_settings(doctype, user_settings, for_update=False):
	...
	appyframe.cache.hset("_user_settings", f"{doctype}::{appyframe.session.user}", json.dumps(current))


def sync_user_settings():
	"""Sync from cache to database (called asynchronously via the browser)"""
	for key, data in appyframe.cache.hgetall("_user_settings").items():
		key = safe_decode(key)
...
def update_user_settings_data(
	...
	if data:
		...
		if update:
			...
			appyframe.cache.hset("_user_settings", f"{user_setting.doctype}::{user_setting.user}", None)
