...
from appyframe.modules.patch_handler import _patch_mode
...
def sync_for(app_name, force=0, reset_permissions=False):
	...
	if app_name == "appyframe":
		# these need to go first at time of install

		FRAPPE_PATH = appyframe.get_app_path("appyframe")
	...
	for module_name in appyframe.local.app_modules.get(app_name) or []:
		folder = os.path.dirname(appyframe.get_module(app_name + "." + module_name).__file__)
...
def remove_orphan_doctypes():
	...
	doctype_names = appyframe.get_all("DocType", {"custom": 0}, pluck="name")
	orphan_doctypes = []
	...
	for i, name in enumerate(orphan_doctypes):
		appyframe.delete_doc("DocType", name, force=True, ignore_missing=True)
