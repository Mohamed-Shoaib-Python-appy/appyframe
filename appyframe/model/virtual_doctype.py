...
class VirtualDoctype(Protocol):
	...
	- DocType controller has to inherit from `appyframe.model.document.Document` class
	...
	@staticmethod
	def get_list(args) -> list[appyframe._dict]:
...
def validate_controller(doctype: str) -> None:
	try:
		...
		appyframe.msgprint(_("Failed to import virtual doctype {}, is controller file present?").format(doctype))
		return
		...
		if not isinstance(method, staticmethod):
			appyframe.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					appyframe.bold(doctype), appyframe.bold(m), appyframe.bold(_as_str(method))
				),
