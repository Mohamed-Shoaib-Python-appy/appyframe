...
import appyframe
from appyframe import _, is_whitelisted, msgprint
from appyframe.core.doctype.file.utils import relink_mismatched_files
from appyframe.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from appyframe.desk.form.document_follow import follow_document
from appyframe.integrations.doctype.webhook import run_webhooks
from appyframe.model import optional_fields, table_fields
from appyframe.model.base_document import BaseDocument, get_controller
from appyframe.model.docstatus import DocStatus
from appyframe.model.naming import set_new_name, validate_name
from appyframe.model.utils import is_virtual_doctype
from appyframe.model.workflow import set_workflow_state_on_action, validate_workflow
from appyframe.types import DF
from appyframe.utils import compare, cstr, date_diff, file_lock, flt, get_datetime_str, now
from appyframe.utils.data import get_absolute_url, get_datetime, get_timedelta, getdate
from appyframe.utils.global_search import update_global_search
...
def get_doc(*args, **kwargs):
	"""returns a appyframe.model.Document object.
...
class Document(BaseDocument):
	...
	def load_from_db(self):
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = appyframe.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = appyframe.new_doc(self.doctype, as_dict=True)
			...
			if not d:
				appyframe.throw(
					...
					appyframe.DoesNotExistError(doctype=self.doctype),
				)
		...
		for df in self._get_table_fields():
			...
			# During appyframe is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to appyframe.db.get_value() as it would crash otherwise.
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `appyframe.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self._handle_permission_failure(permtype)

	def has_permission(self, permtype="read", *, debug=False, user=None) -> bool:
		"""
		Call `appyframe.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return appyframe.permissions.has_permission(self.doctype, permtype, self, debug=debug, user=user)
	...
	def raise_no_permission_to(self, perm_type):
		...
		appyframe.flags.error_message = _(
			"You need the '{0}' permission on {1} {2} to perform this action."
		).format(
			_(perm_type),
			appyframe.bold(_(self.doctype)),
			self.name or "",
		)
		raise appyframe.PermissionError
	...
	def insert(
		...
		if not (appyframe.flags.in_migrate or appyframe.local.flags.in_install or appyframe.flags.in_setup_wizard):
			if appyframe.get_cached_value("User", appyframe.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, appyframe.session.user)
		return self
	...
	def check_if_locked(self):
		...
		if file_lock.lock_age(self.get_signature()) > DOCUMENT_LOCK_SOFT_EXPIRY:
			primary_action = {
				"label": "Force Unlock",
				"server_action": "appyframe.model.document.unlock_document",
		...
		appyframe.throw(
			...
			exc=appyframe.DocumentLockedError,
		)
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Document":
		...
		self.flags.ignore_version = appyframe.flags.in_test if ignore_version is None else ignore_version
	...
	def validate_amended_from(self):
		if appyframe.db.get_value(self.doctype, self.get("amended_from"), "docstatus") != 2:
			message = _(
				"{0} cannot be amended because it is not cancelled. Please cancel the document before creating an amendment."
			).format(appyframe.utils.get_link_to_form(self.doctype, self.get("amended_from")))
			appyframe.throw(message, title=_("Amendment Not Allowed"))
	...
	def update_child_table(self, fieldname: str, df: Optional["DocField"] = None):
		...
		if not (
			...
			or appyframe.get_meta(df.options).is_virtual == 1
		):
			existing_row_names = [row.name for row in all_rows if row.name and not row.is_new()]

			tbl = appyframe.qb.DocType(df.options)
			qry = (
				appyframe.qb.from_(tbl)
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `appyframe.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		appyframe.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in appyframe.db.value_cache:
			del appyframe.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = appyframe.session.user
		...
		if self.is_new() and not (
			appyframe.flags.in_install or appyframe.flags.in_patch or appyframe.flags.in_migrate
		):
		...
		appyframe.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					...
					appyframe.bold(_(df.label, context=df.parent)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), appyframe.bold(_(df.label, context=df.parent))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				appyframe.throw(msg, appyframe.NonNegativeError, title=_("Negative Value"))
	...
	def validate_workflow(self):
		...
		if appyframe.flags.in_install == "appyframe":
			return
				...
				if fail:
					appyframe.throw(
						_("Value cannot be changed for {0}").format(
							appyframe.bold(self.meta.get_label(field.fieldname))
						),
						exc=appyframe.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += appyframe.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in appyframe.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or appyframe.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = appyframe.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = appyframe.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		if self.is_new():
			new_doc = appyframe.new_doc(self.doctype, as_dict=True)
			self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = appyframe.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			appyframe.msgprint(
				...
				raise_exception=appyframe.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.DRAFT:
			if self.docstatus.is_draft():
				...
				raise appyframe.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise appyframe.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise appyframe.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if appyframe.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise appyframe.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			appyframe.throw(_("Could not find {0}").format(msg), appyframe.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			appyframe.throw(_("Cannot link cancelled document: {0}").format(msg), appyframe.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(appyframe.flags.in_import and appyframe.flags.mute_emails)
		...
		from appyframe.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = appyframe.cache.hget("notifications", self.doctype, _get_notifications)
	...
	def _rename(self, name: str, merge: bool = False, force: bool = False, validate_rename: bool = True):
		"""Rename the document. Triggers appyframe.rename_doc, then reloads."""
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = appyframe.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in appyframe.flags.currently_saving:
			appyframe.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		appyframe.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			appyframe.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([appyframe.session.user]), update_modified=False
			)

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		if appyframe.flags.in_patch:
			return
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and not self.meta.get("istable"):
			data = {"doctype": self.doctype, "name": self.name, "user": appyframe.session.user}
			appyframe.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run appyframe.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in appyframe.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", appyframe.session.user)
	...
	def db_get(self, fieldname):
		...
		return appyframe.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from appyframe.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def save_version(self):
		...
		if (
			...
			or appyframe.flags.in_install
			or (not self._doc_before_save and appyframe.flags.in_patch)
		):
			return

		doc_to_compare = self._doc_before_save
		if not doc_to_compare and (amended_from := self.get("amended_from")):
			doc_to_compare = appyframe.get_doc(self.doctype, amended_from)
	...
	def hook(f):
		...
		def composer(self, *args, **kwargs):
			hooks = []
			method = f.__name__
			doc_events = appyframe.get_doc_hooks()
			for handler in doc_events.get(self.doctype, {}).get(method, []) + doc_events.get("*", {}).get(
				method, []
			):
				hooks.append(appyframe.get_attr(handler))
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			appyframe.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or appyframe.EmptyTableError
			)
	...
	def round_floats_in(self, doc, fieldnames=None):
		...
		rounding_method = appyframe.get_system_settings("rounding_method")
		...
		return appyframe.get_doc(
			{
				...
				"comment_email": comment_email or appyframe.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not appyframe.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = appyframe.parse_json(_seen)
			...
			if user not in _seen:
				...
				appyframe.local.flags.commit = True
	...
	def add_viewed(self, user=None, force=False, unique_views=False):
		...
		if unique_views and appyframe.db.exists(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		existing_key = appyframe.db.exists(
			"Document Share Key",
			{
				"reference_doctype": self.doctype,
		...
		if existing_key:
			doc = appyframe.get_doc("Document Share Key", existing_key)
		else:
			doc = appyframe.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", appyframe._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", appyframe._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		return enqueue(
			"appyframe.model.document.execute_action",
			...
			if timeout:
				for _ in range(timeout):
					...
					if not file_lock.lock_exists(signature):
						...
						break
			if lock_exists:
				raise appyframe.DocumentLockedError
		file_lock.create_lock(signature)
		appyframe.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in appyframe.local.locked_documents:
			appyframe.local.locked_documents.remove(self)
		...
		if date_diff(to_date, from_date) < 0:
			...
			if self.meta.istable:
				table_row = _("{0} row #{1}: ").format(
					_(appyframe.unscrub(self.parentfield)),
					self.idx,
				)
			...
			appyframe.throw(
				...
				+ _("{0} must be after {1}").format(
					appyframe.bold(_(self.meta.get_label(to_date_field))),
					appyframe.bold(_(self.meta.get_label(from_date_field))),
				),
				appyframe.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = appyframe.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from appyframe.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from appyframe.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = appyframe.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		if appyframe.message_log:
			msg = appyframe.message_log[-1].get("message")
		else:
			msg = "<pre><code>" + appyframe.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	doctype_meta = appyframe.get_meta(doctype)
	...
	for child_table in doctype_meta.get_table_fields():
		valid_column_map[child_table.options] = appyframe.get_meta(child_table.options).get_valid_columns()
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.owner = doc.modified_by = appyframe.session.user
...
def unlock_document(doctype: str | None = None, name: str | None = None, args=None):
	...
	if not doctype and not name and args:
		...
		name = str(args["name"])
	appyframe.get_doc(doctype, name).unlock()
	appyframe.msgprint(appyframe._("Document Unlocked"), alert=True)
