...
from appyframe.model.dynamic_links import invalidate_distinct_link_doctypes
from appyframe.model.naming import set_new_name
from appyframe.model.utils.link_count import notify_link_count
...
def get_controller(doctype):
	...
	For `custom` type, returns `appyframe.model.document.Document`.

	:param doctype: DocType name as string.
	"""

	if appyframe.local.dev_server or appyframe.flags.in_migrate:
		return import_controller(doctype)

	site_controllers = appyframe.controllers.setdefault(appyframe.local.site, {})
...
def import_controller(doctype):
	from appyframe.model.document import Document
	from appyframe.utils.nestedset import NestedSet

	module_name = "Core"
	if doctype not in DOCTYPES_FOR_DOCTYPE:
		doctype_info = appyframe.db.get_value("DocType", doctype, ("module", "custom", "is_tree"), as_dict=True)
		if doctype_info:
	...
	module_path = None
	class_overrides = appyframe.get_hooks("override_doctype_class")
	if class_overrides and class_overrides.get(doctype):
		import_path = class_overrides[doctype][-1]
		module_path, classname = import_path.rsplit(".", 1)
		module = appyframe.get_module(module_path)
...
class BaseDocument:
	...
	@cached_property
	def meta(self):
		return appyframe.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return appyframe.db.get_value(self.doctype, self.name, key)
			...
			if df:
				if is_virtual_field:
					...
					if (prop := getattr(type(self), fieldname, None)) and is_a_property(prop):
						...
						from appyframe.utils.safe_exec import get_safe_globals

						value = appyframe.safe_eval(
							code=options,
				...
				if isinstance(value, list) and df.fieldtype not in table_fields:
					appyframe.throw(_("Value for {0} cannot be a list").format(_(df.label, context=df.parent)))
	...
	def get_valid_columns(self) -> list[str]:
		if self.doctype not in appyframe.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from appyframe.model.meta import get_table_columns

				valid = get_table_columns(self.doctype)
			else:
				valid = self.meta.get_valid_columns()

			appyframe.local.valid_columns[self.doctype] = valid

		return appyframe.local.valid_columns[self.doctype]
	...
	def as_json(self):
		return appyframe.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and appyframe.db.db_type == "postgres":
			conflict_handler = "on conflict (name) do nothing"

		if not self.creation:
			self.creation = self.modified = now()
			self.owner = self.modified_by = appyframe.session.user
		...
		try:
			...
			if appyframe.db.is_primary_key_violation(e):
				...
				if not ignore_if_duplicate:
					appyframe.msgprint(
						_("{0} {1} already exists").format(_(self.doctype), appyframe.bold(self.name)),
						title=_("Duplicate Name"),
						indicator="red",
					)
					raise appyframe.DuplicateEntryError(self.doctype, self.name, e)

			elif appyframe.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if appyframe.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if appyframe.db.db_type != "postgres":
			...
			appyframe.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise appyframe.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		if getattr(self.meta, "issingle", False):
			appyframe.db.set_single_value(self.doctype, "modified", self.modified, update_modified=False)
		else:
			appyframe.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if docname:
				if df.fieldtype == "Link":
					doctype = df.options
					if not doctype:
						appyframe.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						appyframe.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not meta.get("is_virtual"):
					if not fields_to_fetch:
						...
						values = _dict(name=appyframe.db.get_value(doctype, docname, "name", cache=True))
					else:
						...
						values = appyframe.db.get_value(doctype, docname, values_to_fetch, as_dict=True)

				if getattr(meta, "issingle", 0):
					values.name = doctype

				if meta.get("is_virtual"):
					values = appyframe.get_doc(doctype, docname).as_dict()
				...
				if values:
					...
					if not values.name:
						...
						and appyframe.get_meta(doctype).is_submittable
						and DocStatus(appyframe.db.get_value(doctype, docname, "docstatus") or 0).is_cancelled()
					):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			from appyframe.model.meta import get_default_df

			fetch_from_df = get_default_df(fetch_from_fieldname) or appyframe.get_meta(doctype).get_field(
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (appyframe.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		# data_field options defined in appyframe.model.data_field_options
		for phone_field in self.meta.get_phone_fields():
			phone = self.get(phone_field.fieldname)
			appyframe.utils.validate_phone_number_with_country_code(phone, phone_field.fieldname)
		...
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in appyframe.STANDARD_USERS) and (data in appyframe.STANDARD_USERS):
					continue
				for email_address in appyframe.utils.split_emails(data):
					appyframe.utils.validate_email_address(email_address, throw=True)

			if data_field_options == "Name":
				appyframe.utils.validate_name(data, throw=True)

			if data_field_options == "Phone":
				appyframe.utils.validate_phone_number(data, throw=True)

			if data_field_options == "URL":
				if not data:
					continue

				appyframe.utils.validate_url(data, throw=True)

	def _validate_constants(self):
		if appyframe.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = appyframe.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				appyframe.throw(
					...
					appyframe.CannotChangeConstantError,
				)
	...
	def _validate_length(self):
		...
		type_map = appyframe.db.type_map
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				appyframe.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				appyframe.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		appyframe.throw(
			_("{0}: '{1}' ({3}) will get truncated, as max characters allowed is {2}").format(
				reference, appyframe.bold(_(df.label, context=df.parent)), max_length, value
			),
			appyframe.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = appyframe.get_doc(self.doctype, self.name).as_dict()
				...
				if self_value != db_value:
					appyframe.throw(
						_("{0} Not allowed to change {1} after submission from {2} to {3}").format(
							...
							appyframe.bold(db_value),
							appyframe.bold(self_value),
						),
						appyframe.UpdateAfterSubmitError,
	...
	def _sanitize_content(self):
		...
		for fieldname, value in self.get_valid_dict(ignore_virtual=True).items():
			...
			value = appyframe.as_unicode(value)
	...
	def _save_passwords(self):
		...
		from appyframe.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def get_formatted(
		...
		from appyframe.utils.formatters import format_value
		...
		if (
			...
			currency = appyframe.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = appyframe.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_editor(self):
		from appyframe.core.doctype.file.utils import extract_images_from_doc
