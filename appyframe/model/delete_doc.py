...
from appyframe import _, get_module_path
from appyframe.desk.doctype.tag.tag import delete_tags_for_document
from appyframe.model.docstatus import DocStatus
from appyframe.model.dynamic_links import get_dynamic_link_map
from appyframe.model.naming import revert_series_if_last
from appyframe.model.utils import is_virtual_doctype
from appyframe.utils.file_manager import remove_all
from appyframe.utils.global_search import delete_for_document
from appyframe.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = appyframe.form_dict.get("dt")
		name = appyframe.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			appyframe.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				doc = appyframe.get_doc(doctype, name)
				if not (doc.custom or appyframe.conf.developer_mode or appyframe.flags.in_patch or force):
					appyframe.throw(_("Standard DocType can not be deleted."))

				update_flags(doc, flags, ignore_permissions)
				check_permission_and_not_submitted(doc)
				# delete custom table fields using this doctype.
				appyframe.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", appyframe.model.table_fields)}
				)
				appyframe.db.delete("__global_search", {"doctype": name})

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				appyframe.conf.developer_mode
				and not doc.custom
				and not (
					for_reload
			...
			try:
				appyframe.db.get_value(doctype, name, for_update=True, wait=False)
			except appyframe.QueryTimeoutError:
				appyframe.throw(
					_(
						"This document can not be deleted right now as it's being modified by another user. Please try again after some time."
					),
					exc=appyframe.QueryTimeoutError,
				)
			doc = appyframe.get_doc(doctype, name)
			...
			if not for_reload:
				...
				if not force:
					...
					except appyframe.LinkExistsError as e:
			...
			if not for_reload:
				...
				appyframe.enqueue(
					"appyframe.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and appyframe.flags.in_install != "appyframe":
		appyframe.get_doc(
			dict(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		appyframe.db.delete("Singles", {"doctype": name})
	else:
		appyframe.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if appyframe.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = appyframe.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", appyframe.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		appyframe.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		and appyframe.session.user != "Administrator"
		...
		appyframe.msgprint(
			...
			raise_exception=appyframe.PermissionError,
		)
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from appyframe.model.rename_doc import get_link_fields
	...
	if method == "Delete":
		ignored_doctypes.update(appyframe.get_hooks("ignore_links_on_delete"))
	...
	for lf in link_fields:
		...
		try:
			meta = appyframe.get_meta(link_dt)
		except appyframe.DoesNotExistError:
		...
		if issingle:
			if appyframe.db.get_single_value(link_dt, link_field) == doc.name:
		...
		for item in appyframe.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
...
def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `appyframe.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):
		ignore_linked_doctypes = doc.get("ignore_linked_doctypes") or []

		if df.parent in appyframe.get_hooks("ignore_links_on_delete") or (
		...
		if meta.issingle:
			...
			for refdoc in appyframe.db.sql(
				...
				if (method == "Delete" and not DocStatus(refdoc.docstatus).is_cancelled()) or (
					...
					if reference_doctype in appyframe.get_hooks("ignore_links_on_delete") or (
...
def clear_timeline_references(link_doctype, link_name):
	appyframe.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", appyframe.scrub(doctype))
