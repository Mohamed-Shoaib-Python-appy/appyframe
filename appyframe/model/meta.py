...
from appyframe.model.base_document import (
...
from appyframe.utils import cast, cint, cstr
from appyframe.utils.data import add_to_date, get_datetime
...
def get_meta(doctype, cached=True) -> "Meta":
	...
	if cached and (meta := appyframe.cache.hget("doctype_meta", doctype)):
		return meta

	meta = Meta(doctype)
	appyframe.cache.hset("doctype_meta", meta.name, meta)
	return meta
...
def get_table_columns(doctype):
	return appyframe.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = appyframe.scrub(doctype)
	with open(appyframe.get_app_path("appyframe", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = (
		appyframe._dict(fieldname="creation", fieldtype="Datetime"),
		appyframe._dict(fieldname="owner", fieldtype="Data"),
	)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(appyframe._dict(fieldtype="Data", fieldname="name", label="Name"))

		return fields

	def get_valid_columns(self) -> list[str]:
		if not hasattr(self, "_valid_columns"):
			table_exists = appyframe.db.table_exists(self.name)
	...
	def add_custom_fields(self):
		if not appyframe.db.table_exists("Custom Field"):
			return

		custom_fields = appyframe.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not appyframe.db.table_exists("Property Setter"):
			return

		property_setters = appyframe.db.get_values(
			"Property Setter",
	...
	def check_if_large_table(self):
		...
		if self.istable or not appyframe.db.table_exists(self.name):  # During install, new migrate
			return

		if appyframe.db.estimate_count(self.name) > LARGE_TABLE_SIZE_THRESHOLD:
			recent_change = appyframe.db.get_value(self.name, {}, "modified", order_by="modified desc")
	...
	def set_custom_permissions(self):
		...
		if appyframe.flags.in_patch or appyframe.flags.in_install:
			return

		if not self.istable and self.name not in ("DocType", "DocField", "DocPerm", "Custom DocPerm"):
			custom_perms = appyframe.get_all(
				"Custom DocPerm",
				fields="*",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(appyframe._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if not permission_type:
			permission_type = "select" if appyframe.only_has_select_perm(self.name, user=user) else "read"
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if appyframe.share.get_shared(self.name, user, rights=[permission_type], limit=1):
	...
	def get_permlevel_access(self, permission_type="read", parenttype=None, *, user=None):
		has_access_to = []
		roles = set(appyframe.get_roles(user))
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = appyframe.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other Appyframe applications via hooks.
		"""
		data = appyframe._dict()
		if not self.custom:
			try:
				module = load_doctype_module(self.name, suffix="_dashboard")
				if hasattr(module, "get_data"):
					data = appyframe._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in appyframe.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = appyframe._dict(appyframe.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = appyframe.scrub(self.module)
		doctype = appyframe.scrub(self.name)
		template_path = appyframe.get_module_path(
...
def is_single(doctype):
	try:
		return appyframe.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether %s is single" % doctype)


def get_parent_dt(dt):
	if not appyframe.is_table(dt):
		return ""

	return (
		appyframe.db.get_value(
			"DocField",
			{"fieldtype": ("in", appyframe.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	appyframe.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(appyframe.local, "field_currency", None):
		appyframe.local.field_currency = appyframe._dict()

	if not (
		appyframe.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and appyframe.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = appyframe.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if appyframe.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = appyframe.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			appyframe.local.field_currency.setdefault((doc.doctype, ref_docname), appyframe._dict()).setdefault(
				df.fieldname, currency
			)

	return appyframe.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent") and appyframe.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)


def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from appyframe.utils import get_number_format_info

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(appyframe.db.get_default("currency_precision"))
		if not precision:
			number_format = appyframe.db.get_default("number_format") or "#,###.##"
			decimal_str, comma_str, precision = get_number_format_info(number_format)
	else:
		precision = cint(appyframe.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return appyframe._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return appyframe._dict(fieldname=fieldname, fieldtype="Int")

		return appyframe._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in appyframe.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	appyframe.cache.hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = appyframe.db.get_table_columns(doctype)
	fields = appyframe.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		appyframe.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
