...
def apply():
	rate_limit = appyframe.conf.rate_limit
	if rate_limit:
		appyframe.local.rate_limiter = RateLimiter(rate_limit["limit"], rate_limit["window"])
		appyframe.local.rate_limiter.apply()


def update():
	if hasattr(appyframe.local, "rate_limiter"):
		appyframe.local.rate_limiter.update()


def respond():
	if hasattr(appyframe.local, "rate_limiter"):
		return appyframe.local.rate_limiter.respond()
...
class RateLimiter:
	...
	def __init__(self, limit, window):
		...
		self.key = appyframe.cache.make_key(f"rate-limit-counter-{self.window_number}")
		self.counter = cint(appyframe.cache.get(self.key))
		if not self.counter:
			# This is the first request in this window
			appyframe.cache.incrby(self.key, 0)
			appyframe.cache.expire(self.key, self.window)
	...
	def reject(self):
		raise appyframe.TooManyRequestsError

	def update(self):
		self.record_request_end()
		appyframe.cache.incrby(self.key, self.duration)
...
def rate_limit(
	...
	def ratelimit_decorator(fn):
		...
		def wrapper(*args, **kwargs):
			...
			if not appyframe.request or (
				methods != "ALL" and appyframe.request.method and appyframe.request.method.upper() not in methods
			):
				return fn(*args, **kwargs)

			_limit = limit() if callable(limit) else limit

			ip = appyframe.local.request_ip if ip_based is True else None

			user_key = appyframe.form_dict.get(key, "")
			...
			if not identity:
				appyframe.throw(_("Either key or IP flag is required."))

			cache_key = appyframe.cache.make_key(f"rl:{appyframe.form_dict.cmd}:{identity}")

			value = appyframe.cache.get(cache_key) or 0
			if not value:
				appyframe.cache.setex(cache_key, seconds, 0)

			value = appyframe.cache.incrby(cache_key, 1)
			if value > _limit:
				appyframe.throw(
					_("You hit the rate limit because of too many requests. Please try after sometime."),
					appyframe.RateLimitExceededError,
				)
